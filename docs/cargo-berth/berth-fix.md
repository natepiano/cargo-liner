...
Each variant carries typed `recovery_actions`, serialized in the rejection payload. That field is itself a defined domain type, not a loose list — an empty or untyped action set is the prose-recovery defect this phase exists to end. **The semantic type is `CanonicalWorktreeRoot` (`ledger`, imported at `board/mod.rs:55` and `recovery.rs:24`); `CanonicalRoot` does not exist.** Two emptiness holes must be closed by construction rather than by the gate: `Vec<OsString>` admits an empty command line, and a plural `recovery_actions` field admits an empty set. Introduce `RecoveryCommandLine` (non-empty by construction: private field, fallible constructor) and make `recovery_actions` a non-empty collection type, so the acceptance gate's "non-empty, complete argv" requirement is unrepresentable to violate rather than merely tested:

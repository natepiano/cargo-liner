...
### Phase 17 — Decompose and bound the PostToolUse fixed cost  · status: todo

#### Work Order

**Goal:** The PostToolUse shim path finishes inside 0.20 seconds at both process-cache temperatures, which requires first establishing what the path spends when it asks Git nothing at all, and per-outcome Git process counts become gated ceilings rather than recorded observations.

**Spec:**

Phase 16 measured the installed-engine matrix from independently restored state at both process-cache temperatures across every ready outcome, both durable-proof states, two journal ages, three history shapes, and independent reservation, alert, and incident cardinalities, after removing an instrumentation switch that had been serializing the scoped-patch reads the phase shipped concurrent. **Read Phase 16's as-built bound record in this document for the measured figures; do not carry a number from any earlier draft of this phase, all of which predate the corrected measurement.** The published 0.20-second bound is missed broadly: 273 of 320 cold ready-engine samples and 244 of 320 warm samples land at or above it.

**The dominant term is not the Git process count, and an earlier draft of this phase asserting that it was is wrong.** Fitting the 57 cold per-cell maxima against the Git argv each cell spends gives a slope of roughly **8.6 milliseconds per Git process** over a fixed intercept of roughly **0.234 seconds at zero Git processes**. The intercept alone exceeds the budget. The measurement shows this directly rather than only through the fit: the largest cold sample in the whole matrix is `first_touch_acquisition` at the 214-record journal, 0.590941 seconds, spending **five** Git argv, while `successor_equivalence_positive` spends 22 and reaches 0.409604. Arity reduction is worth having and cannot on its own reach the bound — deleting every Git child from the hottest outcome still leaves it above 0.20 seconds.

A second cost is invisible to any Git-process guard. The independent history grid holds the Git count fixed at nine across both cardinalities, yet `history[profile=deep]` costs 0.249653 seconds at one active subject and 0.418663 at twenty. That 0.17 seconds is per-subject work inside the process, not a subprocess, and no existing cardinality assertion can see it because those assertions compare argv counts, which are equal by construction here.

**This phase's first deliverable is therefore a decomposition of the fixed cost, not an optimization.** Attribute the ~0.234-second intercept across its real components — engine process startup and dynamic linking, ledger open and journal replay, the shim's own interpreter startup and validator execution, generated-consumer loading, and any repository discovery preceding the first Git call — measured on the same production-faithful harness and reported per component with the same cold and warm treatment as the outcome cells. Until that attribution exists, any optimization here is a guess: the plan has already spent one phase optimizing a term that turned out not to dominate, and repeating that with the floor would repeat the error. The attribution is what tells the next phase whether the remaining work is in the engine, in the shim, or in the boundary between them.

Then reduce cost where the attribution shows it lives, plus the arity reductions the measurement already justifies. Each arity candidate below is scoped by query semantics, not by counting `git` in a trace:

- **Pairwise ancestry, where the pair is one of many against a shared base.** `ahead_behind_for_heads` already answers every worktree head against one trunk in a single graph walk; route any remaining site asking that shape one pair at a time through it. `rev-list --left-right --count <left>...<right>` is **not** a general replacement: it reports symmetric-difference counts and says nothing about whether a merge base exists, so related divergence and unrelated history both present as nonzero/nonzero and `AheadBehind::Unrelated` cannot be derived from it. Any site needing that distinction, commit ids rather than counts, cherry equivalence, first-parent order, or a path filter keeps its own query. There is no `rev-list --is-ancestor`; the ancestor predicate is `merge-base --is-ancestor`.
- **Object resolution, where two or more object expressions are known before any decision that depends on the first.** `commit_availability` already batches object ids through `cat-file --batch-check` and returns `CommitAvailability::{Available, ObjectUnknown}` — the pattern to follow, including its semantic result type. Any new batch preserves resolved, missing, ambiguous, and wrong-type as distinct alternatives of a domain enum; never `Option<GitObjectId>`, which collapses three different failures into one absence. Batch-check answers object expressions only: `rev-parse --git-path` sites (`GIT_PATH_ARG`) are repository-path queries and cannot join a batch, and of the cheap engine path's three `rev-parse` calls, two locate rebase state and one resolves an object — no batch exists there.
- **Repeated reads inside one admitted comparison.** `compare_scoped_patch` receives `phase_start_head` already resolved and does not re-resolve it, and the reconciliation budgets admit at most one trunk comparison per target and one successor comparison per pass — the one-versus-twenty traces prove there is no target-cardinality multiplication to hoist out of. Optimize the calls made *within* one admitted comparison, or another query the trace shows genuinely repeated; a hoist across the per-target loop reduces nothing.

**Do not buy the bound with caching, tolerance, or exclusion.** A memoized Git answer that a concurrent worktree can invalidate is a correctness regression, not a cost fix; the retained-verdict set in `reservation/mod.rs` is bounded to specific targets for exactly that reason and is not a precedent for caching raw Git reads. The published bound stays 0.20 seconds, every outcome keeps its cold and warm cells at every dimension Phase 16 measured, and the `NeedsRepair` early return keeps its own row. If the attribution shows the bound is unreachable without a change the plan forbids, that is a finding to report with its evidence, not a licence to widen the limit.

Add the missing ceiling, on the oracle that actually executes the path. `post_tool_use_git_subprocess_count_is_cardinality_invariant` in `tests/gate.rs` invokes berth's **installed Git post-commit hook**, not the external PostToolUse shim — its own comment says so — and it asserts cardinality invariance, never an absolute maximum. Per-outcome ceilings therefore belong beside `PostToolUseTimingCell` in the Python harness, which is the only thing that runs the shim: each ready outcome carries a maximum Git process count, asserted against the same raw unfiltered argv trace the timing matrix already collects. The Rust tests keep and gain absolute command-sequence ceilings for the engine and Git-hook paths they do execute, so a reintroduced per-question process fails a fast test rather than waiting for the matrix.

**Files:**

- `crates/cargo-berth/src/git/mod.rs` — ancestry, object resolution, and scoped-patch reads
- `crates/cargo-berth/src/git/constants.rs` — `GIT_BATCH_CHECK_ARG`, `GIT_LEFT_RIGHT_ARG`, `GIT_COUNT_ARG`, `GIT_STDIN_ARG`, `GIT_PATH_ARG`
- `crates/cargo-berth/src/git/command.rs` — `git_output_dynamic_with_input` for batched stdin reads
- `crates/cargo-berth/src/reconcile.rs` — the per-pass comparison budget
- `crates/cargo-berth/src/drift/execution.rs` — the cheap engine path's process floor
- `crates/cargo-berth/src/ledger/mod.rs` — ledger open and replay, a named component of the fixed cost
- `crates/cargo-berth/tests/gate.rs` — absolute command-sequence ceilings for the engine and Git-hook paths
- `crates/cargo-berth/tests/board.rs` — the existing cardinality equality and `canonical_git_command_sequence`
- `crates/cargo-berth/tests/drift.rs` — the recorded cheap-path command sequence
- `/Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` — the timing matrix, the fixed-cost attribution, and the new per-outcome ceilings; **this file lives in the `/Users/natemccoy/.claude` Git checkout and can never appear in a `cargo-berth` checkpoint commit**
- `docs/cargo-berth/berth-fix.md` — the re-published as-built bound record

**Constraints from prior phases:** Phase 10 fixed the subprocess count for drift provenance and that invariance must survive: any batching added here keeps the count independent of reservation and incident cardinality, which `tests/board.rs` and `post_tool_use_git_subprocess_count_is_cardinality_invariant` already assert. Phase 15 bound the ledger projection to replayed facts; Phase 16 confirmed journal replay is not the term that separates a fast outcome from a slow one at a working repository's record count, but replay remains a named component of the fixed cost this phase must attribute rather than assume away. Phase 16 shipped the concurrent working-tree read (`drift/observation.rs`), `Ledger::open_from_discovered_worktree` on the engine path, concurrent scoped-patch reads in `git/mod.rs`, the selective `-Xno-renames` strategy gated on `ProtectedScopedRename`, and exact Git-synthetic displaced-path matching in `scoped_merge_displaced_reserved_file`; all are correctness or cost fixes and none may be reverted. Phase 16 also shipped the cold-temperature gate in the Python timing harness — each timed child executable is mapped, invalidated with Darwin `msync(MS_INVALIDATE)`, and required to report zero resident pages from `mincore` before timing; that gate is a hard requirement of any re-measurement here, and a cell that will not go cold fails rather than being relabeled warm. Phase 16 further established that **the harness must never set an environment variable the engine reads as a scheduling switch**: tracing and scheduling are separate concerns, and a measurement that changes the code path it measures is void however green it reads. Phase 16 made the reservation-cardinality guards compare Git command names through `canonical_git_command_sequence`, which sorts before comparing, because the concurrent scoped-patch reads make emission order nondeterministic; keep that comparison order-independent and keep the exact argv **totals** asserted separately, since the totals are what catch a per-reservation call returning. Phase 16's harness carries fifteen tests, not fourteen; preserve its production-faithful timing and its per-outcome ceilings rather than substituting a cheaper in-process approximation.

**Acceptance gate:** The fixed-cost attribution is published in this document, naming each component of the zero-Git floor with its cold and warm cost, measured on the production-faithful harness, and summing to within ten percent of the measured zero-Git intercept. All 21 engine outcomes then finish within 0.20 seconds across five samples at each combination of process-cache temperature and durable-proof state, from independently restored state, with `generated_envelope_validation_installation_state=Ready`, at every journal age, history shape, and reservation, alert, and incident cardinality Phase 16's measurement carries — the same 640-sample ready-engine matrix, not a reduced one — and with every cold sample passing its zero-residency gate before timing begins. The `NeedsRepair` early return keeps its own five-cold and five-warm row beside the engine bound. Every ready outcome's Git process count is at or below a per-outcome ceiling asserted beside `PostToolUseTimingCell`, proved on the raw unfiltered argv trace rather than by counting `cargo-berth` invocations; the outcomes Phase 16 names as its worst — `successor_equivalence_positive` at 22 Git argv, `successor_equivalence_negative` at 21, uncached trunk equivalence at 20 and 19, `attribution` at 17, and both lost-evidence recovery variants at 20 and 10 — each spend strictly fewer Git processes than that record shows. The per-subject cost the history grid exposes at fixed Git count is reduced: `history[profile=deep,subjects=20]` finishes within 0.20 seconds rather than the 0.418663 it measures today. Both scoped-equivalence cache-miss rows still report equal Git argv totals and the same multiset of Git command names for one reservation and for twenty, compared order-independently through `canonical_git_command_sequence`. `tests/gate.rs` carries absolute command-sequence ceilings for the engine and Git post-commit-hook paths it executes. The as-built bound record in this document is re-published with the new measured maxima at both temperatures, naming the journal record count, history shape, cardinalities, and installation state it was proved against. `bash ~/.claude/scripts/delegate/verify.sh test cargo-berth`, every scoped `test` line in Delegation Context, and `bash ~/.claude/scripts/delegate/verify.sh lint cargo-berth` green; `python3 /Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` passes in full, including the timing assertion that Phase 16 leaves failing.

### Phase 18 — Semantic roles and bounded optionality  · status: todo
...
- `WorkingTreeStatusPaths` (`drift/git_output.rs:49`) → `WorkingTreeChangePartition`
- `ProtectedScopedRename` (`git/mod.rs:226`), variants `{Absent, Present}` → `ScopedReplayRenameDetection`, variants `{DisabledWithoutProtectedRename, RequiredForProtectedRename}`
...
**The twenty renames are global and mechanical, and the user performs them.** This is a standing preference, not a suggestion: their editor applies a project-wide rename accurately in one action, where a hand-applied sweep across this many symbols is slow and error-prone. The orchestrator presents the twenty old-to-new pairs and waits for the user to apply them; implementation work in this phase is everything else — the new semantic types, the boundary conversions, and the audit — and none of it is blocked on the renames landing first.
...
- `crates/cargo-berth/src/git/mod.rs` — `GitCommandOutputAvailability` call sites, plus `ProtectedTipSuccessorHeadClassification` (declared L1904, produced L1659) and its call site at `reconcile.rs:1529`
...
- `/Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` — created by Phase 12, extended by Phases 13 and 14 and again by Phase 16 to fifteen tests, including typed replay routing, unreadable-validator diagnostics, and installer rollback ordering; extended here with the classifier fixtures this phase's gate names

...
**`Option<ReservationId>` stays inside clap and goes no further.** Convert the parsed flag into a domain selector — `BoardOutputSelection::{CompleteBoard, ReservationLifecycleFor(ReservationId)}` — before anything in `verb/board.rs` sees it, so the two board outcomes are distinguishable by type rather than by an absent value. The lifecycle response has no TUI representation, so `--reservation` **requires** `--json`: reject the combination at the command line, and prove by test that the selector can never reach the TUI path.

**The engine selector alone is inert.** `claim_state.py:2325` rejects every board argv except exactly `["board", "--json"]`, and `/plan:delegate` reaches the board only through that coordinator — so a new engine query would pass every crate test while lost-release recovery still dead-ends. Add a validated coordinator entry point:
...
Its validator requires the echoed id, exactly one lifecycle alternative, the protected tip where the alternative carries one, exit 0 for a known id, and a typed invalid-input reason for an unknown one. Update `/plan:delegate` to use it after a lost release reply — the lost-release reasoning has moved and now sits at `delegate.md:1765–1799`, with `ReservationPresentWithoutProtectedTip` interpreted at `:1776`, `:1783`, and the retention-ref reasoning at `:1787`.

**Install the engine before the coordinator depends on it; do not build a capability fallback.** `claim_state.py` and the three shims are the live, globally registered coordination path for every session on this machine, and they resolve whatever `cargo-berth` is on `PATH` — currently a build predating this plan, which answers `board --reservation` with a usage error. Changing the coordinator first would break live coordination between the edit and the install. So this phase installs the freshly built binary as an ordered step: land the engine selector, install it, prove the installed binary answers the new query, and only then switch `claim_state.py` and `delegate.md` onto it. A typed capability fallback was considered and rejected: once the engine is installed the fallback has no consumer, and permanent surface for a transitional state is exactly the speculative API this plan avoids elsewhere.

**Files:**
- `crates/cargo-berth/src/cli.rs` — the `--reservation` selector on `board`, its `--json` requirement, and the conversion into `BoardOutputSelection`
...
- `/Users/natemccoy/.claude/scripts/berth/claim_state.py` — the `reservation` entry point and validator (~L2325)
- `/Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` — the external coordinator fixtures Phase 12 created, extended with this entry point's known-id and unknown-id cases
- `/Users/natemccoy/.claude/commands/plan/delegate.md` — use it after a lost release reply (~L1765–L1799)
- `docs/cargo-berth/json-contract.md` — the new payload

**Constraints from prior phases:** Phase 1 made `edit_blocking_status` computed and `Released` terminal, so `ReleasedAfterCheckpoint` and `ReleasedWithoutCheckpoint` are genuinely terminal states here. `Reservation::evidence_state` already supplies the four classifications this phase projects — do not duplicate the lifecycle match. Phase 5's lost-evidence alert directs the reader to plain `board --json`, **not** to this selector, precisely so Phase 5 does not depend on this phase; do not retarget that alert text here. Phase 12 established that front ends render typed payload fields without parsing `message` — the coordinator validator follows the same rule, and it created `tests/test_hook_rendering.py`, the external fixture harness this phase extends rather than replaces. **Phase 12 also proved the installed engine and the worktree build have diverged**: an older `cargo-berth` cannot replay a journal a fresh build wrote, which is why the install step above is part of this phase's work rather than an afterthought. Phase 14's canonical-shim decision covers the same engine/consumer pairing; settling it does not remove this phase's obligation to install before switching the coordinator over.

**Acceptance gate:** **Every `Test` command in Delegation Context** green. The timing- and behavior-bearing checks run against the freshly installed binary, not whatever was previously on `PATH`, and the phase proves the install landed before the coordinator switch: the installed binary answers `board --reservation <id> --json` for a known id and a typed unknown-id rejection for an absent one, both asserted from the payload alone with `message` unread, in `tests/test_hook_rendering.py`. Fixtures cover a waiting successor and both unresolved-overlap endpoints — all three omitted from board rows, all three resolvable by id. Existing board JSON stays byte-compatible. `--reservation` without `--json` is rejected at the command line, and a fixture proves `BoardOutputSelection::ReservationLifecycleFor` never reaches the TUI rendering path. The coordinator entry point returns exit 0 for a known id and a typed invalid-input reason for an unknown one. **That reason is named here so Phase 14 has a deterministic contract to generate:** `ReservationLifecycleQueryRejection::UnknownReservation { reservation_id: ReservationId }`, emitted as status `invalid_input`, exit code 5, payload `kind` `reservation`, payload `data.status` `unknown_reservation`, carrying `reservation_id`; a fixture asserts every one of those from the payload alone with `message` unread.
...
2. Rust consumers match **exhaustively, with no wildcard arms** — remove the one at `output.rs:1471`.
...
**Files:**
- `crates/cargo-berth/src/output.rs` — the declaration table, exhaustive matches, no wildcard; the typed replay-failure payload and `ReplayFailureSubject` (`OutputEnvelope` L73, `CommandVerb::Identity` L115, `OutputStatus` L195-197, `OutputFacts` L244-246, integration's nested rejection L383, the wildcard arm L1471)
...
- `crates/cargo-berth/src/reservation/mod.rs` — `ReservationReplayError` (L1913) surfaces its reservation and reason to the payload, generated exhaustively rather than variant by variant
- `crates/cargo-berth/src/ledger/journal.rs` — the journal operation inventory the contract enumerates, including Phase 3's `ScopedPatchEquivalenceChecked` (L404) and `ScopedPatchComparisonAttempted` (L415), plus `CanonicalWorktreeRoot` (L1013), whose schema this contract must pin
- `crates/cargo-berth/Cargo.toml` — `schemars` as a normal dependency; the DTOs derive `JsonSchema`
- `/Users/natemccoy/.claude/scripts/berth/claim_state.py` — generated tables replace the hand-kept ones (~L63, ~L105)

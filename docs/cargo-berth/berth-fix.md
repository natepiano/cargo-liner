...
- **Key files:**
  ...
  - `docs/cargo-berth/json-contract.md` — the stable JSON wire contract for **engine** envelopes and journal records. It does **not** document `cargo-berth-claim-state/v1`, which is declared only at `claim_state.py:46` and whose consumers are `~/.claude/commands/sync.md` and `~/.claude/commands/plan/delegate.md`.
  ...
  - `/Users/natemccoy/.claude/scripts/berth/claim_state.py` — Python coordinator, 3,978 lines. Twenty-four names are imported from `berth.generated.status_payload_tables` at `:22–:44` — nine aliases, five closed sets, four table lookups, five guards, and `valid_outcome_tuple`; no hand-copied literal set remains. The envelope boundary is split: `ValidatedWireEnvelopePayloadValue` (`:236`) echoes the payload with raw `alerts: list[dict[str, object]]`, `ValidatedWireEnvelopeValue` (`:244`) wraps it, and `CoordinatorEnvelopeReadingValue` (`:1180`) carries the semantic reading — typed alerts plus a `CoordinatorPayloadReadingValue` (`:1161`). `ValidatedEnvelope` (`:665`) owns both as `value` and `reading`, with `.payload` (`:684`) and `.alerts` (`:688`) accessors; `EnvelopePayload` no longer exists. `EnvelopeAlertValue` (`:233`) unions `LostIntegrationEvidenceAlertValue` (`:139`) and `OrphanedOutstandingAlertValue` (`:222`), built by `_envelope_alert_value` (`:1472`) and `_wire_and_semantic_envelope_alert_values` (`:1489`). `BlockedCheckPayloadReadingValue` (`:991`) and `BlockedClaimPayloadReadingValue` (`:1000`) are distinct, with `BlockedEditPayloadReadingValue` (`:1008`) as their union alias. `NoPayloadFactsValue` (`:979`) and `DeliberatelyUninspectedPayloadValue` (`:984`) are the two absence readings. `BoardRenderingFieldValue` (`:1130`) and `BoardRenderingValue` (`:1146`) are the typed board seam. `ReservationLifecycleValue` (`:322`), `UnknownReservationLifecycleValue` (`:338`), `ProposalAwaitingApprovalStateValue` (`:569`), `RenderedCoordinationIdentityRecoveryActionValue` (`:819`) with its `kind: str` at `:822`, `ReplayFailureValue` (`:938`) and the `ReplayFailure` dataclass (`:947`). Four `tagged()` erasure points: `:701`, `:834`, `:886`, `:952`. `parse_envelope` (`:1503`) applies `valid_outcome_tuple` (`:1564`). Classifiers all returning `dict[str, object]`: `classify_claim` (`:2258`), `classify_check` (`:2451`), `_validate_board` (`:2527`), `render_board` (`:2581`), `coordinator_state` (`:2862`), whose generic `invalid_input` branch (`:2890`) must stay below `_coordination_identity_classification` (`:3511`) and `_foreign_actor_incursion_resolution_classification` (`:3536`); `_replay_failure_classification` (`:3523`) is called from `:2266`, `:2470`, `:2528`, and `:2868`. `_require_object` (`:1191`) and `_require_list` (`:1208`) are the existing boundary readers. `installed_engine_binary` (`:2656`); `reservation_lifecycle_state` (`:2797`); `CoordinatorRequest` (`:3652`) and `CoordinatorArguments` (`:3662`) converted by `_coordinator_request` (`:3740`); `main` (`:3851`) assigns `command_to_rerun` after classification at `:3901` and `:3961`. Twenty-nine `cast` calls remain.
  ...
  - `/Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` — the only external oracle for the three shims, 6,541 lines of stdlib `unittest`. `HookRenderingTests` (`:782`) holds 27 gated fixtures; `HookTimingTests` (`:6510`) holds the deliberately red timing measurement and is invoked separately. `run_installed_engine_hook` (`:929`) drives the installed engine but hardcodes its payload through `post_bash_payload_for` (`:3224`). Run as `python3 ~/.claude/scripts/berth/tests/test_hook_rendering.py HookRenderingTests`. `pytest` is not installed on this machine.
...
### Phase 22 — Typed alert ownership and the invocation-local refusals  · status: done

#### As-built

Alert ownership is settled as **hook-owned, deliberately**, and enforced by
construction rather than by convention. `berth_post_bash.sh` and
`berth_session_start.sh` render the lost-integration-evidence alert,
`berth_session_start.sh` renders the orphaned-outstanding alert, and the Python
coordinator has no stake in either. Each alert member says so in its own
docstring, and `test_every_envelope_alert_member_has_a_hook_ownership_contract`
holds the pair together: it enumerates the union with `get_args`, asserts each
member's docstring names its rendering hooks, walks the `claim_state` module AST
and asserts the only callables reading `.alerts` are `parse_envelope` and the
`ValidatedEnvelope.alerts` property, and asserts no `coordinator_outcome` symbol
exists. Adding a production reader anywhere in the module fails the test.

The orphan alert is proved where a person meets it: real orphan board results are
driven through SessionStart and the fixtures assert the `ORPHANED OUTSTANDING`
heading together with the recover, retire-orphan, and abandon commands carrying
real reservation identifiers, in both the recoverable and unavailable variants.

The coordinator's contradictory-proposal refusal is proved at the command line
against the real engine. A transparent proxy forwards argv to the installed
engine, rewrites only the authorization answer, and returns the engine's own
envelope and exit status, so the assertion that the ledger is unchanged runs
against a process that could have changed it.

`EnvelopePayload` no longer exists; `ValidatedEnvelope` owns `value` and
`reading` alone.

**Files:**
- `/Users/natemccoy/.claude/scripts/berth/claim_state.py` — the two alert
  docstrings carrying their ownership contract; no production alert reader
- `/Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` — the AST
  ownership guard, the orphan user-surface fixtures, and the transparent-proxy
  refusal proof; 27 gated fixtures

**Binds later work:** ownership is settled and structurally enforced — do not
reintroduce a coordinator-owned alert path, and do not add an `alerts` key to any
emitted payload. Whenever an assertion depends on the engine actually acting,
drive the real engine through the transparent proxy rather than a stub.

**Gotchas:** a structural guard written as a source-string check covers only the
one callable it names; the AST walk is what makes it enforce the whole module.
These files live at `~/.claude/scripts/berth/` and can never join a `cargo-berth`
checkpoint commit — diff them against a pre-edit snapshot, never against `HEAD`.

**Ruled out:** coordinator-owned alerts carried on an `alerts` key added to every
emitted payload — the key reached no reader, and the outcome type wrapping it was
a pass-through carrying `dict[str, object]` built by two new double casts. A stub
engine for the refusal proof — a stub that cannot write a ledger makes a
ledger-non-mutation assertion vacuous. A `getsource(main)` string guard — it sees
only direct reads inside `main`.
...
`classify_claim` (`claim_state.py:2258`), `classify_check` (`:2451`), `_validate_board` (`:2527`), `render_board` (`:2581`), and `coordinator_state` (`:2862`) all return `dict[str, object]`, and **29 `cast` calls** remain in the file — ten in the double-cast classifier returns, nine in board traversal, four at dispatch, and six at parsing and literal boundaries. Every tagged union the coordinator builds is erased at the one boundary a reader inspects. The weakness predates the coordinator work — `classify_claim` returned `dict[str, Any]` at `831e34a` — and repairing only one symbol would leave an inconsistent surface. Not every classifier reaches `dict[str, object]` through `cast`; all five reach it, and the return annotation is the defect regardless of how it is reached.

Name the returns rather than leaving the implementer to invent them: `classify_claim` returns `ClaimClassificationValue`, `classify_check` returns `CheckClassificationValue`, and `coordinator_state` returns **`CoordinatorEngineOutcomeValue`** — the coordinator's reading of the outcome of one engine invocation. It is deliberately not `EngineInvocationStateValue`: it excludes argv, exit status, and stderr, so a name promising the invocation's state would overpromise. Name the single-attempt invocation record `SingleAttemptEngineInvocationValue` and the typed replay hard stop `ReplayHardStopStateValue`. **Every remaining type this phase needs is named here rather than left to the implementer:** the foreign-resolution outcome is `ForeignActorRecordedIncursionResolutionStateValue` — its own state, never folded into a neighbour; the contention pair is `LedgerContentionStateValue` and `RetryReadyLedgerContentionStateValue`; the generic no-facts ledger reading is `LedgerUnreadableWithoutReplayFactsStateValue`; and the replay operator route is `ReplayJournalRecoveryRouteValue`. Every rendered member is named `Rendered` followed by its existing semantic member name.

**The board pipeline is two functions with two different jobs, and today's Work Order conflated them.** A ready-only value cannot represent replay, contention, unconfigured, or unreadable, so a single board return cannot be both the ready rendering and the hard stop. Rename `_validate_board` to **`_classify_board_availability`**, returning a **`BoardAvailabilityState`** whose ready member carries the `BoardRenderingValue` the preceding phases already produce and whose remaining members carry the replay, contention, unconfigured, and unreadable outcomes. `render_board` then takes that state — `render_board(BoardAvailabilityState) -> BoardPresentationStateValue` — so the availability question is answered once, in the classifier, and the renderer never re-derives it. **`coordinator_state` is the only classifier to retype here, not two.** Phase 13 renamed `_generic_state` to the public `coordinator_state`; there is no longer a private twin, and any instruction to retype both names a symbol that no longer exists. Phase 13 also made `installed_engine_binary` (`:2656`) public; `installed_engine_binary() -> str` is already precisely typed and needs no work here.

**Every `tagged()` conversion is an erasure point and all of them close here.** A tagged type erased at its own conversion is the same defect as an untyped classifier, so each gets an explicit typed member in every affected return union rather than a `dict[str, object]`: `EngineInvocation.tagged()` (`:701`), `CoordinationIdentityRejection.tagged()` (`:834`), `ForeignActorIncursionResolution.tagged()` (`:886`), and `ReplayFailure.tagged()` (`:952`) — the last as an explicit `ReplayHardStopStateValue` member.

**The busy-state mutation must not be smuggled into the new unions.** `main` adds `command_to_rerun` to an already-classified contention state after classification returns (`:3901` and `:3961`), so a union member written to match today's flow would need an optional field that is present in one caller and absent in another — the precise shape the type contract forbids. Introduce **two distinct types**: `LedgerContentionStateValue`, the contention state as classified, and `RetryReadyLedgerContentionStateValue`, which carries the command to rerun; or construct the complete state before returning it, so no post-return mutation exists. `NotRequired`, `None`, and `dict[str, object]` are forbidden as the workaround here. Raw mappings remain permitted only at the parsing and emission boundaries.

**`render_board` must not silently change the coordinator's published contract.** The preceding phases produced `BoardRenderingValue` carrying typed fields, paths, and `{path, rendered_json}` actions, while `render_board` today traverses the wire envelope (`:2581`) and returns `{path, value}` actions. Naming its return while it still reads an untyped mapping to build that value moves the erasure inward rather than removing it — so `render_board` takes the `BoardAvailabilityState` produced by `_classify_board_availability` and reads no raw mapping. But the conversion is a refactor, not a redesign: the emitted `cargo-berth-claim-state/v1` JSON must stay equivalent — same markdown order, same paths, same action values — and a parity fixture proves it against the pre-change output rather than trusting the reading of the code.

**Board output is not the only published surface, and `json-contract.md` is not where this contract is written.** That document specifies **engine** envelopes; `cargo-berth-claim-state/v1` is declared only at `claim_state.py:46`, and its consumers are `~/.claude/commands/sync.md` and `~/.claude/commands/plan/delegate.md`. Every state this phase retypes is emitted across that boundary, so the parity fixtures cover the command-line output of **each** of them — identity, replay, invalid input, contention with a rerun command, reservation, claim, and check — and not board output alone.

**One tag Phase 12 left open closes here.** `RenderedCoordinationIdentityRecoveryActionValue` (`:819`) types its `kind` as `str` (`:822`) where the four recovery-action tags are closed literals. That was implementation-only while `tagged()` erased the whole value to `dict[str, object]` — nothing downstream could match on it. Removing that erasure turns the tag into externally typed domain state, so it becomes a rendered union with **one member per recovery action**, and likewise one rendered type per identity rejection, rather than a single type with a widened tag.

**The invalid-input reading is an exhaustive match, and it has two distinct absence states, not one.** It covers Phase 11's three identity rejections, Phase 7's `already_recorded_by_different_coordination_actor`, **Phase 13's `ReservationLifecycleQueryRejection::UnknownReservation` as its own `UnknownReservationLifecycleQueryRejectionValue` member** (`payload.kind = reservation`, `data.status = unknown_reservation`), and a residual member. **That member is a wire-rejection reading and must stay distinguishable from the normalized coordinator state Phase 13 already ships.** `UnknownReservationLifecycleValue` (`:338`) is the *normalized* form the `reservation` operation returns, keyed `kind = unknown_reservation`; the envelope union's member is the *wire* rejection. They carry the same fact in two roles. Keep Phase 13's normalized type under its existing name, give the wire member the query-specific rejection name, and convert between them at exactly one explicit site — inside `reservation_lifecycle_state` and nowhere else.

The residual branch is **not** a single collapsed member. Invalid input can carry the reading where the envelope explicitly has no payload facts, or the reading where the coordinator chose not to inspect a payload domain. Collapsing them erases a distinction the preceding phase audited and shipped.

**But two structurally identical output `TypedDict`s cannot stay distinguishable, so the distinction lives in the domain and not in the emitted shape.** Today's `NoPayloadFactsValue` and `DeliberatelyUninspectedPayloadValue` carry the same members, so nothing — not the type checker, not an exhaustive match — can tell them apart as output types, while the emitted JSON must stay `{kind: "invalid_input", diagnostic: ...}` for both. Introduce nominal domain states **`InvalidInputWithoutPayloadFacts`** and **`InvalidInputWithUninspectedPayload`**, keep the two readings distinct there, and convert both through **one typed emission boundary** that produces the existing JSON. The `NotRequired` on raw wire `payload.data` stays as it is: that is legitimate optionality at a parsing boundary, not a domain type dodging a decision. The typed identity, foreign-resolution, and unknown-reservation branches all ignore `message`; **the residual branch keeps its message-backed diagnostic**, precisely because no semantic payload facts exist there and the diagnostic string is the only thing that does.

**The `ledger_unreadable` reading carries the typed replay failure, and its generic branch means no facts.** `_replay_failure_classification` (`:3523`) returns a `ReplayFailure` (`:947`) reached through `.tagged()` and is called from all four classification entry points — `classify_claim` (`:2266`), `classify_check` (`:2470`), `_classify_board_availability` (`:2528`), and `coordinator_state` (`:2868`) — ahead of each one's generic `ledger_unreadable` branch. It keys on `envelope.status == "ledger_unreadable"` with `payload.kind == "replay_failure"`, so it belongs to the `ledger_unreadable` reading rather than the invalid-input union, and its `ReplayFailureValue` (`:938`) already exists — lift it rather than rebuilding it, and name its operator route `ReplayJournalRecoveryRouteValue`. The generic branch beneath it carries **no payload facts**: `parse_envelope` (`:1503`) applies `valid_outcome_tuple` (`:1564`) before anything else, so a malformed replay payload never crosses validation and can never reach that branch. Type it as `LedgerUnreadableWithoutReplayFactsStateValue`, and keep it below the typed classifier for the same ordering reason the invalid-input branch must.

**Success-tagged lookalikes are rejected at the boundary and must stay that way.** `parse_envelope` rejects a success-tagged envelope carrying a rejection-only nested status before any classifier runs, and the existing identity fixture and the foreign-resolution test (`tests/test_hook_rendering.py:2709` and `:2970`) already expect `EnvelopeValidationError`. Do **not** require these to reach a caller as ordinary engine responses, and do **not** construct an impossible `ValidatedEnvelope` merely to exercise classifier ordering — an exhaustive match over a union whose invalid members cannot be built is the property to preserve, not a fixture that manufactures one.

**Phase 7 left the generic `invalid_input` branch order-sensitive and that ordering is a correctness constraint, not a style preference.** `sequence` and `integrate` identity rejections arrive with status `invalid_input`, so the generic `invalid_input` early return (its generic branch at `:2890`) must stay *below* the two typed classifiers (`:3511` and `:3536`); placed above them, the typed identity states are silently erased into a diagnostic string. A tagged union whose members are matched exhaustively removes the hazard rather than preserving the ordering — prefer that, and do not ship a version that merely re-encodes the current sequence of `if` returns.

**The hook states the session-id contract the engine now enforces.** Phase 18 made the PostToolUse payload parser reject a `session_id` that is overlong or carries control characters, where it previously rejected only an empty one, and `berth_post_bash.sh:422` still tells the user only that `session_id` must be non-empty. Correct that message to state the real contract — 1 to 256 characters, no control characters — and add exact rendering tests for both rejections. The hook does not revalidate an already-typed id; it renders the engine's rejection. `run_installed_engine_hook` (`tests/test_hook_rendering.py:929`) exists but hardcodes its payload through `post_bash_payload_for`, so it cannot express these cases — extend it to take an explicit session id, and drive the 256-character multibyte, 257-character, and control-character cases through it.

**The final cast budget is exact and the acceptance test enforces it.** Of today's 29, the ten double-cast classifier returns and the four dispatch casts disappear with the typed returns, and the nine board-traversal casts are centralized through the existing `_require_object` (`:1191`) and `_require_list` (`:1208`) readers rather than surviving as parse-time board conversions. **Six named boundary casts remain and no others** — the six at parsing and literal boundaries. Name that exact set in the test, so a seventh cast fails the phase and a removed one has to be removed from the list deliberately.

**Implement in this order.** The four stages share the same two Python files and one emission boundary, so a different order leaves a mixed intermediate contract that every later stage has to work around:

1. Freeze the coordinator's current CLI outputs as parity fixtures, and define the domain-to-emission types.
2. Retype the classifiers, the four `tagged()` conversions, and the contention pair.
3. Replace the board pipeline — `_classify_board_availability` then `render_board`.
4. Change the hook text and run the installed-engine fixtures.

**This stays one phase.** Splitting it would publish an intermediate `cargo-berth-claim-state/v1` that neither the old nor the new consumers match.
...
All five classifiers and all four `tagged()` methods carry exact named return annotations, no `dict[str, object]` among them. No `cast` stands between a tagged value and its return, and none remains in state dispatch. **Exactly six casts remain in `claim_state.py`**, each named in the test by the boundary it sits at, so a seventh fails the phase. No file-level type ignore exists in any gated file, and every remaining `# pyright: ignore` is line-level with a named rule.

A fixture carrying Phase 13's `unknown_reservation` rejection reaches a caller as `UnknownReservationLifecycleQueryRejectionValue`, not as a residual member, and the conversion to the normalized form is proved to occur only inside `reservation_lifecycle_state`. The residual invalid-input branch is proved to keep `InvalidInputWithoutPayloadFacts` and `InvalidInputWithUninspectedPayload` as distinct domain states converted through one typed emission boundary, with its message-backed diagnostic intact, while the typed identity, foreign-resolution, and unknown-reservation branches are proved to read no `message`.
...
`_classify_board_availability` returns a `BoardAvailabilityState` covering ready, replay, contention, unconfigured, and unreadable, and `render_board` consumes that state and reads no raw mapping, proved by a test that fails if the wire envelope is reachable from its body. Parity fixtures prove the emitted `cargo-berth-claim-state/v1` JSON is equivalent to the pre-change output — same markdown order, same paths, same action values — for the command line of **every** retyped state: identity, replay, invalid input, contention carrying a rerun command, reservation, claim, and check, not board output alone.

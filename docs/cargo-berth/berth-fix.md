...
### Phase 19 — Acceptance proof for the Phase 18 semantic states  · status: todo

#### Work Order

**Goal:** Every semantic state Phase 18 introduced is constructed by a test that proves what a caller or user observes when it occurs.

**Spec:**

Phase 18 replaced representation-level types with semantic ones and left three states with no test that constructs them. Each is a real reachable state, so each gets one acceptance test named below. This phase adds tests and, where a test cannot reach a state through the public path, the smallest seam that lets it — never a behavior change.

`EnvironmentCoordinationRunSelection::UnusableFallbackToMarker` (`ledger/mod.rs`) is never constructed. `edit_authorization_prefers_environment_then_marker_then_unidentified` covers only `Identified` and `NotSupplied`, so nothing proves that an environment variable holding an unusable coordination run falls back to the worktree marker rather than being treated as absent, rejected, or silently accepted. Add `unusable_environment_coordination_run_falls_back_to_marker_then_unidentified`: set the environment value to text that is not a valid coordination run, assert the resolved authorization is the marker's when a marker exists, and assert it is unidentified when none does.

`WorktreeComparability` (`drift/execution.rs`) ships three states and its sole consumer deliberately normalizes `IdentityUnavailable` and `DeferredPendingRewrite` to the same unchanged, empty drift report. Nothing proves either branch directly, and nothing proves the two normalize identically — which is the property a reader has to take on faith today. Add one test per state: `drift_reports_no_change_when_worktree_identity_is_unavailable` and `drift_reports_no_change_while_a_rewrite_is_pending`, each asserting the same unchanged, empty report, so widening one branch without the other fails.

`HarnessSessionId` at the PostToolUse payload boundary (`cli.rs`, the payload parser) changed an application-observable behavior: the parser now parses `session_id` into `HarnessSessionId` and returns `PostToolUseDriftInvocationError::InvalidPayload` for a value that is overlong or carries control characters, where the previous emptiness-only filter accepted both. That rejection reaches a user through the hook. Prove the Rust half here with two tests — `overlong_harness_session_id_is_an_invalid_payload` and `control_character_harness_session_id_is_an_invalid_payload` — each asserting `InvalidPayload` and each naming the boundary contract it proves: 1 to 256 characters, no control characters. The hook's own message is Phase 21's, not this phase's; do not edit any file under `/Users/natemccoy/.claude` here.

**Files:**
- `crates/cargo-berth/src/ledger/mod.rs` — `EnvironmentCoordinationRunSelection` and `EditAuthorization::resolve_from_sources`, plus the existing precedence test this phase extends
- `crates/cargo-berth/src/drift/execution.rs` — `WorktreeComparability` and the consumer that normalizes two of its states
- `crates/cargo-berth/src/cli.rs` — the PostToolUse payload parser that produces `HarnessSessionId`
- `crates/cargo-berth/src/session/mod.rs` — `HarnessSessionId`'s parse contract, read here as the source of the 1-to-256-character no-control-character rule

**Constraints from prior phases:** Phase 18 shipped `EnvironmentCoordinationRunSelection::{NotSupplied, UnusableFallbackToMarker, Identified(CoordinationRunId)}`, `WorktreeComparability::{Comparable(WorktreeId), IdentityUnavailable, DeferredPendingRewrite}`, and `HarnessSessionId` as `pub(crate)` transported through `PostToolUseDriftInvocation`, its `OnceLock`, and `session::select_current_process_harness_session` rather than as a bare `String`; `from_current_process` no longer re-parses the stored value. Phase 18 also shipped `FilesystemReferenceResolution::{Resolved(GitObjectId), RequiresGitResolution { rejection_if_git_reports_missing }}` in `verb/claim.rs` — two states, not three; the rejected case is carried by the field the producer chooses, so no caller inspects a payload to decide. Phase 11 owns identity concepts: do not restate them in this phase's tests. Phase 17's `git::reference_lookup` returns `Missing` only on exit 2 and propagates every other outcome as `GitError`; a test written here must not collapse that distinction.

**Acceptance gate:** `bash ~/.claude/scripts/delegate/verify.sh test cargo-berth` and `bash ~/.claude/scripts/delegate/verify.sh lint cargo-berth` both pass. The five named tests exist and fail when their state's behavior is changed: reverting the marker fallback fails the first, normalizing only one `WorktreeComparability` branch fails one of the next two, and relaxing the session-id contract back to an emptiness check fails the last two. No file outside `crates/cargo-berth/` is modified.

### Phase 20 — Generated semantic discriminators for the Python consumer  · status: todo

#### Work Order

**Goal:** The generated Python contract exports narrowing types, so the coordinator's discriminants can be typed against the engine's own closed sets instead of against `str`.

**Spec:**

Phase 21 must eliminate `EnvelopeValue.verb: str`, `EnvelopeValue.status: str`, and `EnvelopePayload.kind: str`. It cannot: `scripts/berth/generated/status_payload_tables.py` exports only `frozenset[str]`, which basedpyright cannot use to narrow anything, and hand-copying `Literal` sets into `claim_state.py` would violate Phase 14's single-generated-contract rule by creating a second, unchecked copy of the closed sets. The fix belongs in the generator, which is this phase.

Extend the Rust contract emitter so the generated Python module exports, alongside the existing frozensets: a semantic `Literal` alias per closed set — the verbs, the statuses, and the payload kinds — named for what the set is rather than for its representation; and one boundary `TypeGuard` per alias that narrows an arbitrary `str` (or `object`) to it by testing membership in the corresponding frozenset, so a decoded JSON value is narrowed once at the parse boundary and stays narrowed. The frozensets remain the single source: each alias and each guard is derived from the same contract data in the same generation pass, never restated by hand.

Regenerate with `CARGO_BERTH_REGENERATE_OUTPUT_CONTRACT`, and let the existing byte-compare test that renames a Rust type and asserts every generated artifact is unchanged confirm that wire names, not Rust identifiers, still drive the output.

This phase checkpoints and then installs the engine from that checkpoint. Phase 21 consumes the generated module and must not run against an uncommitted or uninstalled generator: an artifact regenerated from a dirty tree cannot be reproduced, and Phase 21's own digest guard refuses a measurement whose generated consumers change mid-run.

**Files:**
- `crates/cargo-berth/src/output_contract.rs` — the contract emitter, at `:1288` where the generated Python module is written
- `docs/cargo-berth/json-contract.md` — the checked JSON contract, updated to describe the exported aliases and guards
- `/Users/natemccoy/.claude/scripts/berth/generated/status_payload_tables.py` — the generated module, regenerated rather than edited

**Constraints from prior phases:** Phase 14 generated `STATUS_PAYLOAD_KINDS` and `FIXED_STATUS_EXIT_CODES` from the engine contract and pinned every `schemars` definition name to its wire name rather than the Rust identifier, with a test that renames a Rust type and asserts every generated artifact is byte-identical — the aliases and guards added here are bound by the same rule and must key on wire names. Phase 14 added `OutputStatus::LegacyHookOutdated` as its own wire status, so the status set is not what an older reading of this file assumed. The generated module lives in the `/Users/natemccoy/.claude` Git checkout and cannot join a `cargo-berth` checkpoint commit; the checkpoint here covers the Rust emitter and the JSON contract, and the regenerated Python module is committed in that other checkout.

**Acceptance gate:** `bash ~/.claude/scripts/delegate/verify.sh test cargo-berth` and `bash ~/.claude/scripts/delegate/verify.sh lint cargo-berth` both pass, including the artifact byte-compare test. Regeneration is idempotent: a second run with `CARGO_BERTH_REGENERATE_OUTPUT_CONTRACT` leaves every artifact byte-identical. `basedpyright`, run with `/Users/natemccoy/.claude` as the working directory, reports zero errors, zero warnings, and zero notes for the regenerated module. A scratch check proves narrowing actually works: a `str` passed through each exported guard is accepted where its alias is required, and a value outside the set is rejected by the guard at runtime and by the type checker at the use site.

### Phase 21 — Coordinator tagged unions and hook rendering  · status: todo

#### Work Order

**Goal:** The Python coordinator returns tagged unions instead of `dict[str, object]` reached through `cast`, and the hook states the session-id contract it now enforces.
...
**The hook states the session-id contract the engine now enforces.** Phase 18 made the PostToolUse payload parser reject a `session_id` that is overlong or carries control characters, where it previously rejected only an empty one, and `berth_post_bash.sh:421` still tells the user only that `session_id` must be non-empty. Correct that message to state the real contract — 1 to 256 characters, no control characters — and add an exact rendering test for both the overlong and the control-character rejection. The hook does not revalidate an already-typed id; it renders the engine's rejection.

**Phase 13 already built part of this surface — lift or rename it, never recreate it.** `ReleaseDispositionValue` (`claim_state.py:129`) is the four-member `Literal`-tagged union over `integrated`, `rewritten_integration`, `abandoned`, and `retired_orphan`, with `integrated` correctly carrying no `evidence` member; `ReservationLifecycleValue` (`:165`) is the four-member lifecycle union; `ReservationLifecycleQueryStateValue` (`:188`) is the normalized query state; and `reservation_lifecycle_state` (`:2105`) is the strict validator that produces it, with `_release_disposition` (`:2028`) beneath it. All of that already satisfies this phase's contract. **What is left untyped, and is this phase's actual work:** `EnvelopePayload.data: NotRequired[dict[str, object]]` (`:87`) and `EnvelopePayload.alerts: list[object]` (`:86`); the three envelope discriminants; the five classifier returns above; the four `tagged()` conversions and both `expected_verb` APIs; `RenderedCoordinationIdentityRecoveryActionValue.kind: str` (`:614`); `ProposalAwaitingApprovalStateValue.proposal` (`:374`); and every `cast` standing between a tagged value and its return or dispatch.

**This phase edits a dirty external checkout, so it establishes its own baseline before touching anything.** `/Users/natemccoy/.claude` is itself a Git checkout and is dirty from earlier phases, so an ordinary `git diff` there shows several phases' work as one. Snapshot `scripts/berth/claim_state.py`, `scripts/berth/tests/test_hook_rendering.py`, `scripts/berth/generated/status_payload_tables.py`, and `scripts/berth/install/hooks/berth_post_bash.sh` before any edit, and diff against that snapshot rather than against `HEAD`. Run project-root tooling from that root — `~/.claude/pyrightconfig.json` supplies the `executionEnvironments` the type check depends on, and running basedpyright from anywhere else silently applies different settings. Those files cannot join a `cargo-berth` checkpoint commit; the existing commit boundary is unchanged. Invoke `--install-engine` only from a checkpointed tree and only when a measurement is intentionally requested — Phase 20's install is the one this phase consumes.
...
**Files:**
- `/Users/natemccoy/.claude/scripts/berth/claim_state.py` — the five classifiers, the three envelope discriminants, the envelope `data` and `alerts` members, the four `tagged()` conversions, both `expected_verb` APIs, the recovery-action tag, and the proposal type
- `/Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` — created by Phase 12, extended by Phases 13, 14, 16, and 17 to seventeen tests, including typed replay routing, unreadable-validator diagnostics, and installer rollback ordering; extended here with the classifier fixtures this phase's gate names, and split so the deliberately red timing method lives in its own separately invocable timing class
- `/Users/natemccoy/.claude/scripts/berth/install/hooks/berth_post_bash.sh` — the session-id rejection message at `:421`
- `/Users/natemccoy/.claude/scripts/berth/generated/status_payload_tables.py` — read-only input: Phase 20's aliases and guards
- `crates/cargo-berth/src/answer/proposal.rs` — read-only input at `:79`: the complete shape behind `ValidatedOverlapProposalValue`
- `crates/cargo-berth/src/alert.rs` — read-only input at `:29`: the complete alert enumeration behind `EnvelopeAlertValue`

**Constraints from prior phases:** Phase 20 shipped the generated semantic `Literal` aliases and boundary `TypeGuard`s in `berth.generated.status_payload_tables`, checkpointed, and installed the engine from that checkpoint — this phase consumes them and must not regenerate or hand-edit that module. Phase 19 proved the Rust half of the session-id contract; the hook message and its rendering tests are this phase's half, and the hook does not revalidate an already-typed id. Phase 12 already added typed rendering of `CoordinationIdentityRejection` and its `recovery_actions` to this file, and Phase 13 added the `reservation` entry point, its strict validator `reservation_lifecycle_state`, the four-member `ReleaseDispositionValue`, the four-member `ReservationLifecycleValue`, the normalized `ReservationLifecycleQueryStateValue`, and the public `coordinator_state` and `installed_engine_binary`. Phase 13's value types are already correct — lift or rename them, never rebuild them. Phase 14 generated `STATUS_PAYLOAD_KINDS` and `FIXED_STATUS_EXIT_CODES` from the engine contract — they are imported from `berth.generated.status_payload_tables` (`claim_state.py:22`, `:27`) and are inputs here, not hand-edited targets. **Phase 14 also added a sixth classifier this phase must type, and it does not arrive under `invalid_input`.** `_replay_failure_classification` (`claim_state.py:2375`) returns a `ReplayFailure` (`:739`) reached through `.tagged()`, and it is called from all four classification entry points — `classify_claim` (`:1493`), `classify_check` (`:1733`), `_validate_board` (`:1801`), and `coordinator_state` (`:2196`) — ahead of each one's generic `ledger_unreadable` branch. It keys on `envelope.status == "ledger_unreadable"` with `payload.kind == "replay_failure"`, so it belongs to the `ledger_unreadable` reading rather than the `invalid_input` union enumerated above, and its `ReplayFailureValue` (`:730`) already exists — lift it into the typed surface rather than rebuilding it. The generic `ledger_unreadable` branch that remains beneath it in each of those four functions is still reachable for an untyped replay failure and must stay below the typed classifier for the same ordering reason the `invalid_input` branch must. **Phase 18 shipped the reftable fallback's final shape on the Rust side and it is deliberately invisible here.** `ClaimError::GitBackedReferenceRequired` was deleted in Phase 17; the signal is now the private `FilesystemReferenceResolution::RequiresGitResolution { rejection_if_git_reports_missing }` inside the filesystem reference reader, and a genuine Git failure still arrives as `ClaimError::Git(GitError)`. Do not invent a payload member, wire status, or union variant for it. **Phase 17 also shipped four harness guards in `test_hook_rendering.py`** that this phase preserves rather than rediscovers: the exit-20/21/22 protocol, the artifact-digest guard that refuses a measurement whose engine or generated consumers change mid-run, the unresolved-attribution reporting in the timing summary, and a `setUpClass` that refuses to install and directs the operator to the explicit `--install-engine /path/to/cargo-liner` command. Phase 14 further added `OutputStatus::LegacyHookOutdated` as its own wire status, so `ledger_unreadable` is no longer the catch-all it was when this Work Order was written. **Phase 7 left the generic `invalid_input` branch order-sensitive and that ordering is a correctness constraint, not a style preference.** `sequence` and `integrate` identity rejections arrive with status `invalid_input`, so the generic `invalid_input` early return (`coordinator_state` at `claim_state.py:2190`, its generic branch at `:2218`) must stay *below* the two typed classifiers; placed above them, the typed identity states are silently erased into a diagnostic string. **`_inactive_identity_classification` no longer exists** — Phase 12 replaced it with `_coordination_identity_classification` (`:2349`) and `_foreign_actor_incursion_resolution_classification` (`:2422`); do not key any edit or call-site sweep on the old name. **Phase 12 already replaced the obsolete `inactive_session_mapping` / `inactive_marker_run` pair with a semantic `CoordinationIdentityRejectionValue` covering Phase 11's three variants in both their wire placements, plus the matching recovery-action union** — lift **both** of its value types into this phase's exhaustive envelope union rather than recreating them — `CoordinationIdentityRejectionValue` (`claim_state.py:604`) covering the three identity rejections in both wire placements, and `ForeignActorIncursionResolutionValue` (`:660`) covering the foreign-resolver rejection — and treat this part of the identity work as already satisfied. **Phase 12 also bound both classifiers to the envelope's own status**, refusing any envelope whose status is not `invalid_input`, which is the property this phase's exhaustive match must preserve rather than rediscover: a success-tagged envelope carrying a lookalike sub-status stays an ordinary engine response. A tagged union whose members are matched exhaustively removes the hazard rather than preserving the ordering — prefer that, and do not ship a version that merely re-encodes the current sequence of `if` returns.

**Acceptance gate:** `basedpyright scripts/berth/claim_state.py scripts/berth/tests/test_hook_rendering.py scripts/berth/generated/status_payload_tables.py`, run with `/Users/natemccoy/.claude` as the working directory so its `pyrightconfig.json` applies, reports **zero errors, zero warnings, and zero notes** for **all three** files — the fixture file is edited by this phase and is inside the gate, and all three are clean on the tree this phase starts from, so any diagnostic is this phase's own. `python3 /Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py HookRenderingTests` passes in full; the timing class is invoked separately and serially on a quiet machine and its result is reported, not gated — Phase 17 measured the 0.20-second bound as unmet and left it visible, and classifier typing cannot influence it. No file-level type ignore exists in any gated file, and every remaining `# pyright: ignore` is line-level with a named rule.

The three envelope discriminants type-check against Phase 20's generated aliases: assigning an arbitrary `str` to `EnvelopeValue.verb`, `EnvelopeValue.status`, or `EnvelopePayload.kind` is a type error, and each is narrowed at the parse boundary by the generated guard rather than by a hand-written membership test. Both `expected_verb` APIs take the verb alias.

A fixture carrying Phase 13's `unknown_reservation` rejection reaches a caller as `UnknownReservationLifecycleQueryValue`, not as the residual generic member, and the residual member exposes no raw mapping — a test asserting the absence of a decoded-payload accessor on it fails if one is added. Replay fixtures exercise `classify_claim`, `classify_check`, `_validate_board`, `render_board`, and `coordinator_state`; each returns a typed replay-hard-stop member carrying typed reason, subject, effect, and operator route, with no `dict` return, no `cast`, and no `message` read, while a generic `ledger_unreadable` carrying no payload facts stays distinct from it. Fixtures reach each of `EngineInvocation`, `CoordinationIdentityRejection`, and `ForeignActorIncursionResolution` as a typed member rather than through a `tagged()` mapping.

`EnvelopePayload.alerts` is no longer `list[object]`: fixtures cover **every** member of `EnvelopeAlertValue`, including `OrphanedOutstanding` and both `LostEvidenceRecovery` forms, and each reaches a caller carrying its own recovery facts. Two fixtures carry a **success-tagged** envelope whose payload holds an identity-rejection and a foreign-resolution sub-status, and each reaches the caller as an ordinary engine response — the Phase 12 property, re-proved through the exhaustive match rather than assumed. `RenderedCoordinationIdentityRecoveryActionValue.kind` accepts exactly the four action tags, proved by an exhaustive check that fails when a fifth tag is added and when the field is widened back to `str`. Fixtures cover **every** lifecycle alternative and **every** release disposition, not the `active` and `unknown_reservation` pair Phase 13's fixture already exercises: one fixture per `active`, `outstanding`, `released_after_checkpoint`, and `released_without_checkpoint`, and one per `integrated`, `rewritten_integration`, `abandoned`, and `retired_orphan`, each reaching a caller as its own typed member.

The hook renders the corrected session-id contract: rendering tests for an overlong id and for a control-character id each produce a message naming the 1-to-256-character no-control-character rule, and neither path revalidates an already-typed id. The four Phase 17 harness guards are intact — exit-20/21/22 protocol, artifact-digest guard, unresolved-attribution reporting, and the no-install `setUpClass`. The phase's own diff is read against the pre-edit snapshot of the four named files, not against `HEAD`.

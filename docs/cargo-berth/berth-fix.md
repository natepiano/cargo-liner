...
### Phase 16 — Prove the PostToolUse path stays inside 0.20 seconds  · status: todo
...
**Journal age is a third independent dimension, and the matrix does not yet carry it.** Phase 15 bounded what the projection costs to publish; it did not bound what the journal costs to read. `PostToolUse` deserializes every journal record before classification and again around a mutation (`ledger/journal.rs`, `ledger/mod.rs`), so a repository whose journal has aged pays more for identical live state — which is precisely the failure mode Phase 15 was opened to remove from the projection and did not touch here. Run every outcome at two journal ages holding live state equivalent: a short journal, and one aged to the record count a working repository reaches over its life. **The published bound names the journal size it was proved against**, exactly as it names its installation state and its recovery-action set; a bound quoted without that number is a claim about the fixture, not about the path. If an aged cell exceeds the budget, the repair is a new phase that bounds replay cost — never a wider published bound, and never dropping the cell. That phase is deliberately not written yet, because whether it is needed is a measurement this phase has not taken.

...
**Every pathspec is literal and top-anchored.** `ReservationScopePath` (`ids.rs:217`) is a plain `String` newtype that admits a name beginning with git's pathspec magic (`:`) or carrying glob characters (`*`, `?`, `[`), so passing a scope path to `git log` raw lets git reinterpret it and attribute the wrong commits — or none. Prefix each one with `GIT_LITERAL_TOP_PATHSPEC_PREFIX` (`git/constants.rs`), exactly as `git/mod.rs:737`, `:971`, and `:1014` already do; do not introduce a second convention. Fixtures must include a path beginning with `:` and a path containing `*` and `[`, each proving the batched command attributes it as a literal name.
...
**Validate the phase-start ancestry precondition, in one batched call.** When the phase start is not an ancestor of HEAD, `<phase_start>..HEAD` sweeps in unrelated commits reachable only from HEAD, and a fixed subprocess count would then produce wrong attribution faster. Return a typed stale-anchor result or reanchor before attributing. Name that type `IncursionAttributionAnchorState::{UsableAncestor, ObjectUnknown, NotAncestorOfHead}` — the missing, unrelated, and stale fixtures map onto those three variants respectively, and `UsableAncestor` is the only one that attributes. `drift/git_output.rs` owns that result type, since it already owns how a git invocation's outcome is typed for this module. Do **not** issue one ancestry query per phase start — `git::reachability_to_target` (`git/mod.rs:1357`) already takes a **slice** of candidate ancestors and one target and answers all of them at a **fixed cost of two** git invocations, not one: a batched `commit_availability` (`cat-file`) pass over every candidate plus the target, then a single `rev-list <target>` whose output becomes the membership set. Reuse it; a per-anchor loop reintroduces exactly the scaling this phase exists to remove. Its `Reachability` answers map straight onto the anchor states: `ObjectUnknown` for an object git does not have, `NotAncestor` for an object that exists but is not reachable from the target — so `IncursionAttributionAnchorState::ObjectUnknown` takes the first and `NotAncestorOfHead` takes the second, and only `Reachability::Ancestor` yields `UsableAncestor`. Filter the union range's commits back to each anchor through the **parent graph**, the way `descendant_commits` (`git/mod.rs:1408`) does — `rev-list --parents --stdin` once, then a `CommitAncestryGraph` walk in memory — rather than by a second git query per anchor.

**One bad anchor must not poison the batch.** Phase 4 established this: `rev-list --stdin` exits 128 on a single unknown object, which blanked every result until `--ignore-missing` plus a per-item membership check confined the damage to the item actually missing. The shipped precedent is `descendant_commits` (`git/mod.rs:1408`, with `GIT_IGNORE_MISSING_ARG` applied at `:1426`) — **not** `git/mod.rs:1158`, which is `reachable_commits` and unrelated. Apply the same shape here — when one of twenty phase starts is missing, unrelated, or stale, the other nineteen reservations must still produce valid attribution, and only the bad anchor reports the typed stale-anchor result. **Define the two degenerate union cases rather than leaving them to the implementer.** When the anchors share no common base at all — unrelated histories, which `merge-base --octopus` reports by failing rather than by an empty answer — there is no union range to walk: every anchor in that group reports `NotAncestorOfHead` and none attributes, and the run does not fall back to a per-anchor loop. When the common base resolves but a given anchor is not an ancestor of HEAD, only that anchor reports `NotAncestorOfHead`; the union range still covers the rest.
...
- `crates/cargo-berth/src/git/command.rs` — `GitCommandExecution` (~L25) and the facade that makes it reachable from `drift`; the shared `git_command` constructor is at ~L39
...
**Scope boundary: this phase is engine- and git-hook-side, below the Claude PostToolUse shim.**

The plan carries two different hooks under one phrase, and they are not interchangeable. `tests/gate.rs` drives berth's own git `reference-transaction` hook — real, installed, end to end — and never executes `berth_post_bash.sh`. Every assertion about shim rendering or shim cost therefore belongs in `~/.claude/scripts/berth/tests/test_hook_rendering.py`, which **Phase 12 owns creating**; Phase 15 extends it for full-shim timing. This phase creates no external fixture and adds no work outside this repository.

The consequence is a wording requirement, not a coverage gap: this phase's cardinality assertions are git-hook-path measurements and must be published as such. The shim's own fixed process cost — the wrapper plus its `jq` and `mktemp` children — is Phase 15's term to measure against Phase 12's fixture, not this phase's.

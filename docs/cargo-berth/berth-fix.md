...
### Phase 21 — Generated-domain adoption and the semantic envelope boundary  · status: todo

#### Work Order

**Goal:** The coordinator's JSON boundary produces typed values from the engine's own generated domains, and no domain function downstream receives a raw mapping, an untyped namespace, or an empty-string absence sentinel.

**Spec:**

`claim_state.py` decodes an engine envelope into `EnvelopePayload.data: NotRequired[dict[str, object]]` (`:87`) and `EnvelopePayload.alerts: list[object]` (`:86`), then re-inspects those untyped values in every classifier. Phase 20 shipped the generated types that make a typed boundary possible; this phase builds the boundary. The classifier returns, `tagged()` removal, board rendering, and hook text are the following phase's work and are deliberately out of scope here.

**Start from the real baseline, which is red.** Phase 20 tightened `REPLAY_FAILURE_SUBJECT_KINDS` to `dict[ReplayFailureReasonValue, ReplayFailureSubjectKindValue]`, so `REPLAY_FAILURE_SUBJECT_KINDS.get(reason)` at `claim_state.py:2403` is now a type error while `reason` is `str`. That single inherited diagnostic is this phase's starting condition — do not treat it as pre-existing noise, and do not report it as a defect in the generated module. Narrowing `reason` through `is_replay_failure_reason_value` is this phase's first type gate.

**Adopt the generated domains rather than hand-typing them.** Phase 20 exports eleven semantic `Literal` aliases and four `TypeGuard`s from `berth.generated.status_payload_tables`. Nine are coordinator-facing and each has a place here:

- `CommandVerbValue` — `EnvelopeValue.verb`, and both `expected_verb` APIs (`:838`, `:2008`)
- `OutputStatusValue` — `EnvelopeValue.status`
- `OutputPayloadKindValue` — `EnvelopePayload.kind`
- `ReplayFailureReasonValue` and `ReplayFailureSubjectKindValue` — replay parsing, including the `:2403` lookup
- `CoordinationIdentityRejectionKindValue` — rejection parsing and its exhaustiveness check
- `LostIntegrationEvidenceStatusValue` — inside `EnvelopeAlertValue`
- `IntegrationProofValue` and `TrunkObservationAtClaimValue` — the typed board-rendering conversion this phase produces for the next phase to consume

Use the four generated guards — `is_command_verb_value`, `is_output_status_value`, `is_output_payload_kind_value`, `is_replay_failure_reason_value` — to narrow at the parse boundary, and the generated typed sets and map for the remaining domains, while retaining exact per-variant `Literal` tags on the semantic members. Do not hand-copy a literal set into this file; the generated module is the only source. `PayloadDataRequirement` and `OutcomeEmissionDisposition` are generated-validator-only and have no consumer here.

**Two types, not one, at the envelope boundary.** Every coordinator operation echoes `envelope.value`, and the installed lifecycle test inspects the complete echoed `payload.data` (`tests/test_hook_rendering.py:1524`), so a design that discards decoded mappings outright loses data the protocol requires. Introduce a boundary-only `ValidatedWireEnvelopeValue` that preserves the wire shape for echoing, alongside a semantic `CoordinatorPayloadReadingValue` that carries what the coordinator actually read. **Only the wire type may represent protocol-level field absence**; no member of the semantic reading exposes a raw mapping, and no domain-owned payload type retains a `NotRequired` member, because representation-level absence is precisely the protocol state this work exists to name.

The semantic reading names its cases: `NoPayloadFactsValue` for an envelope carrying no payload facts, `DeliberatelyUninspectedPayloadValue` for a payload the coordinator chose not to inspect, and one typed member per classifier-owned fact. The uninspected member names the fact that inspection did not happen; it does not carry the decoded mapping onward for a caller to inspect by hand, which would reintroduce the erasure one member over.

**`EnvelopePayload.alerts` is typed here as a complete union.** It is `list[object]` at `:86`, so every alert reaches a caller as an untyped value. Both `LostIntegrationEvidence` and `OrphanedOutstanding` carry user-actionable recovery facts, so "type the alerts or explicitly ignore them" is not a real alternative. Build `EnvelopeAlertValue` with a named member per alert, including **both** `LostEvidenceRecovery` forms, keying its status on `LostIntegrationEvidenceStatusValue`. The engine-side enumeration is `crates/cargo-berth/src/alert.rs`, with the `Alert` enum at `:32` and its concrete shapes at `:98`, `:145`, and `:175` — read it as authoritative.

**`ProposalAwaitingApprovalStateValue.proposal` (`:374`) carries `ValidatedOverlapProposalValue`** — an overlap-authorization proposal, not a proposal in general, and a `…Value` like every other member of this surface. Its complete shape is `OverlapProposal` at `crates/cargo-berth/src/answer/proposal.rs:108`, carrying an `AuthorizedOverlapSet` defined at `crates/cargo-berth/src/answer/scope_binding.rs:41`. Both are read-only inputs.

**The argparse boundary closes completely, not just `expected_verb`.** `CoordinatorArguments` (`:2465`) carries operation-specific empty values, and `plan`, `phase`, `why`, and `run` flow straight into `_claim_arguments`. Convert the parsed namespace immediately into a per-operation `CoordinatorRequest` union: pair `plan` and `phase` into a named claim-source state, name the purpose and run selection rather than passing bare strings, and reuse the existing `ClaimTransition` for answer state. No domain function receives the namespace, and no empty-string sentinel and no `cast` reaches a domain API. `expected_verb` converts through `is_command_verb_value` immediately after argparse.

**This phase edits a dirty external checkout, so it establishes its own baseline before touching anything.** `/Users/natemccoy/.claude` is itself a Git checkout and is dirty from earlier phases, so an ordinary `git diff` there shows several phases' work as one. Snapshot `scripts/berth/claim_state.py`, `scripts/berth/tests/test_hook_rendering.py`, and `scripts/berth/generated/status_payload_tables.py` before any edit and diff against that snapshot rather than against `HEAD`. Run project-root tooling from that root — `~/.claude/pyrightconfig.json` supplies the `executionEnvironments` the type check depends on, and running basedpyright from anywhere else silently applies different settings. Those files cannot join a `cargo-berth` checkpoint commit.
...
**Files:**
- `/Users/natemccoy/.claude/scripts/berth/claim_state.py` — the envelope `data` and `alerts` members, the three envelope discriminants, both `expected_verb` APIs, the argparse boundary, and the proposal type
- `/Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` — extended with this phase's boundary fixtures
- `/Users/natemccoy/.claude/scripts/berth/generated/status_payload_tables.py` — read-only input: Phase 20's aliases, guards, typed sets, and typed map
- `crates/cargo-berth/src/answer/proposal.rs` — read-only input at `:108`: `OverlapProposal`
- `crates/cargo-berth/src/answer/scope_binding.rs` — read-only input at `:41`: `AuthorizedOverlapSet`
- `crates/cargo-berth/src/alert.rs` — read-only input: the `Alert` enum at `:32`, concrete shapes at `:98`, `:145`, `:175`

**Constraints from prior phases:** Phase 20 shipped eleven generated semantic `Literal` aliases and four boundary `TypeGuard`s in `berth.generated.status_payload_tables`, typed every generated container on them, derived `COORDINATION_IDENTITY_REJECTION_KINDS` so the generated validator and the outcome rules cannot drift, checkpointed, and installed the engine from that checkpoint. This phase consumes those types and must not regenerate or hand-edit that module. Phase 20 also left one inherited type error at `claim_state.py:2403`, described above; the "all three files are clean on the tree this phase starts from" assumption from an earlier reading of this plan is false. Phase 14 generated `STATUS_PAYLOAD_KINDS` and `FIXED_STATUS_EXIT_CODES` — imported at `claim_state.py:22` and `:27`, inputs here, never hand-edited targets. Phase 13's value types are already correct — `ReleaseDispositionValue` (`:129`), `ReservationLifecycleValue` (`:165`), `ReservationLifecycleQueryStateValue` (`:188`), the strict validator `reservation_lifecycle_state` (`:2105`), and `_release_disposition` (`:2028`) — lift or rename them, never rebuild them. Phase 12 already added typed rendering of `CoordinationIdentityRejection` and its `recovery_actions`, shipping `CoordinationIdentityRejectionValue` (`:604`) and `ForeignActorIncursionResolutionValue` (`:660`) — lift both rather than recreating them. **Phase 18 shipped the reftable fallback's final shape on the Rust side and it is deliberately invisible here:** `ClaimError::GitBackedReferenceRequired` was deleted in Phase 17, the signal is now a private field inside the filesystem reference reader, and a genuine Git failure still arrives as `ClaimError::Git(GitError)` — do not invent a payload member, wire status, or union variant for it. **Phase 17 shipped four harness guards in `test_hook_rendering.py`** that this phase preserves rather than rediscovers: the exit-20/21/22 protocol, the artifact-digest guard that refuses a measurement whose engine or generated consumers change mid-run, the unresolved-attribution reporting in the timing summary, and a `setUpClass` that refuses to install and directs the operator to the explicit `--install-engine /path/to/cargo-liner` command. Invoke `--install-engine` only from a checkpointed tree and only when a measurement is intentionally requested.

**Acceptance gate:** `basedpyright scripts/berth/claim_state.py scripts/berth/tests/test_hook_rendering.py scripts/berth/generated/status_payload_tables.py`, run with `/Users/natemccoy/.claude` as the working directory so its `pyrightconfig.json` applies, reports **zero errors, zero warnings, and zero notes** for all three files — including the inherited `:2403` diagnostic, which this phase closes by narrowing through `is_replay_failure_reason_value` rather than by a `cast`. `python3 /Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py HookRenderingTests` passes in full; the timing class is invoked separately and its result is reported, not gated. No file-level type ignore exists in any gated file, and every remaining `# pyright: ignore` is line-level with a named rule.

The three envelope discriminants type-check against the generated aliases: assigning an arbitrary `str` to `EnvelopeValue.verb`, `EnvelopeValue.status`, or `EnvelopePayload.kind` is a type error, and each is narrowed at the parse boundary by the generated guard rather than by a hand-written membership test.

Envelope echoing is proved JSON-equivalent through the wire type for **both** a typed reservation rejection and a deliberately uninspected payload, so the boundary split loses nothing the protocol carries. A test asserting the absence of a decoded-payload accessor on `DeliberatelyUninspectedPayloadValue` fails if one is added.

`EnvelopePayload.alerts` is no longer `list[object]`: fixtures cover **every** member of `EnvelopeAlertValue`, including `OrphanedOutstanding` and both `LostEvidenceRecovery` forms, and each reaches a caller carrying its own recovery facts.

All three `expected_verb` sites take `CommandVerbValue` — the two domain APIs and `CoordinatorArguments`, converted through the generated guard immediately after argparse. Every operation's parsed namespace becomes a `CoordinatorRequest` member before any domain function sees it, proved by a test that fails if a domain function's signature accepts the namespace or an empty-string sentinel.

### Phase 22 — Coordinator classifier unions, board rendering, and hook text  · status: todo

#### Work Order

**Goal:** The Python coordinator's classifiers return tagged unions instead of `dict[str, object]` reached through `cast`, the board renders from typed values, and the hook states the session-id contract the engine now enforces.

**Spec:**

`classify_claim` (`claim_state.py:1485`), `classify_check` (`:1714`), `_validate_board` (`:1800`), `render_board` (`:1893`), and `coordinator_state` (`:2190`) all return `dict[str, object]` and reach it through `cast`, so every tagged union the coordinator builds is erased at the one boundary a reader inspects. The weakness predates the coordinator work — `classify_claim` returned `dict[str, Any]` at `831e34a` — and repairing only one symbol would leave an inconsistent surface.

Name the returns rather than leaving the implementer to invent them: `classify_claim` returns `ClaimClassificationValue`, `classify_check` returns `CheckClassificationValue`, `_validate_board` returns `BoardValidationValue`, `render_board` returns `RenderedBoardValue`, and `coordinator_state` returns `EngineInvocationStateValue` — the value is the coordinator's reading of one engine invocation, and the old `Generic` in its former name said how many cases it covers rather than what it is. **`coordinator_state` is the only classifier to retype here, not two.** Phase 13 renamed `_generic_state` to the public `coordinator_state`; there is no longer a private twin, and any instruction to retype both names a symbol that no longer exists. Phase 13 also made `installed_engine_binary` (`:1968`) public; `installed_engine_binary() -> str` is already precisely typed and needs no work here.

**Every `tagged()` conversion is an erasure point and all of them close here.** A tagged type erased at its own conversion is the same defect as an untyped classifier, so each gets an explicit typed member in every affected return union rather than a `dict[str, object]`: `EngineInvocation.tagged()` (`:493`), `CoordinationIdentityRejection.tagged()` (`:626`), `ForeignActorIncursionResolution.tagged()` (`:678`), and `ReplayFailure.tagged()` (`:744`) — the last as an explicit replay-hard-stop member.

**`render_board` needs a conversion seam ahead of it, not merely a named return.** It walks the complete raw envelope (`:1893`) — payload mappings, path lists, and the user actions it prints. Naming its return `RenderedBoardValue` while it still reads an untyped mapping to build that value moves the erasure inward rather than removing it. Consume the typed board-rendering fields, paths, and user actions produced by the preceding phase's conversion, keyed on `IntegrationProofValue` and `TrunkObservationAtClaimValue`, rather than re-deriving them from the wire envelope.

**One tag Phase 12 left open closes here.** `RenderedCoordinationIdentityRecoveryActionValue` (`:611`) types its `kind` as `str` (`:614`) where the four recovery-action tags are closed literals. That was implementation-only while `tagged()` erased the whole value to `dict[str, object]` — nothing downstream could match on it. Removing that erasure turns the tag into externally typed domain state, so it becomes the closed four-tag literal (or a rendered union with one member per action) at the same time.

**The invalid-input reading is an exhaustive match.** It covers Phase 11's three identity rejections, Phase 7's `already_recorded_by_different_coordination_actor`, **Phase 13's `ReservationLifecycleQueryRejection::UnknownReservation` as its own `UnknownReservationLifecycleQueryValue` member** (`payload.kind = reservation`, `data.status = unknown_reservation`), and a residual generic member. **That member is a wire-rejection reading and must stay distinguishable from the normalized coordinator state Phase 13 already ships.** `UnknownReservationLifecycleValue` (`:181`) is the *normalized* form the `reservation` operation returns, keyed `kind = unknown_reservation`; the envelope union's member is the *wire* rejection, keyed `data.status = unknown_reservation` under `payload.kind = reservation`. They carry the same fact in two roles. Keep Phase 13's normalized type under its existing name, give the wire member the query-specific `UnknownReservationLifecycleQueryValue` name, and convert between them at exactly one explicit site. Every branch is gated with `message` unread.

**The `ledger_unreadable` reading carries the typed replay failure, and its generic branch means no facts.** `_replay_failure_classification` (`:2375`) returns a `ReplayFailure` (`:739`) reached through `.tagged()` and is called from all four classification entry points — `classify_claim` (`:1493`), `classify_check` (`:1733`), `_validate_board` (`:1801`), and `coordinator_state` (`:2196`) — ahead of each one's generic `ledger_unreadable` branch. It keys on `envelope.status == "ledger_unreadable"` with `payload.kind == "replay_failure"`, so it belongs to the `ledger_unreadable` reading rather than the invalid-input union, and its `ReplayFailureValue` (`:730`) already exists — lift it rather than rebuilding it. The generic branch beneath it carries **no payload facts**: `parse_envelope` (`:899`) applies `valid_outcome_tuple` before anything else, so a malformed replay payload never crosses validation and can never reach that branch. Type it as the no-facts reading, and keep it below the typed classifier for the same ordering reason the invalid-input branch must.

**Success-tagged lookalikes are rejected at the boundary and must stay that way.** `parse_envelope` rejects a success-tagged envelope carrying a rejection-only nested status before any classifier runs, and the existing identity fixture and the foreign-resolution test (`tests/test_hook_rendering.py:2048`) already expect `EnvelopeValidationError`. Do **not** require these to reach a caller as ordinary engine responses, and do **not** construct an impossible `ValidatedEnvelope` merely to exercise classifier ordering — an exhaustive match over a union whose invalid members cannot be built is the property to preserve, not a fixture that manufactures one.

**Phase 7 left the generic `invalid_input` branch order-sensitive and that ordering is a correctness constraint, not a style preference.** `sequence` and `integrate` identity rejections arrive with status `invalid_input`, so the generic `invalid_input` early return (`coordinator_state` at `:2190`, its generic branch at `:2218`) must stay *below* the two typed classifiers; placed above them, the typed identity states are silently erased into a diagnostic string. A tagged union whose members are matched exhaustively removes the hazard rather than preserving the ordering — prefer that, and do not ship a version that merely re-encodes the current sequence of `if` returns.

**The hook states the session-id contract the engine now enforces.** Phase 18 made the PostToolUse payload parser reject a `session_id` that is overlong or carries control characters, where it previously rejected only an empty one, and `berth_post_bash.sh:422` still tells the user only that `session_id` must be non-empty. Correct that message to state the real contract — 1 to 256 characters, no control characters — and add exact rendering tests for both rejections. The hook does not revalidate an already-typed id; it renders the engine's rejection.
...
**Files:**
- `/Users/natemccoy/.claude/scripts/berth/claim_state.py` — the five classifier returns, the four `tagged()` conversions, the recovery-action tag, and the exhaustive envelope readings
- `/Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` — extended with this phase's classifier and hook fixtures, and split so the deliberately red timing method lives in its own separately invocable timing class
- `/Users/natemccoy/.claude/scripts/berth/install/hooks/berth_post_bash.sh` — the session-id rejection message at `:422`
- `/Users/natemccoy/.claude/scripts/berth/generated/status_payload_tables.py` — read-only input

**Constraints from prior phases:** The preceding phase typed the envelope boundary, so `EnvelopePayload` already carries a `ValidatedWireEnvelopeValue` for echoing and a `CoordinatorPayloadReadingValue` for what was read, `alerts` is already `EnvelopeAlertValue`, the three envelope discriminants and all three `expected_verb` sites already take generated aliases, and every operation's arguments already arrive as a `CoordinatorRequest` member. Do not rebuild any of that; consume it. Phase 20's generated module is the only source for a closed set — a hand-copied literal set is the defect Phase 14 exists to prevent. **`_inactive_identity_classification` no longer exists** — Phase 12 replaced it with `_coordination_identity_classification` (`:2349`) and `_foreign_actor_incursion_resolution_classification` (`:2422`); do not key any edit or call-site sweep on the old name. Phase 12 also bound both classifiers to the envelope's own status, refusing any envelope whose status is not `invalid_input`, which is the property this phase's exhaustive match must preserve rather than rediscover. Phase 14 added `OutputStatus::LegacyHookOutdated` as its own wire status, so `ledger_unreadable` is no longer the catch-all it was when an earlier version of this Work Order was written. Phase 17's four harness guards in `test_hook_rendering.py` are preserved rather than rediscovered. The files in this phase cannot join a `cargo-berth` checkpoint commit; establish a pre-edit snapshot of them and diff against it rather than against `HEAD`, and run tooling from `/Users/natemccoy/.claude` so its `pyrightconfig.json` applies.

**Acceptance gate:** `basedpyright scripts/berth/claim_state.py scripts/berth/tests/test_hook_rendering.py scripts/berth/generated/status_payload_tables.py`, run with `/Users/natemccoy/.claude` as the working directory, reports **zero errors, zero warnings, and zero notes** for all three files. `python3 /Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py HookRenderingTests` passes in full; the timing class is invoked separately and its result is reported, not gated. No file-level type ignore exists in any gated file, and every remaining `# pyright: ignore` is line-level with a named rule. No `cast` stands between a tagged value and its return or dispatch.
...
`RenderedCoordinationIdentityRecoveryActionValue.kind` accepts exactly the four action tags, proved by an exhaustive check that fails when a fifth tag is added and when the field is widened back to `str`. The identity-rendering fixture is paired with an exhaustiveness check over the generated `COORDINATION_IDENTITY_REJECTION_KINDS`, so a rejection kind added to the engine fails this phase's tests rather than reaching a user as an unrendered value. Fixtures cover **every** lifecycle alternative and **every** release disposition — one each for `active`, `outstanding`, `released_after_checkpoint`, and `released_without_checkpoint`, and one each for `integrated`, `rewritten_integration`, `abandoned`, and `retired_orphan` — each reaching a caller as its own typed member.

`render_board` consumes the preceding phase's typed board-rendering value and reads no raw mapping, proved by a test that fails if the wire envelope is reachable from its body.

The hook renders the corrected session-id contract, proved against the **installed engine** rather than the `run_hook` fixture — that fixture returns its configured exit independently of the payload it is handed, so a rejection-message test written against it passes without ever reaching the engine parser. A 256-character multibyte id produces no invalid-payload feedback; a 257-character id and an id carrying a control character each produce the exact corrected message at `berth_post_bash.sh:422`, naming the 1-to-256-character no-control-character rule. Neither path revalidates an already-typed id. The four Phase 17 harness guards are intact. The phase's own diff is read against the pre-edit snapshot of the named files, not against `HEAD`.

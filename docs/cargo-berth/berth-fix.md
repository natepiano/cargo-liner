...
- **Invariants:**
  ...
  - `docs/cargo-berth/json-contract.md` is the stable wire contract. Additions arrive as new variants; fields are never renamed or removed. Any phase changing the serialized layout updates it in the same phase.
...
- `crates/cargo-berth/tests/board.rs` — cost assertions pinning the batched command forms (L1701)
...
- `crates/cargo-berth/tests/ledger.rs` — coverage for the recorded layout, the `too_long` overflow at 32 KiB, and actor attribution per worktree layout
...
message-free rendering is proved, so a new fixture must follow the same form. The
...
which described a form the engine had already stopped emitting. Rendering keyed to a
...
before decoding the cache's layout, mirroring the journal's own header-first read.
...
**Files:**
- `crates/cargo-berth/src/ledger/projection.rs` — the cache's layout, `ProjectionSchemaHeader`, the header-first read, and the rebuild routing in `read_validated`
...
- Lifting reads out of `compare_scoped_patch`'s per-target loop — `phase_start_head` arrives already resolved and the budgets admit one comparison per target per pass.
...
### Phase 23 — Coordinator classifier unions, board rendering, and hook text  · status: done

#### As-built

The Python coordinator returns named tagged unions at every boundary a reader inspects. `classify_claim` returns `ClaimClassificationValue`, `classify_check` returns `CheckClassificationValue`, and `coordinator_state` returns `CoordinatorEngineOutcomeValue` — the reading of one engine invocation's outcome, excluding argv, exit status, and stderr. `EngineInvocation.tagged`, `CoordinationIdentityRejection.tagged`, `ForeignActorIncursionResolution.tagged`, and `ReplayFailure.tagged` each produce an explicit typed member rather than a mapping; `ReplayFailure.tagged` reconstructs a fresh `ReplayFailureSubjectValue` per subject kind under `assert_never`, so the emitted state never aliases the validated reading.

The board pipeline answers availability once. `_classify_board_availability` returns a `BoardAvailabilityState` covering ready, replay, contention, unconfigured, and unreadable, and `render_board(BoardAvailabilityState) -> BoardPresentationStateValue` consumes that state and reads no raw mapping. Contention is two types — `LedgerContentionStateValue` as classified and `RetryReadyLedgerContentionStateValue` carrying the command to rerun — so no state is mutated after classification returns. Invalid input keeps `InvalidInputWithoutPayloadFacts` and `InvalidInputWithUninspectedPayload` as distinct nominal domain states converted through one typed emission boundary that still produces `{kind: "invalid_input", diagnostic: ...}`, and the residual branch keeps its message-backed diagnostic while the typed branches read no `message`. The `ledger_unreadable` reading carries the typed `ReplayHardStopStateValue` with its `ReplayJournalRecoveryRouteValue`, above a `LedgerUnreadableWithoutReplayFactsStateValue` branch that carries no payload facts.

The wire `unknown_reservation` rejection and the normalized published state stay distinct types: `UnknownReservationLifecycleQueryRejectionValue` is the envelope reading, `UnknownReservationLifecycleValue` the published state. Normalization happens at exactly two emission boundaries — `reservation_lifecycle_state` and `_invalid_input_classification` — held there by an exact reader-set assertion, so every published state carries a `kind` tag. The rendered recovery-action and identity-rejection unions carry one member per tag, checked for exhaustiveness against the generated `COORDINATION_IDENTITY_REJECTION_KINDS`, so a kind added to the engine fails these tests rather than reaching a user unrendered. Parity fixtures hold the emitted `cargo-berth-claim-state/v1` JSON equivalent to the pre-change output — same markdown order, paths, and action values — for identity, replay, invalid input, contention with a rerun command, reservation, claim, check, and board.

`berth_post_bash.sh` states the session-id contract the engine enforces: 1 to 256 characters, no control characters, proved against the installed engine through `run_installed_engine_hook` for a 256-character multibyte id, a 257-character id, and an id carrying a control character. Casts in `claim_state.py` fell from 29 to exactly six named boundary casts, enforced by a test naming that exact set; no file-level type ignore remains in any gated file.

**Files:**
- `~/.claude/scripts/berth/claim_state.py` — the classifier unions, the four typed conversions, the board availability state, the contention pair, and the exhaustive envelope readings
- `~/.claude/scripts/berth/tests/test_hook_rendering.py` — the parity fixtures for every retyped state, the cast-budget and exhaustiveness checks, the installed-engine session-id cases, and `board_presentation_before_typed_rendering`, an independent reimplementation of the untyped mapping walk that serves as the board parity oracle
- `~/.claude/scripts/berth/install/hooks/berth_post_bash.sh` — the corrected session-id rejection message

**Gotchas:**
- `_invalid_input_classification` is an emission boundary, not only a classifier: its return reaches `main` as the published `state` field, so a wire-form reading returned from it is published verbatim and a reading tagged `status` rather than `kind` breaks the published contract silently.
- The `ledger_unreadable` generic branch is total because `parse_envelope`'s status/payload table admits only `no_facts` and `replay_failure` under that status and replay is classified first — the guard is closed two functions upstream, not locally.
- A parity fixture proves nothing about a status/payload combination it omits; `invalid_input` carrying a `reservation` payload is the combination that hid a published-form defect behind a fully green suite.
- These files live outside the repository and can never join a `cargo-berth` checkpoint commit. Diff against a pre-edit snapshot rather than `HEAD`, and run tooling from `~/.claude` so its `pyrightconfig.json` applies.

**Ruled out:**
- Restoring `dict(self.value["subject"])` to isolate the replay subject — it reintroduces the untyped conversion this phase removed.
- Widening the normalized-reader assertion to a subset check or deleting it — an exact set is what makes a third conversion site fail.
- Constructing `ValidatedEnvelope` states `parse_envelope` would reject in order to exercise classifier ordering — static return-union membership checks carry that proof, with runtime proof kept on the board rejection that can actually occur.

...
**Pending decision: whether Phase 17's two new boundaries join this phase's type-design scope**

Actual problem:
Phase 17 introduced two surfaces carrying exactly the defect this phase exists to remove, and neither is in its Work Order today. `ReferenceFileReadError` (`crates/cargo-berth/src/verb/claim.rs:157`) is a read-error type whose new `GitResolutionRequired` case is not an error at all but a resolution outcome, and `PostToolUseDriftInvocation` (`crates/cargo-berth/src/cli.rs:1008`) carries the harness session identity as a bare `String` through a `OnceLock` and `select_current_process_harness_session`, discarding the validated `HarnessSessionId` that already exists.

What exists now:
- `ReferenceFileReadError` mixes genuine read failures with the reftable signal that a Git resolution is required, so a caller reads the variant name and infers a resolution policy the type never states.
- The session identity is validated into `HarnessSessionId` and then flattened back to `String` for transport, so every downstream consumer re-derives a guarantee that had already been proved.
- This phase's goal line says "Eighteen types" while its rename list carries twenty, so the count is already wrong independent of this decision.

What should change:
- Replace `ReferenceFileReadError` with `FilesystemReferenceResolution::{Resolved, RequiresGitResolution, Rejected}`.
- Pass the existing `HarnessSessionId` through `PostToolUseDriftInvocation`, the `OnceLock`, and `select_current_process_harness_session` instead of `String`.
- Add `crates/cargo-berth/src/verb/claim.rs` and `crates/cargo-berth/src/session/mod.rs` to **Files**, and add both boundaries to the **Acceptance gate**.
- Correct the goal to say twenty renames.

Recommendation:
Take both. They are the same defect as the eighteen already in scope, they sit in code this phase is already opening, and splitting them into a later phase would ship a plan that knowingly leaves two fresh instances of the exact pattern it just spent a phase removing. This stays one cohesive phase and does not merge with Phase 19.
...
`PriorClassification` does not name its pre-lock foreign-path role; `ReservationRow` (`board/mod.rs:145`) names a display representation rather than what it holds; `CommandExecution` does not state who owns presenting the result; and `GitCommandExecution` (`git/command.rs:25`), added by Phase 2, names an action when its actual guarantee is whether a completed process output exists — `CouldNotRun`, which carries the spawn `io::Error`, versus a completed `Output`, which is precisely an availability, not an execution. `overlap_authorization_request` exposes six bare `Option<T>` parameters and `EditAuthorization::resolve_from_sources` accepts `Option<OsString>`, so readers must infer overlap-selection and environment-identity states from representation and control flow. Two further bare optionals carry domain states of their own: `comparable_worktree` (`drift/execution.rs:366`) returns `Result<Option<WorktreeId>>`, which collapses "identity is missing" and "comparison is deferred pending a rewrite" into the same absent value; and `first_touch_disposition_description` (`output.rs:2484`) returns `Option<String>`, leaving "no disposition applies" indistinguishable from "a disposition applies but has no text".

Phase 3 shipped four more names that state their representation rather than their semantic role. `ScopedPatchEquivalenceCache` (declared beside `RetainedScopedPatchTargetVerdict` in `reservation/mod.rs`) is not a cache in the sense the name implies — it is a bounded set of retained verdicts for specific targets. `ScopedPatchEquivalenceCacheLookup` (same module) names the act of looking up; its two cases say whether a verdict is available. `ScopedPatchComparisonAttemptHistory` (same module) is not a history anyone reads back — it is the round-robin schedule that decides which target is compared next. `ScopedPatchEvaluationMemo` (`reconcile.rs:448`) is not a memo — it is the one-comparison-per-target budget for a single reconciliation pass, and its lifetime is exactly that pass.
...
- `WorkingTreeStatusPaths` (`drift/git_output.rs`, beside `WorkingTreeChangePartition`'s consumers) → `WorkingTreeChangePartition`
- `ProtectedScopedRename` (`git/mod.rs:349`), variants `{Absent, Present}` → `ScopedReplayRenameDetection`, variants `{DisabledWithoutProtectedRename, RequiredForProtectedRename}`
...
**`BoardReservationSnapshot`, not `BoardReservationState`.** The type (`board/mod.rs:145`) combines journal-derived lifecycle with computed integration evidence, visibility, freshness, holder liveness, and live `ahead_behind_main`:
...
- `crates/cargo-berth/src/board/mod.rs` — `BoardReservationSnapshot` (declared at `:145`; also constructed in `reservation_visibility`'s caller)
...
- `crates/cargo-berth/src/git/command.rs` — `GitCommandOutputAvailability` (the type today named `GitCommandExecution`)
- `crates/cargo-berth/src/reservation/mod.rs` — `RetainedScopedPatchTargetVerdict`, `RetainedScopedPatchTargetVerdicts`, `ScopedPatchTargetVerdictAvailability`, `ScopedPatchTargetEvaluationSchedule`, and Phase 4's successor twins `RetainedSuccessorScopedPatchTargetVerdict`, `RetainedSuccessorScopedPatchTargetVerdicts`, `SuccessorScopedPatchTargetVerdictAvailability`, `SuccessorScopedPatchTargetEvaluationSchedule` — anchor on the symbol names, which the eviction sites at `:262-264` and `:330-332` bracket
- `crates/cargo-berth/src/reconcile.rs` — `ReconciliationScopedPatchEvaluationBudget` (`:448`) and `ReconciliationSuccessorScopedPatchEvaluationBudget` (`:458`)
- `crates/cargo-berth/src/git/mod.rs` — `GitCommandOutputAvailability` call sites, plus `ProtectedTipSuccessorHeadClassification` (`:2565`) and its consumers around `reconcile.rs:1786`
- `crates/cargo-berth/src/alert.rs` — `LostEvidenceRecoveryCommand` (the type today named `alert::RecoveryAction`)
- `crates/cargo-berth/src/recovery.rs` — `PostCommitRecoveryMarkerAction` (the type today named `recovery::RecoveryAction`)
- `crates/cargo-berth/src/drift/execution.rs` — the semantic result replacing `comparable_worktree`'s `Result<Option<WorktreeId>>` (`:366`)
- `crates/cargo-berth/src/output.rs` — the semantic result replacing `first_touch_disposition_description`'s `Option<String>` (`:2484`)
...
- `/Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` — created by Phase 12, extended by Phases 13 and 14, again by Phase 16, and again by Phase 17 to seventeen tests, including typed replay routing, unreadable-validator diagnostics, and installer rollback ordering; extended here with the classifier fixtures this phase's gate names

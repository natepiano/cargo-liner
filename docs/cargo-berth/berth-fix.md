...
Extend the Rust contract emitter so the generated Python module exports, alongside the existing frozensets: a semantic `Literal` alias per closed set — `CommandVerbValue` for the verbs, `OutputStatusValue` for the statuses, and `OutputPayloadKindValue` for the payload kinds; and one boundary `TypeGuard` per alias — `is_command_verb_value`, `is_output_status_value`, and `is_output_payload_kind_value` — each taking `object` and narrowing it by testing membership in the corresponding frozenset, so a decoded JSON value is narrowed once at the parse boundary and stays narrowed.

**Exporting the aliases is not the work; using them is.** An alias nothing consumes leaves the generated contract exactly as stringly typed as it was, with a type a downstream consumer may adopt by hand — which is the copy Phase 14 exists to prevent, one indirection over. Replace `str` with the corresponding alias inside the generated module itself: the `OutcomeRule` fields, the table keys, and `valid_outcome_tuple`'s parameters. The frozensets remain the single source: each alias and each guard is derived from the same contract data in the same generation pass, never restated by hand.
...
- `docs/cargo-berth/generated/output-contract.json` — the checked contract artifact the regeneration run rewrites, and the source the installed Python module is derived from; regenerated rather than edited, and inside this phase's checkpoint

**Constraints from prior phases:** Phase 14 generated `STATUS_PAYLOAD_KINDS` and `FIXED_STATUS_EXIT_CODES` from the engine contract and pinned every `schemars` definition name to its wire name rather than the Rust identifier, with a test that renames a Rust type and asserts every generated artifact is byte-identical — the aliases and guards added here are bound by the same rule and must key on wire names. Phase 14 added `OutputStatus::LegacyHookOutdated` as its own wire status, so the status set is not what an older reading of this file assumed. The generated module lives in the `/Users/natemccoy/.claude` Git checkout and cannot join a `cargo-berth` checkpoint commit; the checkpoint here covers the Rust emitter, `docs/cargo-berth/json-contract.md`, and the regenerated `docs/cargo-berth/generated/output-contract.json`, and the regenerated Python module is committed in that other checkout.

**Acceptance gate:** `bash ~/.claude/scripts/delegate/verify.sh test cargo-berth` and `bash ~/.claude/scripts/delegate/verify.sh lint cargo-berth` both pass, including the artifact byte-compare test. Regeneration is one command, run from the repository root: `CARGO_BERTH_REGENERATE_OUTPUT_CONTRACT=1 bash ~/.claude/scripts/delegate/verify.sh test cargo-berth`. It rewrites `docs/cargo-berth/generated/output-contract.json`, which carries the embedded Python module the installer derives. Regeneration is idempotent: a second run leaves every artifact byte-identical. `basedpyright`, run with `/Users/natemccoy/.claude` as the working directory, reports zero errors, zero warnings, and zero notes for the regenerated module. A scratch check proves narrowing actually works: a `str` passed through each exported guard is accepted where its alias is required, and a value outside the set is rejected by the guard at runtime and by the type checker at the use site.

...
**Gotchas:**
- `_invalid_input_classification` is an emission boundary, not only a classifier: its return reaches `main` as the published `state` field, so a wire-shaped reading returned from it is published verbatim and a reading tagged `status` rather than `kind` breaks the published contract silently.
...
- A parity fixture proves nothing about a status/payload combination it omits; `invalid_input` carrying a `reservation` payload is the combination that hid a published-shape defect behind a fully green suite.

...
**`claim_state.py:543` still models a two-name taxonomy that no longer exists.** It carries only `inactive_session_mapping` and `inactive_marker_run`, while Phase 11 emits three differently named variants, reachable either directly under `payload.kind = coordination_identity` or nested under `integrate.data.reason`. Establish a semantic `CoordinationIdentityRejectionValue` covering those exact three shapes, plus the matching recovery-action union, and classify both placements through it. Phase 17 lifts these types into its exhaustive envelope union rather than recreating them.

**A recovery action set never contains a command a front end cannot execute.** Phase 11 guarantees every published `argv` is directly runnable, and omits `RerunFromHoldingWorktree` entirely when the original command cannot be reproduced faithfully — so a shim must render whatever actions are present and must never assume that particular action exists. Rendering is driven by the action list, not by a fixed expectation of which actions a rejection carries.
...
**Files:**
- `/Users/natemccoy/.claude/scripts/berth/claim_state.py` — replace the obsolete two-name identity taxonomy and render the three rejections from typed fields (~L543)
...
**Constraints from prior phases:** Phase 12 already added typed rendering of `CoordinationIdentityRejection` and its `recovery_actions` to this file, and Phase 13 added the `reservation` entry point and its validator. Phase 14 generated `STATUS_PAYLOAD_KINDS` and `FIXED_STATUS_EXIT_CODES` from the engine contract — those generated tables are inputs here, not hand-edited targets. **Phase 7 left `_generic_state` order-sensitive and that ordering is a correctness constraint, not a style preference.** `sequence` and `integrate` inactive-identity rejections arrive with status `invalid_input`, so the generic `invalid_input` early return (`claim_state.py:1847`) must stay *below* `_inactive_identity_classification`; placed above it, the typed identity states are silently erased into a diagnostic string. **Phase 12 already replaced the obsolete `inactive_session_mapping` / `inactive_marker_run` pair with a semantic `CoordinationIdentityRejectionValue` covering Phase 11's three variants in both their wire placements, plus the matching recovery-action union** — lift those types into this phase's exhaustive envelope union rather than recreating them, and treat this part of the identity work as already satisfied. A tagged union whose members are matched exhaustively removes the hazard rather than preserving the ordering — prefer that, and do not ship a version that merely re-encodes the current sequence of `if` returns.

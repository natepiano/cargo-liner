...
**Goal:** A resolve that leaves the incident resolved and the same durable journal actor responsible exits 0 with a payload naming what it did.
...
`LedgerTransactionOutcome::Appended` already proves the current invocation wrote the resolution and already returns success, so the work is entirely in the **pre-existing-resolution** branch — the `else` arm at `recovery.rs:280-292` that yields `RecoveryRejection::IncursionIncidentAlreadyResolved`, not the `Display` text at `recovery.rs:781`.
...
Responsibility means **equality of the resolved worktree and coordination-run ids as the journal recorded them** — not the same process invocation, which the journal cannot express, and not a claim that the recorded actor is the physically correct one. Phase 6 centralized identity resolution and instrumented its inputs, but the historical misattribution it was written to explain never reproduced and remains undiagnosed; nothing here may assume a corrective fix landed. Compare the ids the journal holds. Outcomes:
...
**Constraints from Phase 6:** `ledger::resolve_identity(&WorktreeContext)` is the single identity entry point and returns `ResolvedJournalMutationActor`, carrying `worktree_id`, `coordination_run_id`, and the `EditAuthorization` resolved in that same read. Take the acting actor from it once; do not resolve identity a second time inside the resolve path. The retained `JournalActor` on `IncursionIncidentStatus::Resolved` is compared against those ids.

**Files:**
- `crates/cargo-berth/src/recovery.rs` — the three outcomes (~L105, ~L262, ~L280-292)
- `crates/cargo-berth/src/reservation/mod.rs` — `IncursionIncidentStatus::Resolved` retains the actor (~L414)
- `crates/cargo-berth/src/ledger/journal.rs` — `JournalActor`, the retained actor type (~L116)
...
- `crates/cargo-berth/src/gate/mod.rs` — same (~L1165); retire the second identity read spanning ~L644 and ~L722

...
**Generate correlated outcome tuples, not four independent inventories.** Phase 12 proved the failure this distinction exists to stop: a legal outer status paired with a legal payload kind can still carry a rejection sub-status underneath a *success* envelope, and two consumers classified on the inner status alone — so an engine success rendered as a refusal while the command exited 0. Separate lists of statuses, exit codes, payload kinds, and nested statuses cannot express that, because every one of those values is individually legal. The generated contract's unit is therefore the whole tuple — verb, envelope status, exit code, payload kind, and nested payload status together — with the legal combinations enumerated and everything else rejected. Retained legacy outcomes are tuples too, marked decodable-but-unemitted rather than omitted. Generate negative fixtures for exactly the Phase 12 shape: a success-tagged envelope carrying a coordination-identity rejection sub-status, and a success-tagged envelope carrying a foreign-actor resolution sub-status, each proving a consumer keyed on the tuple refuses to read it as a refusal.

**Phase 13's `reservation` payload is two wire shapes under one payload kind, and the contract must say so.** `ReservationLifecycleQueryPayload` (`output.rs`) is `#[serde(untagged)]`, so the success shape is `(board, board_ready, 0, reservation)` carrying `data.lifecycle.status` — one of `active`, `outstanding`, `released_after_checkpoint`, `released_without_checkpoint` — while the rejection shape is `(board, invalid_input, 5, reservation)` carrying `data.status = unknown_reservation` and **no `lifecycle` member at all**. `data.status` does not exist on the success shape. Enumerate these as two semantic payload-shape alternatives of the `reservation` kind, not as one shape with an optional nested status: an optional member would make a mixed envelope — a `board_ready` success carrying `data.status`, or an `invalid_input` rejection carrying `data.lifecycle` — legal to both shapes at once, which is the same class of ambiguity the outcome tuple exists to remove. Generate positive fixtures for the rejection shape and for all four lifecycle tags, and a negative fixture for the mixed shape that every consumer must reject.
...
2. Rust consumers match **exhaustively, with no wildcard arms** — remove the one at `output.rs:1547`.
...
**Generated schema validation is added beside the semantic checks, never in place of them.** This phase edits all three canonical shims, so the whole of Phase 12's external oracle is inside its blast radius. `valid_live_incursion_state` in `berth_post_bash.sh` asserts something no schema can: that an incident is in exactly one of the board's outstanding and recorded sections, so a contradictory board fails closed rather than silently suppressing the stop text. A generated fragment validates shape; that check validates agreement between two sections. Keep it, and keep every other Phase 12 semantic assertion, as a gate this phase inherits.
...
**Files:**
- `crates/cargo-berth/src/output.rs` — the declaration table, exhaustive matches, no wildcard; the typed replay-failure payload and `ReplayFailureSubject` (`OutputEnvelope` L74, `CommandVerb` L94 with `Identity` L116, `OutputStatus` L130 with `SessionMappingCleared` L196 and `SessionMappingUnavailable` L198, `OutputFacts` L217 with `Reservation` L227, `Identity` L247 and `CoordinationIdentity` L249, integration's nested rejection L420, the wildcard arm L1547)
- `crates/cargo-berth/src/coordination_identity.rs` — `CoordinationIdentityRejection` and its recovery-action types, generated in both their top-level and integration-nested placements
- `crates/cargo-berth/src/alert.rs` — `LostIntegrationEvidenceStatus`, three variants, converted at the construction site
- `crates/cargo-berth/src/reservation/mod.rs` — `ReservationReplayError` (L1960) surfaces its reservation and reason to the payload, generated exhaustively rather than variant by variant; `ReservationLifecycleSnapshot` (L529) derives `JsonSchema` and pins the four lifecycle wire names
- `crates/cargo-berth/src/reservation/evidence.rs` — `ProtectedReservationTip` (L27), reached recursively from `ReservationLifecycleSnapshot`; derives `JsonSchema` and pins its wire schema name
- `crates/cargo-berth/src/reservation/lifecycle.rs` — `ReleaseDisposition` (L179), reached recursively from the two released lifecycle alternatives; derives `JsonSchema` and pins its wire schema name
- `crates/cargo-berth/src/ids.rs` — `ReservationId` (L105, minted through `uuid_identifier!`); the lifecycle query payload carries it in both wire shapes, so the identifier macro must produce a `JsonSchema` impl with a pinned name rather than a per-type Rust identifier
- `crates/cargo-berth/src/ledger/journal.rs` — the journal operation inventory the contract enumerates, including Phase 3's `ScopedPatchEquivalenceChecked` (L404) and `ScopedPatchComparisonAttempted` (L415), plus `CanonicalWorktreeRoot` (L1012), whose schema this contract must pin
- `crates/cargo-berth/Cargo.toml` — `schemars` as a normal dependency; the DTOs derive `JsonSchema`
- `/Users/natemccoy/.claude/scripts/berth/claim_state.py` — generated tables replace the hand-kept ones (~L64, ~L107)
- `/Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` — Phase 12's external shim oracle, rerun in full against the generated fragments
...
**Whichever model wins must also own the engine, not only the scripts.** The shims and `claim_state.py` resolve whatever `cargo-berth` is on `PATH`, and Phase 12 proved that binary has already drifted behind this plan's build. A canonicity model that keeps the scripts current while leaving the engine stale reproduces the same split it exists to close, so the answer must say who installs the engine and when. Phase 13 installs it as an ordered step for its own selector; this decision settles whether that stays an ad-hoc step or becomes part of a single owned install.

**Constraints from prior phases:** **Must follow Phases 1, 4, 5, 6, 7, 11, 12, and 13** — every one adds statuses, journal records, or payload variants that this contract must enumerate. Phase 4 is on that list because it adds the distinct successor-equivalence cache records, and the contract cannot be frozen before they exist. Specifically: Phase 1 added `ReservationReplayError::{WidenRequiresUnreleased, ResnapshotRequiresOutstanding}` — the widen failure is named for the release boundary, and Phase 2 widened it to accept an `Outstanding` reservation, so every fixture and message must say "unreleased", never "active" — both currently untyped in the envelope, and retained `reblocked_active_constraint` as a reserved-but-unreachable board wire value; Phase 2 added the nested `IntegrationProof` variants and the untagged `trunk_at_claim` alternatives, which the generated inventory must cover; Phase 3 added the journal records `ScopedPatchEquivalenceChecked` and `ScopedPatchComparisonAttempted` (`ledger/journal.rs:404`, `:415`) and the types they carry — `IntegrationProofSubjectRevision` (`reservation/mod.rs:76`) and `ScopedPatchEquivalenceVerdict` (`reservation/mod.rs:85`) — all of which the generated journal inventory must enumerate, plus the three replay hard stops named in the Spec; Phase 4 adds **two** journal records, not one — `SuccessorScopedPatchEquivalenceChecked` (`ledger/journal.rs:424`) and `SuccessorScopedPatchComparisonAttempted` (`:435`) — plus the nested `SuccessorScopedPatchEquivalenceVerdict::{Equivalent, Different}` wire values they carry; Phase 5 added the lost-evidence alert with its two `LostEvidenceRecovery` variants; **Phase 7 added three `resolve` payload statuses, not two** — `recorded_now` and `already_recorded_by_same_coordination_actor` under status `incursion_resolved`/exit 0, and `already_recorded_by_different_coordination_actor` under status `invalid_input`/exit 5 with payload `kind` `resolve` carrying `reservation_id`, `incident_id`, `resolving_worktree_id`, `resolving_coordination_run_id`, `resolution_event_id`, and `resolved_at`; it also replaced `RecoveryRejection::IncursionIncidentAlreadyResolved(IncursionIncidentId)` with `IncursionIncidentAlreadyResolvedByDifferentCoordinationActor { reservation_id, incident_id, resolving_actor, resolution_event_id, resolved_at }` (`recovery.rs:751`), added `IncursionResolutionNotAppended` (`recovery.rs:774`), added `resolving_actor: JournalActor` to `IncursionIncidentStatus::Resolved` (`reservation/mod.rs:416`), and left `ResolvePayload::IncursionResolved` (`output.rs:451`) with no producer — deliberately retained so older envelopes stay decodable, and the generated contract must express it as decodable-but-unemitted rather than dropping it. **`invalid_input` now admits the `resolve` payload kind**: the hand-kept `STATUS_PAYLOAD_KINDS` had to be edited by hand before the coordinator would accept the new envelope at all, which is direct evidence for generating these tables; **Phase 11 added a complete generated-contract surface, not one type**, and the constraints must enumerate all of it: the verb `CommandVerb::Identity` (`output.rs:116`); two new statuses, `SessionMappingCleared` and `SessionMappingUnavailable` (`output.rs:196`, `:198`), the first reached by two distinct outcomes and the second carrying exit `UsageError`; all three `IdentityPayload` outcomes — `SessionMappingRemoved`, `SessionMappingAlreadyAbsent`, and `CurrentSessionUnavailable` (`output.rs:521`) — which are the only thing that distinguishes clearing a mapping from clearing nothing, because two of them share a status; the top-level `coordination_identity` payload (`OutputFacts::CoordinationIdentity`, `output.rs:249`) carrying `CoordinationIdentityRejection` with its typed `recovery_actions`, emitted under status `invalid_input` and exit code 5; and **the same rejection nested inside the integration payload** (`output.rs:420`), which is a second wire placement of one type and must be generated as such rather than as a separate shape. **Every published `recovery_actions` `argv` is directly executable**, and an action whose command cannot be reproduced faithfully is omitted from the set rather than degraded — so the generated schema describes a non-empty list of runnable commands whose membership varies, never a fixed roster; **Phase 11 also added `GateError::LegacyReferenceTransactionHook`**, raised when a hook installed before issuing-directory capture reports no issuing checkout — `integrate` currently folds it into `ledger_unreadable` (`verb/integrate.rs:197`) alongside every other unbucketed gate error, which tells a consumer routing on status alone to send the operator to journal review when the actual repair is rerunning `cargo-berth init`. Give it a status whose meaning is "the installed hook is out of date", so the recovery is derivable without reading `message`; **Phase 13 added two outcome tuples under one payload kind, not one type**: `board` / `board_ready` / exit 0 / payload kind `reservation` carrying `data.lifecycle.status` in `{active, outstanding, released_after_checkpoint, released_without_checkpoint}`, and `board` / `invalid_input` / exit 5 / payload kind `reservation` carrying `data.status = unknown_reservation`. `invalid_input` therefore now admits the `reservation` payload kind, exactly as Phase 7 made it admit `resolve`. The payload enum is `#[serde(untagged)]`, so the two shapes share no discriminant member and the recursive types `ProtectedReservationTip`, `ReleaseDisposition`, and `ReservationId` are all reachable from it. **Phase 6 added the journal record's optional `identity_inputs` envelope**, and the generated contract must express three things about it: the field is absent on records written before it existed and on every record written by an older binary, so the schema makes it optional rather than required; when present its `status` is `recorded`, whose five members are `invocation_directory`, `cargo_berth_session_id`, `cargo_berth_run`, `git_dir`, and `git_common_dir`; and each member is itself a tagged state — the directory as `utf8`/`too_long`/`non_utf8`/`unavailable`, each environment value as `unset`/`utf8`/`too_long`/`non_utf8`, with `too_long` carrying only `observed_bytes`. The Rust `Unrecorded` variant is an internal absence marker that never serializes; it must not appear in the generated contract as a wire value. Unifying the contract before these land means doing it twice. **Phase 10 added an `origin` field to every incursion commit in the `drift` payload**, and the generated contract must enumerate its three wire values — `phase_authored`, `already_on_trunk`, and `unknown` — alongside the existing `commit`, `subject`, and `paths` members of `payload.data.results[].effects[].commits[]`. `unknown` is not an error state: it is emitted whenever the trunk cannot be resolved or the origin query fails, and the report is still delivered, so the contract must express it as a normal value of a complete response rather than a degraded one. `docs/cargo-berth/json-contract.md` already documents the field and is the hand-kept source this phase replaces.

**Acceptance gate:** **Every `Test` command in Delegation Context** green. Adding a new `OutputStatus` variant fails engine tests until the generated Python and `jq` artifacts are regenerated and checked in. Malformed status/payload/exit combinations remain rejected. Python and `jq` validators execute against generated valid and invalid fixture envelopes. No hook invocation reads a manifest at runtime. A fixture per replay failure proves each is distinguishable from the payload alone, names its exact typed subject, and identifies itself as a hard stop without reading `message` — starting with a `Release` → `Widen` journal, a `Released` `Resnapshot` journal, and Phase 3's three (`IntegrationProofSubjectRevisionExhausted`, `ActiveScopedPatchComparison`, `IntegrationProofSubjectMismatch`). Adding a variant to `ReservationReplayError` must fail engine tests until the generated reason inventory and every consumer artifact regenerate; a test asserts the generated inventory covers the enum exhaustively rather than a hand-kept subset. The generated journal inventory likewise covers every `JournalOperation` variant, including Phase 3's `ScopedPatchEquivalenceChecked` and `ScopedPatchComparisonAttempted` and Phase 4's `SuccessorScopedPatchEquivalenceChecked` and `SuccessorScopedPatchComparisonAttempted`, including the nested `SuccessorScopedPatchEquivalenceVerdict::{Equivalent, Different}` wire values. The generated board contract accepts `reblocked_active_constraint` as a reserved value while no fresh engine fixture emits it. **Generated fixtures cover all three of Phase 7's `resolve` outcomes with their exact combinations** — `recorded_now` and `already_recorded_by_same_coordination_actor` as status `incursion_resolved`/exit 0/payload kind `resolve`, and `already_recorded_by_different_coordination_actor` as status `invalid_input`/exit 5/payload kind `resolve` carrying all six typed fields — and the generated Python tables admit `invalid_input` with payload kind `resolve`, which the hand-kept table did not. `ResolvePayload::IncursionResolved` is accepted for decoding and absent from every fresh generated output, so a retained-for-compatibility variant cannot silently become emittable again. `generated_contract_covers_identity_clear_session_outcomes` proves the contract enumerates `CommandVerb::Identity`, both new statuses, and all three `IdentityPayload` outcomes, and that a fixture distinguishes a removed mapping from an already-absent one from the payload alone. `generated_contract_covers_coordination_identity_rejections` proves all three rejections appear in both wire placements — top-level `coordination_identity` and nested under the integration payload — as one generated type rather than two, and that a generated invalid fixture rejects a recovery action whose `argv` is empty. `generated_contract_covers_both_reservation_lifecycle_wire_shapes` proves the contract enumerates the `reservation` payload kind as two outcome tuples — `board_ready`/exit 0 with all four lifecycle tags, and `invalid_input`/exit 5 with `unknown_reservation` — that the generated Python tables admit `invalid_input` with payload kind `reservation`, and that a generated negative fixture mixing the two shapes (a success envelope carrying `data.status`, and a rejection envelope carrying `data.lifecycle`) is refused by the Rust, Python, and `jq` consumers alike. `generated_contract_covers_both_lost_evidence_wire_forms` proves the generated contract enumerates both `LostEvidenceRecovery` variants in the envelope form and the flattened board form, and generated invalid fixtures prove an `integrated` evidence status is rejected in both — the wire value `LostIntegrationEvidenceStatus` makes unrepresentable. A test renames a Rust type and asserts every generated artifact is byte-identical, proving definition names are pinned to wire names rather than tracking Rust identifiers. **Phase 12's external shim suite is rerun in full against the generated fragments**, every case still passing — the identity-rejection renderings, both lost-evidence forms, the typed foreign-actor rejection, the recorded and outstanding incursion cases, the malformed board, the contradictory board, and the constant board-read cost — and `valid_live_incursion_state`'s exclusive-membership check survives the rewrite rather than being replaced by schema validation. **Generated fixtures are outcome tuples, not independent values:** a fixture asserts that a legal verb, envelope status, exit code, and payload kind combined with an illegal nested status is rejected, and two negative fixtures carry a success-tagged envelope with a coordination-identity and with a foreign-actor resolution sub-status, each proving a tuple-keyed consumer does not read it as a refusal. The generated inventory also proves that **nested** enum variants stay synchronized across all three consumers, not only top-level statuses: `IntegrationProof::{ProtectedTipAncestor, ScopedPatchEquivalent}` inside `IntegrationEvidenceStatus::Integrated`, and the untagged `trunk_at_claim` alternatives, must appear in the Rust envelope, the generated Python tables, and the generated `jq` validators, and adding a variant to any nested enum must fail engine tests until all three regenerate.
...
- `/Users/natemccoy/.claude/scripts/berth/tests/test_hook_rendering.py` — created by Phase 12, extended by Phase 13 to eleven fixtures; extended here with the classifier fixtures this phase's gate names

...
`Edge::readiness` (`edge/mod.rs:317`) asks whether the successor contains the predecessor's **original protected tip**:
...
**The equivalence predicate must accept more than one target.** `predecessor_descendants` (`reconcile.rs:1338`, called from `reconcile.rs:640`) currently classifies **all** successor heads with one batched reachability invocation, while Phase 2's scoped-equivalence predicate accepts a single target commit. Taking the shipped single-target form as-is turns one batched call into one call per successor and loses the batching Phase 2 established. Either widen the predicate to multiple targets in one invocation or bound the per-pass invocation count some other way, and prove it at scale rather than on a single edge.

**Bounding the comparison per target is not enough here.** Phase 3's `ScopedPatchEvaluationMemo` (`reconcile.rs:328`) admits **one** scoped comparison per trunk target, which bounds a single-trunk reconciliation but does not bound this phase: twenty distinct successor heads are twenty distinct targets, so a per-target budget still buys twenty comparisons. This phase must therefore do one of two things, and say in code which:

- **Batch all successor targets into one invocation**, so the argv count is fixed regardless of how many heads are pending; or
- **Persist a fixed-budget round-robin schedule** over successor targets, reusing Phase 3's shipped `ScopedPatchEvaluationPriority` / attempt-history model (`reservation/mod.rs:137`, `reservation/mod.rs:154`) so a target that loses a pass is preferred on the next one.

If the round-robin form is chosen, the schedule must guarantee **eventual coverage** — every pending successor target is compared within a bounded number of passes — and must not starve on a target whose comparison returns `ObjectUnknown`, which is never cached and therefore always re-eligible. An edge whose comparison was deferred rather than run keeps reporting `AwaitingSuccessorIncorporation`; a deferral never reads as incorporation.

**`Edge::readiness` is not where the git work happens.** It reads a `repository_snapshot`; the grouped reachability facts in that snapshot are produced by `reconcile.rs::predecessor_descendants` (`reconcile.rs:1338`, called from `reconcile.rs:640`). The equivalence check and its cache both belong there, or the batching Phase 2 established is lost to one query per edge.

Cache a typed successor-equivalence verdict — positive and negative alike — keyed by the predecessor's `IntegrationProofSubjectRevision` and the successor head. This is a distinct key from Phase 3's trunk-keyed `ScopedPatchEquivalenceCache` and needs its own persistent owner alongside it, with its own bounded retention constant beside `SCOPED_PATCH_TARGET_RETENTION_LIMIT` (`reservation/constants.rs:6`).

**Files:**
- `crates/cargo-berth/src/edge/mod.rs` — `Edge::readiness` consults the snapshot's equivalent-content outcome (L317)
...
- `crates/cargo-berth/src/edge/graph.rs` — exhaustive graph replay must handle the new successor-cache journal operations
- `crates/cargo-berth/src/reconcile.rs` — `predecessor_descendants` produces the equivalence verdict in the same batched pass (L640, L1338); the successor budget sits beside `ScopedPatchEvaluationMemo` (L328)
...
- `crates/cargo-berth/src/reservation/constants.rs` — the successor cache's bounded retention limit
- `crates/cargo-berth/src/gate/mod.rs` — successor-cache operations must survive the committed-hook entering filter (`entering_reservations`, ~L801)
...
**Constraints from prior phases:** Phase 2 defined scoped patch equivalence and exposed it as a reusable predicate taking `phase_start_head`, a `ReservationScopeSet`, a protected tip, and a target commit; the baseline is `phase_start_head` only, never the checkpoint trunk snapshot. **That predicate accepts one target and composes roughly a dozen git invocations per evaluation**, so calling it per successor is what this phase must avoid. Phase 3 shipped the shared proof-subject and cache model, and these are its **actual** names: `IntegrationProofSubjectRevision` (`reservation/mod.rs:75`) advances on `Widen`, `Resnapshot`, and release-disposition replacement; `ScopedPatchEquivalenceCache` (`reservation/mod.rs:122`) retains bounded definitive verdicts and `ScopedPatchEquivalenceCacheLookup` (`reservation/mod.rs:128`) answers `Hit`/`Miss` as a semantic state rather than an absent value; `ScopedPatchEquivalenceVerdict` (`reservation/mod.rs:84`) is the cached verdict and `DurableScopedPatchComparison` (`reservation/mod.rs:94`) its reusable projection; only definitive verdicts are cached, never an `ObjectUnknown` comparison that could not run; and the cache is journalled and replayed. Phase 3 also shipped the per-target scheduling primitives this phase reuses: `ScopedPatchEvaluationPriority` (`reservation/mod.rs:137`), the attempt history behind it (`reservation/mod.rs:154`), and the per-reconciliation `ScopedPatchEvaluationMemo` (`reconcile.rs:328`), whose budget is **one comparison per trunk target** — a bound that does not by itself bound a phase with many targets. Follow all of these here. The successor path needs its own cache key, because it compares against the successor head rather than trunk. **This phase runs after Phase 3** because it reuses that model rather than inventing a second one.

**Acceptance gate:** **Every `Test` command in Delegation Context** green, with a fixture where the successor carries equivalent rewritten content and does **not** contain the predecessor's original protected tip, and the edge reports `Fulfilled`. A negative fixture where the successor lacks the content keeps `AwaitingSuccessorIncorporation`. A `git` shim proves both verdicts are cached: one equivalence query on the first readiness call and **zero** across twenty subsequent calls at unchanged successor head and predecessor proof subject, for the positive and the negative fixture alike, and both survive a process restart from the journal alone. An `ObjectUnknown` comparison is **not** cached. The shim reports **exact total git argv** for **one** pending successor head and for **twenty** pending successor heads on a cold pass, and the two totals must be equal — not merely sublinear. If the round-robin form is chosen, a further fixture drives repeated passes over twenty pending heads and asserts every head is compared within a bounded number of passes, that a head whose comparison returns `ObjectUnknown` does not starve the others, and that every head still awaiting its turn reports `AwaitingSuccessorIncorporation` rather than `Fulfilled`.
...
**Validate the phase-start ancestry precondition, in one batched call.** When the phase start is not an ancestor of HEAD, `<phase_start>..HEAD` sweeps in unrelated commits reachable only from HEAD, and a fixed subprocess count would then produce wrong attribution faster. Return a typed stale-anchor result or reanchor before attributing; `drift/git_output.rs` owns that result type, since it already owns how a git invocation's outcome is typed for this module. Do **not** issue one ancestry query per phase start — `git::reachability_to_target` (`git/mod.rs:1182`) already takes a **slice** of candidate ancestors and one target and answers all of them in a single batched invocation. Reuse it; a per-anchor loop reintroduces exactly the scaling this phase exists to remove.
...
**Project it from `ReservationEvidenceState`; do not restate the lifecycle rules.** `Reservation::evidence_state` (`reservation/mod.rs:1609`) already returns exactly these four classifications — `Active`, `Outstanding`, `Released`, `ReleasedWithoutCheckpoint` (`ReservationEvidenceState`, `reservation/mod.rs:377`) — each carrying its protected tip where one exists. Map from it and drop the evidence fields this caller does not need. A second hand-written lifecycle match is a second place for the Phase 1 invariant to drift out of.

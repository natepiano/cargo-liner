...
3. `compare_mapped_phase` runs scoped replay with `ScopedPatchTargetHistory::MappedDestinations { commits }`. The mapped variant skips `target_scoped_change_position`: an empty destination list is `Unproven`, and a non-empty one is `Contiguous`. Shared-history and protected-change queries still run.
4. `Equivalent` appends `Resnapshot { Outstanding { protected_tip, trunk_oid, phase_start_head: Some(..) } }`. `Different` records a `CompletedRewriteSubject`. `Unavailable`, `HistoricalEvidenceUnavailable`, a budget deferral, or unreadable history defers the subject and keeps the marker with its progress.

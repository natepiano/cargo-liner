...
| --- | --- |
| `verb` | String. A consumer accepts unfamiliar command names. |
| `status` | String. A consumer accepts unfamiliar status names. |
...
A consumer accepts additional envelope members. In particular, `reservations`,
`payload`, and `output_contract_version` remain available to specialized
readers, but none belongs to the stable front-end shell. Payload contents never
determine whether the front end may use `exit_code`, `message`, `blocked_by`,
or `presentation`.
...
released reservations are always `clear`. Consequently, a row never pairs
`lifecycle.stage = released` with `edit_blocking_status = blocking`.
...
- `enrolled[]`: `{ "reservation_id", "worktree_root", "branch" }` for each
  ...
  the full ref, or `detached at <sha>`.
- `overlaps[]`: `{ "first_reservation_id", "second_reservation_id",
  "shared_scopes", "sequence_commands" }` for each enrollment pair still
  awaiting a `sequence` answer, including pairs whose reservation has ended.

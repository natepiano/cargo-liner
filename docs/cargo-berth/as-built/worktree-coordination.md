...
`worktree/` classifies whether a reservation's holder still exists. `reconcile.rs` runs the reconciliation pass that turns a dead holder into an alert. Nothing is removed on inference: reconciliation records what it observed and raises an alert, and abandoning, retiring, or recovering a reservation takes a user action. The one release reconciliation performs itself is settlement: an `Outstanding` reservation whose evidence is `Integrated` against the actual trunk, with no unproven reserved work in its merge extent, is released as `Integrated` or `RewrittenIntegration(witness)` in that pass. The committed hook and evidence against a proposed trunk never settle.
...
**Nothing is removed on inference.** The tool can prove a worktree is gone; it cannot prove the work is abandoned. Auto-removing a reservation on that evidence would silently discard someone's claim on the strength of an inference. An alert plus an explicit `resolve` keeps the decision with the person who has the missing context. Settlement is the one automatic release, and it rests on proof: git shows the whole scoped phase on the trunk actually checked out, and the merge guard shows no reserved work outside that proof.

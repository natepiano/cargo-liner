...
- `OutputStatus::InvalidInput` carries two conditions on the post-commit path: a request that never ran, and a run that completed a report and was refused only its acquisition. The presented text separates them; the status does not, so a consumer keying on status alone cannot tell them apart.

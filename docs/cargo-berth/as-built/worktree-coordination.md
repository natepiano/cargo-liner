...
For a present non-trunk target checked out in a registered worktree, reconciliation keeps a cover reservation in that checkout. The cover targets the integration branch's own parent and protects the branch's whole committed diff plus dirty paths. An existing non-released reservation in that checkout with the same recorded parent target also covers it, regardless of its claim source. A new engine cover takes the checkout's coordination-run marker or creates and publishes one; it publishes no session mapping, so an unmapped session joins through the marker. The cover extent refreshes each pass as lanes land. Lanes targeting the branch settle only when an existing cover was observed at that branch's current tip in the pass. A cover created in the pass qualifies on the next pass. When the branch lands on its parent, the cover's empty extent ends its run. Missing targets need no cover; a present target checked out nowhere raises `target_uncovered`.

Overlap containment compares a foreign holder's committed remainder with the acting checkout's HEAD. When the holder's recorded target differs from the acting target, it also limits that remainder to paths the holder would bring to the acting target's tip. Dirty paths always remain protected. A lane targeting its integration branch therefore sees no committed overlap from that branch's cover; a checkout acting from `main` meets the cover's full diff as an ordinary overlap. The edit check reads target refs from loose or packed files and keeps its lock-free path.
...
The two schemas version independently. The journal's `CURRENT_SCHEMA_VERSION` is 2 and every record must carry it, while the projection owns `CURRENT_PROJECTION_SCHEMA_VERSION`, at 3. `read_once` reads a small `ProjectionSchemaHeader` and validates its version before decoding the rest, mirroring the journal's own header-first read. `read_validated` answers `ProjectionSynchronization::RebuildRequired` for a missing file and for any schema version other than the current one, so an unreadable cache is discarded and rebuilt rather than failing the command. Malformed bytes, a repository-identity mismatch, a cache ahead of the journal, and a fingerprint mismatch stay fatal, because none of them establishes that the file is a readable cache for *this* repository.
...
`Claim` carries the origin of the reservation as `ClaimSource::{WorkPlan, FirstTouch, Explicit, Enrolled}` — a claim made under a named plan and phase, one created by first touch, one a user stated outright, or one `init` enrolled from a worktree's existing changes (see [worktree-enrollment.md](worktree-enrollment.md)). `Widen` carries a reason distinguishing drift-driven widening from an explicit one.
...
`resolve_from_sources` consults the harness session mapping first, then `CARGO_BERTH_RUN`, then the on-disk coordination-run marker, and falls back to `Unidentified`. `session/mod.rs` keeps `session-identities.json` current: `apply_journal_event` publishes a mapping on `Claim` (except a `ClaimSource::Enrolled` claim, which `init` makes for other worktrees) and `Widen`, and retires every entry pointing at the reservation on `Checkpoint` and `Release`. Marker removal reports `CoordinationRunMarkerRemoval::{Removed, AlreadyAbsent, PreservedDifferentRun, PreservedMalformed}` — a marker belonging to another run or one that cannot be parsed is left in place rather than deleted.
...
Both terms of the rule now arrive in the types. The holder's is its recorded `CoordinationIdentityProvenance`, read inside the predicate. The acting side is a `PresentedCoordinationRun`, threaded through `validate_worktree_occupancy` and `RetainedReservationSet::worktree_occupancy`: its field is private and its only two constructors name the two places a caller can present a run — `from_run_argument` for the `--run` flag, reached once at the `cli` claim conversion, and `from_edit_authorization`, which yields `Some` for `EditAuthorization::Environment` and `None` for a session mapping, a marker, and an unidentified caller. So the three call sites obtain their acting side there rather than each repeating a variant match, and a fourth cannot ask the question one-sidedly: a bare `CoordinationRunId` does not convert. A caller presenting nothing is still never refused, and that narrowness is deliberate — the engine's own markerless post-commit first touch reaches this same validation, so refusing an unpresented identity would refuse the engine itself.

**The resolution order is a precondition of the rule, not a tie-break.** `resolve_from_sources` consults a live harness session mapping before `CARGO_BERTH_RUN`, so a caller inside a mapped session resolves to `EditAuthorization::Session` whatever it exports, presents no run, and is never asked the occupancy question at all. Reversing that order would put every mapped session one exported variable away from refusing itself in its own worktree. `a_live_session_mapping_outranks_a_presented_run` pins it through `PresentedCoordinationRun`, which is what the rule actually consults.

The overlap chain still consults no acting-side term, and that asymmetry stays — neither `AuthorizedEditingIdentity::is_foreign`, which `conflicts_for_edit` and `conflicts_for_first_touch` reach the predicate through, nor `conflicts_for_drift`, `blocking_coverage_for_drift` and `bind_widened_scopes`, which read `is_foreign_to_coordination_run_in_worktree` directly; every hop carries a bare `CoordinationRunId`. A same-worktree holder that presented an identity is foreign to any other run there, whatever that run presented.
...
Claims from older journals replay as `RecordedTarget::Unrecorded`. `init` appends one `unrecorded_targets_pinned` event when any remain, assigning the repository trunk at that moment. `retarget <reservation> --target <branch>` appends `retarget` for an unreleased holder, replacing its target and comparison commit and clearing its retained integration proof. Reconciliation and release evaluate each reservation at its recorded target. `resolve --integrated-as` checks reachability from that target after replay. A missing local target ref makes the repository trunk the judging branch for these operations and rewrite re-anchoring; a Git read failure does not trigger that fallback. Drift uses the acting session reservation's target for committed origin attribution, or each reporting reservation's target for a run without a mapped session; it batches origin reads by distinct target. The reference-transaction gate evaluates the repository trunk and every live recorded target. Every unreleased reservation whose commit newly reaches the repository trunk enters its gate decision, including reservations recorded against another target; a non-trunk proposal enters only reservations judged there. `gate-targets` is a sorted, replay-derived shell filter under the shared ledger directory; the binary makes decisions from the journal. Confirmed journal reinitialization republishes that filter from the empty replay. A prepared transaction moving two gated refs is refused before any gate record is appended.
...
`Released` is terminal. `resnapshot` accepts only `Outstanding` and `apply_widen` accepts only `Active`; a journal that resnapshots a released reservation fails replay.

Two dispositions rest on a decision rather than on a checkpoint's protected tip, so `release_without_checkpoint` admits `Abandoned` and `RetiredOrphan` from `Active` as well as from `Outstanding`. `RetiredOrphan` has a second source besides the `resolve --retire-orphan` verb: `reconcile::append_orphan_retirements` writes it for an `Active` holder that `WorktreeRegistry::classify` answers `Orphaned` for, and whose merge extent carries a completed observation of emptiness — `MergeExtent::proved_empty_key` — taken under a key whose head was trunk with no tracked and no untracked path. Without it such a reservation has no ending available at all: `Reservation::is_terminal` requires `!is_active()`, `ReservationLifecycle` leaves `Active` only at a checkpoint or a disposition, and a deleted worktree can never observe again, so the row stays an `active_constraint` with `edit_blocking_status: blocking` until a user runs `resolve`. The guard is a strict conjunction and every other liveness retains the reservation, including the prunable `OrphanCandidate` and the locked `Unavailable`: a retained dead row costs a reader one puzzled look, while retiring a live holder stops protecting its work in every session at once. An `Outstanding` orphan is retained too — it checkpointed a commit, and `alert::for_orphaned_outstanding` reports it with a `RecoverabilityVerdict` so the work can be recovered.
...
Ancestry is the fast answer, not the only one. When a reservation's protected tip stops being an ancestor of trunk, the change the reservation made *inside its own scopes* — measured from `phase_start_head` — is compared against trunk history: first against the earliest historical trunk commit that could contain it, then against current trunk. An amended, rebased, or squashed commit whose scoped content survives still certifies; the same paths carrying different content do not. `IntegrationEvidenceStatus::Integrated { trunk_oid, proof, witness }` carries the trunk the evidence was evaluated against, the proof as `IntegrationProof::{ProtectedTipAncestor, ScopedPatchEquivalent, RewrittenWitnessAncestor}`, and the commit containing the work as `IntegrationWitness::{EvaluatedTrunk, Historical(..)}`. Records written before `proof` decode as `ProtectedTipAncestor`, and records without `witness` as `EvaluatedTrunk`. A released `RewrittenIntegration` revalidates by one ancestry query against its witness and never reaches scoped replay. `docs/cargo-berth/as-built/rebase-settlement.md` covers candidate discovery, the witness, settlement, and rewrite capture.
...
The proof is cached against the pair that produced it. `IntegrationProofSubjectRevision` versions the baseline, protected content, and scopes a proof was checked under, and advances on `Widen`, `Resnapshot`, a `RewrittenIntegration` release, and release-disposition replacement — never on ordinary revalidation. `RetainedScopedPatchTargetVerdicts` retains definitive verdicts (`Integrated`, `NotIntegrated`, `TrunkRewritten`), each with its witness and `ScopedPatchEvaluatorVersion`, for the two most recent targets; lookup ignores a `Legacy` negative so the subject is reevaluated under current rules. An `ObjectUnknown` comparison is never cached, because it is a transient environment fact and storing it would make one failed subprocess durable across restarts, and a historical proof that could not be evaluated is journalled as an attempt rather than a negative verdict. The cache and its schedule reach the journal as `ScopedPatchEquivalenceChecked` and `ScopedPatchComparisonAttempted`, so both survive a restart from replay alone.

Reconciliation admits **one** cold scoped comparison per trunk target per pass; historical candidate discovery, certification, current-trunk replay, and mapped rewrite acceptance share that slot. Targets that lose the slot are scheduled round-robin over a bounded attempt history, so a skipped subject is preferred next pass and a subject whose comparison keeps returning `ObjectUnknown` cannot starve the others. A deferral is not neutral: `DeferredScopedPatchIntegrationStatus` decides what the materialized evidence still proves — `StillValid` only for an equivalence proof bound to the trunk actually observed, and `Degraded` to `NotIntegrated` both for a protected-tip proof reachability has just refuted and for an equivalence proof bound to an earlier target. Degradation is durable: the `EvidenceRevalidated` append precedes the schedule update, so the correct answer is the one that replays.

Successor edges use the same proof on a different axis. `SuccessorIncorporationEvidence` — `ProtectedTipAncestor`, `ScopedPatchEquivalent`, `NotIncorporated`, `ObjectUnknown` — is reported per predecessor as `PredecessorSuccessorIncorporation`. The subject is the predecessor's checkpoint tip, or the witness commit for a `RewrittenIntegration` release; only a checkpoint tip enters scoped comparison, so a successor holding equivalent content without the witness commit keeps holding. `Edge::readiness` reads only the snapshot; the git work assembles every predecessor's subject into one `descendant_commits` call. Successor verdicts have their own cache, keyed by the predecessor's proof-subject revision and the successor head, with its own retention limit (512, against 2 for trunk targets) because twenty heads are twenty targets. A deferred head keeps reporting `AwaitingSuccessorIncorporation`: a deferral never reads as incorporation.

Git cost is bounded by batching, and the standard is exact argv equality at one subject and at twenty rather than a sublinear trend. Predecessor ancestry, worktree ahead/behind, retention-ref availability, and retention-ref repair are each one invocation regardless of count. Incursion attribution runs the path log over every committed path a present foreign holder covers and the `rev-list --parents` graph giving every anchor's range membership concurrently; it then reads origin membership once per distinct reservation target, using the acting session's reservation or each reporting reservation for run or unidentified identities, and only when some anchor is `UsableAncestor`. They run before the ledger lock, in `provenance::read_committed_foreign_paths`, over the paths `PreLockForeignPathClassification` found, and not at all when it found none; classification under the lock and the report naming after it both read that one batch, so the lock holds no git. `IncursionAttributionAnchorState` (`UsableAncestor` / `NotAncestorOfHead` / `ObjectUnknown`) records each anchor's relation to the target, so an unreadable anchor reports nothing rather than defaulting into a false `Unchanged`.
...
An edge between reservations judged at the same branch uses that branch's integration evidence. An edge across branches waits for the predecessor's protected tip or any commit that previously proved its integration at its target to reach the repository trunk. Retaining earlier proof commits matters when the target advances or rewrites after one of those commits has landed on trunk. The edge can fulfill sooner when the successor incorporates the predecessor directly. Once the predecessor reaches trunk, the successor still waits until its head incorporates that work. `RepositorySnapshot` compares recorded branch identities, with a missing target falling back to trunk and an unreadable target remaining unavailable; equal branch tips do not make two branches the same target. A cross-target predecessor judged at the trunk has `RepositoryTrunk` ordering presentation and prescribes `cargo-berth integrate <predecessor>`; one judged at another branch prescribes landing that branch on the trunk. Reconciliation derives cross-target evidence from each completed reservation snapshot, extending the trunk's existing reachability batch for commits introduced during the pass. A proposed trunk update re-derives that evidence, including retained target proof commits; a proposed non-trunk target update carries the actual trunk evidence and substitutes only that target's reservation evidence. `integrate` moves the reservation's judging branch, including the repository trunk when its recorded target is missing.

### Liveness and reconciliation

`worktree/` classifies whether a reservation's holder still exists. `reconcile.rs` runs the reconciliation pass that turns a dead holder into an alert. A dead holder is never removed on inference: reconciliation records what it observed and raises an alert, and abandoning, retiring, or recovering it takes a user action. Reconciliation releases a live reservation itself in two cases. A merged run ends as described under **Merging ends the run.** Settlement releases an `Outstanding` reservation whose evidence is `Integrated` against the actual trunk, with no unproven reserved work in its merge extent, is released as `Integrated` or `RewrittenIntegration(witness)` in that pass. The committed hook and evidence against a proposed trunk never settle.

### Alerts and recovery

`Alert` carries three variants. `MergeExtentUnavailable` keeps a reservation's last observed protection while a branch observation fails. `OrphanedOutstanding` is a protected reservation with no validated worktree holder. `LostIntegrationEvidence` is a released reservation that no longer has affirmative integration evidence; it carries the reservation id, protected tip, evidence status, and a recovery split into `NameCarryingTrunkCommit { trunk_oid, action }` and `ResolveTrunkFirst { action }`, so neither case emits an `--integrated-as <trunk-oid>` instruction naming a trunk that does not carry the work, and `resolve` refuses such a commit. It is derived on every reconciliation from replayed state and the already-materialized trunk, so the *first* drift envelope detecting a rewrite reports it, and the derivation is pure — it adds no git subprocess and no per-reservation cost. Alerts travel on every envelope. The board's `BoardAlert` adds the board-only views `StaleReservation` and `UnrecordedBypasses`. An orphan alert carries everything needed to act: `protected_tip`, `BoardBranchRefStatus`, `ObjectAvailability`, `BoardRetentionRefStatus`, `RecoverabilityVerdict`, `OrphanRecoveryConsequence`, and an `OrphanResolutionAction` built from the already-observed trunk: `Recover` offers `resolve <id> --recovered` plus `--integrated-as <commit>` only when reconciliation proved a trunk commit carries the work, otherwise `--recovered`, `--retire-orphan`, and `--abandon` with the resolved trunk named as not containing the tip, or states that trunk must resolve first, and `RetireOrAbandon` offers `--retire-orphan` and `--abandon` when the commit is unavailable. An orphan whose work reaches trunk settles without any of them. `recovery.rs` implements `resolve` and `renew` against those verdicts.
...
The `reference-transaction` hook exports `CARGO_BERTH_REFERENCE_TRANSACTION_ISSUING_DIRECTORY=$PWD` before it changes directory to the policy worktree, and the binary reads it as the issuing directory. There is no fallback to the process's own working directory. A hook that does not export it is refused with `GateError::HookReportedNoIssuingDirectory` before any worktree is resolved; the refusal exits 5 and names both repairs — rerun `cargo-berth init`, or set `CARGO_BERTH_BYPASS=1` to proceed now.
...
`drift/` answers whether a worktree edited outside its recorded scope. Two comparison modes exist behind `DriftComparisonChoice`: a cheap fingerprint delta costing two git calls, and a full phase-start comparison whose committed history is one batched read however many reservations report. The outcomes are silent (nothing moved), auto-widen (the path is free and joins the scope), incursion (someone else held the path when it was written), and collision (two reservations both touched it). A working-tree path was written just now, so its holders are the ledger's present ones; a path that arrived only through a commit is judged against the holders in force when that commit was made. `DriftPathAttributionOutcome` names all six results including the wire tags `first_touch_claimed` and `post_write_incursion`. `PostWriteFreePathProtection` covers the case where a write landed on a path nobody had claimed. `DriftEffect` is what the run actually applied.
...
**An incursion names the commits behind its entered paths.** `DriftEffect::Incursion` carries an `IncursionCommit` per commit in the reservation's phase range that introduced an entered path, each with its subject, the paths it introduced, and an `IncursionCommitOrigin` of `phase_authored`, `already_on_trunk`, or `unknown`. Only paths from the committed component are looked up, so a working-tree incursion carries none: there the write that caused it is the one the reader just made. The commits named are also only those made while a holder the incident names stood: `provenance::commits_for_paths` drops any commit older than the earliest `claimed_at` among the incident's holders, so a standing incursion never accuses a commit that predates the claim it entered. `name_incursion_commits` reads no git of its own; it names the report from the batch the pre-lock pass already took. Without this the report is not wrong about the paths, it is silent about why they moved, and a path that arrived on a commit taken from trunk reads exactly like a path the worktree wrote.

**A committed path is judged against the holders in force when it was committed.** The ledger answers for the present, and a phase range keeps every commit until the phase advances, so asking the present about a committed path let a claim taken days later over a long-lived shared file turn every earlier commit to it into an incursion the operator was told to stop and resolve — a commit cannot enter a claim that did not exist when it landed. The pre-lock pass (`PreLockForeignPathClassification`) therefore names the paths a phase committed into a present foreign holder's scope, `provenance::read_committed_foreign_paths` runs the three batched attribution reads over exactly those paths before the lock — the attribution record now carries `%ct`, a `CommitterTime` — and `classification::blocking_coverage` drops any foreign holder whose `claimed_at` follows the path's latest commit in the subject's phase range, filing the path as `NoForeignStanding` when none survives. The same batch names the report's commits afterwards, so the git cost is what it was: one path log, one range-membership query, one origin resolution, and none of them when no committed path has a foreign holder. Only a path the working tree is not holding open qualifies (`ObservedDriftChanges::committed_only`) — a staged, unstaged, or untracked path was written just now, and the present is the right moment for it. `RecordedAt::follows` compares at whole seconds, which is what `%ct` carries, so a claim and a commit from the same second are not ordered and the holder is kept. A holder's `claimed_at` also stands in for a scope it widened onto later, dating that coverage earlier than it began, so such a holder is kept rather than dropped. A holder that appears between the pre-lock pass and the lock was never a candidate, so no commit time was read for its path and the locked pass judges it against the present, as every path was before. `a_claim_taken_after_a_commit_is_not_entered_by_it` and `a_claim_taken_before_a_commit_is_entered_by_it` pin both sides.

**Incursion coverage is decided one path at a time, and two suppressions answer different questions.** An observation is a `BlockedIncursionPathSet`, each entered path paired with the holders that block it, so a union of several paths' holders cannot be expressed; the journal's `incursion` record carries the same pairs as `blocked_paths`, and reads the older two-array layout by pairing every path with the whole holder set. `RetainedReservationSet::observe_incursion` asks, per path, whether a retained incident of the *same* reservation already names that path against holders it already names against it: an outstanding one reports under that incident and creates nothing, a resolved one stays answered, and only genuinely new paths create an incident, naming those paths alone. Matching on whole-set equality instead created a second incident every time a straying edit grew by one path, each re-covering the previous one's paths and each needing its own disposition. `outstanding_incursion_covers` in `drift/classification.rs` answers the other question — whether a *sibling* reservation reported in the same run already stands accused of this overlap — and drops the effect outright, which is why it excludes the reporting reservation itself. Its `ClaimSource::FirstTouch` condition is deliberate rather than an oversight: two explicitly claimed reservations are separately accountable for the same entered path, which `markerless_post_commit_reports_every_incursion_without_ambiguous_widens` pins.
...
**One post-commit branch decides acquisition without asking, and two things make that right.** `execution::execute_inner` returns early when the reporting set is empty and a post-write first touch is required, stating `DriftScopeAcquisition::Permitted` before reaching the lock that decides refusal. The acquisition it performs is not unguarded: it goes through `claim::acquire_first_touch`, which asks the same occupancy question through the same `validate_worktree_occupancy` inside its own ledger transaction, so a second presented run reaching here is refused by the claim. The reported `Permitted` holds for a separate reason — post-commit selection drops the acting run from its filter and matches every `Active` reservation in the worktree, so an empty reporting set means no `Active` holder exists here at all and occupancy has nothing to find. `an_empty_post_commit_reporting_set_means_this_worktree_has_no_incumbent` pins the two moving together; re-adding a run term to that filter would break the implication while the branch kept compiling.

**A refusal outranks the two conditions whose remedies it would refuse.** In `OutputEnvelope::drift`, `ScopeAcquisitionRefused` is selected above `DriftAttributionRequired` and `ObjectUnknown` — but below every drift effect, so an incursion, a collision, or a widen still wins. The reason is the remedy each of the lower two names: `drift --reservation <id>` for an ambiguous attribution and `drift --full` for an unreadable phase start, both of which this same rule would refuse. Neither combination arises through the claim path, because the occupancy rule prevents two presented runs from both holding `Active` reservations in one worktree, so the ranking is pinned in `drift/report.rs` against reports assembled directly rather than through a ledger fixture.
...
**An unclaimed path is attributed once, at the commit that introduced it.** A phase range keeps every commit until the phase advances, so a path an earlier commit introduced sits inside every later full comparison. Its own post-commit check already asked who it belonged to: a widen covered it, and an ambiguous or withheld answer left it unclaimed. Asking again at every later commit raised the same ambiguity for a path nobody touched this time, which is how a lockfile committed once appeared as ambiguous drift at every checkpoint after it. `classification::classify_locked` therefore skips an unclaimed path that reached the observation only through the range (`ObservedDriftChanges::committed_only`) and not through `HEAD`'s own commit (`ObservedDriftChanges::head_commit_introduced`). Foreign standing is unaffected: an incursion or collision on a committed path is still judged against the holders in force when it was committed. `post_commit_drift_does_not_name_a_path_an_earlier_commit_touched` pins it.
...
**A rewritten branch re-anchors its phases.** `<phase_start_head>..HEAD` means "what this phase changed" only while HEAD's history still contains `phase_start_head`, and a rebase, amend, or reset makes that false. The committed `reference-transaction` gate detects a rewrite when the previous tip is not an ancestor of the proposed one, captures git's rewrite map and the commits the rewrite created, and emits `Resnapshot { Active }` in that invocation for each `Active` reservation claimed on that branch. Where map pairs touch the old phase, the mapped interval must pass scoped replay before the anchor moves. Where none do, `git::rewritten_phase_anchor` finds the replacement by position: the phase's commits survive a rebase as patch-equivalents contiguous at the tip, so the anchor is the commit beneath the last of them. Counting them is not enough and neither is patch identity alone — a rebase drops a commit whose patch already reached the new base, and the upstream commit carrying that patch is itself an equivalent, so both read a dropped commit exactly like a replayed one. `Outstanding` reservations re-anchor later, in ordinary reconciliation, from the `branch_rewrite` pending marker the hook writes. Drift stands aside entirely while `rebase-merge` or `rebase-apply` exists, because git runs `post-commit` for every replayed commit and nothing re-anchors until the branch reference moves at the end.
...
The rendered `reference-transaction` hook classifies phase/ref pairs in shell and spawns the binary only for actionable ones: `preparing`, `aborted`, and unknown phases exit before the binary; `prepared` invokes when the transaction names the configured trunk ref or a full ref listed in `gate-targets`; `committed` invokes for any local `refs/heads/*`, because it captures a local rewrite's map, reanchors phase starts, and consumes forced-integration permits. In the committed phase, the binary checks ancestry and reads `gate-targets` before the journal; an ordinary fast-forward to an unlisted branch stops there. The same shell filter gates the bypass recording. Two classifier stages run per fire at a cost independent of ref count: `LC_ALL=C grep -q` routes straight to the binary on any byte outside tab and printable ASCII — grep's own error exit counts as a bad byte — and then one `awk` pass classifies the surviving records. The hook passes the filter path and trunk ref through the environment so awk retains literal backslashes. The byte scan must precede awk, because awk truncates a record at NUL. Stdin is copied to a protected temporary file and the *unchanged bytes* are redirected into the binary; a buffering failure refuses and prints a retry instruction rather than replaying a partial transaction. Anything unclassifiable invokes the binary: skipping is not a failure mode this table produces.
...
The artifact holds four top-level keys — `$schema`, `contract`, `version`, `schemas` — and pins the shape of each wire value, not the combinations they may appear in. No table enumerates which verb, status, exit code, and payload kind may occur together, so nothing rejects a success envelope carrying a rejection sub-status. Every `schemars` definition name is pinned to its wire name, so no Rust rename can move the generated bytes — a property a test proves by declaring a genuinely distinct type carrying the same wire name.

`ReplayFailurePayload { reason, subject, effect }` types every replay hard stop: `reason` is generated exhaustively from the replay error enums, `subject` is a three-arm identity union, and `effect` is `HardStop`. Without it the roughly twenty replay invariant failures collapse into one untyped `ledger_unreadable` envelope. `docs/cargo-berth/as-built/json-contract.md` is the document independent consumers read.
...
- Only definitive scoped-patch verdicts are cached, keyed by the trunk or successor target together with the proof-subject revision. An `ObjectUnknown` comparison is transient and is never made durable, and a historical proof that could not be evaluated is journalled as an attempt, never as a negative verdict.
...
- A worktree admits one coordination run at a time, and the refusal holds only between two runs that both presented an identity: same worktree, another run, `Active`, and `Presented` on the holder, with the acting side carried as a `PresentedCoordinationRun`. Both terms are enforced by construction, so a new call site cannot apply the rule to one side only; the session-mapping-before-environment resolution order is what keeps a mapped session out of the question entirely.
...
- Every outstanding reservation has a retention ref at its protected tip. A released reservation keeps its ref only while a nonterminal dependent needs it, pointing at the witness for `RewrittenIntegration`. Refs move inside the same lock hold as the record that justifies them, only after that append succeeds.
...
- The session identity mapping is maintained only by `apply_journal_event`, and only `Claim`, `Widen`, `Checkpoint`, and `Release` move it. An enrolled `Claim` publishes no mapping.
...
| Incursion attribution | 3 further git calls, batched and run before the lock, only when a committed path has a present foreign holder: one path log carrying `%ct`, one `rev-list --parents` range graph, one origin resolution |
...
- `DriftPathAttributionOutcome::Ambiguous` exists only in the command payload and is never journalled — it describes a question the run could not answer, not a fact about the repository.
...
- Git committer time is whole seconds and `RecordedAt` is milliseconds. `RecordedAt::follows` truncates the record to the second before comparing, and a tie keeps the holder; comparing at millisecond precision would drop a holder that claimed in the same second the commit landed.
- The order in `execute_inner` is fixed: observation, pre-lock classification, the attribution read, the lock, then report naming. Moving the attribution read after the lock puts three git processes inside the mutation lock; moving it before the pre-lock pass reads history for every path instead of the ones with a foreign holder.
...
- `.claude/config/berth.toml` is not tracked, so `git worktree add` does not bring it along. A linked worktree without one reads the main worktree's file (`WorktreeContext::configuration_lookup` names both roots; `BerthConfig::read` tries them in that order), so a worktree added after `init` coordinates from its first edit. A file the linked worktree does have wins. Only when neither exists is the worktree `unconfigured`, and the path it reports is the main worktree's, because `init` writes the file there from any worktree. A linked worktree of a bare repository has no main worktree and reads only its own file.
...
**Claiming happens on first touch.** Declaring a reservations block up front fails on the thing that actually happens: work discovers files. A declared block is either stale within minutes or maintained by hand at exactly the moment the user has the least attention to spare. First-touch claiming makes the reservation a consequence of the edit, so it cannot drift from what the worktree is doing. The accepted cost is that the claim is created by the same command that answers the question, which makes `check` a mutating verb and forces the acquisition into the transaction.
...
**Nothing is removed on inference.** The tool can prove a worktree is gone; it cannot prove the work is abandoned. Auto-removing a reservation on that evidence would silently discard someone's claim on the strength of an inference. An alert plus an explicit `resolve` keeps the decision with the person who has the missing context. Automatic release rests on proof instead. Settlement releases an outstanding checkpoint when git shows the whole scoped phase on the trunk actually checked out and the merge guard shows no reserved work outside that proof.

**Merging ends the run.** A run that has done work — an observation once found unmerged paths, its checkout committed past the phase start, or its claim is `ClaimSource::Enrolled` — ends on the first reconciliation that sees its checkout clean, with no net branch change, at a head trunk contains. An uncommitted edit to `.claude/config/berth.toml` does not make a checkout dirty for this purpose, tracked or not. Reconciliation checkpoints it at that head and settles it as integrated in one append, so it becomes the same released reservation settlement produces. Nothing in the ordinary workflow calls `release`: sessions commit and merge, and a first-touch reservation that waited for a release verb stayed active for days, widening across unrelated tasks and blocking every other run in its checkout. A clean tree can hide work in a stash or an ignored file, but in this repository `git stash` is denied and the tool has one user, so that risk does not justify a release step nobody runs. A clean claim that has not written yet keeps running, so a pre-edit check is never ended before its write lands.

**A stacked checkout already holds the work it was stacked on.** A foreign holder protects only what its head would still bring to the acting checkout's HEAD — the paths `git merge-tree` of the two heads changes relative to that HEAD, plus its conflicted paths — plus the holder's uncommitted paths. A worktree branched from a holder's branch therefore edits those committed paths freely, while commits the holder makes afterwards stay protected. `ActingHeadContainment` in `reservation/containment.rs` reads HEAD and one merge-tree plus one name-only diff per distinct foreign holder head before the decision, and nothing when no foreign holder has committed protection. The locked replay matches each holder by the head it observed; a holder whose head moved, whose extent has no key, or whose read failed keeps its complete protection. `RetainedReservationSet` carries the choice as `ForeignProtectionPolicy::{FullProtection, ExcludeContainedWork(ActingHeadContainment)}`, set by `with_acting_head_containment`. Check, claim, first touch, drift and enrollment all decide through this narrowing.

...
Claims from older journals replay as `RecordedTarget::Unrecorded`. `init` appends one `unrecorded_targets_pinned` event when any remain, assigning the repository trunk at that moment. `retarget <reservation> --target <branch>` appends `retarget` for an unreleased holder, replacing its target and comparison commit and clearing its retained integration proof. Reconciliation and release evaluate each reservation at its recorded target. `resolve --integrated-as` checks reachability from that target after replay. A missing local target ref makes the repository trunk the judging branch for these operations and rewrite re-anchoring; a Git read failure does not trigger that fallback. Drift uses the acting session reservation's target for committed origin attribution, or each reporting reservation's target for a run without a mapped session; it batches origin reads by distinct target. The trunk gate still evaluates the repository trunk.

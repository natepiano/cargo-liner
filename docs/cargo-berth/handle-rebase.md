...
### Phase 2 — Rebase re-anchor from git's rewrite map  · status: todo

#### Work Order

**Goal:** After a rebase, reconciliation re-anchors each outstanding reservation from git's rewrite map, accepting the new interval only when the scoped replay contains the whole phase.

**Spec:**

*Capture (no new hook).*
- In `evaluate_reference_transaction` (`src/gate/reference_transaction.rs:243`), when `rewrite::branch_rewrites` (:284) returns rewrites for the `Committed` phase, copy the rewrite map into a pending marker before `reanchor_rewritten_phases` (`src/gate/rewrite.rs:79`) takes the ledger lock.
- Map source, in the worktree's git dir (the per-worktree admin dir `rewrite_in_progress` reads, `src/git/discovery.rs:118`):
  - merge backend: `rebase-merge/rewritten-list`;
  - apply backend: `rebase-apply/rewritten`;
  - neither present: the rewrite is an amend or reset, and the map is one line per `BranchRewrite` (`src/gate/rewrite.rs:29`) from old tip to new tip.
- Verified with git on 2026-09-14: at the committed update of `refs/heads/<branch>`, both files hold every `<old> <new>` line, including many-to-one lines for squash and fixup (a fixup of two commits and a squash of two commits each mapped both old commits to one new commit). Git deletes the file when the rebase finishes, so the copy must happen in this hook invocation.
- Capture context: the managed hook records its original directory and then changes to the policy worktree (`src/gate/install.rs:300`), while the existing context at `src/gate/reference_transaction.rs:294` is discovered from `invocation_directory`. Discover the capture context from `issuing_directory`, read and record that checkout's administrative directory, and persist the map before either committed trunk auditing or active-reservation re-anchoring takes a ledger lock.
- The marker reuses the pending-marker mechanism in `src/gate/permit.rs` (`write_pending_marker` :248 into the common git dir, `PENDING_BYPASS_FILE_PREFIX` :46, payload `PendingEnvironmentBypass` :77), carrying the map pairs and the worktree admin dir. `PendingEnvironmentBypass` requires bypass-specific facts and its recovery creates `JournalOperation::Bypass`, so decode markers into `PendingReconciliationMarker::{EnvironmentBypass, BranchRewrite}`, defaulting legacy markers to `EnvironmentBypass`. Exclude rewrite markers from `pending_environment_bypass_count` (:277, which today counts filenames without decoding), bypass audit operations, and recovered/unrecorded-bypass notices. Rewrite-marker cleanup must not depend on `RecoveredBypassReporting::Report`, which controls bypass deletion today (`src/reconcile.rs:2885`).

*Import.*
- Reconciliation reads rewrite markers beside the pending bypass markers (`pending_bypass_imports` taken at `src/reconcile.rs:1102`, applied :2829). Import a marker only when `git::rewrite_in_progress` is false for its recorded worktree admin dir; a missing dir counts as not in progress.
- Ordering: `prepare_reconciliation_transaction` (`src/reconcile.rs:1081`) completes `build_plan` before it takes pending imports, and `with_pending_settlements` (`src/reservation/retention.rs:252`) projects only evidence and releases. So preflight mapping and acceptance before the transaction, validate the captured proof-subject revisions under lock, project accepted resnapshots before `build_plan`, and prepend their journal operations before evidence and settlement; construct ancestry batches, snapshots, successor observations, and retention actions from that projected state. Keep these operations out of bypass `recoverable_operations`, which append separately after ordinary reconciliation operations (`src/ledger/handle.rs:456`).
- Delete the marker after the append succeeds. A lock timeout or failed append leaves the marker for the next pass. An aborted rebase updates no branch, so it leaves no marker.
- Budget: `ReconciliationScopedPatchEvaluationBudget::evaluate` (`src/reconcile.rs:740`) admits work by `ScopedPatchEvaluationKey.target_trunk`. Charge mapped acceptance to the observed trunk's shared subject budget and keep the actual comparison destination (the mapped tip) separate, so mapped tips never become admission keys. A deferred subject keeps the marker, which a later pass finishes; already-resnapshotted reservations no longer match the map, so reprocessing is idempotent.
- The same pass then evaluates evidence against actual trunk from the projected anchors, so a trunk fast-forward that already landed settles per Phase 1.

*Selection.* An `Outstanding` reservation is a candidate when a commit of its protected range `phase_start_head..protected_tip` appears as an old commit in the map. `Active` reservations keep today's selection in `resnapshot_operations` (`src/gate/rewrite.rs:130`).

*Mapping (pure function, new module `src/gate/rewrite_map.rs`).*
- Inputs: the map pairs, the old protected range commits oldest first, and the new first-parent history oldest first, starting with a base commit so each entry's parent is the entry before it.
- A phase commit absent from the map that still appears in the new history is its own destination. One absent from both was skipped because the new base already carried it, and has no destination.
- New protected tip: the latest destination in new history order. New phase start: the entry before the earliest destination. No destination, or an earliest destination with no entry before it: refusal.
- Squash, fixup, reorder, and split map many-to-one or move endpoints; no bijection is required.

*Acceptance.*
- Extend `ScopedPatchTargetHistory` (`src/git/patch.rs:116`) with a variant carrying the mapped destinations. In `compare_scoped_patch` (:349) that variant counts as located: skip `target_scoped_change_position` (:618) and proceed as `Contiguous`. Still run the history relationship check, `protected_scoped_changes`, and the scoped replay against the new tip.
- Reason: `classify_target_phase_integration_commits` (:670) needs exactly one unmatched target commit and returns `Unresolved` when earlier rebased phases or upstream commits touch the same paths, which rejects conflict-resolved and split phases.
- Call with the old phase start, the scopes, the old protected tip, and the new tip as target. Pass: `Resnapshot` with the new anchors. Fail: old anchors stay, and the reservation is left to a later proof or a manual answer.

*Journal and revision check.*
- Extend `ReservationSnapshot::Outstanding { protected_tip, trunk_oid }` (`src/ledger/journal.rs:1366`) with an optional wire `phase_start_head`, serde default and omitted when absent. Convert it before replay into `PhaseStartHeadUpdate::{Preserve, Replace(ProtectedPhaseStartHead)}`: absent is `Preserve`. `apply_resnapshot` (`src/reservation/retention.rs:1146`) takes that domain type, never a bare `Option`.
- The mapping step records each reservation's `IntegrationProofSubjectRevision` (`src/reservation/scoped_patch_evaluation.rs:22`) in memory. The transaction validation closure, which replays current state under the lock as in `src/gate/rewrite.rs:92-111`, rejects a resnapshot when that revision advanced or the reservation is no longer `Outstanding`. Nothing new is journaled for this check; a rejected validation appends nothing and runs no committed action.
- Move retention refs in the committed-action callback after the append, as `resnapshot_operation` (`src/verb/release.rs:560`) does.

*Tests.*
7. Unit `rewrite_map_derives_phase_interval` in `src/gate/rewrite_map.rs`. Rows: plain rebase, reorder, squash across a checkpoint boundary, fixup, split, unchanged commits, a skipped commit, and a map that excludes the phase and selects nothing.
8. Unit mapped acceptance in `src/git/patch.rs` with `PatchEquivalenceFixture` (`src/git/fixture.rs:36`). Passes: a conflict resolution that keeps the protected hunk; a split; two checkpoints on one path with the second conflict-resolved; two resolutions that each add edits; a commit skipped because the base carried it. Rejects: a resolution that discards the hunk; a deliberate drop.
9. Unit `resnapshot_validation`, and extend `proof_subject_changes_remove_retained_verdicts_after_widen_resnapshot_and_replacement` (`src/reservation/retention.rs:1893`): an advanced revision rejects; a released reservation rejects; a legacy snapshot without a phase start preserves it.
10. Integration `checkpoint_ranges_survive_two_rebases` in `tests/lifecycle.rs`: checkpoint, rebase with a conflict in a later phase, checkpoint, amend the tip, rebase again, fast-forward trunk, delete the branch. Run the rebases through the installed hook from a linked checkout whose policy worktree differs. Assert earlier checkpoints separately from the final branch tip: each accepted reservation already on trunk settles in its accepting pass, and settlement may already happen in the prepared gate or drift hook. Progress stays bounded across reconciliations until the marker drains; then the marker is gone, no alert remains, and a rerun after the settling pass appends nothing. An aborted rebase partway through leaves no marker. A refused acceptance keeps old anchors for both a live and an orphaned holder, and the orphan gets the existing recovery action. A mixed bypass-plus-rewrite marker set reports only the bypass and cleans up the rewrite marker.
11. Extend `failed_journal_append_does_not_move_the_retention_ref` (`tests/lifecycle.rs:486`) with a re-anchor case: the marker and retention ref stay, and the next `board` completes it. A `board` run while a rebase is stopped leaves the marker.

**Files:**
- `crates/cargo-berth/src/gate/rewrite_map.rs` — new: mapping function; unit test 7
- `crates/cargo-berth/src/gate/mod.rs` — register `rewrite_map`
- `crates/cargo-berth/src/git/patch.rs` — mapped `ScopedPatchTargetHistory` variant and its handling in `compare_scoped_patch`; unit test 8
- `crates/cargo-berth/src/gate/reference_transaction.rs` — copy the map before the re-anchor lock
- `crates/cargo-berth/src/gate/rewrite.rs` — map source reading beside `BranchRewrite`
- `crates/cargo-berth/src/gate/permit.rs` — rewrite marker kind
- `crates/cargo-berth/src/reconcile.rs` — marker import, selection, resnapshot transaction, retention callback
- `crates/cargo-berth/src/ledger/journal.rs` — optional `phase_start_head` on `ReservationSnapshot::Outstanding`
- `crates/cargo-berth/src/reservation/retention.rs` — `apply_resnapshot` honors `PhaseStartHeadUpdate`; unit test 9
- `crates/cargo-berth/src/verb/release.rs` — snapshot constructor at :572 supplies the new field
- `crates/cargo-berth/src/board/rows.rs` — unit fixture snapshot constructor at :1134 supplies the new field
- `crates/cargo-berth/tests/lifecycle.rs` — tests 10 and 11
- `crates/cargo-berth/tests/{board,gate,hooks,drift}.rs` — fixtures affected by projected re-anchoring and rewrite markers; mixed-marker acceptance

**Seats:** 2 writers + 1 tester — capture, mapping, markers, and the comparator split from projection, journal conversion, and snapshot constructors by file group.
- `impl` — `src/gate/{rewrite_map,mod,reference_transaction,rewrite,permit}.rs`, `src/git/patch.rs`; capture, mapping, marker handling, comparator, unit tests 7 and 8; hub: `src/gate/mod.rs` (module registration)
- `review` — opens as impl: `src/reconcile.rs`, `src/ledger/journal.rs`, `src/reservation/retention.rs`, `src/verb/release.rs`, `src/board/rows.rs`; projected resnapshots, journal conversion, snapshot constructor updates (`src/verb/release.rs:572`, unit fixture `src/board/rows.rs:1134`), unit test 9; hub: `src/reconcile.rs`. It calls the mapping function and the marker kind through the signatures in this Spec.
- `test` — `tests/*.rs` and integration support, excluding frozen `tests/fixtures/reader_compat` bytes; tests 10 and 11 and affected board, gate, hook, drift, and lifecycle fixtures

**Constraints from prior phases:**
- Phase 1: actual-trunk settlement is selected in `complete_reconciliation_plan` (`src/reconcile.rs:1379`) through `settlement_selection` (:2208) and `append_settlement_operations` (:2251). It releases ancestry proofs as `Integrated` and equivalence proofs as `RewrittenIntegration(<evaluated actual trunk>)`, and only when the evidence's `trunk_oid` equals actual trunk.
- Phase 1: actual-trunk settlement is selected in `complete_reconciliation_plan`; prepared decisions project it before proposed-trunk constraints, committed audits preserve lifecycles for forced-permit consumption, and proposed-trunk evidence never settles. `GateReconciliationPurpose::{PreparedDecision, CommittedAudit}` (`src/reconcile.rs:764`); keep `tests/gate.rs:2130` and `:2315` green, including the bypass audit record.
- Phase 1: after projecting accepted anchors, `complete_reconciliation_plan` can settle earlier mapped checkpoints in the same pass when the holder head is already in actual trunk and reserved paths are clean; remaining committed work or reserved dirt still blocks settlement (`has_unproven_merge_work`, `src/reservation/retention.rs:90`).
- Phase 1: the `RewrittenIntegration` revalidation arm is ancestry-only in `src/reconcile.rs` and `src/verb/release.rs`.
- Phase 1: failed acceptance preserves the old anchors and outstanding state; board/check retain the merge-protection evidence, and proven orphans receive the existing recovery action through `OrphanResolutionAction` (`src/alert.rs:193`) and `BoardOrphanResolutionAction` (`src/board/alerts.rs:212`). The orphan alert requires `Outstanding` and `WorktreeLiveness::Orphaned` (`src/alert.rs:457`).
- Phase 1: every journal append republishes the session mapping, and many existing fixtures in `tests/{answers,drift,gate,hooks,liveness,board,edges,lifecycle}.rs` were adjusted so reservations stay outstanding (a dirty reserved source) or expect automatic settlement.

**Acceptance gate:** `bash ~/.claude/scripts/delegate/verify.sh check cargo-berth`, `bash ~/.claude/scripts/delegate/verify.sh test cargo-berth`, and `bash ~/.claude/scripts/delegate/verify.sh lint cargo-berth` green; `verify.sh test cargo-berth lifecycle` green with tests 10 and 11 present; unit tests 7, 8, and 9 pass. Behavior: a checkpointed branch rebased with a conflict and fast-forwarded into trunk leaves no outstanding reservation and no alert.
...
**Spec:**
- Today `IntegrationEvidenceStatus::Integrated { trunk_oid, proof }` carries one OID and a payload-free `IntegrationProof`. `settlement_selection` (`src/reconcile.rs:2208`, equality check near :2233) releases only when `trunk_oid` equals actual trunk and records that same OID as the `RewrittenIntegration` witness. `integration_status_from_retained_scoped_patch_comparison` (`src/reconcile.rs:2096`) rebuilds evidence from retained verdicts and would discard any distinction.
...
- `IntegrationProof::ProtectedTipAncestor` promises trunk contains the protected commit (`src/reservation/lifecycle.rs:28`), but `RewrittenIntegrationTrunkCommit::revalidate_ancestry` (:325) returns it for witness ancestry, including after the protected tip is collected. Add a distinct proof for rewritten-witness ancestry. Existing serialized values keep their names; the new value is additive. If the serialized proof reaches the output contract, regenerate `docs/cargo-berth/generated/output-contract.json` per the Invariants and update `docs/cargo-berth/json-contract.md`.
- This phase adds no producer of a historical witness: every current producer records the evaluated trunk as witness, so observable behavior changes only in the proof reported on witness revalidation. The historical trunk candidate phase produces historical witnesses.
...
B. Unit in `src/reservation/retention.rs`: a `ScopedPatchEquivalenceChecked` record with a historical witness replays into evidence carrying that witness; a legacy record without the field yields the evaluated trunk as witness.
C. Extend `witness_survives_pruning_and_controls_successors` (`tests/edges.rs:933`): witness revalidation reports the witness-ancestry proof, not checkpoint ancestry.
...
- `crates/cargo-berth/src/output_contract.rs`, `docs/cargo-berth/generated/output-contract.json`, `docs/cargo-berth/json-contract.md` — only if the serialized proof changes
...
- `impl` — `src/reservation/{evidence,lifecycle,scoped_patch_evaluation,retention,mod}.rs`, unit test B; hub: `src/reservation/mod.rs` (re-exports)
- `review` — opens as impl: `src/reconcile.rs`, `src/ledger/journal.rs`, `src/output_contract.rs`, the generated contract, `docs/cargo-berth/json-contract.md`; unit test A; hub: `src/reconcile.rs`
- `test` — `tests/*.rs` excluding frozen `tests/fixtures/reader_compat` bytes; test C and fixtures asserting the proof name
...
- Phase 1: witness revalidation is ancestry-only through `RewrittenIntegrationTrunkCommit::revalidate_ancestry` in `src/reconcile.rs` and `src/verb/release.rs`; `RetentionCommitResolution::IncludeSettlementWitness` (`src/reconcile.rs:812`) retains a newly settled witness in the same pass; `SuccessorIncorporationSubject::RewrittenIntegrationWitness` (:551) never enters scoped comparison.
- Phase 2: re-anchored reservations settle through the same selection and produce no historical witness.

**Acceptance gate:** `bash ~/.claude/scripts/delegate/verify.sh check cargo-berth`, `bash ~/.claude/scripts/delegate/verify.sh test cargo-berth`, and `bash ~/.claude/scripts/delegate/verify.sh lint cargo-berth` green; `verify.sh test cargo-berth edges` green with test C; unit tests A and B pass; frozen reader fixture bytes unchanged. Behavior: a retained equivalence verdict carrying a witness older than trunk's tip survives a restart and settles with that witness.
...
- Matches come from the existing `phase_equivalent_commits` cherry-mark query (`src/git/patch.rs:238`, `rev-list --cherry-mark --left-right --no-merges <a>...<b> ^<phase_start>`), run between the protected tip and trunk with `--right-only` to list trunk-side `=` commits. No new patch-id pipeline.
- The candidate is the **earliest** first-parent trunk commit that has every match as an ancestor, read from the batched first-parent history in `target_history_after_phase_start` (`src/reconcile.rs:434`). The latest such commit is the trunk tip, which is the replay that already fails.
...
- The same comparator serves current-trunk and successor checks, which stay safe because the replay still validates content. Check `intervening_unrelated_commit_does_not_separate_the_phase_integration` (:1552) against the new rule.
...
- Successors need it too: successor evaluation uses the same comparator (`src/reconcile.rs:2743`), but a retained `Different` returns `NotIncorporated` before evaluation (:2696-2717), so without a version existing successor holds never see the new rule. Rewritten-integration witnesses stay ancestry-only and never enter the comparison.
- Ship the version in the same step as the candidate and the contiguity rule, so no binary writes versioned negatives from the old evaluator. Re-anchoring does not need it: a resnapshot advances the proof subject revision, which already clears retained verdicts.

*Tests.*
12. Unit candidate cases in one `PatchEquivalenceFixture` in `src/git/patch.rs`:
    - extend `one_equivalent_commit_does_not_certify_partial_integration` (:1735) with final-only and reordered matches;
    - extend `duplicate_context_does_not_relocate_the_protected_change` (:1576): equal patch ids nominate the wrong site and the comparator rejects it;
    ...
    - update `separated_target_equivalents_do_not_prove_one_replayed_phase` (:1757) for the relaxed rule, with a protected-path gap that still rejects;
...
- `crates/cargo-berth/src/reservation/{lifecycle,evidence,mod}.rs` — only where a historical witness needs a constructor on the Phase 3 witness state
...
**Constraints from prior phases:**
- Phase 1: actual-trunk settlement is selected in `complete_reconciliation_plan`; prepared decisions project it before proposed-trunk constraints, committed audits preserve lifecycles for forced-permit consumption, and proposed-trunk evidence never settles. Keep `tests/gate.rs:2130` and `:2315` green, including the bypass audit record.
- Phase 1: witness revalidation is ancestry-only; `SuccessorIncorporationSubject::RewrittenIntegrationWitness` (`src/reconcile.rs:551`) never enters scoped comparison; `RetentionCommitResolution::IncludeSettlementWitness` (:812) retains a newly settled witness in the same pass.
...
- Phase 2: a resnapshot advances the proof subject revision and clears retained verdicts, so versioned verdicts matter only for unmapped reservations.

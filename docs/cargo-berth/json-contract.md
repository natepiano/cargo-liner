...
generated in the `cargo-berth` test binary from the serialized DTOs and one
outcome declaration table. Ordinary tests regenerate it in memory and require
an exact byte match. The same contract contains the reproducible Python tables,
static jq validator, positive fixtures, and negative fixtures used by the hook
consumers. Consumers validate the correlated verb, envelope status, exit code,
payload kind, and nested payload status as one outcome; membership of each
value by itself is not sufficient.

The generated Python module exports a semantic `Literal` alias for every
closed string domain it publishes:

- `CommandVerbValue`, `OutputStatusValue`, and `OutputPayloadKindValue` type
  the top-level outcome correlation.
- `PayloadDataRequirement` and `OutcomeEmissionDisposition` type the two
  decisions carried by each `OutcomeRule`.
- `IntegrationProofValue`, `TrunkObservationAtClaimValue`, and
  `LostIntegrationEvidenceStatusValue` type the enum-backed validation sets.
- `ReplayFailureReasonValue` and `ReplayFailureSubjectKindValue` type the
  replay-failure map.
- `CoordinationIdentityRejectionKindValue` types the rejection set derived
  from the emitted outcome rules and used by the identity validator.

`OutcomeRule`, every generated set and map, and `valid_outcome_tuple` use those
aliases directly. At a decoded-JSON boundary, `is_command_verb_value`,
`is_output_status_value`, `is_output_payload_kind_value`, and
`is_replay_failure_reason_value` accept an `object`, test the corresponding
generated frozenset, and narrow it to the matching alias.
...
board value and is absent from fresh engine fixtures.

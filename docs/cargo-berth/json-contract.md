...
Ordinary tests regenerate it in memory and require an exact byte match. The
artifact also embeds the reproducible Python module and static jq validator
installed by the hook layer.

Those two consumers validate only the stable front-end shell documented below.
They do not inspect `payload`, enumerate payload kinds, correlate an outcome
tuple, or read `output_contract_version`. The Python entry point is
`valid_contract_envelope`; the jq entry point is
`cargo_berth_valid_contract_envelope`.

Schema definition names in that artifact are stable wire names. They do not
track Rust identifiers, so internal type renames leave the generated bytes
unchanged. `reblocked_active_constraint` remains accepted only as a reserved
board value and is absent from fresh engine responses.

For what the tool is and how to use it, see the [README](../../crates/cargo-berth/README.md).

Journal replay invariant failures use `status = "ledger_unreadable"`, exit code
...
Every engine response supplies these members for front-end decisions and
rendering:

| Member | Required JSON value |
| --- | --- |
| `verb` | String. The shell validator accepts unfamiliar command names. |
| `status` | String. The shell validator accepts unfamiliar status names. |
| `exit_code` | Integer. JSON booleans are not integers here. |
| `message` | String, including an empty string. |
| `blocked_by` | Array containing only reservation-id strings; the array may be empty. |
| `presentation` | One of the two tagged objects below. |

`presentation` has exactly two producer states:

```json
{"kind":"not_provided"}
```

`not_provided` means the engine supplied no presentation. The other state is:

```json
{
  "kind": "rendered_blocks",
  "blocks": [
    {"summary": "Reservation claimed", "detail": "file:src/output.rs"}
  ]
}
```

Every member of `blocks` is a `RenderedOutputBlock` object with required string
members `summary` and `detail`. The named members state the two presentation
roles without a positional convention. An empty `blocks` array means the engine
explicitly has nothing to show; it is distinct from `not_provided`.

The current producer emits only those two states. A consumer validates
`rendered_blocks` because it reads `blocks`; it accepts an unfamiliar string
`kind` without inspecting any other member, allowing the front end to use the
envelope's `message` until it learns that presentation state.

The validators accept additional envelope members. In particular,
`reservations`, `payload`, and `output_contract_version` remain available to
specialized readers, but none participates in shell validation. Payload
contents never determine whether the front end may use `exit_code`, `message`,
`blocked_by`, or `presentation`.
...
```json
{
  ...
  "presentation": {
    "kind": "rendered_blocks",
    "blocks": [{
      "summary": "The reservation board was read.",
      "detail": "Two reservations are present."
    }]
  },
  "payload": {
    "kind": "board",
...
constant. This is reported information and gates nothing. The generated Python
and jq consumers never read it, and no consumer refuses an envelope because its
reported version differs from installed assets.
...
When `check` has no usable harness-session mapping and more than one active
reservation is eligible for the acting coordination run and worktree, it
returns `status = "ambiguous_active_run_reservations"` and exit code `1`
without selecting one by replay order. Its payload is:

```json
{
  "kind": "first_touch_reservation_selection",
  "data": {
    "status": "ambiguous_active_run_reservations",
...
`candidate_reservation_ids` contains every eligible reservation id in
ascending deterministic order. The envelope's `reservations` array repeats
that same list, while `blocked_by` is empty because the candidates are not
foreign conflicts. This outcome appends or widens no reservation and publishes
no harness-session mapping.

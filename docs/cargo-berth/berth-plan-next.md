...
- [ ] **Publish `cargo-berth` after the hana loop is proven and `tui_pane 0.8.0` is published**
  ...
  - Why needed: the crate is publish-ready and its README tells readers to run `cargo install cargo-berth`, but nothing has been published; the versionless `tui_pane` release pin currently resolves to 0.7.0 while this workspace builds against `tui_pane 0.8.0-dev`.
  - Completion condition: the live-hooks item below is satisfied in `hana`; `tui_pane 0.8.0` is published; `/release cargo-berth 0.1.0` completes its dry run and publish flow; and a fresh `cargo install cargo-berth` succeeds.

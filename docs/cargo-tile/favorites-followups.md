...
- **Key files:**
  ...
  - `crates/cargo-tile/src/favorites_overlay.rs` — `column_descriptors` (`:445`), `FavoritesSurfaceBindings::footer` unconditional move/load/delete (`:527`), `finish_navigation` indexing blanks and headings (`:618`), `FavoritesOverlay` state (`:684`), `open` calling `favorites::load` (`:715`), `FavoriteRowsView::from` matching `FavoriteRowRecognition::Unrecognized { diagnostic, .. }` (`:218`), `FavoriteSelection` (`:1308`) and `favorite_selection` (`:1054`), two-cell marker prefix in `rendered_line` (`:1327`), `build_line_plan` (`:1368`), `Attract:` section heading (`:1448`), `append_unrecognized` emitting `CachedOverlayLine::Static` (`:1501`), `favorite_cells` (`:1721`), private `mode_label` (`:1748`), `close_overlay_with` taking `impl FnMut(FavoriteRemovalTarget)` (`:1121`). **1,802 non-test lines**; `#[cfg(test)]` at 1803.
...
**Constraints from prior phases:** Phase 7 shipped `UnrecognizedFavoriteRemovalLocator` and `FavoriteRemovalTarget::{Recognized, Unrecognized}`, both exported from `favorites/mod.rs`. The locator rides on `FavoriteRowRecognition::Unrecognized { diagnostic, removal_locator }` — now a **struct** variant, so every match site names its fields — and is created during the recognition refresh, before the display sort. It is opaque: a raw table index plus a private serialized-text fingerprint, and nothing outside `rows.rs` may read either. `remove` takes a `FavoriteRemovalTarget`; the refusal when a locator no longer identifies exactly one raw table is the named variant `FavoritesMutationError::UnrecognizedFavoriteChanged`, so match on it rather than on message text, and surface it to the user rather than swallowing it. A refused removal leaves the file byte-identical. Phase 7 also updated `terminal.rs` and the overlay's close path (`close_overlay_with` now takes `impl FnMut(FavoriteRemovalTarget)`) so recognized removal already goes through that type. Phase 8 split the overlay into `favorites_overlay/{mod,content,bindings,line_plan}.rs` with `line_plan` depending on `content` and `bindings`, and kept `favorite_cells` in `content.rs`; the line refs in this Spec are to the pre-split file, so locate by item name. Three of its visibility choices bind this phase. `CachedLinePlan` exposes `lines`, `navigation_line_index` and `last_horizontal_column_page` as `pub(super)`, but `selectable_line_index` is private with a `#[cfg(test)] pub(super) fn selectable_line_index(&self) -> &[usize]` accessor — read the navigation index directly and assert on the selectable one through that accessor. `FavoriteRowsView` has no `crate::favorites_overlay::` path: `mod.rs` re-exports only `FavoritesOverlayContent` and `UnrecognizedFavoritesView`, and the rows view is reached as the payload of `FavoritesOverlayContent::Rows`. Each leaf module's test module carries only `#[expect(clippy::expect_used, reason = "tests should panic on unexpected values")]`, while `mod.rs`'s also carries `clippy::panic` and `clippy::unchecked_time_subtraction`; warnings are denied and an unfulfilled `#[expect]` is itself an error, so a test needing `panic!` adds that lint to its own module's block and a test that does not must not copy the wider block.
...
**Constraints from prior phases:** Phase 9 gave the cached line a `FavoriteRowIdentity::{Recognized(FavoriteId), Unrecognized(UnrecognizedFavoriteRemovalLocator)}` with non-row lines carrying none, put only real rows in the navigation index, made the selection carry that identity or nothing, and kept selection out of the identity as render-time state — currency joins it there. The prefix Phase 9 renders is still two cells; this phase widens it. Unrecognized rows never carry a currency mark: they have no settings to compare. `Attract::current_settings()` takes `&mut self` (`attract/mod.rs:697`), so the caller must hold the attract state mutably — which is why the snapshot is taken in `globals.rs` and `terminal.rs` rather than inside the overlay. Phase 8 gave each leaf module of `favorites_overlay/` its own narrow test-module lint block — `content.rs`, `bindings.rs` and `line_plan.rs` carry only `#[expect(clippy::expect_used, …)]`, while `mod.rs` also carries `clippy::panic` and `clippy::unchecked_time_subtraction`. Warnings are denied and an unfulfilled `#[expect]` is itself an error, so a new test adds only the lints its own module actually trips.

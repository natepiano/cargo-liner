...
- **Key files:**
  ...
  - `crates/cargo-tile/src/globals.rs` — app-globals scope; toasts pushed with `app.framework.toasts.push_timed(...)` (`:148`, `:217`) and no paired scheduling call; the overlay-open handler (`:116`), which snapshots `app.attract.current_settings().into()` into `OpenFavoritesCurrentParameters` before opening — as does the fall-through path of `show_random_favorite_with`, the success path having already applied the settings; private `mode_label`.
...
**The policy needs an ownership path, and the framework does not have one yet.** `Keymap` has no presentation field, and a client can configure a `Keymap` only through `KeymapBuilder` before it is finalized — cargo-tile builds its keymap from `Keymap::builder()` (`cargo-tile/src/keymap.rs:69`) and never touches the finished map. So the policy is a builder-set field that `finalize` moves onto `Keymap`, beside the existing `dismiss_fallback` and the monomorphized row renderers. Three files carry it besides `keymap/mod.rs`: `keymap/builder/mod.rs` holds the field on `KeymapBuilder` and its setter, `keymap/builder/transition.rs` carries it across the typestate transition — every builder field is moved there by hand, so one omitted here silently drops the policy — and `keymap/builder/finalize.rs` moves it onto the constructed `Keymap`. Export both types from the crate root beside `GlobalShortcutRow` (`lib.rs:149`) so cargo-tile never names a `keymap::` path.
...
**The stale-locator refusal must say what to do about it.** `deletion_refusal_message` (`favorites_overlay/mod.rs:960`) currently says only that the file changed and nothing was deleted. Retrying without reopening necessarily fails again — the locator was captured when the overlay loaded — and the overlay has no reload key, so the user is told a fact with no action attached. Make the notice read exactly: `The favorites file changed after this row was loaded; nothing was deleted. Close and reopen favorites, then try again.`

**Files:**
- `crates/tui_pane/src/keymap/mod.rs` and `crates/tui_pane/src/keymap/global_action.rs` — `FrameworkGlobalShortcutPresentation` and `FrameworkGlobalShortcutVisibility`, and the non-optional `Keymap` field holding the policy, consulted by `global_shortcut_rows` (`:439`) and **not** by `keymap_help_rows` (`:515`), defaulting to showing every row
- `crates/tui_pane/src/keymap/builder/mod.rs` — the matching non-optional `KeymapBuilder` field and its setter, defaulting to the show-everything policy
- `crates/tui_pane/src/keymap/builder/transition.rs` — carry the field across the typestate transition
- `crates/tui_pane/src/keymap/builder/finalize.rs` — move the field onto the constructed `Keymap`
- `crates/tui_pane/src/lib.rs` — export both types from the crate root beside `GlobalShortcutRow` (`:149`)
- `crates/tui_pane/src/overlays/keymap_edit.rs` — `edit_selected_global_shortcut` (`:178`) already indexes `global_shortcut_rows` by viewport position; it owns the test that a hidden row above the selection does not shift which row the editor opens
...
- `crates/cargo-tile/src/favorites_overlay/mod.rs` — `deletion_refusal_message` (`:960`) gains the recovery sentence
...
2. `CaptureWindowSelectionMethod::{Identified, Frontmost, Size}` — how the id was reached. Its own enum rather than a string or an integer, so a log line cannot claim a method that does not exist.
...
- `crates/tui_pane/src/backdrop/monitor.rs` — hold `LatestCaptureAttemptWindowSelection` beside the latest attempt status, advance the sequence, and expose both
...
- Tests proving: an attempt that fails before window selection reports `SelectionNotReached`; each of `Identified`, `Frontmost` and `Size` is reachable and reported; two consecutive attempts with an identical outcome produce two records with distinct sequence values rather than one; and a monitor that has completed no attempt reports `WaitingForFirstResult`
...
**Goal:** Two named types carry window selection — identification progress on the monitor, capture target on the capture path — replacing three owned options whose `None` means "nothing settled, use frontmost or size".
...
**Preserve Phase 15's distinction.** `CaptureWindowTarget` says what the capture was *asked* to do; `CaptureAttemptWindowSelection` says what it actually did. They are not the same value and must not be merged: an attempt can be asked for `FrontmostOrSizeHeuristic` and report `Selected { method: Frontmost }`, or be asked for `Identified(id)` and report `SelectionNotReached`. Keep the per-attempt record, its `CaptureAttemptSequence`, and the transition line's `LatestCaptureAttemptWindowSelection` working exactly as Phase 15 shipped them. When this phase lands, no domain-owned window-selection `Option<u32>` remains anywhere in `backdrop/`.

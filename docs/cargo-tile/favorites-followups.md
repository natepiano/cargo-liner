...
### Phase 17 — Window selection stops being a bare optional number  · status: done

#### As-built

- `WindowIdentificationState` (private to `monitor.rs`) is the window search's whole state: `NotAttempted`, `PendingBeforeMarker`, `PendingWithMarker`, `Identified`, `Fallback`. It owns the phase-dependent data the five loose `BackdropMonitor` fields `pinned`, `attempts`, `attempted_at`, `asked` and `titles` used to hold, so no combination can contradict another, and offers two projections — `report() -> WindowIdentification` and `capture_window_target() -> CaptureWindowTarget` — beside `identify()`, `run_first_attempt()`, `run_marker_setup_attempt()` and `run_marker_lookup_attempt()`. `BackdropMonitor::identify` is a single delegating call. Its own fields carry no bare `Option`; the per-window `Option<String>` inside `previous_titles` is the exception, because a listed window genuinely may have no title.
- `CaptureWindowTarget { PreferWindow { window_id: u32 }, TerminalWindowHeuristic }` in `desktop.rs` is the capture target threaded through `CaptureRequest`, `Desktop::capture`, both platform implementations, `select_capture_window` and the `#[doc(hidden)]` `capture_attempt_for_test`. Only `select_capture_window` matches on it; every other site passes it along.
- `TerminalWindowSearchOutcome { NotFound, Found { window_id: u32 } }` is the answer of both window lookups, built by the target-independent wrappers `desktop::window_titled` and `desktop::window_at`. The macOS `platform` lookups they wrap, and their non-macOS stubs, keep `Option<u32>` as foreign-boundary reads.
- The worker message is `CaptureRequest`, and its `window: Option<u32>` field is `window_target: CaptureWindowTarget`. Its one construction site remains `request_capture_if_worker_available`.
- All three types are crate-private: `WindowIdentification` is the public report on the search, and `CaptureAttemptWindowSelection` the public record of what a capture actually did. The target and the record stay separate — an attempt asked for `TerminalWindowHeuristic` can report `Selected { method: ClosestSizeMatch { .. } }`, and one asked for `PreferWindow { window_id }` can report `SelectionNotReached`.
- Both projections are covered for every state by the five `*_state_projects_report_and_capture_target` tests in `monitor.rs`. Behavior is unchanged throughout: every assertion and expected value in the backdrop suite is unmoved.

**Files:**
- `crates/tui_pane/src/backdrop/monitor.rs` — the window-identification state machine, its two projections, `CaptureRequest`, and the projection tests
- `crates/tui_pane/src/backdrop/desktop.rs` — `CaptureWindowTarget`, `TerminalWindowSearchOutcome`, the target-independent lookup wrappers, and the macOS platform reads they wrap
...
**Ruled out:**
- Returning `TerminalWindowSearchOutcome` from the four `platform` lookups and their non-macOS stubs — it makes `Found` constructible only under `#[cfg(target_os = "macos")]` and breaks the Linux lint gate.
- Silencing that Linux failure with `allow(dead_code)`.
- Making the public `WindowIdentification` the monitor's field — it would leave `attempts`, `attempted_at`, `asked` and `titles` beside it as fields that can contradict it.

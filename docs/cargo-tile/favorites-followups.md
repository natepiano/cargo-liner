...
1,802 non-test lines followed by a single 34-test module (`#[cfg(test)]` at 1803). The clusters are real but **not peers** — splitting them as equals produces circular imports or a wave of `pub(super)`. `FavoritesOverlay` directly owns the bindings, the line plan, the width cache, the notice and the removal state (`:683`), and `build_line_plan` consumes both content and bindings (`:1368`). Split with the direction stated:
...
- `content.rs` — `FavoritesOverlayContent`, `FavoriteRowsView`, `FavoriteRowView`, `UnrecognizedFavoritesView`, row lifecycle, the conversion-time formatting the row constructor calls (`:309`), and **`favorite_cells` (`:1721`)**
- `bindings.rs` — `FavoritesSurfaceBindings`, `ModeColumnBindings`, `ParameterColumnDescriptor`, `column_descriptors` (`:445`), `footer` (`:527`), the private `mode_label` (`:1748`) which keeps its name
- `line_plan.rs` — `CachedOverlayLine`, `CachedLinePlan`, `CachedSurfaceWidth`, `FavoriteSectionTableLayout`, `finish_navigation` (`:618`), `build_line_plan` (`:1368`), `rendered_line` (`:1316`), `append_unrecognized` (`:1501`)

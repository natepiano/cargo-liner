...
- **Key files:**
  ...
  - `crates/cargo-tile/src/terminal.rs` — input dispatch ladder. The local toast-scheduling types are gone; the framework deadline supplies the repaint cadence. The overlay consumes every key in the modal branch of `handle_key` (`:511`–517), which calls `handle_unmapped_key()` so any unmapped key cancels a delete confirmation, and `:290` commits `FavoritesOverlayFrameOutcome::CommitRemoval` for both recognized and unrecognized rows; `Event::Resize` records a pending area only (`:458`) and the post-drain `Resized::Yes` block acts on it (`:268`); `keyed_mode` caller and `Dismiss` dispatch follow.
...
`Attract::current_settings()` (`attract/mod.rs:697`) supplies the comparison. Snapshot it when the overlay opens and hold it in the open-overlay state. `build_line_plan` takes the content immutably (`line_plan.rs:191`) and `rendered_line` (`:98`) receives only an already-cached line, so neither can reach back to a view to mark it: pass the snapshot **into** `build_line_plan` and cache the per-row outcome on the `CachedOverlayLine::Row` itself, as a named `FavoriteRowCurrentParameters::{Unrecognized, Different, Matching}` beside its identity. `rendered_line` then combines that with `FavoriteSelection` to pick the three-cell prefix. The overlay consumes every key while open (`terminal.rs`, the modal branch of `handle_key` at `:511`–517), so steering cannot stale the snapshot; a resize that reclamps settings recomputes it. This is one comparison per recognized row on open and on resize, none per frame, no allocation.
...
**Name the marker state and give it an owner.** The four selected/current combinations are one named four-state value, not two booleans and not a tuple. Store the open overlay's settings snapshot **inside the open-overlay state** — an `OpenFavoritesOverlayState` holding a named `OpenFavoritesCurrentParameters` — or in another named state that says what it is; do not hang an `Option<AttractSettings>` on the outer controller, where "no overlay is open" and "the overlay is open and has no snapshot" would collapse into the same value.

**What the mark claims.** `Attract` keeps settings, not the `FavoriteId` they came from — loading returns `AttractSettings` alone — and a hand-edited file can hold several rows with equal settings and different ids, which `push` does not normalize. So the honest claim is "this row matches the current parameters", and **every** matching row is marked. Say so where the user can see it: a legend reading `● matches the current parameters`, in the overlay heading. That exact glyph and wording are what Phase 14 documents, so a test asserts them. A single authoritative "the favorite that is running" would mean carrying `FavoriteId` provenance through load, steer, randomize, undo and save and clearing it on every edit; that is a different, larger feature and is not this phase.
...
**Files:**
- `crates/cargo-tile/src/favorites_overlay/line_plan.rs` — three-cell prefix, the four-state marker value, width budget, `build_line_plan` taking the snapshot and caching `FavoriteRowCurrentParameters` on each `CachedOverlayLine::Row`
- `crates/cargo-tile/src/favorites_overlay/mod.rs` — hold the snapshot in the open-overlay state, pass it into `build_line_plan`, render the legend
- `crates/cargo-tile/src/app.rs` — the open-overlay state that owns the snapshot
- `crates/cargo-tile/src/globals.rs` — **two** production paths open the overlay and both must snapshot: the `AppGlobalAction::OpenFavorites` handler (`:117`) calling `open`, and `show_random_favorite_with` (`:175`) calling `open_file_state`. `terminal.rs:712` and `favorites_overlay/mod.rs:1309`, `:1925`, `:1940` are test openers and take the snapshot too
- `crates/cargo-tile/src/constants.rs` — add a **new** `FAVORITE_ROW_PREFIX_WIDTH: usize = 3` for the favorites row prefix. Do **not** rename or retune `CURSOR_WIDTH` (`:206`, value 2): `settings.rs:19`, `:90` uses the same constant for the settings pane, so changing it there would silently shift an unrelated layout. Switch the favorites uses — `line_plan.rs:447`, `:632`, `:655` and the test at `favorites_overlay/mod.rs:1617` — to the new constant and leave `settings.rs` on `CURSOR_WIDTH`
...
**Constraints from prior phases:** Phase 9 gave the cached line a `FavoriteRowIdentity::{Recognized(FavoriteId), Unrecognized(UnrecognizedFavoriteRemovalLocator)}` with non-row lines carrying none, put only real rows in the navigation index, made the selection carry that identity or nothing, and kept selection out of the identity as render-time state — currency joins it there. The prefix Phase 9 renders is still two cells; this phase widens it. Unrecognized rows never carry a currency mark: they have no settings to compare. `Attract::current_settings()` takes `&mut self` (`attract/mod.rs:697`), so the caller must hold the attract state mutably — which is why the snapshot is taken in `globals.rs` and `terminal.rs` rather than inside the overlay. Each leaf module of `favorites_overlay/` carries its own narrow test-module lint block: `content.rs` and `bindings.rs` expect only `clippy::expect_used`; `line_plan.rs` expects `clippy::expect_used` **and** `clippy::panic` (`line_plan.rs:558`, added in Phase 9); `mod.rs` expects `clippy::panic` and `clippy::unchecked_time_subtraction` as well. Preserve only the expectations each module actually triggers. Warnings are denied and an unfulfilled `#[expect]` is itself an error, so a new test adds only the lints its own module actually trips.
...
**Goal:** cargo-tile's compact shortcut overlay stops listing a key that does nothing there, and a delete refused by a changed file tells the user how to recover.

**Spec:**

The `x` Dismiss row is listed in cargo-tile's shortcut overlay but does nothing there. `GlobalAction::Dismiss` is bound to `'x'` as a shared framework default (`global_action.rs:70`, `:241`) that `cargo-port` relies on, while cargo-tile's tests require `x` not to close framework overlays. So the shared default must not change — but the filter cannot live in cargo-tile alone either: `Keymap::keymap_help_rows` and `global_shortcut_rows` enumerate `GlobalAction::ALL` unconditionally, and `tui_pane` offers a client no way to hide a framework-global row from its own rendered help.

**Add that hook to the framework.** The cause is a missing framework affordance, so the change goes where the cause is: a client-supplied *presentation* policy in `tui_pane`. It must not touch dispatch or rebinding — a hidden action still fires when its key is pressed and still appears in the full keymap editor; only the compact rendered list omits it.

**Apply it to `global_shortcut_rows` only, and leave `keymap_help_rows` unfiltered.** The two functions feed different surfaces, and getting this backwards would hide the action from the very editor that is supposed to rebind it. `keymap_help_rows` feeds `overlays/keymap_ui.rs:166`, which renders the full keymap overlay — the rebinding catalog, which must stay complete. `global_shortcut_rows` feeds exactly two callers that must agree with each other: `overlays/global_shortcuts.rs:133` renders the compact overlay and sets its viewport length from the row count, and `overlays/keymap_edit.rs:178` (`edit_selected_global_shortcut`) indexes the same vector by that viewport position to find the row to open the editor on. Filtering inside `global_shortcut_rows` keeps those two consistent by construction; filtering at only one call site would desynchronize the index and open the editor on the wrong row.

**Name the policy; do not spell it as an option or a boolean.** The policy is always present, defaulting to showing everything: a named `FrameworkGlobalShortcutPresentation` returning `FrameworkGlobalShortcutVisibility::{Shown, Hidden}` per row. Not an `Option<fn(...)>`, where "no client policy" and "a policy that hides nothing" would be two spellings of one behavior, and not a bare `bool`, which names neither answer. Dispatch and the keymap TOML are unchanged.

**The stale-locator refusal must say what to do about it.** `deletion_refusal_message` (`favorites_overlay/mod.rs:931`) currently says only that the file changed and nothing was deleted. Retrying without reopening necessarily fails again — the locator was captured when the overlay loaded — and the overlay has no reload key, so the user is told a fact with no action attached. Make the notice read exactly: `The favorites file changed after this row was loaded; nothing was deleted. Close and reopen favorites, then try again.`

**Files:**
- `crates/tui_pane/src/keymap/mod.rs` and `crates/tui_pane/src/keymap/global_action.rs` — `FrameworkGlobalShortcutPresentation` and `FrameworkGlobalShortcutVisibility`, consulted by `global_shortcut_rows` (`:439`) and **not** by `keymap_help_rows` (`:515`), defaulting to showing every row
- `crates/cargo-tile/src/keymap.rs` — supply the policy that hides the inactive `x` Dismiss row from cargo-tile's compact shortcut overlay
- `crates/cargo-tile/src/favorites_overlay/mod.rs` — `deletion_refusal_message` (`:931`) gains the recovery sentence

**Constraints from prior phases:** Phase 9 shipped `deletion_refusal_message` in `favorites_overlay/mod.rs`, reached when `FavoritesMutationError::UnrecognizedFavoriteChanged` refuses a removal whose locator no longer identifies exactly one raw table; match that error by variant, never by message text. Phase 9 also made unrecognized rows selectable and deletable behind a two-press confirmation that any other key cancels, and gave `FavoritesSurfaceBindings` a cached footer whose every segment is gated on `ResolvedBinding::Bound`. Invariant 2 binds the framework half: `cargo-port` supplies no presentation policy and must keep showing and dispatching `GlobalAction::Dismiss` exactly as it does today.

**Acceptance gate:**
- `bash ~/.claude/scripts/delegate/verify.sh test tui_pane` — the default policy hides nothing; a supplied policy removes only the named row from `global_shortcut_rows`; `keymap_help_rows` still lists a hidden action so it stays rebindable; dispatch still reaches a hidden action; and `edit_selected_global_shortcut` opens the editor on the row the compact overlay actually shows when a row above it is hidden
- `bash ~/.claude/scripts/delegate/verify.sh lint tui_pane`
- `bash ~/.claude/scripts/delegate/verify.sh test cargo-tile` — cargo-tile's compact shortcut overlay omits the `x` Dismiss row; an exact-text regression test asserts the stale-locator refusal notice, including its recovery sentence
- `bash ~/.claude/scripts/delegate/verify.sh lint cargo-tile`
...
**Constraints from prior phases:** Phase 9 made unrecognized rows selectable and deletable behind a two-press confirmation that any intervening key cancels; do not call those rows diagnostics in the README, since a row this version cannot read may simply have been written by a newer cargo-tile. Phase 10 widened the row prefix to three cells — `"   "`, `"▸  "`, `" ● "`, `"▸● "` — and renders the legend `● matches the current parameters`, marking every matching row rather than one authoritative running favorite. Phase 11 named the save outcome `FavoriteSaveOutcome::{Added, Refreshed}`. Phase 13 hid the `x` Dismiss row from cargo-tile's compact shortcut overlay while leaving it rebindable in the full keymap editor, so the README must not list `x` as a cargo-tile global. Document what those phases actually shipped — read the code before writing the section rather than describing the plan.
...
### Phase 15 — The second window's capture failure gets its actual cause  · status: todo
...
Add a named attempt state, `LatestCaptureAttemptWindowSelection::{WaitingForFirstResult, SelectionNotReached, Selected { window_id, method: CaptureWindowSelectionMethod }}`, with `CaptureWindowSelectionMethod::{Identified, Frontmost, Size}` naming how the id was reached. Three points, each load-bearing. Not an `Option<u32>`: "failed before choosing a window" and "chose this window and then failed" are the two answers the reproduction turns on, and an option collapses them. `WaitingForFirstResult` is a distinct third state, because an instance that has completed no attempt at all is exactly the instance under investigation and must not read as one that failed before selecting. And `method` is its own enum rather than a string or an integer, so a log line cannot claim a method that does not exist.

Carry the attempt state inside a named `CaptureAttemptResult` alongside a `CaptureAttemptSequence` counter, so **every completed attempt** emits a record — the Spec above asks for every attempt, which a transition-only signal cannot deliver — while the existing `backdrop:` summary line stays transition-only and unchanged in cadence. Carry it out of `capture` on both the success and the failure path, hold it on `BackdropMonitor` beside the latest attempt status, export it through `backdrop/mod.rs` and `lib.rs`, and add it to the `backdrop: …` transition line `Attract::identify` already emits so the reproduction records it.

This is distinct from Phase 16's `CaptureWindowTarget`, which says what the capture was *asked* to do. This says what it actually did.

**Files:**
- `crates/tui_pane/src/backdrop/desktop.rs` — `LatestCaptureAttemptWindowSelection`, `CaptureWindowSelectionMethod` and `CaptureAttemptResult`, carried out of `capture` on both paths; then window resolution, filter construction, or capture, as the evidence directs
...
### Phase 16 — Window selection stops being a bare optional number  · status: todo

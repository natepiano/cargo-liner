...
`drawn_parent` (`processes.rs:1070`) conflates the same two meanings today — an optional pid standing for both "no visible parent" and "the parent is not drawn" — and gains a named visible-parent type in this phase alongside the identity it keys on.
...
*10.. Two capture-facing boundaries the migration must also name.* `CapturedRun` (`processes.rs:505`) is the **unverified** nearest-registration lookup — its `Unregistered` arm is a real outcome — so its name states no guarantee and must not be read as this phase's verified direct association; rename it for what it is and keep the two distinct. Separately, `RunState::working` (`progress.rs:172`) and `CaptureLookup::working` (`:191`) return a domain-owned `Option<(Phase, Progress)>` consumed at `render.rs:1769`, with no semantic replacement assigned anywhere in this plan. Make the gauge consume named counter states instead, preserving the blocked, unavailable and unregistered behavior it has today. The measurement and roster replacements specified elsewhere in this phase stay as written. The home-boundary and pinning replacements are **phase 9's**, specified there, not here.

**Files:**
- `crates/cargo-tile/src/processes.rs` — `InvocationId`, `is_shim` (`:1005`), `captured_run` (`:1181`), the exclusion retain at `:617`, `Capture::take(roots)` at `:631`, `drawn_parent` (`:1070`), and `CpuSmoothing`'s pid-keyed history (`:709`)
...
- `crates/cargo-tile/src/roster.rs` — matching (`:195`), the family maps, and `TrackedRow`'s three optionals (`:26`)
...
- `crates/cargo-tile/src/terminal.rs` — **two** `CargoProcess` row literals, in `scan_process` (`:748`) and its unavailable case (`:770`); the `CaptureKey` construction and `Capture::read` call at `:919`; and the `scan_process(state)` call at `:927`, which imports `DirectoryIdentity`
...
- `test` — opens as impl: `src/roster.rs` including every row fixture, `src/tiles.rs`, and `src/render.rs` — the consumers keyed on a bare pid today, plus `TrackedRow`'s three named states, and `src/terminal.rs` — its two row literals (`:748`, `:770`) plus the `CaptureKey` construction and `Capture::read` call at `:919`, so it migrates with the consumers
...
- Phase 5's verified anchors, replacing the stale ones this Work Order carried: `Capture::read` (`progress.rs:673`) **takes a `CaptureKey`, not a pid**; `Capture::confirmed` (`:689`); `registered_runs` (`:756`); `take_roots` (`:507`); `scan_root` (`:564`); `ConfirmedCapture` (`:427`), which now carries `key: CaptureKey`; `CaptureRootSource` (`:268`); `CaptureRoots::resolve` (`:303`). In `processes.rs`: exclusion retain `:617`, the `Capture::take(roots)` call `:631`, `is_shim` (`:1005`), `drawn_parent` (`:1070`), `captured_run` (`:1181`), `annotate_capture` (`:1245`), `CapturedRun` (`:505`), and `spawn(config: &Config)` (`:519`) whose only production caller is `terminal.rs:209`. `drain_scans` remains at `terminal.rs:401`.
...
- **Two precedence rules already hold and must both survive the identity migration.** `captured_run` (`processes.rs:1181`) selects the nearest registered ancestor first, and only then the lowest root index for that pid; own-root precedence does not override a nearer ancestor. The regression at `processes.rs:2057` holds the second rule in place — an unconfirmed reading in the preferred root stops another root's verified metadata from annotating the row. Keep both, and keep that regression passing.
- Phase 5 added a `CargoProcess` row literal to `terminal.rs`, and phase 7 split it in two: `scan_process` (`:748`) and its unavailable case (`:770`). The `CaptureKey` construction and `Capture::read` call sit at `:919`, and `scan_process(state)` at `:927`. Every key, type-name and identity change in this phase reaches both literals.
...
- **Phase 7 supplies three named measurement types, and rows built here are constructed out of them.** `Measurement<T>` is `Reading(T) | Unavailable(MeasurementAbsence)`; `MeasurementAbsence` is `FirstObservation | ReadFailed | Unproven` — a first sample, a read that failed, and a sample no evidence can classify. `CompilerObservation` is `Unknown | None | Running(Compiler)`, where `None` is an **observed** absence rather than a missing reading, so "distinguishing a measured zero" means the observed-absence arm and not the unknown one. A row carries `cpu: Measurement<String>` and `managed: Measurement<usize>`. `aggregate_cpu` (`processes.rs:1413`) and `cpu_label` (`:1462`) are `pub(crate)`; `aggregate_compilers` (`:1383`) stays private. `Add for Measurement<T>` propagates absence, so a total with one unavailable member is unavailable, and every unavailable value renders as `UNAVAILABLE_MEASUREMENT` (`constants.rs`). Rows built here use these names directly and must not reintroduce a bare `Option` for any of the three.
- **Phase 7's CPU smoothing is keyed by pid twice over, and both maps plus its publication clock retire with the identity migration.** `CpuSmoothing` (`processes.rs:709`) holds `settled` and `reported`, both keyed by `Pid`, beside `publication: CpuPublication` (`:688`), which is `NeverPublished | Published(Instant)`. Re-key both maps onto the new identity — re-keying one and leaving the other keeps a pid-keyed history alive — and keep these three regressions passing under it: `the_table_holds_a_reading_for_the_whole_report_interval` (`:3191`), `a_never_published_smoother_is_distinct_from_one_holding_a_reading` (`:3253`), and `unavailable_cpu_replaces_a_stale_reading_before_its_publication_deadline` (`:3266`).
- **`assign_families` (`roster.rs:388`) now reads an unavailable count as unknown rather than as zero.** A family survives an unavailable managed count instead of being released, can be established from a retained child's parent link, and is released only on a **measured** zero. The family maps are keyed by pid today; the re-key must preserve all three behaviors.
- **The CPU baseline still pairs on pid plus a whole-second start time, and this phase is where that ends.** `CpuBaseline` (`processes.rs:670`) matches a previous sample to a current one on pid and a start time sysinfo exposes only in seconds, so a pid replaced inside one second whose replacement's accumulated counter is not lower passes the identity filter as the same process. Phase 7 accepted that as unclassifiable because it had no lifetime evidence to separate the two; this phase has it. Establish baseline continuity from the same process-lifetime evidence the invocation identity carries, preserving the pre-refresh snapshot at `processes.rs:570` that supplies the previous sample.
- **Phase 6's root-state contract survives this migration unchanged, and phase 6 owns its surfaces.** `RootReadStatus` (`processes.rs:454`) separates a readable root, a default root deliberately not created and reported quietly, a retryable `Unavailable(PathFailure)`, and an `Invalid(CaptureFailure)` from startup validation that needs a corrected configuration and a restart. `CleanupRefusal` (`capture_root.rs:76`) separately reports foreign ownership as informational, writable mode bits as repairable, effective-user failure as session-long, and access, incomplete-inventory and continuity refusals with the path each affects. `RootStatus` travels on `Scan` into `App` and is read by settings and by `drain_scans`. This phase re-keys identity underneath all of it and builds no replacement surface; keep these tests passing as written: `settings.rs:749`, `:799`, `:853`, `:1063`, `:1137`, and `terminal.rs:802` and `:840`.
...
*2. Precedence is per field, never per row.* Preferring a whole process-table row discards a working directory the registration has, because `row()` (`processes.rs:1427`) degrades a missing cwd to `UNRESOLVED_PATH` (`:1436-1437`) rather than failing. That is the ordinary runner-machine case, not a corner: there argv is readable and cwd is denied, so the row survives with `path: "unavailable"` while the registration holds the real directory. The process row keeps its real pid, its tree and its measured values; an absent cwd is filled from the matching registration; and the merge happens **before** any field becomes display text.
...
- `crates/cargo-tile/src/roster.rs` — the literal row fixtures (`:609`), which change with every identity field a row gains
- `crates/cargo-tile/src/render.rs` — grouping on account, root and working directory (`:1353`), pin selection (`:1401`), the heading prefix
- `crates/cargo-tile/src/constants.rs` — the heading prefix's presentation literals
- `crates/cargo-tile/src/terminal.rs` — **two** `CargoProcess` row literals, in `scan_process` (`:748`) and its unavailable case (`:770`), both built from the row schema this phase changes
...
- `impl` — opens as impl: `src/processes.rs`, `src/progress.rs`, and `src/roster.rs` including every row fixture — the registration-sourced row, the per-field merge at `row()` (`:1427`) where `path` degrades to `UNRESOLVED_PATH` but `command` fails the whole row, and the row-sourcing accessor beside `confirmed`; hub: `src/processes.rs`, the row builder both writers feed and read. Owns the source-merge tests. Keeps the `roster.rs` fixtures current through every repair round, plus `src/terminal.rs` — its two `CargoProcess` row literals (`:748`, `:770`) are built from the row schema this seat changes and cannot be migrated by anyone else
- `test` — opens as impl: `src/render.rs` — `group_by_path` (`:1353`) keys on verified account, root and working directory, headings gain the account prefix, and pin selection (`:1401`) migrates with the key — plus `src/constants.rs` for the prefix literals, this writer's alone in this phase; writes into the existing `#[cfg(test)]` module (`:2038`)
...
- Phase 5 verified anchors: `Capture::read` (`progress.rs:673`) **takes a `CaptureKey`, not a pid**; `Capture::confirmed` (`:689`); `ConfirmedCapture` (`:427`), which now carries `key: CaptureKey`; `row()` (`processes.rs:1427`); `registration_directory` (`processes.rs:1511`); `group_by_path` and pin selection stay in `render.rs`.
- Phase 5 added a `CargoProcess` literal to `terminal.rs`, and phase 7 split it in two and changed its schema: `scan_process` (`:748`) and its unavailable case (`:770`) now carry `cpu: Measurement<String>`, `compiler: CompilerObservation` and `managed: Measurement<usize>` — **not** the `String` / `Option` / `usize` this Work Order previously stated. Any row-schema change here has to migrate both.

...
*Root-qualified capture identity.* `Capture` (`progress.rs:295`) holds **two** pid-keyed stores, and both need the root: `readings`, the map of read outcomes, and `confirmed`, the verified registrations `Capture::confirmed()` (`:417`) hands out. With more than one root, two roots can hold a registration under the same pid number. Qualify each store on the interned root index plus the pid, and qualify their consumers with them — `annotate_capture` (`processes.rs:934`) selects a confirmed registration by pid alone, so qualifying only `readings` leaves registration metadata free to attach to a row from a different root while the readings map stays correct. Do not attempt the wider identity change here — Phase 8 carries root-qualified identity through the roster, families and tiles; this phase's obligation is that two roots holding the same pid number cannot collide in either store.
...
- `crates/cargo-tile/src/progress.rs` — `root()` becomes the resolved list; both capture stores, `readings` and `confirmed`, gain the root index in their key
...
- `crates/cargo-tile/src/progress.rs` — `take_roots` (`:312`) and `registered_runs` (`:438`) retain the errors they discard today, and `Capture` carries them
...
`Capture::read` (`progress.rs:409`) returns `Unregistered | Registered(CaptureRead)` over
`Progress | NoCurrentProgress | Unreadable(CaptureFailure)`, and `CaptureFailure` (`:211`) carries
the error kind and the message. What is still discarded sits on either side of that, and this phase
retains all three: `take_roots` (`:312`) drops the error from a root that would not open,
`registered_runs` (`:438`) drops the error from an individual registration read, and a record that
...
`CpuSmoothing`'s `taken` field (`processes.rs:517`) is itself one of the optionals this phase exists to remove: it distinguishes a smoother that has never published from one holding a previous reading, and those are different states with different consequences once a sample can go unavailable. Give it a name rather than an `Option`.

Reach: `Census::take` (`:595`), `CpuSmoothing` (`:507`), the group totals `aggregate_compilers` (`:1070`) and `aggregate_cpu` (`:1090`), `Roster::assign_families` (`roster.rs:382`, the `managed > 0` test), and the call sites in `render.rs`.
...
**Files:**
- `crates/cargo-tile/src/processes.rs` — `Census::take` (`:595`), `CpuSmoothing` (`:507`), the row fields, and the group totals `aggregate_compilers` (`:1070`) / `aggregate_cpu` (`:1090`)
...
- `impl` — `crates/cargo-tile/src/processes.rs` — `Census::take`, `CpuSmoothing`, the row fields, **and the group totals: `aggregate_compilers` (`:1070`) and `aggregate_cpu` (`:1090`) live here, not in `render.rs`**; plus `crates/cargo-tile/src/constants.rs`; hub: `crates/cargo-tile/src/processes.rs` (the measurement type all three files read)
...
`drawn_parent` (`processes.rs:806`) has the same shape today — an optional pid standing for both "no visible parent" and "the parent is not drawn" — and gains a named visible-parent type in this phase alongside the identity it keys on.
...
*3. `is_shim` accepts absence as evidence.* `is_shim` (`processes.rs:741`) compares `subcommand(outer.cmd()) == subcommand(inner.cmd()) && outer.cwd() == inner.cwd()`. When both sides are `None` the cwd test passes, so two unavailable fields count as a match — and cwd is exactly the field the runner machine denies. Require observed equality, not matching absence.

*4. Membership is resolved before progress is parsed — and already is.* `captured_run` (`processes.rs:917`) now stops at the nearest registered capture whether or not its log parses, and `nearest_empty_capture_does_not_inherit_an_enclosing_progress_reading` (`:1553`) and `nearest_unreadable_capture_does_not_inherit_an_enclosing_progress_reading` (`:1569`) hold it there. What remains is narrower: carry invocation identity through that association, and keep **direct** association distinct from **enclosing** membership, preserving the existing `CaptureLookup` / `CaptureRead` outcomes and the once-per-scan reads behind them.

*5. Consume the scan's capture results; do not build a second cache.* This item asked for a once-per-scan read because every row was believed to reopen its log. **Phase 3 already shipped that.** `Capture::scan_root` (`progress.rs:327`) reads each selected log during the scan and stores the result; `Capture::read` (`:409`) is a map lookup documented as performing no later filesystem access, and `captured_run` (`processes.rs:917`) walks parents over that lookup alone. `a_capture_keeps_its_reading_without_reopening_a_replaced_log` (`progress.rs:994`) holds it in place. There is no per-row I/O left to remove and no new cache to add here.
...
- `test` — opens as impl: `src/roster.rs` including every row fixture, `src/tiles.rs`, and `src/render.rs` — the consumers keyed on a bare pid today, plus `TrackedRow`'s three named states
- `review` — opens as test: `tests/shim_registration.rs` — nested invocation identity and exclusion regressions driven through the production binary; claims no writer file
...
This is the item the feature exists for. Phase 4 already opens and verifies the registration — `registered_runs` (`progress.rs:438`) reads each record against fresh kernel evidence, and `Capture::confirmed()` (`:417`) hands out the verified ones — so the fields the grid needs are in hand. What is missing is a row built from them: a registration can *supply* a row rather than only annotate one, and an unreadable argv costs no columns.

*1. The new direction.* Today the linkage runs one way: `captured_run` (`processes.rs:917`) walks **upward** from a cargo pid to find a registered ancestor and read its state. That walk stays as it is for every row the process table can already build. What is new is the other direction, for a run the process table cannot describe: the registration names a live shim pid, and the fields the row is missing are already parsed. `Capture::confirmed()` (`progress.rs:417`) is the source — it hands out `ConfirmedCapture` (`:264`), each a verified registration with its file's modification time — and `Capture::read` (`:409`) already returns a named outcome rather than an optional. Build rows from `confirmed()`, taking the direct-association type phase 8 defines rather than reaching for the candidate record.
...
**Files:**
- `crates/cargo-tile/src/progress.rs` — the row-sourcing accessor beside `confirmed` (`:417`)
- `crates/cargo-tile/src/processes.rs` — the registration-sourced row, the per-field merge, `row()` (`:1095`), and `registration_directory` (`:1179`)
...
- `impl` — opens as impl: `src/processes.rs`, `src/progress.rs`, and `src/roster.rs` including every row fixture — the registration-sourced row, the per-field merge at `row()` (`:1095`) where `path` degrades to `UNRESOLVED_PATH` but `command` fails the whole row, and the row-sourcing accessor beside `confirmed`; hub: `src/processes.rs`, the row builder both writers feed and read. Owns the source-merge tests. Keeps the `roster.rs` fixtures current through every repair round

...
- **Key files:**
  ...
  - `crates/cargo-tile/src/progress.rs` — reads the capture root. `Progress`
    `:89`; `Phase` `:119`; `RunState` `:145`; `CaptureLookup` `:171` —
    `Unregistered | Registered(CaptureRead)`; `CaptureRead` `:191` —
    `Progress(RunState) | NoCurrentProgress | Unreadable(CaptureFailure)`;
    `CaptureFailure` `:211`, which retains the error kind and message;
    `RegisteredRuns` `:234`; `RegistrationEvidence` `:255`; `ConfirmedCapture`
    `:264` — a verified registration with its descriptor-bound modification time
    held as `Result<SystemTime, CaptureFailure>`, so verification can succeed
    while the timestamp read fails; `RegistrationName` `:272` — `Legacy |
    Generated | Staging | Unrelated`, a classification of a filename that proves
    nothing about process identity; `Capture` `:295`, holding two pid-keyed
    stores, `readings` (read outcomes) and `confirmed` (verified registrations),
    with `take()` `:304`, `take_from(root, observe)` `:307`,
    `take_roots(roots, observe)` `:312` — the single per-scan removal allowance
    across every root — `scan_root` `:327`, `read(pid) -> CaptureLookup` `:409`,
    a map lookup that performs no filesystem access, and `confirmed() ->
    &[ConfirmedCapture]` `:417`; `root()` `:433` — resolves `CARGO_TILE_ROOT` or
    the default, the single site phase 5 turns into a list;
    `registered_runs(scan, observe)` `:438` — reads `<root>/state/pids` through
    the access layer, verifies each record against fresh kernel evidence for that
    pid, and returns `RegisteredRuns`; `parse_state` `:542`. Inline
    `#[cfg(test)]` module `:679`. Reading a log goes through
    `capture_root::ScanEntry::read_log` `:525`, and reading a registration
    through `read_registration` `:533`.

    One ordering is an invariant rather than implementation detail, and phase 4
    moved the sweep onto identity. `RootScan::open` (`capture_root.rs:126`)
  ...
  - `crates/cargo-tile/src/processes.rs` — the two-phase process scan and row
    ...
    Result<CargoArguments, RowAbsence>` `:1278`, which reports an unavailable
    argv as its own outcome — the macOS empty-argv gate. `row(...) ->
    Result<CargoProcess, RowAbsence>` `:1095`: `path` degrades a missing cwd to
    `UNRESOLVED_PATH` via `map_or_else` (`:1104-1107`) while `command:
    command_text(process.cmd(), home)?` (`:1121`) fails the whole row — the
    asymmetry phase 9's per-field merge resolves. `CpuSmoothing` `:507`;
    `Census` `:579` with `take` `:595`, phase 7's collection boundary; `is_shim`
    `:741`; `drawn_parent` `:806`; `captured_run` `:917`, returning
    `CaptureLookup`; `annotate_capture` `:934`, which selects a confirmed
    registration by pid alone; the exclusion retain `:448`, whose `.is_ok()`
    discards the `RowAbsence` distinction; `Capture::take()` at `:462`;
    `aggregate_compilers` `:1070` and `aggregate_cpu` `:1090`, phase 7's
    aggregation boundary and in this file rather than in `render.rs`;
    `registration_directory` `:1179`, which collapses a directory to `~` only
    when the record's writer home matches the scanner's own. Inline
    `#[cfg(test)]` blocks `:167` and `:1381`.
...
- **Invariants:**
  ...
  - **Never edit `/etc/nixos`.** Reads are fine at any time. It is one shared
    working tree and index between the `linux-host` session and the local-runners
    ...
    `linux-host` session, not by a phase delegate.
  - **No agent session rebuilds or switches NixOS generations.** The user does that.
  - **The linux-host scan-path gap is unreproduced and is not a phase.** On the
    linux-host Linux box no runner cargo has yet appeared in the grid, and the cause
...
*A retained validation failure is a sixth refusal condition, and it cannot recover.* `CaptureRoot.path` is `Result<PathBuf, CaptureFailure>`: a configured entry that fails validation — today, a relative path — is **retained** as an error, and `take_roots` (`progress.rs:467`) skips it silently beside the roots it cannot open. Give it a row showing the original `capture.roots` entry, its error, and the fact that correcting the path and restarting is what retries it, because resolution runs once at startup and no later scan revisits it. This is the one status whose displayed path is **not** absolute, so the absolute-path requirement above admits it as the exception. A relative `CARGO_TILE_ROOT` behaves differently — `resolve_environment` (`:293`) makes it absolute — and the two must read differently.

*One row per effective root means one row per **deduplicated** root, listing every source that named it.* A root reached as both the environment override and a configuration entry, or twice through different spellings of one directory, is a single row carrying both sources and both original spellings. The singular `default | environment | config` wording would throw that provenance away, which is exactly what an operator debugging a misspelled entry needs.

*Say which root supplied an association, and when another root's proof went unused.* `captured_run` (`processes.rs:940`) takes the nearest registered ancestor first and only then the lowest root index for that pid. An unconfirmed reading in the preferred root therefore suppresses a **confirmed** proof held by another root, deliberately — it is the anti-cross-root rule — but today nothing tells the operator it happened. Name the supplying root on the row and word the suppressed-proof case; it is otherwise indistinguishable from a run whose registration was never found.
...
- `crates/cargo-tile/src/birth_stamp/` — `mod.rs`, `linux.rs` and `macos.rs`: the named diagnostic for a boot read whose failure is cached for the session
- `crates/cargo-tile/src/progress.rs` — `take_roots` (`:467`) and `registered_runs` (`:608`) retain the errors they discard today, and `Capture` carries them
...
- `impl` — `crates/cargo-tile/src/capture_root.rs`, `crates/cargo-tile/src/progress.rs`, `crates/cargo-tile/src/processes.rs`, `crates/cargo-tile/src/terminal.rs` and `crates/cargo-tile/src/app.rs`, with their inline tests — producing the diagnostics and carrying them to the screen: the split classification, the read outcomes behind it, the retained root, registration and log errors from `progress.rs`, the session-cached boot-read diagnostic in `crates/cargo-tile/src/birth_stamp/`, root status on `Scan`, the `drain_scans` (`:401`) redraw when status changes while the command groups do not, and the retained status `App` holds. The Delegation Context lists `app.rs` as read-only for this plan and this phase is the exception. Hub: `crates/cargo-tile/src/processes.rs` (the `Scan` status type the renderer reads). Producing a diagnostic and transporting it are one change; splitting them across two writers puts the same type on both lines.
...
- Phase 5 resolves, deduplicates and interns the roots **once inside the scanner worker**, before its scan loop — `spawn` (`processes.rs:375`) only snapshots configuration on the caller, so terminal startup never waits on the filesystem (regression at `processes.rs:1436`). Responsiveness is the user-visible consequence; the private resolver helper needs no status row and no public API.
- Phase 5 retains a **missing** configured path through resolution and reopens it every scan, so this phase's missing-then-present gate shows recovery **without a restart**. A path retained as a *validation failure* is the opposite case and cannot recover: resolution runs once.
- Phase 5's `CaptureRoot.sources` is a `Vec<CaptureRootSource>` — `Default`, `Environment { path }`, `Configuration { entry, path }` (`progress.rs:253`) — so a deduplicated root carries **every** source that named it, with the original spelling of each. The `source` column renders all of them, not the first.
- Root-qualified capture identity (`CaptureKey { root, pid }` keying both stores) is observed only by capture scanning and annotation and needs no operator surface. Keep the shipped regressions at `progress.rs:1238` and `processes.rs:1687` passing.
- Phase 5 verified anchors: `take_roots` (`progress.rs:467`), `scan_root` (`:488`), `Capture::read` (`:577`) **taking a `CaptureKey`**, `Capture::confirmed` (`:593`), `registered_runs` (`:608`), `ConfirmedCapture` (`:410`), `CaptureRoots::resolve_environment` (`:293`). `drain_scans` remains at `terminal.rs:401`.
- Phase 5 supplies the resolved, deduplicated, interned root list and the `default` / `environment` / `config` distinction.
- Phase 4 supplies the read outcomes this phase renders — `CaptureLookup` (`progress.rs:176`), `CaptureRead` (`:196`) and `CaptureFailure` (`:216`) — and `ConfirmedCapture` (`:410`), which is what the confirmed count counts. Verification runs against `birth_stamp::observe(pid)`, whose boot read is cached for the life of the process including its failure; that cache is one of the two session-long failures this phase must word.
- Phase 3 supplies per-scan ownership and enumeration outcomes, including the unavailable-versus-empty live set that `permission denied: state/pids` and `partial` report. It produces **five distinct refusal conditions that today all look identical to an empty root**, and this phase owes each one its own wording:
  ...
  - an effective-user read that failed. `effective_user()` (`capture_root.rs:712`) caches its first result in a `OnceLock` **including the failure**, so one failed read at startup disables cleanup for the entire session with no retry. This one is new as of phase 3's repair round and currently has no surface anywhere — it is the condition most likely to be reported as "it just stopped tidying up".

  `RootScan::access` (`capture_root.rs:228`) collapses all five into one private `RootAccess::Foreign`, so that value cannot become the displayed classification and this phase cannot render the list above without changing it. Split the two questions the name currently conflates: who owns the root, and whether this scan may delete in it. Report the reason path-qualified — the failing directory named, not the root alone — so `permission denied: state/pids` says which directory. That makes `capture_root.rs` and its inline tests this phase's to edit.

*An unreadable capture is a status, not an absence.* Phase 4 already keeps the outcome:
`Capture::read` (`progress.rs:577`) takes a `CaptureKey` and returns `Unregistered | Registered(CaptureRead)` over
`Progress | NoCurrentProgress | Unreadable(CaptureFailure)`, and `CaptureFailure` (`:216`) carries
the error kind and the message. What is still discarded sits on either side of that, and this phase
retains all three: `take_roots` (`:467`) drops the error from a root that would not open,
`registered_runs` (`:608`) drops the error from an individual registration read, and a record that
only annotates leaves no completeness evidence of its own in `Capture`. Retain each observation
**path-qualified** — naming the root, the registration, or the log that failed, never the root
alone — carry it through `Scan` into `App`, and render it there. Keep a registration that cannot be
verified distinct from a root that is genuinely empty. This is the visible face of the `0600`
failure the capture ordering exists to prevent: when it does happen the operator sees nothing at all
and is left to find it with `cat`.

*Counting, and what a cached failure means.* `Capture.readings` holds unknown and legacy
registrations alongside confirmed ones, so `active — N captures` counts **confirmed published
registrations** and nothing else; an annotation-only record, an unreadable log and a `.tmp` staging
file each stay out of that number and earn their own wording. Two failures are also cached for the
life of the process and cannot recover through another scan: `effective_user()`
(`capture_root.rs:712`) and the boot read behind `birth_stamp::observe` (`birth_stamp/linux.rs:17`,
`birth_stamp/macos.rs:27`), each a `OnceLock` that retains its failure. Both need a named diagnostic
saying that cleanup or verification is disabled for the session and that restarting is what retries
it, worded distinctly from a record whose identity is merely unknown this scan and may resolve on
the next one. Producing that diagnostic is this phase's writer work, which makes `birth_stamp/` its
file too. Conservative retention is deliberate and must not be worded as a promise of eventual
deletion: an artifact whose identity can never be established stays on disk.

**Acceptance gate:** Build, Test and Lint green. A test asserting `settings::rows` performs no filesystem access. A test asserting each of the five refusal conditions above — a short or failed enumeration, an incomplete registration inventory behind a complete directory enumeration, a genuinely foreign read-only root, a root whose mode bits the operator can fix, and an unavailable effective user — renders wording distinct from one another and from a root that is genuinely empty. A test asserting the effective-user wording says cleanup is disabled for the session and that restarting is what retries it, since nothing else will. A test asserting a permission failure names the directory that failed, not only the root. A test asserting a root whose status changes while the command groups do not still requests a redraw. A test asserting a configured root that is missing at first and appears later changes its displayed status, with the command groups unchanged across both scans. A test asserting an empty `CARGO_TILE_ROOT` displays the default as its source, not the environment. A test asserting a capture the reader cannot open **with no process-table row at all** still displays its access failure rather than an empty root, and that when the log becomes readable the next scan returns it to a normal status and requests a redraw, with the command groups unchanged across both scans. A test asserting a mixed root — one confirmed registration, one annotation-only record, one unreadable log, one staging file — reports the confirmed count alone and names the other three separately. A test asserting the cached boot-read diagnostic is worded distinctly from a record whose identity is unknown only this scan, and says restarting is what retries it. A test asserting that retention wording for a permanently unverifiable artifact does not promise eventual deletion. A test asserting a registration that cannot be verified is distinct from a root with nothing in it. A test per status string. A test asserting a **configured root retained as a validation failure** — today a relative `capture.roots` entry — renders its original spelling, its error, and that correcting the path and restarting is what retries it, distinctly from a root that is merely missing. A test asserting a relative `CARGO_TILE_ROOT` is made absolute and reported as a working environment root, so the same mistake reads differently in the two places it can be written. A test asserting a root named by both the environment and a configuration entry — including the `state/..` spelling — renders **one** row listing both sources with their original spellings. A test asserting a row names which root supplied a run's association, and that where a nearer or lower-indexed root held an unconfirmed reading the row says another root's proof went unused rather than showing nothing.
...
**Decision 1: where the macOS process-identity reader gets built and run before phase 8 depends on it**

Actual problem:
`crates/cargo-tile/src/birth_stamp/macos.rs` is compiled only on macOS and every gate in this plan
runs on Linux, so the sysctl reader that produces process birth evidence — the plan's only `unsafe`
— has never been compiled, run, or linted. This phase is the first to edit that directory, and
phase 8 keys invocation identity on what it returns.

What exists now:
- `birth_stamp/macos.rs` reads `kern.boottime` and `KERN_PROC_PID`; `BirthStamp::macos`
  (`birth_stamp/mod.rs:69`) truncates the birth to whole seconds to match what the shim can read
  from `ps`.
- The Linux reader regressions drive Darwin utility adapters, which proves the shim's conversion
  wording and nothing about the Rust reader.
- `.github/workflows/ci.yml` runs every job on `ubuntu-latest` apart from one `windows-latest`
  job. There is no macOS runner.

What should change:
- Add a macOS job to `.github/workflows/ci.yml` running this crate's build, test and lint, so the
  reader is compiled and its current-process and writer/reader tests execute on the platform that
  owns them; or
- run those three gates by hand on a Mac once before phase 8 and record the result here; or
- accept the gap and restate phase 8's identity guarantees as Linux-only, with the macOS path
  documented as unverified.

Recommendation:
Add the macOS CI job, in this phase, alongside the `birth_stamp/` diagnostic work it already owns.
It is the only option that keeps the evidence standing rather than expiring after one run, and it
is what makes phase 8's identity guarantees true on both platforms rather than one. The other two
leave a `unsafe` reader shipping with no build behind it.
...
`CpuSmoothing`'s `taken` field (`processes.rs:540`) is itself one of the optionals this phase exists to remove: it distinguishes a smoother that has never published from one holding a previous reading, and those are different states with different consequences once a sample can go unavailable. Give it a name rather than an `Option`.

Reach: `Census::take` (`:618`), `CpuSmoothing` (`:530`), the group totals `aggregate_compilers` (`:1088`) and `aggregate_cpu` (`:1108`), `Roster::assign_families` (`roster.rs:382`, the `managed > 0` test), and the call sites in `render.rs`.
...
**Files:**
- `crates/cargo-tile/src/processes.rs` — `Census::take` (`:618`), `CpuSmoothing` (`:530`), the row fields, and the group totals `aggregate_compilers` (`:1088`) / `aggregate_cpu` (`:1108`)
...
- `crates/cargo-tile/src/terminal.rs` — the `CargoProcess` row literal (`:758`), which constructs the measurement fields by hand
...
- `impl` — `crates/cargo-tile/src/processes.rs` — `Census::take`, `CpuSmoothing`, the row fields, **and the group totals: `aggregate_compilers` (`:1088`) and `aggregate_cpu` (`:1108`) live here, not in `render.rs`**; plus `crates/cargo-tile/src/constants.rs`; hub: `crates/cargo-tile/src/processes.rs` (the measurement type all three files read)
...
*A generation is a filename, not a lifetime.* `RegistrationName::Generated { generation, .. }` (`progress.rs:272`) classifies a filename suffix and proves nothing on its own. Phase 4 made that suffix non-repeating — `same_pid_and_calendar_second_receive_different_generations` (`tests/shim_registration.rs:1019`) holds the publisher to it — and supplied the boot-qualified birth stamp, so `(root, shim pid, generation)` no longer repeats at publication. What this phase owes is the consumer side, and it needs two values kept apart:

- **Comparison precision.** `BirthStamp::macos` (`birth_stamp/mod.rs:69`) truncates the birth to whole seconds, deliberately: the shim can read only `ps -o lstart=` at second resolution and the two have to compare equal, which `macos_discards_fractional_seconds_but_linux_keeps_individual_ticks` (`:280`) fixes in place. On macOS the birth stamp is therefore evidence that a claimed identity still holds — not a value that separates two processes born inside one second.
- **Process-lifetime identity.** `RunId` and `ProcessIdentity` are the non-repeating values, and the non-repeating generation is what distinguishes them, with the birth stamp qualifying rather than replacing it. Where neither is available, say so through a named unavailable-lifetime state rather than a value that might collide, and never merge two rows on unavailable evidence.

Root replacement invalidates every retained association keyed on a root, and this phase needs something it can observe that through. `RootScan::open` (`capture_root.rs:126`) already computes replacement once per scan, but the continuity it derives stays private to that module. Expose a **root incarnation** — an identity value that changes when the root at a pathname is replaced — without exporting any cleanup capability alongside it. `CpuSmoothing`'s history (`processes.rs:507`) is keyed on the bare pid and belongs in the same migration: otherwise a reused pid inherits the previous invocation's smoothed CPU.
...
`drawn_parent` (`processes.rs:829`) conflates the same two meanings today — an optional pid standing for both "no visible parent" and "the parent is not drawn" — and gains a named visible-parent type in this phase alongside the identity it keys on.
...
- `crates/cargo-tile/src/terminal.rs` — the `CargoProcess` row literal (`:758`), which constructs `CaptureKey`, calls `Capture::read` and imports `DirectoryIdentity`
...
- `test` — opens as impl: `src/roster.rs` including every row fixture, `src/tiles.rs`, and `src/render.rs` — the consumers keyed on a bare pid today, plus `TrackedRow`'s three named states, and `src/terminal.rs` — its row literal (`:758`) constructs `CaptureKey`, calls `Capture::read` and imports `DirectoryIdentity`, so it migrates with the consumers
...
**Constraints from prior phases:**
- Phase 5's verified anchors, replacing the stale ones this Work Order carried: `Capture::read` (`progress.rs:577`) **takes a `CaptureKey`, not a pid**; `Capture::confirmed` (`:593`); `registered_runs` (`:608`); `take_roots` (`:467`); `scan_root` (`:488`); `ConfirmedCapture` (`:410`), which now carries `key: CaptureKey`; `CaptureRootSource` (`:253`); `CaptureRoots::resolve` (`:288`). In `processes.rs`: exclusion retain `:468`, the `Capture::take(roots)` call `:485`, `is_shim` (`:764`), `drawn_parent` (`:829`), `captured_run` (`:940`), `annotate_capture` (`:958`), `CapturedRun` (`:361`), and `spawn(config: &Config)` (`:375`) whose only production caller is `terminal.rs:209`. `drain_scans` remains at `terminal.rs:401`.
- **`fn root()` no longer exists.** Phase 5 deleted it; `CaptureRoots::resolve` (`progress.rs:288`) resolves the list in its place. Any lookup this phase re-keys starts from `CaptureKey { root, pid }`, never from a bare pid.
- **Two precedence rules already hold and must both survive the identity migration.** `captured_run` (`processes.rs:940`) selects the nearest registered ancestor first, and only then the lowest root index for that pid; own-root precedence does not override a nearer ancestor. The regression at `processes.rs:1687` holds the second rule in place — an unconfirmed reading in the preferred root stops another root's verified metadata from annotating the row. Keep both, and keep that regression passing.
- Phase 5 added a `CargoProcess` row literal to `terminal.rs:758` which constructs `CaptureKey` directly, calls `Capture::read`, and imports `DirectoryIdentity`. Every key, type-name and identity change in this phase reaches it.
- **This phase's platform guarantee depends on phase 6's pending macOS decision.** Until that is settled, `birth_stamp/macos.rs` has never been compiled, run, or linted by any gate in this plan, and an identity guarantee stated here would be Linux-only in fact. Do not turn a two-platform claim into an acceptance item before that decision lands.
...
This is the item the feature exists for. Phase 4 already opens and verifies the registration — `registered_runs` (`progress.rs:608`) reads each record against fresh kernel evidence, and `Capture::confirmed()` (`:593`) hands out the verified ones — so the fields the grid needs are in hand. What is missing is a row built from them: a registration can *supply* a row rather than only annotate one, and an unreadable argv costs no columns.

*1. The new direction.* Today the linkage runs one way: `captured_run` (`processes.rs:940`) walks **upward** from a cargo pid to find a registered ancestor and read its state. That walk stays as it is for every row the process table can already build. What is new is the other direction, for a run the process table cannot describe: the registration names a live shim pid, and the fields the row is missing are already parsed. `Capture::confirmed()` (`progress.rs:593`) is the source — it hands out `ConfirmedCapture` (`:410`), each a verified registration with its file's modification time — and `Capture::read` (`:577`) already returns a named outcome rather than an optional. Build rows from `confirmed()`, taking the direct-association type phase 8 defines rather than reaching for the candidate record.
...
*4. A fallback row is for a run with no process row, never a second view of one that has one.* `Capture::confirmed()` (`progress.rs:593`) hands out proofs from every root, and a pid can be confirmed under a root the process-row association did not select. Build a fallback row **only** where the process table describes the shim pid nowhere. A non-selected proof for a pid that already has a row never becomes a second row and never overwrites that row's own root's metadata — one live invocation is one tile. Phase 6's root status is where an operator learns that another root held a proof and why it went unused; this phase owes the row-count rule and its tests, not the wording.

**Files:**
- `crates/cargo-tile/src/progress.rs` — the row-sourcing accessor beside `confirmed` (`:593`)
- `crates/cargo-tile/src/processes.rs` — the registration-sourced row, the per-field merge, `row()` (`:1113`), and `registration_directory` (`:1197`)
...
- `crates/cargo-tile/src/terminal.rs` — the `CargoProcess` row literal (`:758`), built from the row schema this phase changes
...
- `impl` — opens as impl: `src/processes.rs`, `src/progress.rs`, and `src/roster.rs` including every row fixture — the registration-sourced row, the per-field merge at `row()` (`:1113`) where `path` degrades to `UNRESOLVED_PATH` but `command` fails the whole row, and the row-sourcing accessor beside `confirmed`; hub: `src/processes.rs`, the row builder both writers feed and read. Owns the source-merge tests. Keeps the `roster.rs` fixtures current through every repair round, plus `src/terminal.rs` — its `CargoProcess` row literal (`:758`) is built from the row schema this seat changes and cannot be migrated by anyone else
...
**Constraints from prior phases:**
- Phase 5 supplies the root-qualified capture key. `Capture::confirmed()` (`progress.rs:593`) returns proofs from **every** root, while the process-row association selects one root per pid, so the two sets can disagree about the same shim pid. **A fallback row exists only for a shim pid the process table cannot describe at all.** A confirmed proof in a root the process row did not select never creates a second row: one live invocation is one tile, and a second row for it is precisely the duplicate phase 5's deduplication exists to prevent. Where the two disagree, phase 6's root status is the surface that explains it.
- Phase 5 verified anchors: `Capture::read` (`progress.rs:577`) **takes a `CaptureKey`, not a pid**; `Capture::confirmed` (`:593`); `ConfirmedCapture` (`:410`), which now carries `key: CaptureKey`; `row()` (`processes.rs:1113`); `registration_directory` (`processes.rs:1197`); `group_by_path` and pin selection stay in `render.rs`.
- Phase 5 added a `CargoProcess` literal to `terminal.rs:758` carrying `cpu: String`, `compiler: Option` and `managed: usize`. Any row-schema change here has to migrate it.

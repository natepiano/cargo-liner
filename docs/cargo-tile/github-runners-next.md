...
**Constraints from prior phases:** none that bind; no earlier phase touched `capture_root.rs`. Phase 3 proved the crate compiles and tests natively on macOS, so this phase's macOS-only target has a native route as well as the workflow's `macos-latest` job (`.github/workflows/ci.yml:348`); both are subject to the three pre-existing `shim_registration` failures recorded in Delegation Context, which this target does not touch. Phase 6 made the Settings popup scroll and keep its selected row visible, so a long refusal string rendered by `cleanup_refusal` wraps into continuation lines that each already carry their row's identity and need no positioning work here. Row identity is now `SettingsRowIdentity::Selectable(SettingsRowPayload)` (`crates/tui_pane/src/settings_store/row.rs`), and a selectable payload must equal that row's zero-based position among selectable rows — `push_value` (`crates/cargo-tile/src/settings.rs:385`) assigns it, so let the builder number the row rather than choosing an id.

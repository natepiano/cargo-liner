...
- `crates/cargo-tile/src/settings.rs` — `capture_diagnostic` (`:625`); also the Settings consumer of the startup-notice state (`rows`, `:153`)
- `crates/cargo-tile/src/cargo-capture-shim.sh` — framing `v3`, version line in the header
- `crates/cargo-tile/src/hook.rs` — `Hook::install` (`:630`) version comparison and outcome
- `crates/cargo-tile/src/cli.rs` — child protocol encoding and ordinary install result rendering
- `crates/cargo-tile/src/capture.rs` — `stand_up` (`:23`) handles the downgrade refusal as its own startup note, not as a failure; the existing failure collection it must not use is at `:81`
...
- `impl` — `crates/cargo-tile/src/registration.rs`, `crates/cargo-tile/src/progress.rs`, `crates/cargo-tile/src/processes.rs`, `crates/cargo-tile/src/settings.rs`, `crates/cargo-tile/src/constants.rs`; hubs: `crates/cargo-tile/src/registration.rs` (the error type both sides name) and `crates/cargo-tile/src/settings.rs`, which holds **both** Settings consumers — the unsupported-version diagnostic and the kept-newer-shim notice
- `review` — opens as impl: `crates/cargo-tile/src/cargo-capture-shim.sh`, `crates/cargo-tile/src/hook.rs`, `crates/cargo-tile/src/cli.rs`, `crates/cargo-tile/src/capture.rs`, `crates/cargo-tile/src/app.rs`; owns the downgrade outcome through child encoding, decoding, account reduction, and startup handling, and supplies the agreed startup-notice type where `app.rs` declares it plus its production in `capture::stand_up`; hub: `crates/cargo-tile/src/hook.rs`. It does not edit `settings.rs` — rendering that notice is `impl`'s — so the two settle the notice type's cases on the board before either writes against it
...
`crates/cargo-tile/src/capture_root.rs` (1,224 lines before its inline-test module, 26 top-level types after Phase 7) holds three clusters — directory inspection (`InspectedDirectory`, `Inventory`, `ScanEntry`, and Phase 7's `AclWriteAccess` with the native Darwin ACL inspector), root identity (`RootHistory`, `TreeIdentity`, `RootIncarnation`), and sweep policy (`SweepBudget`, `SweepDisposition`, `OwnedRoot`) — under a module name that no type carries. The anchor type is `RootScan`, not `CaptureRoot`.

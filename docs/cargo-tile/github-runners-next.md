...
- `crates/cargo-tile/src/registration.rs`, `crates/cargo-tile/src/birth_stamp/`, `crates/cargo-tile/src/processes.rs` — only if registration, identity verification, or attribution is the failing stage
...
**Seats:** 1 writer + 1 tester + reserve — the fix lands in one stage; the scenario and fixture are separate; the third seat stays the cold read.
- `impl` — the one source file the reproduction names, with that file as the hub; `progress.rs` is the placeholder, not the answer

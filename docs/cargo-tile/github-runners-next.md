...
### Phase 11 — A cargo row reports the processor time its invocation actually causes  · status: todo

#### Work Order

**Goal:** A live cargo row carries a numeric CPU percentage for the whole time it is compiling, including when a compiler cache performs the compile outside the invocation's process tree, and a group lead counts each contributing process once.

**Spec:**
Three defects, all in the same aggregation.

**Compile work performed by a compiler-cache server is attributed to nobody.** With `RUSTC_WRAPPER` set to `sccache`, a cargo invocation's direct children are thin `sccache` client processes. The compile itself runs as a `rustc` child of the long-lived `sccache` **server**, which is a child of init and is not a descendant of any cargo invocation. Observed during `cargo build -p cargo-tile` on this tree: the invocation (`cargo-tile-real`, pid 1515795) held roughly twenty-five direct `sccache` children, each under two seconds old at `0.0` CPU, while the one `rustc` at `100` CPU named the sccache server (pid 354655, a child of pid 1) as its parent. `Census::attribute_cpu` (`crates/cargo-tile/src/processes.rs:1475`) walks `owning_cargo`, so none of that work reaches the row. The compiler **count** column is unaffected and already reports `sccache×1`, because `attribute_compilers` (`:1399`) counts the client processes that are descendants.

The same build on Darwin splits the work instead of moving all of it out. Sampled during `cargo build` on `~/rust/cargo-liner` with the current binary installed: `rustc` pids 38790 (`22.6`) and 38929 (`16.2`) were children of `sccache` clients that were themselves children of the invocation (pid 38298), while pids 38723, 38755, 38760, 38777 and 39107 (`79`–`93`) were children of the server (pid 15856, a child of pid 1). A row therefore publishes a number on Darwin and understates it by whatever share the server took, and publishes nothing on Linux when no compile lands inside the tree. One defect, two appearances — neither platform may be treated as the working control.

**A descendant that has not accumulated measurable time blanks the whole row.** `Measurement<T>` (`:221`) is `Reading(T)` or `Unavailable(MeasurementAbsence)`, and `MeasurementAbsence` (`:230`) is `FirstObservation`, `ReadFailed`, or `Unproven`. `impl<T: Add<Output = T>> Add for Measurement<T>` (`:258`) returns `Unavailable(reason)` when either side is unavailable, and `attribute_cpu` adds every descendant's share into the invocation's total, so one unavailable descendant makes the total unavailable. `Census::take` (`:1299`) measures every process in the scan, and `Census::measure_cpu` (`:1348`) returns `Unavailable(MeasurementAbsence::FirstObservation)` for a pid with no previous baseline carrying the same lifetime and `Unavailable(MeasurementAbsence::Unproven)` when the previous accumulated counter is zero. A cache client lives less than one scan interval and accumulates no measurable time, so it satisfies both — and there is always one present.

Measured against this tree with a terminal-emulator probe rendering the live grid:

| Invocation | Descendants | Row CPU |
| --- | --- | --- |
| build whose build script spins for 200 s | one, long-lived, busy | `98%`, `100%`, `99%`, `100%`, `100%` at 16–154 s |
| build whose build script sleeps for 200 s | one, long-lived, idle | `--` at 16, 34, 64 and 94 s |
| `cargo build -p tui_pane`, cache active | 5–8 `sccache` clients, turning over | `--` at 18 s |
| `cargo build -p cargo-tile --release -j 4`, `build.rustc-wrapper=""` | 3–4 `rustc`, seconds each | `397%` at 19 s, `403%` at 34 s |
| `cargo port`, 17 h old | stable | `3%`–`5%` throughout |

Rows one and two differ in one variable and establish that measurement and attribution both work for a long-lived descendant. Rows three and four differ in one variable — whether the compiler cache is in use — and isolate the reported defect.

Target: an invocation's CPU comes from processor time accumulated across the invocation between scans, rather than from a sum of per-process instantaneous rates. A descendant that appears between scans contributes the time it has used; one that exits contributes the time its parent inherited when it reaped it. `Unavailable` then states that the invocation's own total could not be read, never that a descendant is new or idle. On Linux the accumulated total including reaped children is `utime + stime + cutime + cstime` from `/proc/<pid>/stat`; `sysinfo`'s `Process::accumulated_cpu_time` reports a process's own time alone, so the children's half needs its own read. Darwin exposes no equivalent for another process's reaped children — retain each descendant's last observed accumulated total and keep it in the invocation's running sum once that descendant disappears. `measure_cpu`'s evidence rules for the invocation's own counter stay as they are: a zero previous counter, a decrease within one lifetime, and an unchanged counter under a claimed positive rate each remain unproven, because a failed Darwin task-info read also reports zero.

Attributing a cache server's work is this phase's own design work, and the parent chain cannot answer it. A `rustc` under the server names its output directory on its command line, and that path sits inside the requesting invocation's target directory, which is the link to follow; `command_text` and `cargo_split` (`:2740`) already read process command lines in this module. Attribute each such compile to exactly one invocation, charge it to no invocation when the owner cannot be established rather than spreading it, and leave a `rustc` that is already a cargo descendant counted once.

`CpuSmoothing` (`:1181`) and its `settle` (`:1203`), `CpuBaseline` (`:1142`), `CpuPublication` (`:1160`) and `cpu_label` (`:2654`) keep their roles: smoothing and the reporting interval operate on whatever the invocation-level measurement produces.

**A group lead charges a shared process twice.** `aggregate_cpu` (`:2424`) folds each member's pid bucket, so when two invocation identities resolve to one pid that bucket is counted twice. `tests/summary_totals.rs` holds member readings of 128, 1, 2, 4, 16 and 32 — 183% when each pid is charged once — and its shared-pid case asserts the lead at `187%` (`:525`). Phase 5 corrected the promoted summary row through `subtree_cpu` (`:2439`) and left the command-row lead alone, because its contract was to leave command rows unchanged. Charge each contributing pid once in command rows and in unpromoted summary rows, and keep a lead unavailable when a member's contribution genuinely cannot be established.

Both existing render cases stay green under their current names: `unavailable_cpu_never_renders_as_a_measurement` (`render.rs:2407`) and `measured_cpu_including_zero_remains_visible` (`:2433`). A measured zero remains a `0%` reading.

**Files:**
- `crates/cargo-tile/src/processes.rs` — hub: `Census::take`, `measure_cpu`, `attribute_cpu`, `attribute_compilers`, `CpuBaseline`, `CpuSmoothing`, `aggregate_cpu`, `subtree_cpu`, `Census::groups`, and the command-line reading already in `cargo_split`
- `crates/cargo-tile/src/constants.rs` — only if the cache-server attribution needs a name or path constant
- `crates/cargo-tile/src/render.rs` — only if the CPU cell needs a change; the display already separates a reading from an absence
- `crates/cargo-tile/tests/summary_totals.rs` — the shared-pid lead, in a command row and in an unpromoted summary row
- `crates/cargo-tile/tests/shim_registration.rs` — a captured invocation whose descendants turn over, and one whose compile runs under a wrapper server outside its process tree

**Seats:** 1 writer + 1 tester + reserve — the measurement change is one source file, and the two test targets are independent of it.
- `impl` — `crates/cargo-tile/src/processes.rs` (hub), plus `constants.rs` and `render.rs` if they are needed
- `test` — `crates/cargo-tile/tests/summary_totals.rs`, `crates/cargo-tile/tests/shim_registration.rs`
- `review` — reserve

**Constraints from prior phases:** Phase 5 added subtree totals to `Census::groups` and made a promoted summary row unavailable when a member's contribution is unestablished; that rule is correct and survives. Phase 1 replaced the harness's nested readiness wait: a scenario that waits for a nested fixture calls `wait_for_fixture_pane(markers)` (`shim_registration.rs:508`), which polls until exactly one command pane holds every marker and returns that screen, and asserts rows on the screen it returns; `fixture_pane` (`:503`) is for a screen already known ready; no test scrubs `CARGO_TILE_ROOT`. Phase 8 bumped the shim framing to `v3`. Every harness carrying a `#[path]` block carries a full copy of it — `shim_registration.rs:3`, `summary_totals.rs:3`, and `capture_root_acl.rs` — and all three name `progress.rs` and `processes.rs`.

**Acceptance gate:** `bash ~/.claude/scripts/delegate/verify.sh check cargo-tile`, `bash ~/.claude/scripts/delegate/verify.sh test cargo-tile`, `bash ~/.claude/scripts/delegate/verify.sh lint cargo-tile` green; `bash ~/.claude/scripts/delegate/verify.sh test cargo-tile summary_totals` green with a shared pid charged once in a command row and in an unpromoted summary row; `bash ~/.claude/scripts/delegate/verify.sh test cargo-tile shim_registration` green with a case where a captured invocation's descendants turn over across scans and the row publishes a reading on every scan after the first, and a case where the work runs in a process outside the invocation's tree and is charged to it. Smoke (orchestrator): the live grid shows a numeric percentage on a cargo row throughout a multi-crate build with the compiler cache active, which is the configuration that fails today.
...
Phase 7 shipped the macOS ACL ownership check inside `capture_root.rs`, and this move preserves every part of it:

- `AclWriteAccess` — `NoNonOwnerWriteGrant` on every platform, plus macOS-only `NonOwnerWriteGrant` and `InspectionFailed(CaptureFailure)`. It states its evidence without a bare `Option`; do not reduce it to one.
- `InspectedDirectory` carries an `acl_write_access` field and `InspectedDirectoryMetadata::refusals` takes an `&AclWriteAccess` parameter. Both signatures survive the split.
- Inspection happens when a directory is opened **and again on every revalidation**, so a grant added after open is caught before any unlink.
- `revalidate_paths` returns `Result<(), CleanupRefusal>`. The private `revalidate` wrapper is gone and both unlink sites consume the typed refusal directly.
- `CleanupRefusal::AclWritableByOthers(PathBuf)` is macOS-only, and `cleanup_refusal` in `settings.rs` is the only exhaustive match on that type — a second exhaustive match must gate the arm or it compiles on Linux and fails on macOS alone.
- Ownership stays available when inspection fails: the refusal names the directory and the original cause rather than discarding the owner and file list.
- The `CAPTURE_ACL_*` constants stay in `constants.rs`, including the two `cfg(all(test, target_os = "macos"))` fixture arrays, and both test-only adapters keep their gates: `RootScan::fail_acl_inspection_for_test` and `settings::cleanup_refusal_for_test`.
- The native inspector's bindings and its narrowly scoped `allow(unsafe_code)` belong to the directory-inspection submodule, never the module root.

This phase owns preserving both user-actionable ACL outcomes. A non-owner write grant reaches the owning reader's Settings as a path-qualified cleanup refusal; an inspection failure reaches it through `Access(PathFailure)` carrying the original diagnostic, and repairing the named directory lets a later scan retry. The cases that establish that — `acl_refusal_rendering_names_each_directory_and_write_reason`, the three `every_non_owner_write_permission_on_*` cases, and `acl_inspection_failure_names_each_directory_and_preserves_owner_and_files` — and the permitting cases beside them (`directories_without_acls_still_sweep`, `directories_with_explicitly_emptied_acls_still_sweep`, both read-only-grant cases, `owner_write_acl_on_each_directory_still_sweeps`, and `non_owner_deny_entry_is_not_a_write_grant`) all live in `tests/capture_root_acl.rs` and keep their names through the move.

**Acceptance gate:** `bash ~/.claude/scripts/delegate/verify.sh check cargo-tile`, `bash ~/.claude/scripts/delegate/verify.sh test cargo-tile`, `bash ~/.claude/scripts/delegate/verify.sh lint cargo-tile` green with the package tests unchanged; every file under `crates/cargo-tile/src/root_scan/` and under `crates/cargo-tile/src/birth_stamp/` is under the style guide's line threshold or holds one cluster. **A green Linux gate says nothing about the ACL target.** `tests/capture_root_acl.rs` carries a whole-file `#![cfg(target_os = "macos")]` at `:3`, so on Linux the target compiles to nothing and runs nothing — neither its adapters, nor its imports, nor any native body is checked there. Only the Mac establishes that the moved module still compiles for Darwin and that the fifteen cases still pass; Phase 16's native preflight owns that execution, and until it runs this lane's ACL evidence is carried forward, not proven. Deferred to the run's administrative smoke batch, not a gate a delegate can pass: `verify.sh check cargo-tile` and `verify.sh test cargo-tile` green on the Mac, which needs the three pre-existing `shim_registration` failures recorded in Delegation Context resolved first — they are a recorded next item that no phase owns.
...
- `impl` — every file in the corrected relocation map, all three harness include blocks included; hubs: `crates/cargo-tile/src/progress/mod.rs` and `crates/cargo-tile/src/root_scan/mod.rs` where it is edited
...
**Constraints from prior phases:** Phase 13 renamed `capture_root` to `root_scan`; Phase 5 added subtree totals to `Census::groups`; Phase 8 added the unsupported-version diagnostic path in `registered_runs`. All of it moves as-is. Phase 7's ACL refusal reaches Settings through this module's status production, so a refusal a scan observes still arrives with its directory and its reason after the relocation; `AclWriteAccess` and `CleanupRefusal::AclWritableByOthers` are not this phase's to reshape, and the fifteen cases in `tests/capture_root_acl.rs` keep their names. That target is whole-file `#![cfg(target_os = "macos")]`, so a green Linux gate compiles none of it — its evidence is carried forward to Phase 16's native preflight, not established here. Every harness carrying a `#[path]` block carries a full copy of it — `shim_registration.rs:3`, `summary_totals.rs:3`, and `capture_root_acl.rs` — and all three name `progress.rs` and `processes.rs`.
...
- `test` — opens as test: the split itself adds no behavior, but the named-outcome work does. Writes the cases the gate names — the two ancestry outcomes, and the three classification outcomes at the raw-argv boundary — and confirms the package compiles against the new paths and that every inline test travelled with its type
- `review` — reserve

**Constraints from prior phases:** Phase 14 moved types out of `processes.rs` by ownership and split `progress.rs`, and **its relocation map decides this phase's clusters** — read the map rather than assuming, `CaptureDiagnostic` included, which may no longer sit with capture attribution once ownership is applied. Phase 5's subtree totals live in `Census::groups`; Phase 8's `CaptureDiagnostic` variant joined the attribution cluster where the map leaves it. Phase 7's ACL refusal travels through this module's scan transport, so a refusal still reaches Settings with its directory and reason after the split; `tests/capture_root_acl.rs` is one of the three harnesses whose include block must follow, and being whole-file macOS-gated it runs none of its fifteen cases under a Linux gate — Phase 16's native preflight is where that evidence is established. Before dispatch the writer confirms the consumer list against the tree — `grep -rln 'processes::' crates/cargo-tile/src crates/cargo-tile/tests`, plus the `mod` declaration in `main.rs` — because Phases 5 and 8 each add files to it.

Phase 5's subtree work is more than "totals live in `Census::groups`", and the split must carry all of it together. `Census::groups` (`:2012`) builds a `process_rows` set of **invocation identities** at `:2031`; both the subtree seed at `:2038` and the per-row CPU overwrite at `:2053` are gated on that set, because two distinct invocation identities can share one pid and charging a pid bucket per member would count it twice. `subtree_cpu` (`:2439`) is a private free function taking `&HashSet<InvocationId>`, with exactly one call site, and aggregates raw buckets before any formatting — it stays beside that call site. Registration rows are injected **after** the `process_rows` snapshot, which is what makes an injected row unproven. The `cfg(test)` field `Census.registration_rows` and the two adapters `groups_with_cpu_for_test` (`:2931`) and `groups_with_registration_rows_for_test` (`:2941`) travel with the `Census` cluster; they are implementation-only, giving a test binary access to private assembly without widening any production visibility. The three regressions in `tests/summary_totals.rs` — a shared pid, a registration-only member, and an unproven group lead — must still pass under the package gate after the split. `summary_rows` in `render.rs` remains the only consumer of the subtree total, drawing a number when every contribution is established and the unavailable marker when one is not.

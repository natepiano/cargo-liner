...
- **Key files:** (line refs verified against HEAD `3fb313b0`; the `cargo-tile` `cli.rs`, `hook.rs`, and `tests/support/shared_capture.rs` refs re-verified against the tree Phase 2 left; drift noted inline)
  ...
  - `crates/cargo-tile/src/hook.rs` — shim installation, account discovery, credential switching. `AccountInstallOutcome` `:85`, `AccountInstallReport` `:159`, `Hook::install` `:409`, `Hook::remove` `:462`, `account_groups` `:612`, `install_accounts` `:673`, `install_account` `:689`, `ToolchainInstallOutcome` `:96`, `ToolchainInstallReport` `:111`, `impl TryFrom<&str> for ToolchainInstallReport` `:118`, `AccountInstallReport::from_output` `:179`, report `Display` implementations `:143` and `:229`, `is_shim` `:765`. Also holds `SHIM_SOURCE`, `SHIM_MARKER`, `system_accounts`, `stage_installer`, and three `allow(unsafe_code)` FFI items at `:562`, `:609`, `:721`.
...
**Constraints from prior phases:** Phase 2 added `ToolchainInstallOutcome` and `ToolchainInstallReport`; `AccountInstallReport` now retains `toolchains: Vec<ToolchainInstallReport>` beside its `AccountInstallOutcome`, and `install_account` returns the complete report. `AccountInstallReport::from_output(account, &Output)` and `TryFrom<&str> for ToolchainInstallReport` replace `impl From<Output> for AccountInstallOutcome`. `account_groups` is behaviorally unchanged.
...
**Spec:**
Today `install_all_accounts()` (`crates/cargo-tile/src/cli.rs:182`) re-executes the binary per account through `install_accounts` / `install_account` (`crates/cargo-tile/src/hook.rs:673`, `:689`), which passes `--{ACCOUNT_INSTALL_REPORT_FLAG}` (`:705`) to the child; the protocol is hard-coded to install. Its remaining silent paths are in toolchain discovery (`Hook::in_rustup_home`, `hook.rs:358`, discards directory-entry errors) and `is_shim` (`hook.rs:765`, folds an open or read failure into "not installed"); `AccountInstallReport::from_output` (`hook.rs:179`) already retains valid toolchain rows despite malformed lines or an unsuccessful child exit and includes the failures in the account summary. `Hook::remove` (`:462`) cannot restore an orphaned toolchain that has no saved `cargo`.
...
**Seats:** 1 writer + 1 tester + reserve.
...
- `crates/cargo-tile/src/hook.rs` — `Hook::install` (`:409`) version comparison and outcome

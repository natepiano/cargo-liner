...
### Phase 11 — README, changelog, and publish readiness  · status: done
...
### Phase 12 — Config and init in hana  · status: done
...
These shims compile nothing, but they parse concrete exit codes and payload tags, so the engine checkout phase 12 recorded is a read-only input to this phase and its exact files are named rather than left to be searched for:
...
- `crates/cargo-berth/src/exit.rs` — read only, in the engine checkout; the eight-code table itself, which is where a code's meaning is established rather than in `cli.rs`.
- `crates/cargo-berth/src/verb/board.rs` — read only, in the engine checkout; the proof that exit `7` belongs to the terminal view alone and that `board --json` cannot return it, which is what lets this phase's gate name `7` unreachable instead of synthesizing it.
...
- `crates/cargo-berth/src/output.rs`, `crates/cargo-berth/src/board/mod.rs`, `crates/cargo-berth/tests/board.rs` — read only, in the engine checkout phase 12 recorded; the frozen `board --json` contract `/sync board` renders.
- `crates/cargo-berth/README.md` — read only; phase 11's board section, the prose statement of that same contract.

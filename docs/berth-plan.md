...
### Phase 18 — The /sync skill  · status: done

#### As-built

`/sync` wraps the installed engine for a person: `board`, `claim`, `release`,
`sequence`, `integrate`, and a `check` that runs `drift`'s full phase-start
comparison rather than the `check` verb's proposed-path question. `/sync board`
invokes `board --json` and renders the payload itself, because a redirected bare
`board` prints only a pointer line.

One shared Work Order parser and validator serves `/sync` and all three plan
commands. It validates the complete **Goal**/**Spec**/**Files**/**Reservations**
structure — not the reservations block alone — and returns tagged
`ReservationDeclaration::{Declared, Missing}` and
`ReservationCoverageMode::{Advisory, Required}` results. Path validation is
lexical, never filesystem existence: a path that does not exist yet is valid, and
scope sets reduce to a minimal antichain. It also answers a pairwise collision
question offline, with no ledger and no engine call, reproducing the engine's
component-ancestry overlap, `file:`/`tree:` asymmetry, and case folding.

A claim coordinator owns the Blocked → Proposal awaiting approval → Claimed
sequence across both an interactive claim and a dispatch boundary. It parses the
engine envelope defensively — type-checking every frozen field, requiring the
process status to agree with the envelope's `exit_code`, and rejecting an unknown
status or payload-kind tag — rather than reading a rendered message. It appends
the plan document and its derived `-next.md` sibling as exact exclusive `file:`
scopes at resolution (R33), treats `unconfigured` as terminal for every command,
and treats exit `6` as an exhausted outcome with no shim-side retry.

`integrate` returns a typed inactive-identity rejection:
`IntegrationPayload::Rejected { reason: IntegrationRejectionKind }` with
`InactiveSessionMapping` and `InactiveMarkerRun`, each carrying the stale
`CoordinationRunId`. The envelope keeps `invalid_input` / exit `5` and names the
requested reservation in `reservations`. The two cases previously collapsed into
an untyped `invalid_input` with a `no_facts` payload, which made a distinction the
skill must draw unrenderable.

**Files:**

- `crates/cargo-berth/src/output.rs` — the rejection payload, its kind enum, and
  the `integration_rejected` constructor.
- `crates/cargo-berth/src/verb/integrate.rs` — `gate_error` maps the two
  inactive-identity errors to that rejection; a regression test asserts the
  serialized shape.
- `/Users/natemccoy/rust/hana/.claude/commands/sync.md` — the skill.
- `~/.claude/scripts/berth/work_order.py` — the shared parser and validator.
- `~/.claude/scripts/berth/claim_state.py` — the claim coordinator.
- `~/.claude/commands/plan/{to_phased_plan,phase_review,delegate}.md` — each calls
  the shared validator instead of parsing Markdown itself.

**Binds later work:**

- The typed integrate rejection is classified on `reason.kind`, never on
  `message`. Its end-to-end surface, and the proof that the two kinds render as
  distinct recoveries, belong to "End-to-end proof, then enforce".
- The tagged declaration and coverage results are the dispatcher's coverage
  contract: "/plan:delegate integration" replaces its config-derived selection
  with one named rollout policy set to `advisory`, and "End-to-end proof, then
  enforce" flips that same policy to `required`.
- R33 plan-document scopes are appended at claim resolution, not written into
  declared blocks, so a pairwise declaration comparison correctly excludes them.
  Their ledger-visible effect is owned by "/plan:delegate integration"'s
  claimed-footprint assertion and the final phase's hook proof.
- The installed executable is phase 17's build and predates this phase's engine
  change, so every later engine-invoking phase installs from
  `/Users/natemccoy/rust/cargo-berth-init` and records the digest it gets.

**Gotchas:**

- `~/.claude/scripts/berth/` is a namespace package with no `__init__.py`. The
  invocation is `PYTHONPATH="$HOME/.claude/scripts" python3 -m berth.work_order`
  (likewise `berth.claim_state`); direct execution works only through an explicit
  `sys.path`/`__package__` bootstrap.
- The coordinator's status-to-payload-kind and status-to-exit-code tables mirror
  the engine's pairings by hand, so an engine that adds a status makes the front
  end reject valid replies until both tables are updated.
- Identity validation runs before the entering-reservation calculation
  (`gate/mod.rs:553-557`, versus `ReservationNotEntering` at line 796). A
  stale-identity fixture must satisfy the entering condition as well, or it is
  refused for the unrelated reason.
- The skill and both Python artifacts live outside every repository, appear in no
  `git status`, and enter no checkpoint.

**Ruled out:**

- Distinguishing the two inactive-identity cases in the front end by matching the
  rendered message — the engine now carries the distinction structurally.
- Adding a second missing-declaration parser in the dispatcher — the shared
  validator already answers it.

### Phase 19 — /plan:delegate integration  · status: done

#### As-built

`/plan:delegate` coordinates each delegated phase with the `cargo-berth` engine
across a durable boundary rather than in conversation.

- `<CoordinateDelegatedPhaseReservation/>` is the only pre-dispatch coordination
  boundary. It resolves to exactly one of four persisted
  `DelegatedPhaseReservationState` values before any dispatch:
  `RepositoryNotEnrolled`, `ReservationDeclarationMissingUnderAdvisory`,
  `Active(ActivePhaseReservation)`, and
  `CheckpointCommittedAwaitingReleaseConfirmation(CheckpointReleaseConfirmationPending)`.
  The record lives at `${SESSION_DIR}/delegated_phase_reservation_state.json`,
  is written atomically, and is read back before it is trusted. None of the four
  is representable as an absent record or a bare optional.
- `<ComposeWorkOrder/>` takes `reservation_declaration.kind` only from the shared
  validator's tagged result. It never searches the Markdown for a Reservations
  heading and never infers enrollment from a configuration path.
- `RESERVATION_COVERAGE_POLICY` is a named workflow constant, currently
  `advisory`. Repository configuration never selects it; the rollout is one
  value, not a property of each checkout.
- `<CheckpointCommit/>` step 5 runs drift's full phase-start comparison, bound to
  the record's `reservation_id` and `phase_start_head`. Identity ambiguity, an
  incursion, a collision, an attribution requirement, or a malformed response
  blocks the checkpoint.
- `<CheckpointCommit/>` step 6 commits, then atomically replaces `Active` with
  `CheckpointCommittedAwaitingReleaseConfirmation` carrying that commit as
  `checkpoint_commit`, reading the new record back. That durable transition
  succeeds before release is invoked.
- `<CheckpointCommit/>` step 7 releases exactly once, reading both the
  reservation id and `checkpoint_commit` from the record; an `Active` record at
  this step means the transition did not complete and refuses release. When a
  release reply is lost after its `Checkpoint` was journalled, recovery
  re-invokes release once, reads the validated board, classifies the reservation
  as `ProtectedTipObserved`, `ReservationPresentWithoutProtectedTip`,
  `ReservationAbsent`, or `BoardInconsistent`, and confirms the checkpoint only
  on an observed protected tip equal to the read-back `checkpoint_commit`.
- `<RetainDelegatedPhaseReservation/>` retains both the engine reservation and
  the durable record on every unsuccessful ending. Release is never invoked from
  an error handler, a cancellation path, single-mode completion, a run summary,
  or session cleanup.
- `<CompactionContract/>` and `<RecordPhaseCompletion/>` both know the fourth
  state: the record is re-read after compaction rather than remembered, and a
  phase whose `Active` or pending state survives is not complete.
- A stranded phase is user-actionable: the retained record plus the exact board
  or full-drift command the workflow names is what a person acts on.

**Files:**

- `~/.claude/commands/plan/delegate.md` — holds
  `<DelegatedPhaseReservationContract/>`,
  `<RetainDelegatedPhaseReservation/>`, `<CoordinateDelegatedPhaseReservation/>`,
  the `RESERVATION_COVERAGE_POLICY` and `WORK_ORDER_RESERVATION_DECLARATION`
...
- `release` re-run after a journalled checkpoint can never return `checkpointed`:
  `operation_for_state` maps `Active -> Checkpointed` and
  `Outstanding -> Resnapshotted | EvidenceRevalidated`. Recovery accepts
  `resnapshotted` or `evidence_revalidated` and proves the checkpoint from the
  board instead.
- `validate_release_transaction` rejects before the journal append, so an
  ordinary failed release appended nothing and is not a recovery case.
- The board deliberately omits a lifecycle from two positions
  (`board/mod.rs:618-640`): a waiting `successor`, and either endpoint of an
  unresolved overlap. A reservation there has no lifecycle-bearing row.
- A retention ref proves the protected tip and never the stage. A `Released`
  reservation reaches that same constraint-only position with its ref intact —
  `release.rs:85` returns `EvidenceRevalidated` before dispatch regardless of
  lifecycle, `apply_evidence` preserves `Released`, `reconcile.rs:678` keeps the
  ref, and `board/mod.rs:795` marks the row `ReblockedActiveConstraint`.
- An `orphaned_outstanding` alert is the one exception:
  `for_orphaned_outstanding` returns empty unless the lifecycle is
  `Outstanding { protected_tip }` and copies that same tip, so the alert
  independently proves both stage and tip even where the row is omitted.
- No read-only engine output can select one named reservation and expose its
  lifecycle — `board` enumerates but takes no id, `check`'s payload carries no
  lifecycle, and `drift --reservation` is not read-only — so
  `ReservationPresentWithoutProtectedTip` can only ever retain.
- Recovery must never re-derive the checkpoint commit from current `HEAD`: a
  later commit in the same worktree silently supplies a wrong answer the
  comparison would accept.
- `~/.claude/scripts/berth/` is a namespace package with no `__init__.py`;
  invoke it as `PYTHONPATH="$HOME/.claude/scripts" python3 -m berth.work_order`
  or `berth.claim_state`.

**Ruled out:**

- Deriving reservation coverage from `.claude/config/berth.toml` presence — it
  makes the rollout a property of each repository instead of one switch.
- Demanding release payload status `checkpointed` on the recovery path — the
  engine cannot produce it after the checkpoint is journalled.
- Consulting the retention ref to establish the outstanding stage — it proves
  only the protected tip, and stays in the text as an explicit non-proof.
- Requiring a single board row with `lifecycle.stage = outstanding` — two board
  positions carry no lifecycle at all.
- Adding `checkpoint_commit` as a field on `Active` — the tag would keep naming a
  stage the engine no longer reports once the checkpoint is journalled.
...
### Phase 21 — Deciding when two worktrees meet  · status: done

#### As-built

A blocked edit renders as a screen offering exactly five answers, and the person picks one. `claim_state.py` classifies the blocked-check envelope into `RefusalPresentationValue`, whose `answers` is a list of `RefusalAnswerValue` — a tagged union of `ReasonedClaimRefusalAnswerValue` (`selection` one of `before`, `after`, `defer`, `override`, carrying `label`, `adds_ordering_edge`, and `consequence`) and `LeaveAloneRefusalAnswerValue`, which appends nothing. Every answer offers one coordinator invocation to run; no engine flag is named anywhere in either rendering, and `CARGO_BERTH_BYPASS=1` is never offered, because no edit path reads it. Only `before` and `after` add an ordering edge; `defer` and `override` add none and stay visible on the board, which each answer's `consequence` states.

`RefusalScopeFactsValue` is a second tagged union recording where the paths came from: `ExactRequestedScopeFactsValue` carries `requested_scopes` directly, and `HolderSharedScopeFactsValue` carries nothing, because the paths are readable only through each holder's `overlapping_scopes`. Consumers read `scope_facts.kind`, never the presence of a sibling key.

The answered invocation returns `ProposalAwaitingApprovalStateValue` at exit `3`: holder facts, the selected answer, its reason, its consequence, the proposal, and the token — and no answer menu, since the choice is already made. Approval is a separate invocation; a token is never minted and spent in one turn. Two envelope rejections enforce the state's internal agreement: a proposal must display exactly one conflict whose reservation id equals its answer's blocker, and its consequence must equal the one its selected answer implies.

A holder claimed on first touch renders explicitly as a first-touch claim with no plan and no phase, never as an explicit claim and never as an empty field. A granted claim whose `session_mapping_publication.status` is `unavailable` reports as nonblocking degraded success: the diagnostic shows, the reservation id is named from then on, and the flow continues. `berth_pre_edit.sh` accepts a clear check carrying one reservation plus an `acquisition` record for each of `appended`, `widened`, and `already_held`, and accepts `first_touch` as a holder source.

**Files:**
- `~/.claude/scripts/berth/claim_state.py` — the classifier producing both tagged unions, the refusal presentation, and the proposal state.
- `~/.claude/scripts/berth/install/commands/sync.md` — the `/sync` refusal and proposal rendering.
- `~/.claude/scripts/berth/install/hooks/berth_pre_edit.sh` — the inline mid-edit refusal feedback.
- `~/.claude/scripts/berth/install/` — the canonical home for every repository-installable file (`commands/sync.md`, the three `hooks/*.sh`), with a `README.md` stating the install and removal procedure. Registering these absolute paths is what keeps a proving repository's worktree clean.

**Binds later work:** The end-to-end proof reads `state.answers[].kind` to distinguish a reasoned claim from leaving the block alone, and `state.scope_facts.kind` to know whether requested scopes are carried directly or only through each holder. The exit-`3` screen omits the answer menu; the exit-`1` refusal carries all five. The engine's answer-to-consequence authority is `crates/cargo-berth/src/answer/proposal.rs:196`.

**Gotchas:** `berth_pre_edit.sh` validates the engine envelope, not the coordinator's `state` presentation, so restructuring the refusal types touches no shim assertion — the two can drift apart silently. `claim_state.py` lives outside every repository and has been uncommitted since `831e34a`, so no diff of it can be scoped to a single phase; any review of it needs an explicit scope note.

**Ruled out:** Naming the engine claim flag beside each answer as "the mechanism" — nothing read it, and a second command register beside the one to type is what this phase removed. Carrying the full five-answer menu into the post-choice proposal state — no consumer, and it puts a menu on a screen where the choice is made. Installing the shims into a proving repository for the duration of a run and deleting them afterwards — it leaves the hook registration naming deleted paths.
...
5. An `--after` claim **holds** — the refusal-time operation is `claim --after`; the separate `sequence` verb changes an answer that already exists and is not what this scenario exercises. Start from the neutral exit `1` block, take the user's `--after` answer and reason to obtain exit `3`, apply the exact token only after separate approval, then show both worktrees editing freely while B's merge to `main` is refused with A unmerged.
6. Landing in order **releases the edge from scenario 5** — A merges, and B's hold clears only when A's `protected_tip` is an ancestor of B's `HEAD` (R69). It is `HEAD` ancestry that clears the edge; the rebase is what makes B's `HEAD` contain that tip, and the resnapshot separately updates B's own protected checkpoint. Do not let either stand in for the ancestry check.
...
- `~/.claude/scripts/berth/install/hooks/berth_post_bash.sh` — **editable, and the canonical copy.** Phase 21 moved every file a repository installs into `~/.claude/scripts/berth/install/` (`commands/sync.md` and the three `hooks/*.sh`), because they had no durable home and `hana`'s worktree must stay clean; its `README.md` states the install and removal procedure. **Register these canonical paths directly — do not copy them into `hana`.** A hook command is an absolute path and each shim reads its repository from the invoking working directory, so registering `~/.claude/scripts/berth/install/hooks/...` leaves a runtime installation that still resolves after the scenario run ends, while `hana`'s worktree never gains a file. Copying them in and deleting them afterwards would leave the registration in `settings.local.json` naming three paths that no longer exist. On the shim itself: `valid_widening`, **not** `valid_effect`, is the predicate governing `.payload.data.widening.status`, whose variants are `DriftPathAttributionOutcome`'s at `crates/cargo-berth/src/drift/mod.rs:146` — a different union from the `results[].effects[].kind` values `valid_effect` accepts. Teach `valid_widening` the `first_touch_claimed` and `post_write_incursion` statuses phase 20 added, and render each; extending `valid_effect` would leave both still rejected.
- `~/.claude/scripts/berth/install/commands/sync.md`, `~/.claude/scripts/berth/install/hooks/berth_pre_edit.sh`, `~/.claude/scripts/berth/install/hooks/berth_session_start.sh`, `~/.claude/scripts/berth/install/README.md`, `~/.claude/scripts/berth/claim_state.py` — read only; the renderings, the two shims this phase does not edit, the procedure the `README.md` states, and the classifier the scenarios exercise, as phase 21 left them. All live outside every repository. Every registered worktree must resolve these exact canonical files — confirm each registered command path matches before a scenario begins, because a registration pointing at a copy that has drifted proves nothing about the shipped shim.
...
**Constraints from prior phases:** `hana` currently has no `.claude/commands/sync.md` and no `.claude/hooks/` at all — phase 21 removed them so that worktree stays clean, and the canonical copies now live in `~/.claude/scripts/berth/install/`. **Nothing is copied back into `hana`.** The three hook registrations name the canonical `~/.claude/scripts/berth/install/hooks/*.sh` paths as absolute commands, and `/sync` is made invocable by placing `commands/sync.md` at `~/.claude/commands/sync.md`, where it is available in every repository. That path is one of the protected locations under `~/.claude`: a shell write to it fails with `Operation not permitted`, so write it with the Edit/Write tools rather than reaching for a sandbox override. This leaves a runtime installation that keeps working after the run ends, which a copy-then-delete cycle would not — and it is why the cleanup step below removes no shim files. A scenario run against a checkout whose registered hooks do not fire passes by measuring nothing, so confirm all three fire before running any scenario.

**This phase spans three repositories and only one of them takes a checkpoint.** The engine, the plan, and the next-items file are in `/Users/natemccoy/rust/cargo-berth-init`, and that is the only repository this plan commits to — dispatch from there, not from `hana`, because the Work Order validator resolves every relative path against the plan's own repository root. `hana` contributes `.claude/config/berth.toml` (tracked, and the final `gate_mode` flip is a real commit in that repository) and `.claude/settings.local.json` (untracked and ignored, so it never appears in any commit and never dirties that worktree). The canonical shims, `/sync`, and `claim_state.py` live in the `~/.claude` repository, which is the user's own configuration and is theirs to commit — this phase edits `berth_post_bash.sh` there and must say so in its report rather than committing it. Record the ordered sequence in the transcript: edit the canonical shim, register, run the scenarios, flip `gate_mode` durably and commit that in `hana`, then checkpoint the plan in `cargo-berth-init`. Every prior phase is exercised here. Phase 9's observe-only default is what makes this safe to run against the real repo — do not flip it early, and do not leave it flipped after a failed scenario. Phase 16 established the ledger and the config; phase 17 built the three hook shims but left them unregistered, which is why this phase's first step is registering them. **Phase 20 removed the reservation declaration entirely**, so there is no coverage policy to flip and no Work Order backfill to verify — a phase claims by editing. Phase 21 owns the refusal rendering the scenarios read; do not reimplement it here, and do not accept a scenario that asserts only the exit code where phase 21 promised a rendered choice. Phase 19's coordination boundary is what this phase exercises end to end: a phase persists exactly one tagged `DelegatedPhaseReservationState` — `RepositoryNotEnrolled`, `Active`, or the checkpoint-release state — reads that record back rather than remembering a reservation id, runs drift's full phase-start comparison before the checkpoint commit, and releases only after that commit succeeds. Every unsuccessful ending retains both the engine reservation and the durable record. Do not invoke release from an error, cancellation, or cleanup path. **Phase 20 changed engine source, so no earlier digest is the artifact to select.** Before this phase's first engine invocation, confirm that `/Users/natemccoy/rust/cargo-berth-init`'s `HEAD` contains the phase 20 checkpoint, run `cargo install --path crates/cargo-berth` from that checkout unconditionally, and record the source `HEAD`, the absolute installed path, and the resulting SHA-256. `cargo-berth --version` reports `0.1.0-dev` for every build and proves nothing. The acceptance transcript names the newly recorded digest.
...
**Acceptance gate:** No `verify.sh`. The `hooks` key is present, registers all three shims, and the rest of `settings.local.json` is byte-identical to its prior contents. An unenrolled checkout keeps its silent opt-out unchanged — the coordination boundary answers `unconfigured` first and persists `RepositoryNotEnrolled`, claiming nothing. A session started against a ledger carrying an orphaned-outstanding alert surfaces it. All seven scenarios pass with transcripts recorded — six against two real worktrees, and the ledger-corruption scenario against its own separate scratch repository or clone per the constraint above, never against the only ledger there is, each transcript naming the `gate_mode` in force when it ran. Scenario 1's refusal is read as a rendered screen, not an exit code: it lists exactly five answers — the four `reasoned_claim` selections in the order `before`, `after`, `defer`, `override`, then the single `leave_alone` — with no answer that bypasses the trunk, and each `reasoned_claim` naming whether it adds an ordering edge and what follows from choosing it. Every answer offers one coordinator invocation to run; an engine flag spelled out as the thing to type is a regression phase 21 closed. That refusal also carries `exact_requested_scopes`, so the transcript asserts the requested path itself; scenario 5's neutral claim carries `holder_shared_scopes_only` instead, where the paths are read through each holder's `overlapping_scopes` and no requested-scope list exists to assert. Both tagged variants are exercised, one per scenario. Scenario 2 proves the silence-means-safe invariant by journal replay: a permitted edit leaves a reservation held by the editing run over exactly the edited path, and the next run is refused on it. Scenario 5 records exit `1`, exit `3`, explicit token approval, and the resulting edge before scenario 6 clears that same edge — and records the exit-`3` screen itself, because that screen is what a user reads before approving. It names the sole holder and the exact shared scopes, then the selected answer, the reason given, the consequence that follows, the proposal, and the token; it **omits** the five-answer menu, which belongs to the refusal and not to a choice already made; and it ends by waiting for a separate approval rather than acting. Minting a token and spending it in one step does not satisfy this. The coordinator's two envelope rejections — a proposal whose blocker disagrees with its single displayed conflict, and a consequence that disagrees with its selected answer — stay implementation-only and are not scenarios here: only a self-contradictory engine envelope can produce either, so phase 21's fixture tests own them. The scenario 5, 6, and 7 transcripts each record the reported ordering status at every step, showing scenario 5's hold changing to the incorporate-the-predecessor state once A lands, scenario 6 reaching fulfilled only on `HEAD` ancestry and distinguishing that from the resnapshot, and scenario 7 keeping the bypass annotation separate from the edge's own derived status. The journal used by scenario 3 is either disposable or verified clean afterwards. The scratch-repository scenario runs `SessionStart` twice after a pending bypass marker is drained and asserts the recovered set is reported on the first run, absent on the second, and that the bypass remains in the board's audit history throughout. The failed-scenario rollback to observe mode is tested. `gate_mode = "enforce"`; an out-of-order merge to `main` is then refused in a real terminal, and `CARGO_BERTH_BYPASS=1` still lands it.
...
| `.claude/config/berth.toml` (in hana) | The repo's dialect: `trunk`, `gate_mode`, and R4's two limits. R34 and final D3 withdrew the announce-not-claim rule and D9 replaced declaration with first-touch claiming, so no coverage list remains. Sits beside `mirror.toml` and `release.toml`. |

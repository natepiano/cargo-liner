...
**Goal:** In `cargo-liner`: `release` works, reservations move `Active → Outstanding → Integrated`, and every integration question is answered from retained git evidence rather than a live branch tip.

**Spec:**

**This phase owns `release` end to end** — its behavior, its CLI dispatch, its output status, and its tests. Phase 3 deliberately does not implement it, because a release is meaningless without the protected tip, the retention ref, and the `Outstanding` state defined here.
...
`IntegrationEvidenceStatus` is **revalidated on every stateful check, never trusted as terminal** (R41). A trunk rewrite that removes `protected_tip` returns the reservation to blocking and emits `TrunkRewritten`; a missing object is `ObjectUnknown` and also blocks. `Resnapshot` (R52) records the replacement tip after a rebase.

**Whether a reservation blocks an edit is materialized into the journal, never derived by `check`.** Revalidation is a git computation, and phase 13's pre-edit hook calls `check` on every edit and must make no git call — phase 3b's `Ledger::read_for_edit_check` walks the filesystem for exactly that reason and hands back raw `JournalEvent`s and nothing else. A reservation that returns to blocking after a trunk rewrite is therefore invisible to a git-free reader unless the verb that ran the git computation wrote the conclusion down. Every verb that revalidates evidence appends the resulting edit-blocking state, and `LiveReservationSet::replay` reads it rather than recomputing it. `check` keeps its no-git, no-lock contract unchanged; only writers call git.

**`release` removes the coordination-run marker it is retiring.** Phase 3b's `claim` publishes `cargo-berth-run-id` in the worktree administrative directory and removes only the value its own attempt wrote. The matching removal belongs here, with the release that ends the run's last reservation: remove the marker only when it still names the run being released **and** no other reservation held by that run is still active. A marker naming a different run, or one still backing an active reservation, is left untouched. Phase 5 sweeps markers left behind by a crash; it does not own the ordinary path.

**Add exit `6` — `BerthExit::BlockedByContention`.** `OutputEnvelope::contention` currently maps `LedgerTransactionError::LockContention` onto `BerthExit::LedgerUnreadable`, whose documented meaning is "the ledger cannot be read; edit paths fail open, `integrate` fails closed". A five-second wait on `mutation.lock` is a transient busy state that the caller should retry, not an unreadable ledger, and phases 8 and 13 key their fail-open and fail-closed rules on exit `4`. `release` is the second mutating verb, so the conflation stops being theoretical here. The expansion is additive — codes `0`–`5` keep their exact meanings — so every parser written against the phase-1 surface keeps working. Update the exit-code table in `src/exit.rs`'s module documentation in the same change.

**Files:**

- `crates/cargo-berth/src/reservation/lifecycle.rs` — the four types above, transitions, confirmation gates.
...
- `crates/cargo-berth/src/verb/release.rs` — the verb, new here.
- `crates/cargo-berth/src/reservation/mod.rs` — **exists**; phase 3b's `LiveReservationSet::replay` derives live state and this phase changes what "live" means. Rename `LiveReservation`, which stops being truthful once a reservation can be `Outstanding` or `Released` and still be retained.
- `crates/cargo-berth/src/ledger/journal.rs` — **exists**; the restructured `Release` record and the materialized edit-blocking state below.
- `crates/cargo-berth/src/ledger/projection.rs` — **exists**; carries the materialized state so a rebuilt projection agrees with journal replay.
- `crates/cargo-berth/src/exit.rs` — **exists**; the new retryable exit code below.
- `crates/cargo-berth/src/cli.rs` — **exists**; dispatch `release` into the engine.
- `crates/cargo-berth/src/output.rs` — **exists**; the release and evidence payload variants, and the contention envelope's corrected exit code.
...
**Constraints from prior phases:** Phase 3b implements `claim` and `check` and deliberately leaves `release` to this phase — its CLI dispatch is still the phase-1 `unimplemented` path. Phase 2's transaction wrapper, complete journal union, typed envelope payload, and `src/git/constants.rs` naming convention bind. All reservation records carry the UUID-v7 `ReservationId` phase 2 validated. Phase 2 already ships `ReservationSnapshot` and `ReleaseDisposition` — refine those single definitions in place, never create a second. `JournalOperation::Release` currently pairs a disposition with an unrelated `String`, so a rewritten integration loses the verified trunk oid while an ordinary one is forced to carry a meaningless reason. Restructure it into variants that each carry exactly their own evidence: the verified trunk oid for a rewritten integration, a required reason for an abandonment, and nothing for an ordinary one. Phase 3b shipped `LiveReservationSet::replay`, which is the **only** way live reservation state is derived — it currently drops a reservation on `Release` and treats `Checkpoint` and `Resnapshot` as revision-only. Any record this phase adds or reshapes must be handled there or replay silently ignores it. Phase 3b also shipped `EditCheckLedgerSnapshot`, which exposes raw events and worktree context and nothing else; that is the reader the materialized-state rule above exists to serve.

**Acceptance gate:** `verify.sh test cargo-berth` green, including: a checkpoint writes a retention ref and the commit survives deleting its branch and running `git gc --prune=now`; an `Active` reservation blocks and the type makes its missing protected tip unrepresentable rather than absent; `Integrated` reverts to blocking after `git reset --hard` removes the tip from trunk; a rebase + resnapshot updates the tip and the ref; `release` no longer returns `unimplemented` in either output mode, and its process exit status, envelope `exit_code`, and semantic status agree. Also: every lifecycle state survives a round trip through journal replay with the projection deleted, across `Active → Outstanding → Released` and across a trunk rewrite that returns an integrated reservation to blocking; a `check` run with `git` removed from `PATH` reports that returned-to-blocking reservation as blocking, proving the state was read and not derived; `release` removes the coordination-run marker when it names the released run and no other reservation of that run is active, and leaves it in place in both negative cases; and lock contention returns exit `6` with a retry message while an unreadable ledger still returns exit `4`, asserted end to end through process status, envelope `exit_code`, and semantic status.

...
Edge record: its own `EdgeId`, `before: ReservationId`, `after: ReservationId`, the validated non-empty overlap scope set, reason, and its journal `EventId`. The reason is a role-specific non-empty type — why this ordering was declared — minted at the boundary and never a bare `String`; this is its first use, so its representation is settled here rather than migrated later. **`EdgeId` and `EventId` are not the same thing.** Phase 6 already mints the durable `EdgeId` inside `ConflictAuthorization::Sequence`; replay pairs that embedded id with the claim or widen event id that recorded the authorization. `sequence` resolving a `Defer` mints an edge id in `ResolveDefer`. An edge persists independently of either declaring event: `EdgeId` is what the board renders, duplicate checks compare, and bypass records name, while `EventId` identifies the journal fact that introduced or resolved it.
...
- `crates/cargo-berth/src/ids.rs` — **exists**; phase 6 already made `EdgeId::new` live, and this phase also uses it when `ResolveDefer` creates an edge.
...
- `crates/cargo-berth/src/ledger/journal.rs` — **exists**; remove the unused `DeclareOrderingEdge` variant, expose the narrow `JournalEvent::event_id` accessor needed by replay, and give `ResolveDefer` its role-specific ordering reason.

...
> **Status: BUILT.** `cargo-berth`, a git-worktree reservation engine, exists in the `cargo-liner` workspace and is wired into the `hana` repo's Claude Code environment, with the trunk gate enforcing. This plan lives in `cargo-liner` because phases 1–15 and 20 built here; phases 16–19 and 21–22 ran in `/Users/natemccoy/rust/hana`.
...
### Phase 22 — End-to-end proof, then enforce  · status: done

#### As-built

The whole coordination loop is proven live against the installed binary: a first-touch claim on a free path, a refusal naming the holder on a held path, all four coordinator answers (`before`, `after`, `defer`, `override`), the two-turn proposal handshake, drift and its widen, incursion recording, the board's alert and incursion sections, marker drain and its idempotence, the unenrolled silent opt-out, rollback to observe, and the orphan alert. An ordering edge's three holding states and its settled state are each observable in turn. `hana` runs with `gate_mode = "enforce"`: an out-of-order merge into `main` exits 128, and the refusal names the blocking reservation, its protected paths, the ordering edge with its recorded reason, and three recoveries — release the predecessor, single-shot `integrate --force --why`, or `CARGO_BERTH_BYPASS=1`.

The three hook shims are registered in `hana`'s `.claude/settings.local.json` against their canonical absolute paths under `~/.claude/scripts/berth/install/hooks/`, so the installation keeps working after any run ends. Those shims have only ever been driven by direct invocation; no live Claude Code session has fired them, and this environment cannot register hooks for itself, so the session-level wiring remains unexercised. Two proof reservations persist in `hana`'s append-only journal, released with `--abandon`, so no orphan alert is outstanding and both appear in the board's resolved audit section.

**Files:**
- `~/.claude/scripts/berth/install/hooks/berth_post_bash.sh` — the `PostToolUse` shim; its `valid_widening` predicate accepts and renders the `first_touch_claimed` and `post_write_incursion` widening statuses.
- `/Users/natemccoy/rust/hana/.claude/config/berth.toml` — `gate_mode = "enforce"`, `trunk = "main"`, and the two graph bounds.
- `/Users/natemccoy/rust/hana/.claude/settings.local.json` — registers all three shims at their canonical paths; gitignored, so `hana` stays clean.
- `/Users/natemccoy/rust/cargo-berth-init/docs/berth-plan.md` — status line records the design as built.

**Gotchas:** The trunk gate needs an *entering* reservation to attribute a merge to — a branch holding nothing merges silently, which is correct because nothing was ordered, so a refusal needs reservations on both sides. An ordering edge settles on `HEAD` ancestry alone; no resnapshot on the successor is involved. One coordination identity is shared by every call from a single Claude session, because `CLAUDE_CODE_SESSION_ID` overwrites `CARGO_BERTH_SESSION_ID`; distinct actors need distinct session ids. `CARGO_BERTH_BYPASS=1` always records `skipped_holds: "override_preceded_ledger_read"` even when it skipped a real hold, because the override short-circuits before the ledger read. Pending bypass markers live directly in `.git/`, not under `.git/cargo-berth/`, and only the `SessionStart` shim drains them. Enforcement depends on the managed `reference-transaction` hook and its recorded executable remaining available, not on the config key alone. The `berth.toml` trunk key is `trunk`; `trunk_branch` is rejected as an unknown key. On a stale coordination identity, `claim.rs` advises retrying the command, which re-reads the same stale source and reproduces the diagnostic verbatim.

**Ruled out:** Re-running the bypass half of the enforce proof against `hana` — it advances `hana`'s trunk. Using `hana` as the merge target for ordering scenarios — it compiles too slowly for a scenario loop. Fixing the stale-identity recovery advice here — it lives in the engine and fails closed without losing state.

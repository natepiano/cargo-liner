...
**`init` installs a commit-time warning hook, and it can only warn.** A `post-commit` hook runs `drift` after every commit and reports an incursion. Git discards a `post-commit` hook's exit status, so a commit-time check that blocks is unrepresentable rather than merely discouraged, and `git commit --no-verify` does not skip it — that flag covers the pre-commit and commit-message hooks. It runs the **full comparison**, not the cheap delta: the question at commit time is whether committed content strayed outside what was claimed, and the cheap delta's baseline is the last observation, which a just-finished commit has already moved past. It installs through phase 8's managed-hook path, so it is idempotent per hook name and preserves an unrelated existing `post-commit` hook. **The hook has no reservation to name, so it checks every reservation the invoking worktree holds.** The selection rule below makes a parameterless `drift` a usage error when several active reservations match the acting identity, and the hook can never supply `--reservation` because git gives it no arguments. Resolving that by having the hook fall silent whenever a worktree holds more than one reservation would make the commit-time warning quietly stop working in exactly the case it matters most. So the hook runs the full comparison once and classifies the changed set against each of the acting identity's active reservations in turn, reporting every widen and every incursion it finds; a worktree holding one reservation is the ordinary case of that rule, not a separate path. This is the hook's own subject rule and it does not relax the command-line rule: a human running `drift` by hand with several candidates still gets the usage error, because a hand-run widen must be attributed to a reservation the user chose. **The hook must not change directories the way phase 8's reference-transaction hook does.** That hook deliberately `cd`s to the recorded policy worktree, because a trunk-gate decision is repository-wide and must read the same configuration wherever git invoked it. A commit-time drift check is the opposite: its whole subject is the worktree whose commit fired it, so it must run where git invoked it and resolve its configuration and its acting identity from that worktree. Copying the trunk gate's script shape here would make every linked worktree report drift for the policy checkout instead of itself.

The hook records nothing of its own — whatever `drift` journals for a widen or an incursion is the whole record — and it states an outcome for every way the check can fail to produce an answer: `CARGO_BERTH_BYPASS=1` skips it before the ledger is read, an unreadable ledger (exit `4`) and an exhausted lock deadline (exit `6`) each name what could not be checked and the command to run by hand. Every one of these leaves the commit standing.
...
**Acceptance gate:** `verify.sh check cargo-berth` and `lint cargo-berth` green (`missing_docs` is denied, so the crate-level docs must be complete). `cargo publish --dry-run -p cargo-berth` succeeds. Every command in the README runs as written against a scratch repo, and the collision transcript matches the real exit-`1` block, exit-`3` proposal, and token-bearing application output byte-for-byte. The README attributes approval only to the submitting repository, worktree, and coordination run, never to a person. Every verb's `--help` runs and names each of its flags. The README's account of the commit-time warning matches shipped behavior — it appears after the commit is made, it never blocks, and it names both the on-demand `drift` command and the environment override that skips it. The installed `cargo-berth` executable runs from a directory outside the workspace and reports the expected version. Also: the README states that the gate ships observe-only and that enforcing is a configuration choice; it states the fail-closed rule for the journal and the permit-and-explain rule for an unreadable configuration together with why they differ; it names all three of the gate's other fail-open cases with the route back for each, including a preserved unmanaged hook and where `init` reports it; it documents all three `init` branches with their exact effects, naming confirmed reinitialization as the only route back from a corrupt ledger; and it documents `CARGO_BERTH_BYPASS=1` including that its audit fact carries no reason and names nothing it skipped, and that when neither audit destination is writable the tool warns and still permits.
...
**Acceptance gate:** No `verify.sh`, and no compile. `taplo fmt --check .claude/config/berth.toml` passes; `cargo-berth board --json` runs and reports an empty ledger with "no integration order declared"; the hook file exists in the common dir; a `git merge` into `main` succeeds and the observe-only log records the evaluation. By this phase `init` manages two hooks, not one, and it resolves the effective `core.hooksPath` rather than always writing into the common git directory — so checking for one file in the common directory proves nothing. Assert instead that `init` reports both `reference-transaction` and `post-commit` as active in its `hooks` payload, and that each managed marker is present in the effective hook directory git actually reads. Also assert that `.claude/config/berth.toml` is committed before any linked worktree is created, and that a clean in-order merge is silent — the gate ships observe-only and has nothing to say when nothing is violated, so expecting output from a clear gate is expecting the wrong thing; observe-only violation logging is phase 17's to prove.

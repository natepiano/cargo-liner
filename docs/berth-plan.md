...
<!-- RENUMBERED. Phase ids were bare integers with four letter suffixes; the
     suffixes are gone and every id from 4 onward shifted. Checkpoint commit
     messages for the done phases carry the OLD id. Old -> new:
       1->1  2->2  3->3  3b->4  4->5  5->6  6->7  7->8  8->9  9->10
       9b->11  10->12  10b->13  10c->14  11->15  12->16  13->17  14->18
       15->19  16->20  17->21
     Every cross-reference in this doc and in berth-plan-next.md uses the NEW
     ids. -->
...
### Phase 4 — Claims, scopes, and overlap  · status: done
...
### Phase 5 — Reservation lifecycle and git evidence  · status: done
...
### Phase 6 — Worktree liveness, reconciliation, and orphan alerts  · status: done
...
### Phase 7 — Overlap answers  · status: done
...
### Phase 9 — The trunk gate  · status: done
...
### Phase 10 — Drift detection  · status: done
...
### Phase 11 — Drift records and hook identity, finalized  · status: done
...
### Phase 12 — The board: model, incidents, and `--json`  · status: done
...
The projection phase 9 built for gate decisions is extended here to carry
...
**Binds later work:**
- Phase 13 hardens this phase's board model: it adds a per-invocation
  recovered-bypass set (phase 17's one-time recovery report needs it and this
  ...
  `RecordedAnswer::RevalidatedWiden` before phase 15 freezes their serialized
  tags — phase 15 depends on phase 13, not on this phase alone, because every item
  phase 13 carries is unfixable after the freeze without a version bump.
- Phase 14 renders this model in a terminal and must add no fact of its own;
...
- Phases 15, 17, 18, and 19 parse the `--json` payload this phase froze — its
...
- A board row for `DriftWideningOutcome::{Ambiguous, CoordinationRunRequired}`
  ...
  immediate output, phase 17's shim, and phase 15's README carry that instead.
...
### Phase 13 — Pre-publish contract hardening  · status: done

#### As-built

Four contract repairs to phases 11 and 12, landed before phase 15 freezes the
...
### Phase 14 — The board TUI  · status: done

#### As-built

`cargo berth board` has three output modes, selected in `src/cli.rs` after the output mode resolves so `--json` never acquires a terminal. With a terminal on stdout it opens a `tui_pane` front end over phase 12's board model and exits clear once the terminal is restored, printing nothing afterward. Redirected, it prints a pointer line to `--json` plus any recovered-bypass notices — not the board rows. `--json` prints the frozen payload unchanged and is the only mode that carries the facts. The view renders and decides nothing: six pane field lists partition all sixteen top-level model fields, and construction fails if a field is unassigned or a named field is absent.

A terminal failure has two outcomes and they are deliberately different. Failing before the first frame keeps the board-bearing envelope, exits `0`, and attaches the terminal diagnostic. Failing after the first frame reports an internal failure carrying no facts and exits `7` — `BerthExit::TerminalViewFailed`, with a matching `OutputStatus::TerminalViewFailed`. `event_loop` returns `BoardTerminalViewInteractionOutcome::{Completed, FailedBeforeFirstFrame, FailedAfterFirstFrame}`, and `finish_terminal_view` is a total match over that crossed with the restoration result, so a failed rollback of raw mode or terminal acquisition is named rather than leaving a wedged terminal silent. `Command::execute` returns `CommandExecution`, which distinguishes an envelope to emit from a board already presented on a restored terminal.

**Files:**
- `crates/cargo-berth/src/board/tui.rs` — the terminal view, its pane partition, its failure taxonomy, and its tests.
- `crates/cargo-berth/src/verb/board.rs` — the authoritative mapping from a terminal outcome to an exit code and a response status.
- `crates/cargo-berth/src/exit.rs` — eight process exit codes and the doc table giving each its one-line meaning.
- `crates/cargo-berth/src/output.rs` — the two board response constructors for the before-opening and after-opening terminal failures.
- `crates/cargo-berth/src/cli.rs` — command dispatch, `CliOutputFormat` routing, and the no-envelope restored-terminal path.
- `crates/cargo-berth/Cargo.toml` — now consumes `crossterm`, `ratatui`, `unicode-width`, and `tui_pane`.

**Binds later work:** The exit table is eight codes; exit `7` means the terminal failed after the board was visible. `--json` is the only mode that emits facts, so every scripted caller and every documented example uses it and bare `board` would hang a shim. Exit `7` is unreachable off the terminal path. `board --json`'s payload is unchanged from phase 13.

**Gotchas:** Reconciliation runs in reporting mode before any output-mode branch, which is what retires and reports recovered-bypass markers on the terminal path too; moving it after the branch loses that on every interactive run. The root workspace manifest was untouched — all four dependencies were already declared there for other crates, and only the crate manifest and the lockfile moved. A pty with no window size renders an empty frame, so the terminal view cannot be smoke-tested through `/usr/bin/script`. Text rendering shows no status field, so a terminal outcome is legible only through the exit code or `--json`.

**Ruled out:** Reusing an existing exit code for a terminal failure — it conflates a tool-level failure with a berth-domain outcome. Treating every terminal failure as a failure to the caller — a failure before the first frame still has a board to deliver.

### Phase 15 — README, changelog, and publish readiness  · status: done

#### As-built

The crate README replaces the phase-1 placeholder and is the prose statement of
the frozen `board --json` contract: first-use flow, a real collision transcript
through its exit-`1` block and exit-`3` proposal, the enforcement boundaries and
every fail-open case with the route back from each, `drift` with both comparison
modes and all three consequences, harness identity and `EditAuthorization`'s
resolution order, the three `board` output modes, the configuration fields, the
bypass audit, the recovery procedures, and the eight-code exit table.
`crates/cargo-berth/CHANGELOG.md` and the root workspace-member row ship beside
it.

The help audit corrected `src/cli.rs`: `board` states that it opens the
full-screen view only when both standard input and standard output are
terminals and points at `--json` otherwise, `check` states that a bare path
means one exact file while `file:` and `tree:` override it, and
`integrate --force` names unresolved deferrals as well as ordering edges.

`cargo-berth 0.1.0-dev` is installed at `/Users/natemccoy/.cargo/bin/cargo-berth`
from `/Users/natemccoy/rust/cargo-berth-init` on `init/cargo-berth`, and runs
under both `cargo-berth` and `cargo berth` from outside the workspace.

The publish dry run is deferred, not failed. The path-only dependency check
passed — `tui_pane` is the sole path-only runtime dependency and the versionless
`[[publish_path_pins]]` entry in `.claude/config/release.toml` already covers it
— and the registry dependencies resolved with declared MIT/Apache-family
licenses. Nothing was published.

**Files:**

- `crates/cargo-berth/README.md` — the user guide, and the prose contract phases
  13–15 parse against.
- `crates/cargo-berth/CHANGELOG.md` — release notes.
- `README.md` — the workspace-member row.
- `crates/cargo-berth/src/cli.rs` — the help surface an agent reads, plus the
  parser test `operational_help_explains_board_streams_check_defaults_and_force_scope`.

**Binds later work:** Track-B phases invoke the installed executable by name and
compile nothing. `--version` reports `0.1.0-dev` for every build of this crate,
so the installed file's SHA-256 —
`eab08809bbc2df03800428ed7cc0a187831676065e5d92a7df296bc2bde65095` — is the
immutable identifier, checked with one `shasum -a 256` call and repaired by
reinstalling from the named checkout. A complete default `drift` call measured
0.09–0.12 s across five runs in a scratch repository; 0.20 s is the documented
bound track B is written against. Publishing `cargo-berth` requires
`tui_pane 0.8.0` on crates.io first, because the versionless pin otherwise
resolves to 0.7.0 while this workspace builds on 0.8.0-dev.

**Gotchas:** Installing the artifact is not a one-time step inside a phase — the
binary was installed before review found the help defect, so the corrected text
sat in source while the executable kept printing the superseded wording. Only a
live invocation catches that; no diff can. Every test in this repository builds
from source and is therefore blind to it.

**Ruled out:** Publishing `tui_pane 0.8.0` to unblock the dry run — irreversible,
and this plan publishes nothing. Running the dry run against the pin's resolved
0.7.0 — it would check the crate against an older API than it was built on, so
the result carries no information either way. Recording the phase's own
checkpoint hash as the artifact's provenance — the checkpoint contract forbids a
phase's commit hash in the plan, and the executable digest is both available now
and directly checkable.

### Phase 16 — Config and init in hana  · status: done

#### As-built

`/Users/natemccoy/rust/hana` carries `.claude/config/berth.toml` — committed as `e052361e`, holding trunk, R4's `V`/`E` limits, and `gate_mode = "observe"` — plus an initialized ledger. `cargo-berth init` is idempotent and manages two hooks, `reference-transaction` and `post-commit`, installed into the effective hook directory git actually resolves rather than always into the common git directory. `config.rs` defines `Enrollment<T>`, making the absence of configuration a state rather than an error so the compiler forces every reader to handle non-enrollment: `BerthConfig::read` returns `Result<Enrollment<Self>, ConfigError>` and `Ledger::read_for_edit_check` returns `Result<Enrollment<EditCheckLedgerSnapshot>, LedgerError>`. An unenrolled repository answers `unconfigured` at exit `4` with a `no_facts` payload and is silent — no diagnostic on commit, no shared-state write. Enrollment is a property of the repository, not of the worktree a developer stands in — a trunk ref update is repository-wide, and a gate a worktree could escape by deleting its own configuration would not be a gate — so the managed hook changes into the policy worktree before the tool decides. `CARGO_BERTH_BYPASS=1` is the explicit override and never fails a ref transaction; the tool reads and parses the hook's stdin before deciding, recording exactly one audit fact when a local-branch update names the trunk reference and none when no update does.

**Files:**
- `/Users/natemccoy/rust/hana/.claude/config/berth.toml` — hana's per-repo dialect: trunk, reservation and ordering-edge limits, observe-only gate mode.
- `/Users/natemccoy/rust/cargo-berth-init/crates/cargo-berth/src/config.rs` — `Enrollment<T>` and `BerthConfig::read`.
- `/Users/natemccoy/rust/cargo-berth-init/crates/cargo-berth/src/output.rs` — the frozen envelope, including `unconfigured` at exit `4`, and its silent post-commit rendering.
- `/Users/natemccoy/rust/cargo-berth-init/crates/cargo-berth/tests/overlap.rs` — `check` against a repository that loses its configuration, clear and conflicting.

**Binds later work:** The installed binary's SHA-256 is `d503043d5f848557d43e6f345fa81cbe12120365cd0f2afd39aa90da13444150`, built from `/Users/natemccoy/rust/cargo-berth-init` on branch `init/cargo-berth`; `cargo-berth --version` reports `0.1.0-dev` for every build of this crate and so proves nothing about freshness — the digest is the only check, and phase 15's digest along with two intermediate digests taken during this phase are superseded. `unconfigured` and `ledger_unreadable` share exit `4` and `no_facts`, so only the envelope's `status` field distinguishes them: the first must be ignored silently by automated surfaces, the second keeps its fail-open diagnostic.

**Gotchas:** Git runs the reference-transaction hook once per transaction phase, and one fast-forward merge produces three `prepared` invocations — `ORIG_HEAD`, `refs/heads/main`, and `AUTO_MERGE` — of which only the middle one names trunk. The audit fact has two possible homes and the binary's exit status selects between them: zero means it retained the fact or there was nothing to retain and the hook stops; nonzero means a trunk bypass it could not retain, and the hook writes its own pending marker at an absolute path, so the fallback still works when retention failed for a reason tied to the working directory. Either way the hook exits zero, so no bypass path can fail a git command. `hana_clerestory_recovery` is a linked worktree of hana on branch `enh/recovery`, not a separate repository, and its branch predates the configuration commit; the requirement that the configuration be committed before any linked worktree is created is superseded — not satisfiable retroactively and no longer required, because an unenrolled checkout is now silently not enrolled for both reads and writes. hana's `core.hooksPath` formerly pointed into a directory named `bevy_diegetic` that no longer existed; that setting is unset and the phantom directory is gone, so `git rev-parse --git-path hooks` resolves to `/Users/natemccoy/rust/hana/.git/hooks`.

**Ruled out:** Naming the enrollment type `RepositoryCoordinationEnrollment` — `Enrollment<T>` carries the same two states and is generic over what enrollment yields. Teaching the hook's shell fallback to parse ref names — the tool parses, and its exit status is the only thing the shell needs to read. Treating an unenrolled worktree as an error rather than a state.

### Phase 17 — Hook shims and settings wiring  · status: done

#### As-built

Three hook shims live in `/Users/natemccoy/rust/hana/.claude/hooks/`, unregistered — `.claude/settings.local.json` is untouched, and the end-to-end activation phase adds the `hooks` key. `berth_pre_edit.sh` (`PreToolUse` on `Edit`/`Write`/`NotebookEdit`) converts the payload's absolute `file_path`/`notebook_path` to a repo-relative path lexically, calls `check --json`, and refuses a path another worktree holds, naming every holder's reservation, branch, plan, phase, reason, and overlapping scopes; a `Write` naming a file that does not exist yet is handled normally, and an edit outside the repository is permitted rather than an error. `berth_post_bash.sh` (`PostToolUse` on `Bash`) sets `CARGO_BERTH_SESSION_ID` from the payload's `session_id` and `CARGO_BERTH_POST_COMMIT=1`, calls `drift --json`, and emits typed feedback — `ImmediateStop` for an incursion, collision, or required attribution, `WideningNotification` for an automatic widen — classified from the envelope's structured fields across every `results[].effects[]`, with no prose fallback. `berth_session_start.sh` reads `board --json` and reports orphan alerts, the one-time recovered-bypass drain, and outstanding incursion incidents, each with the `resolve` command that answers it. All three read `status` rather than exit code alone: `unconfigured` is silent, `ledger_unreadable` keeps its fail-open diagnostic and permits the edit.

The engine changed with them. `drift --json` under `CARGO_BERTH_POST_COMMIT=1` rendered the human stderr warning instead of its typed envelope, so `cli.rs` gained `PostCommitHookRequest` and `CommandResponseRendering`, with `command_response_rendering` the total function mapping output format and hook request onto one of them; only a text-format post-commit invocation renders the warning.

**Files:**
- `crates/cargo-berth/src/cli.rs` — `PostCommitHookRequest`, `CommandResponseRendering`, and `command_response_rendering`; the JSON post-commit guarantee is at `cli.rs:281`.
- `crates/cargo-berth/tests/drift.rs` — `text_post_commit_drift` for the three warning-asserting tests, the JSON helper for envelope consumers, and `json_post_commit_reports_every_active_reservation_without_warning_rendering` (`drift.rs:490`).
- `/Users/natemccoy/rust/hana/.claude/hooks/berth_pre_edit.sh`, `berth_post_bash.sh`, `berth_session_start.sh` — the three shims, untracked in `hana` and deliberately unregistered.

**Binds later work:** `drift --json` under `CARGO_BERTH_POST_COMMIT=1` now returns its envelope reporting every active reservation — `cli.rs:281` and `drift.rs:490` are the read-only sources. The installed artifact is this phase's build, digest `efceb301d2c4f61ee2a835694b5d6563bcfcfa01446b292f90119cdc85be19d9`; earlier recorded digests are stale. The shared coordinator inherits this phase's defensive envelope parsing — per-field JSON type validation, process status agreeing with `exit_code`, unknown tags rejected, decisions derived from structured fields. Every Work Order must declare the plan-document and next-items scopes, or the registered `PreToolUse` hook blocks the dispatcher's own plan edits. The end-to-end activation phase owns the `PostToolUse` cost measurement, since it runs the loop under real use.

**Gotchas:**
- The installed git post-commit hook passes no `--json` — `gate/install.rs:232` runs `CARGO_BERTH_POST_COMMIT=1 {executable} drift --full` — so it still resolves to the warning rendering, which is what made splitting the two paths safe.
- `jq`'s `length` is `0` for `null`, a missing key, `""`, and `{}` alike, so an emptiness check that has not first asserted `type == "array"` accepts three malformed envelopes as one valid answer.
- Classifying a stop-versus-notify outcome by grepping rendered text is unsound: a reservation path containing the grepped word flips a notification into a stop. Classify from the same structured query that renders the text.
- A linked worktree reads `.claude/config/berth.toml` from its own checkout, and `cargo-berth init` writes that file after the seed commit, leaving it untracked — a worktree created before it is committed answers `unconfigured` and every scenario in it passes vacuously. Commit the enrollment before `git worktree add`.
- The journal lives inside the git directory at `.git/cargo-berth/journal.ndjson`, so a `find` excluding `*/.git/*` finds nothing.
- A claim made from the holder's own worktree is correctly permitted, because the run recognizes itself through its marker file; proving refusal takes two worktrees.
- The complete `PostToolUse` path measures 0.259 seconds against the published 0.20-second bound; the pre-edit clear path measures 0.069 seconds, inside it.

**Ruled out:** Correcting the JSON post-commit defect inside the shim, since the rendering decision lives in the engine; treating the elapsed-time overrun as a blocker, since it is a measured performance fact with no correctness consequence.

### Phase 18 — The /sync skill  · status: todo
...
**The validator's subject is the complete Work Order, not its `Reservations` block alone.** R35 exists because `/plan:delegate` accepts the mere presence of a `#### Work Order` heading and copies expected fields without validating them: live Tool Graph 60 lacks the standard **Goal**, **Spec**, and **Files** structure, and Tool Graph 69 and 70 lack **Files**, and none of that is caught today. A reservations-only check leaves that failure mode exactly as it found it. So the validator rejects a missing or empty **Goal**, **Spec**, or **Files** section alongside an invalid path, a duplicate or contained scope, and a malformed block — and every writer revalidates the whole Work Order after changing **Files** or any implementation path, because a `Files` edit that is not mirrored into `Reservations` is precisely the divergence R35 names.
...
The validator also answers *"do these two path sets collide?"* offline, from two parsed blocks alone, with no ledger and no engine call. **Offline means it reimplements the engine's rules, so it must reproduce all four or it will disagree with the tool it fronts:** overlap is path-component ancestry rather than string prefix, so `crates/hana_kana` does not collide with `crates/hana_kana_extra`; a `file:` scope and a `tree:` scope over the same spelling behave differently for descendants; a path that does not exist yet participates normally; and comparison folds case when the repository sets `core.ignoreCase`, which this one does. Phase 20's backfill needs exactly that, and `cargo-berth check` cannot provide it: `check` compares one candidate against **live** reservations, so run against an empty ledger it reports no collision no matter how badly two Work Orders overlap.

**Every Work-Order claim also carries the plan document's own scopes (R33).** `/plan:delegate` edits its plan doc during decision resolution, phase review, shrink, and checkpoint, and may create or edit the derived `-next.md` sibling. Neither is an implementation file, so no **Files** section names them — and once phase 21 registers the `PreToolUse` hook, a claim that omits them lets that hook block the dispatcher's own plan edits, while Bash-mediated edits surface later as unexplained drift. So resolving a Work Order to paths appends two exact exclusive `file:` scopes — the plan document and its derived next-items path — to the declared block, in the same reservation, outstanding with the phase commit. Adding them at resolution rather than in the document is what keeps them out of 28 hand-written blocks. Session files outside the repository need no reservation.

`/sync claim` owns one shared authorization state machine used at both an interactive claim and phase 19's dispatch boundary:

- **Blocked** — the neutral claim returned exit `1`. Render every current holder and shared scope, then ask the user for one direction and a non-empty reason. Phase 7's CLI answers exactly one named blocker; if several conflicts remain, require the user to narrow the requested scopes rather than pretending one answer covers all of them.
...
- **Claimed** — the separately approved token-bearing invocation returned exit `0`. Return its `ReservationId` to the caller. The session mapping is already written by then and nothing downstream records it: phase 11's `claim` writes the mapping itself when its own process environment carries `CARGO_BERTH_SESSION_ID`, so this skill sets that variable on every engine invocation, derived from `CLAUDE_CODE_SESSION_ID`, and a claim made without it silently produces no mapping at all. Read `session_mapping_publication` from the claim payload and treat `unavailable` as a **degraded success**, not a failure — the reservation is held and the journal is durable, only the mapping is missing, so report the diagnostic and continue with the returned `ReservationId` named explicitly from there on. Phase 11 also added a distinct `inactive_session_mapping` rejection that `sequence` and `integrate` return when the mapping names a reservation that is no longer active. It is a usage error with its own recovery — name the coordination run and reservation explicitly — and it must not be rendered as the coordination-run marker being stale, which is a different rejection with a different fix.
...
- `~/.claude/scripts/berth/work_order.py` — new; the shared Work Order parser and validator — the complete **Goal**/**Spec**/**Files**/**Reservations** structure, not the reservations block alone — plus the offline pairwise overlap comparison, invoked by all three commands below so none of them grows a second parser (R35).
...
- `crates/cargo-berth/src/output.rs`, `crates/cargo-berth/src/board/mod.rs`, `crates/cargo-berth/tests/board.rs` — read only, in the engine checkout phase 16 recorded; the frozen `board --json` contract `/sync board` renders.
- `crates/cargo-berth/README.md` — read only; phase 15's board section, the prose statement of that same contract.
...
- `~/.claude/commands/plan/to_phased_plan.md`, `~/.claude/commands/plan/phase_review.md`, `~/.claude/commands/plan/delegate.md` — each calls the shared validator instead of growing its own parser (R35). `delegate.md` is also phase 19's file; keep the two phases' edits disjoint.

**Constraints from prior phases:** Phase 7 defines the four answers and that they arrive as `claim` flags, not as separate verbs — there is no standalone `override` verb. Phase 1's six envelope fields plus phase 2's typed payload are the parse target. **Parse that envelope defensively; it is process output, not a trusted value.** Validate each frozen field's JSON type before reading it, require the process exit status to agree with the envelope's own `exit_code`, reject an unknown status or payload-kind tag rather than falling through to a default, and derive every decision from the structured fields rather than from a rendered message. The emptiness test is the trap: a `null`, a missing key, an empty string, and an empty object all report length zero in `jq`, so a check that has not first asserted the type accepts three malformed envelopes as one valid answer. Phase 19 inherits this coordinator, so the malformed-envelope cases live here. Phase 17 owns the installed executable; invoke `cargo-berth`, and compile nothing. Phase 17 repaired the engine under a build exception — the `cli.rs` fix that made a `--json` post-commit invocation return its typed envelope instead of the stderr warning — then rebuilt and reinstalled, so phase 15's and phase 16's digests are both stale. The artifact every later phase consumes is `efceb301d2c4f61ee2a835694b5d6563bcfcfa01446b292f90119cdc85be19d9` from the `init/cargo-berth` checkout at `/Users/natemccoy/rust/cargo-berth-init`; `cargo-berth --version` reports `0.1.0-dev` for every build and proves nothing, so compare `shasum -a 256 "$(command -v cargo-berth)"` against that value before the first invocation. **Phase 7 shipped a blocked/proposal/apply protocol, and this skill is the only thing that performs its approval transition.** A neutral conflicting claim returns exit `1`; after the user supplies an answer and `--overlap-why`, the next invocation returns exit `3` carrying an `OverlapProposal`; only a separately approved invocation passes it back as `--proposal <token>`. The engine recomputes under the lock and distinguishes a refreshed proposal from malformed input. This skill is also where R54 is actually enforced, because the tool cannot enforce it: render the escalation payload to the user, take the direction and the reason **from the user's own answer**, and never select a permissive answer, compose its reason, or spend a token on the user's behalf. Read the shipped `src/cli.rs` for the flags. Phase 3 owns `EditAuthorization` and its resolution, which phase 11 reordered — the session-keyed mapping first, then `CARGO_BERTH_RUN`, then the worktree marker file, failing closed to `Unidentified`. Read it; never re-derive it, and never pass a run id to `check`. Phase 4's `claim --run` is the one provenance boundary where a run id is an argument; phase 11's session mapping records that same value against the harness session — the same UUID-v7, not a second identifier — and `--plan` and `--phase` travel together or the command line is a usage error. A successful claim returns the reservation identity, and the `claim` process itself records it against the harness session when this skill supplies `CARGO_BERTH_SESSION_ID`. Phase 17's `PostToolUse` shim passes only that session id and lets the engine resolve the reservation; it never supplies `--reservation` and never reads the mapping file. The escalation material comes from phase 4's typed `ClaimPayload`, which names **every** holder of the paths rather than only the first, and reports each holder's provenance through `ClaimSource` — including its variant for a claim made with no plan or phase, which the escalation must render rather than assume away. **`/sync check` runs drift's full comparison, not its cheap delta.** Phase 10 splits the two: the cheap delta is the post-`Bash` hook contract and answers only whether anything changed since the last observation, while the full four-command comparison against the phase-start baseline is what a user asking "what has changed against what I claimed" actually wants. Select the full path explicitly rather than taking the verb's default. The retry budget is phase 9's single 10-second total deadline measured across all attempts, not five attempts each paying the engine's internal lock wait. **Phase 6 changed what a marker-derived identity buys.** `EditAuthorization` is now source-preserving — `Environment(CoordinationRunId)`, `Marker { coordination_run_id, worktree_id }`, or `Unidentified` — and a `Marker` identity is honored only when replay shows that same run still holds an **active** reservation minted in that same worktree. An environment-supplied run is trusted as given. A stale marker left by a crashed run therefore no longer grants the holder's own exemption, which is exactly why phase 11's session-keyed mapping is the primary path and `CARGO_BERTH_RUN` and the marker file are the fallbacks behind it. **`/sync check` means drift's full phase-start comparison, not the `check` verb.** `/sync check` answers "did anything stray outside what was claimed", which is `cargo-berth drift` with the full comparison selected explicitly rather than the verb's cheap default. The `check` verb answers a different question — whether a proposed path or edit would collide — and stays reachable for that. The Spec states both; keep them stated in the acceptance gate too. **The engine already owns the only retry budget, so a shim must not open a second one.** Phase 9 set the contract and phase 5's lock implements it: one 10-second total deadline measured across all attempts, with exponential backoff from 50 ms inside it. Exit `6` is returned only once that whole deadline is spent, so a caller that sees `6` has no budget left to retry with — a second-layer loop just multiplies a ten-second wait by however many times it runs and turns a busy ledger into a hang. Treat `6` as the named exhausted outcome it is: report that the tool was busy and nothing was decided, name the command to run again, and let the person decide. Do not write a retry loop around a verb, and do not assert one in an acceptance gate. **`unconfigured` is a terminal outcome for every `/sync` command, not an input to its state machine.** Phase 16 made an unenrolled repository answer `unconfigured` at exit `4` with a `no_facts` payload, so `board` has no board to render and `claim`, `release`, `sequence`, `integrate`, and `drift` must not enter their normal transitions. Because `/sync` is invoked by a person rather than by a hook, it says so plainly: render the exact expected configuration path the message carries and name `cargo-berth init` as the remedy. Read `crates/cargo-berth/src/config.rs` and `crates/cargo-berth/src/output.rs` for that contract.


**Phase 14 gave `board` an interactive terminal view, so `/sync board` reads `--json` and renders the result itself.** Run with a terminal on stdout, bare `cargo-berth board` opens a full-screen view and waits for a keypress; redirected, it prints only a pointer line and any recovered-bypass notices, not the board rows. So `--json` is the only mode that carries the facts, and `/sync board` is a second consumer and renderer of the payload phase 15 freezes — not a passthrough. Render every top-level field and every user action the payload names; a field the skill silently drops is a coordination fact the user never sees. Exit `7` belongs to the terminal view alone and is unreachable on the `--json` path.

Read the frozen contract from three read-only engine sources rather than re-deriving it — `crates/cargo-berth/src/output.rs` for the envelope and its statuses, `crates/cargo-berth/src/board/mod.rs` for the sections, tagged row states, alerts, waiting reasons, the transient recovered-bypass set, and the persistent audit history, and `crates/cargo-berth/tests/board.rs` for the shipped assertions the rendering must agree with — plus phase 15's README board section, which is the prose statement of that same contract.
**Acceptance gate:** No `verify.sh`, and no compile. `/sync board` invokes `cargo-berth board --json` exactly once and its rendering is asserted to preserve every top-level field of that payload and every user action it names, including the one-time recovered-bypass notice, which appears on the read that adopted it and is absent on the next read while the bypass stays visible in the audit history; a field present in the payload and absent from the rendering fails the gate. The skill never invokes `board` without `--json`. `/sync claim --from-work-order docs/hana/tool-graph.md <phase>` resolves a real Work Order to the right paths and claims them. A forced collision follows the complete shared state machine: the neutral invocation becomes **Blocked** on exit `1`; a user-supplied direction and reason produce **Proposal awaiting approval** on exit `3`; only a later explicit approval applies that exact token and reaches **Claimed** with a reservation id. A stale token that returns exit `3` replaces the proposal and requires approval again, a changed conflict set that returns exit `1` returns to **Blocked**, and malformed-token exit `5` is reported separately and never treated as approval. No path mints and spends in one turn. Every path the skill emits is lexically valid and repo-relative, **whether or not it exists yet**; the validator rejects an absolute path, a `..` escape, and a `.git` component; two hand-written blocks that overlap are reported as colliding with no ledger present; a `claim` that returns exit `6` is reported as busy rather than as clear, asserted to have invoked the verb exactly once with no shim-side retry loop; all four commands resolve the same validator. Also: `/sync check` runs drift's full phase-start comparison and is asserted to issue that comparison's four commands, not the `check` verb's proposed-path question; and `check` remains reachable for the proposed-path and proposed-edit questions it actually answers. Also: every `/sync` command run in an unenrolled repository reaches the unconfigured terminal outcome, prints the expected configuration path and the `cargo-berth init` remedy, and enters no state machine — asserted separately from a genuine ledger failure, which still reports as such. Also: the validator rejects a Work Order missing **Goal**, **Spec**, or **Files** as well as one carrying an invalid or contained path, and a `Files` edit left unmirrored in `Reservations` fails revalidation. Also: resolving any Work Order to paths appends the plan document and its derived next-items path as exact exclusive scopes (R33), asserted present in the claimed footprint of a Work Order whose own declaration names neither. Also: a malformed envelope is refused rather than defaulted — a field of the wrong JSON type, a process status disagreeing with the envelope's `exit_code`, an unknown status or payload-kind tag, and a frozen field that is `null`, missing, an empty string, or an empty object each fail validation instead of reading as a valid empty answer.

### Phase 19 — /plan:delegate integration  · status: todo
...
**Spec:** Claim **before the first implementation dispatch**, recording the phase's starting `HEAD` as the fingerprint baseline. The dispatcher invokes phase 18's shared `/sync claim --from-work-order` flow; it does not call `cargo-berth claim` directly or implement another proposal/token loop. Release through `/sync release` at the checkpoint boundary that already exists — which records `Outstanding { protected_tip }`, not disappearance (phase 5).

**The release order is fixed, because `release` takes no commit argument: it snapshots the invoking worktree's current `HEAD` (`crates/cargo-berth/src/verb/release.rs:347`).** So run drift's full phase-start comparison, take the checkpoint commit and confirm it succeeded, and only then invoke `/sync release` — asserting the recorded `protected_tip` equals that commit. Releasing first snapshots the parent and publishes a protected tip that does not contain the phase's work, and phase 21's ordering edges are computed from exactly that value, so the error surfaces as another worktree's hold clearing early rather than as a failure here.

**Every ending that is not a successful checkpoint keeps the reservation.** A phase can stop short in several ways and they share one disposition: a cancelled phase that produced no diff, a run halted by an error, a checkpoint commit that failed, a dirty `single`-mode run, a user stop, and a failed release attempt each leave the reservation held and the durable `ActivePhaseReservation` record on disk, and report what happened. Only a successful release deletes that record. Releasing on a path that never committed would free paths whose work is still uncommitted in a worktree, and discarding the record without releasing would strand a reservation nothing can name.

A phase whose Work Order has no `**Reservations:**` block will become a hard stop with an actionable message, not a silent skip — a phase that claims nothing is invisible to every other worktree, which is the exact decay this design exists to prevent. Implement the check here behind an explicit **advisory** coverage mode: before activation, a missing block reports that mandatory coverage is not active and permits the phase to continue, while a present block uses the full claim lifecycle. Phase 21 changes that named mode to **required** only after phase 20 finishes the backfill; required mode refuses a missing block before dispatch. This phase's dry run uses a Work Order that already carries a block, and its gate proves a Phase 20 Work Order remains dispatchable while coverage is advisory.

On a blocked neutral claim, `/plan:delegate` pauses before dispatch and enters phase 18's shared **Blocked** state. The `/sync` flow alone gathers the user's answer and reason, presents **Proposal awaiting approval**, and applies an explicitly approved token. Only **Claimed** may continue to implementation. A claim or release that exits `6` is contention rather than a verdict, and it arrives with phase 9's whole retry budget already spent inside the engine — one 10-second total deadline across all attempts, with exponential backoff from 50 ms inside it. Do not retry the invocation: stop with a busy-ledger message naming the command to run again, rather than dispatching as though the claim had succeeded.
...
**Constraints from prior phases:** Phase 18's shared validator does the Work-Order resolution and its shared claim coordinator owns **Blocked → Proposal awaiting approval → Claimed** — `/plan:delegate` invokes that `/sync` entry point rather than parsing markdown or driving token application itself. Phase 5's release semantics mean checkpoint does not free the paths. Phase 3's provenance flags are what carry the plan, phase, coordination run, and starting `HEAD` into the claim. Compile nothing: the installed executable is phase 17's, digest `efceb301d2c4f61ee2a835694b5d6563bcfcfa01446b292f90119cdc85be19d9` from the `init/cargo-berth` checkout at `/Users/natemccoy/rust/cargo-berth-init`. Verify it with `shasum -a 256 "$(command -v cargo-berth)"` before the first invocation — `cargo-berth --version` reports `0.1.0-dev` for every build — and reinstall from that checkout on a mismatch, because nothing here builds and a stale executable fails as wrong behavior rather than as a build error. Phase 3 owns `EditAuthorization` and its resolution, which phase 11 reordered — the session-keyed mapping first, then `CARGO_BERTH_RUN`, then the worktree marker file, failing closed to `Unidentified`. Read it; never re-derive it, and never pass a run id to `check`. Phase 4's `claim --run` is the one provenance boundary where a run id is an argument; phase 11's session mapping records that same value against the harness session — the same UUID-v7, not a second identifier — so a run that claims and a later check in that worktree agree on who is acting. `--plan` and `--phase` are additive optional flags that travel **together**; supplying one without the other is a usage error, not a partial record. `--head` records the commit the phase started from and is what phase 10's drift fingerprint later diffs against, so the dispatcher must pass the real starting `HEAD` rather than omitting it. A claim can succeed while reporting `marker_publication: unavailable`; phase 11's session mapping is what carries identity through the phase in that state, rather than the marker file. **The claimed `ReservationId` has to survive the phase, and the session mapping is not where it survives.** Two facts about phase 11's shipped mapping decide this. It holds exactly one reservation per harness session — a `BTreeMap` keyed by session id — so a second claim in the same session **replaces** the first rather than joining it. And no command reads it back out: `release` and `renew` take the reservation as a required positional argument, and nothing prints the mapping. The mapping is a disposable projection that answers one question for the engine — who is acting in this process — and it answers it well; it is not orchestration memory. **`/plan:delegate` runs in every repository, so coordination must be optional without being silent about failure.** An `unconfigured` answer means this repository never opted in: skip claim, release, and reservation-coverage enforcement without a word. A `ledger_unreadable` answer means coordination is broken and must stop the run — never treat it as opt-out, or real corruption reads as a repository that simply is not enrolled. For the same reason, the absence of an `ActivePhaseReservation` must not be the way non-enrollment is represented, since absence would then mean both "not enrolled" and "lifecycle state lost"; carry a tagged state such as `PhaseCoordination::{NotEnrolled, Claimed(ActivePhaseReservation)}` instead.

So `/plan:delegate` keeps its own record. Persist a named `ActivePhaseReservation` under the run's `SESSION_DIR` when **Claimed** returns — the reservation id, the coordination run, the phase it belongs to, and the starting `HEAD` — and read the release argument from there at checkpoint. `SESSION_DIR` is where that command already keeps durable run state and is what survives a compaction, a resume, or an orchestrator that lost its context mid-phase; a value held only in conversation does not, and a value held only in the mapping cannot be read back. Delete the record when the release succeeds. The mapping stays exactly what phase 11 built it for: the `PostToolUse` shim passes the harness session id and the engine resolves the acting reservation from it, with no shim reading the file and none supplying `--reservation`. **The pre-checkpoint check runs drift's full comparison.** The cheap delta answers only whether anything changed since the last observation; before releasing, the question is what changed against the phase-start baseline, which is phase 10's four-command comparison selected explicitly. The retry budget is phase 9's single 10-second total deadline across all attempts, not five attempts each paying the engine's internal lock wait. **Phase 6 changed what a marker-derived identity buys.** `EditAuthorization` is now source-preserving — `Environment(CoordinationRunId)`, `Marker { coordination_run_id, worktree_id }`, or `Unidentified` — and a `Marker` identity is honored only when replay shows that same run still holds an **active** reservation minted in that same worktree. An environment-supplied run is trusted as given. A stale marker left by a crashed run therefore no longer grants the holder's own exemption, which is exactly why phase 11's session-keyed mapping is the primary path and `CARGO_BERTH_RUN` and the marker file are the fallbacks behind it. **The engine already owns the only retry budget, so a shim must not open a second one.** Phase 9 set the contract and phase 5's lock implements it: one 10-second total deadline measured across all attempts, with exponential backoff from 50 ms inside it. Exit `6` is returned only once that whole deadline is spent, so a caller that sees `6` has no budget left to retry with — a second-layer loop just multiplies a ten-second wait by however many times it runs and turns a busy ledger into a hang. Treat `6` as the named exhausted outcome it is: report that the tool was busy and nothing was decided, name the command to run again, and let the person decide. Do not write a retry loop around a verb, and do not assert one in an acceptance gate.

**Acceptance gate:** No `verify.sh`. A dry run over a real todo phase invokes the shared `/sync` flow, claims the right paths, records the run and reservation in phase 11's session mapping, and releases at checkpoint leaving the reservation `Outstanding` and retiring that mapping; a forced collision pauses before dispatch and uses the same blocked/proposal/claimed transitions as interactive `/sync`, including stale-versus-malformed handling; a claim held off by a busy lock produces exactly one dispatcher invocation — the engine's own lock attempts stay inside the single 10-second deadline, no phase is dispatched, and the user receives an actionable message naming the command to run again — and the dispatcher is asserted never to reinvoke the verb; two phased claims made under **distinct** harness session ids each map to their own reservation and each `drift` resolves unambiguously, while a second claim made under the same session id is shown to replace the first in the mapping — which is why the dispatcher's own `ActivePhaseReservation` record, not the mapping, is what the release argument comes from; a checkpoint taken after the orchestrator's context is discarded still releases the right reservation by reading that record; and the pre-checkpoint check runs the full comparison against the recorded phase-start `HEAD`. While coverage mode is advisory, a Phase 20 Work Order with no `**Reservations:**` block reports the missing declaration but remains dispatchable; required-mode refusal is activated and tested in Phase 21. Also: required reservation coverage is exercised in both an enrolled and an unenrolled repository — the first enforces, the second proceeds silently — while a repository whose ledger cannot be read stops the run rather than proceeding as if it were unenrolled. Also: the release order is asserted — full drift comparison, then a successful checkpoint commit, then `/sync release` — with the recorded `protected_tip` equal to that commit rather than to its parent. Also: each ending that is not a successful checkpoint is exercised separately — a cancelled no-diff phase, a run halted by an error, a failed checkpoint commit, a dirty `single` run, a user stop, and a failed release attempt — each leaving the reservation held and the `ActivePhaseReservation` record intact, with only a successful release deleting it.
...
Three have no `**Files:**` block and must be authored by reading the phase: **Tool Graph 60, 69, 70**. Tool Graph 60 falls further short of the standard shape — it carries no **Goal** and no **Spec** either — so normalize all three to the complete Work Order structure rather than bolting a reservations block onto an incomplete one. Phase 18's validator rejects a Work Order missing any of those fields (R35), so a backfill that added only reservations would leave these three failing the very check this phase exists to satisfy.
...
- `crates/cargo-berth/src/cli.rs`, `crates/cargo-berth/tests/drift.rs` — read only, in the engine checkout phase 17 recorded; `cli.rs:281` is the rendering split that guarantees a `--json` post-commit invocation returns its typed envelope rather than the stderr warning, and `drift.rs:490` is the shipped assertion that it also reports every active reservation. The `PostToolUse` scenarios below depend on both.

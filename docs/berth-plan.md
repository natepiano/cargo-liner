...
**Spec:** **Compile nothing.** Phase 11 already installed the verified binary and recorded its version and path; this phase invokes `cargo-berth` and never runs `cargo install`, `cargo build`, or anything else that builds. Nothing is published yet. **Confirm the executable is current before relying on it.** The installed binary corresponds to phase 11's checkpoint on branch `init/cargo-berth` in `/Users/natemccoy/rust/cargo-berth-init`, and phase 11 had to reinstall once because a repair landed in `src/cli.rs` after its first install left the executable printing superseded help. No track-B phase builds, so nothing here would reveal a stale executable. **Run the freshness preflight before the first invocation, and treat a mismatch as blocking.** `cargo-berth --version` reports `0.1.0-dev` for every build of this crate and therefore proves nothing, so compare `shasum -a 256 "$(command -v cargo-berth)"` against the SHA-256 phase 11 recorded. On a match, proceed. On a mismatch, run `cargo install --path crates/cargo-berth --force` from the engine checkout phase 11 names, re-read the digest, and record the new one in this phase's as-built notes so the phases after this one compare against a current value. That reinstall is the one build this phase performs and it exists to restore the artifact the plan already promised — it is not this phase implementing engine work.
...
**Acceptance gate:** No `verify.sh`, and no compile. `taplo fmt --check .claude/config/berth.toml` passes; `cargo-berth board --json` runs against the empty ledger and its envelope carries the exact frozen fields `status = "board_ready"`, `payload.kind = "board"`, and `payload.data.integration_order = "undeclared"` — assert those three, not prose. `"no integration order declared"` is the terminal board's rendering of that state and is neither the serialized value nor the envelope message, so a gate written against the sentence passes on nothing; `crates/cargo-berth/tests/board.rs` (`empty_board_is_headless_and_declares_no_integration_order`) is the shipped assertion to match. `init` manages two hooks by this phase and resolves the effective `core.hooksPath` rather than always writing into the common git directory, so assert that `init` reports both `reference-transaction` and `post-commit` as active in its `hooks` payload and that each managed marker is present in the effective hook directory git actually reads. A clean in-order `git merge` into `main` succeeds and is **silent** — the gate ships observe-only and has nothing to say when nothing is violated, so expecting output from a clear gate is expecting the wrong thing; observe-only violation logging is phase 17's to prove. Also assert that `.claude/config/berth.toml` is committed before any linked worktree is created.
...
- `crates/cargo-berth/src/output.rs`, `crates/cargo-berth/src/board/mod.rs`, `crates/cargo-berth/tests/board.rs` — read only, in the engine checkout phase 11 recorded; the frozen `board --json` contract `/sync board` renders.

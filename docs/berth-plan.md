...
A phase whose Work Order has no `**Reservations:**` block will become a hard stop with an actionable message, not a silent skip — a phase that claims nothing is invisible to every other worktree, which is the exact decay this design exists to prevent. **Phase 18 already installed complete Work Order validation in `<ComposeWorkOrder/>`; do not add another missing-declaration parser or check.** Replace its current config-presence-derived coverage selection with one named rollout policy set to **advisory**: `Declared` enters the shared claim lifecycle, while `Missing` reports that mandatory coverage is not active and continues without a claim. Phase 21 changes only that named policy to **required** after phase 20 validates the backfill; required mode refuses a missing block before dispatch.

**No live `hana` Work Order carries a declaration before phase 20, so the dry run cannot use one.** Exercise the claimed lifecycle in an enrolled scratch Git repository containing a repository-local scratch copy of one real todo Work Order plus the declaration emitted by `berth.work_order`; separately exercise an unchanged Phase 20 Work Order to prove `Missing` remains dispatchable under advisory coverage. Run the completed dispatcher proof in a fresh delegate session so it loads the edited command, and assert that the claimed footprint contains the declared scopes plus exact scopes for the scratch plan and its derived `-next.md` sibling.
...
- `crates/cargo-berth/src/gate/mod.rs`, `crates/cargo-berth/src/output.rs`, `crates/cargo-berth/src/verb/integrate.rs`, `crates/cargo-berth/tests/gate.rs` — read only; the inactive-identity path phase 18 typed, and the shipped gate assertions the scenarios must agree with.

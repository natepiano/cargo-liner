...
Surface: `claim <paths> --before|--after|--defer|--override <blocker> --overlap-why <text>`. **`--why` and `--overlap-why` are two different facts and both may be present.** `--why` is phase 3b's `ReservationPurpose` — why the paths are being protected — and it belongs to the claim whether or not an overlap is being answered. `--overlap-why` is R54's approved reason — why this specific overlap was accepted — required by, and only by, a permissive answer. Neither substitutes for the other.
...
The enclosing `Claim` or `Widen` already supplies the requester's identity, so the answer does not repeat it. Phase 3 shipped `AuthorizedOverlap { reservation_id, reservation_revision, scopes }`: each answer stores the **holder's** `ReservationId`, the holder's `ReservationRevision` at the moment the answer was given, and the **normalized overlap antichain at that moment** (R55) — no requester generation is stored, matching phase 2's ruled-out decision. Refine the two possibly-empty vectors before first use: `ConflictAuthorization` carries a non-empty `AuthorizedOverlapSet`, and each `AuthorizedOverlap` a non-empty `AuthorizedOverlapScopeSet`. Suppression covers only that recorded set.

**The recorded set is the exact intersection, not the holder's scopes.** Phase 3b's `ReservationConflict::overlapping_scopes` collects each *whole held scope* that intersects the request, so a holder of `tree:crates` conflicting with a request for `crates/a/lib.rs` reports `tree:crates` — authorizing that set would suppress the holder's entire tree on the strength of one file the user was shown. Compute the intersection of the two scope sets instead: for each conflicting pair, the narrower of the two scopes, reduced to a minimal antichain, and non-empty by construction. `AuthorizedOverlap::scopes` is currently `Vec<ReservationScopePath>` and drops `ScopeKind` entirely, which would let an authorized *file* replay as covering a *tree*; it carries complete `ReservationScope` values so the kind survives replay. Both runs receive only the authorization actually shown. A widen recomputes the intersection and re-blocks anything uncovered. An answer is never transitive to a third reservation, and a new `ReservationId` never inherits one.
...
- `crates/cargo-berth/src/verb/claim.rs` — **exists**; the four flags already reach it and are discarded during request conversion. Carry them into the transaction instead of extending the surface.
- `crates/cargo-berth/src/reservation/mod.rs` — **exists**; the exact-intersection computation beside the conflict construction that currently reports whole held scopes.
- `crates/cargo-berth/src/verb/check.rs` — **exists**; a check must not report a path an answer already covers as blocking.
...
- `crates/cargo-berth/src/main.rs` — **exists**; owns the `mod` list, which `mod answer` is added to.
- `crates/cargo-berth/tests/answers.rs`

**Constraints from prior phases:** Phase 3's validating transaction is where the payload lands — extend it, do not add a second path. Phase 3b's antichain reduction computes the recorded overlap set. Exit `3` from phase 1 is needs-authorization. **Collapse clap's option bag at the boundary.** Phase 1 shipped `claim`'s resolution as four mutually exclusive `Option<ReservationId>` fields guarded by a clap `ArgGroup`, plus a separate optional reason — a shape that lets the parser hand the engine four `None`s, or an `Override` with no reason, and forces every consumer to re-derive which answer was actually given. Convert it once, where clap's types end, into one semantic authorization request that makes the direction and its required reason inseparable; the engine sees only that type. Do the same for `Reason` and `Timestamp` if they appear here — name them for the role they play (why an overlap was authorized; when a journal event occurred), not for what they are made of. **`--why` already means something else and must keep meaning it.** Phase 3b refined it into `ReservationPurpose::{Explained, NotProvidedByCaller}` — why the paths are being protected — while R54's approved reason is why an *overlap* was accepted. They are different facts recorded in different places; derive the authorization reason as its own type rather than overloading the purpose, and require it wherever a permissive answer is given. The escalation payload names both plans and phases through phase 3b's typed `ClaimSource`, which has a variant for a claim made with no plan or phase at all — read that variant rather than assuming every holder has provenance. Phase 2 already ships `ConflictAuthorization`, `AuthorizedOverlap`, and `OrderingDirection`, and every verb-keyed `OutputPayload` variant exists as `PendingPayload`. Move, expose, or refine those definitions; do not introduce a second one alongside. **`ReservationRevision` is the wrong thing to bind an authorization to.** Phase 4 advances it on renewal, checkpoint, release, rebind, and evidence-only revalidation, so binding an answer to it expires a permission the user already granted every time an unrelated revalidation touches the holder — and R54 makes re-granting it expensive, because it costs a human turn. Bind the answer to a revision that changes only when the holder's authorization-relevant scopes change: derive an `OverlapScopeRevision` from the holder's scope set and store that beside the recorded antichain. The general revision stays what it is and remains available for proposal freshness; it simply is not what suppression is keyed on. **Two reasons, two flags — the frozen surface has only one.** Phase 1's `claim --why` maps to phase 3b's `ReservationPurpose`, so R54's authorization reason has no flag at all and cannot be supplied; the surface line above is corrected accordingly. Add `--overlap-why <text>` as its own argument, required by every permissive answer and meaningless without one, and convert the pair at the clap boundary so the semantic request carries the protection purpose and the authorization reason as two distinct values. The expansion is additive and phase 1's frozen-surface tests extend to cover it. **Phase 5 changed what `claim` does before it resolves anything.** A claim now reconciles first, resolves its acting run against the locked replay, and attaches any reconciliation alerts to its own envelope; a `Marker`-derived identity is honored only when that replay shows the same run still holding an active reservation minted in that same worktree. Preserve all three when the authorization payload joins the transaction — the payload rides the existing transaction and never gets a path that skips reconciliation or re-resolves identity.

**Acceptance gate:** `verify.sh test cargo-berth` and `verify.sh test cargo-berth answers` both green, including: answering a first collision succeeds in one transaction and a crash between validate and append leaves no reservation and no answer; a widen into a path the answer never covered re-blocks; an answer between A and B does not authorize A against C; `Defer` permits both edits and blocks both integrations; a holder of `tree:crates` answered against a request for one file inside it has only that file suppressed, and its remaining paths still block; an authorized file scope replays as a file scope and does not suppress its siblings or its parent tree; a `check` for an answered path reports clear while an unanswered path under the same holder still blocks; a permissive answer given with no reason is refused; every permissive answer returns exit `3` with the full escalation payload — process exit status, envelope `exit_code`, semantic status, and the user-facing recovery message all agreeing. Also: a holder renewed, checkpointed, or revalidated after an answer was given still has that answer honored, while the same holder widening its scopes invalidates it and re-blocks. Also: a permissive answer supplying only `--why` is refused as a usage error while one supplying both records the protection purpose and the authorization reason as separate facts that both survive replay; and a claim carrying an authorization payload still reconciles first, still refuses a stale marker identity, and still reports any reconciliation alert on its own envelope.

**Pending decision: how a permissive answer actually gets the user's authorization**

Actual problem:
R54 requires every permissive answer to be authorized by the user, and the surface as frozen cannot enforce it. `cargo-berth claim <paths> --override <blocker> --why "..."` is a single command that both proposes the answer and applies it. Either that one invocation is sufficient — in which case an agent authorizes its own block and R54 is violated — or it returns exit `3`, and phase 14's skill re-invokes exactly the same command with nothing added, which either loops forever or is bypassed by whatever makes the second attempt succeed.

What exists now:
- `src/cli.rs` — `claim` accepts `--before|--after|--defer|--override <blocker>` with `--why`, in one shot, with nothing distinguishing a proposal from an approved application.
- Phase 6's spec — the answer must ride the same locked transaction that discovered the block (R53), so a separately appended approval is not an option.
...
Why the recommended answer does not close this on its own:
A proposal token is returned to the same caller that asked for it, over the same channel. Nothing about holding one proves a human ever saw the conflict — an agent can make the first call, read the token out of its own exit-`3` payload, and make the second call, entirely unattended. The token binds an authorization to a *specific conflict*, which is genuinely worth having and is what R55 already records; it does not bind it to a *person*. So it fixes the transitivity and staleness problems and leaves R54 exactly where it was.

Closing it requires naming a boundary the requesting agent cannot stand on both sides of. Three shapes exist, and they differ in what gets built:
- **Out-of-band approval.** The escalation goes somewhere the agent cannot read from, and the approval comes back through a channel the agent cannot write to. Strongest, and the only one that actually satisfies R54 as written; it means `cargo-berth` grows a notion of an approval channel, which today it has none of.
- **Trusted intermediary.** Phase 14's skill is treated as the trusted party — it is the thing that talks to the user — and it holds a capability the engine checks. This moves the trust boundary into the harness rather than the tool, and is only as strong as the harness.
- **Weaken R54 deliberately.** Accept that the tool records who authorized what and cannot enforce it, make the answer loudly auditable — it is in the journal, it names the reason, the board shows it — and say so plainly in the README the way the enforcement gap is already stated there. This is honest, cheap, and consistent with how the design already treats the edit-hook gap.

Recommendation:
Take the proposal/apply protocol **and** decide which of the three above it sits inside — the token is a necessary component of every one of them and is not sufficient for any. Whichever is chosen, the acceptance test is the same and must exist: a bare `cargo-berth claim --override` driven from a shell, with no human in the loop, cannot land an authorized overlap.

Whoever answers this is choosing how much enforcement `cargo-berth` claims to provide, which is a product decision rather than an implementation one, and it also determines what phase 14's skill and phase 11's README have to say. It cannot be resolved by the implementation.
...
Edge record: its own `EdgeId`, `before: ReservationId`, `after: ReservationId`, the validated non-empty overlap scope set, reason, and its journal `EventId`. The reason is a role-specific non-empty type — why this ordering was declared — minted at the boundary and never a bare `String`; this is its first use, so the shape is settled here rather than migrated later. **`EdgeId` and `EventId` are not the same thing and this is the phase that makes the difference real** — phase 1 froze `EdgeId` with no consumer, and identifying an edge by the event that happened to record it breaks the moment `sequence` changes an existing answer, since the edge persists across the new event. `EdgeId` is what the board renders, what a duplicate check compares, and what a bypass record names.

`sequence <first> <then> --why` changes an answer already given; it never creates the first one (that is phase 6). **Turning an existing `Defer` into an ordering edge is a journal operation in its own right** — phase 2's union carries it — because replay must be able to tell a deferral that was later ordered from one that was ordered from the start, and the deferral's hold on both integrations lifts only when the edge exists. **No `edge_satisfied` or `edge_dissolved` journal variant** (R60, R68) — status is derived on every read. **There is no `Integrated` lifecycle state and this phase must not assume one.** Phase 4 shipped exactly three — `Active`, `Outstanding`, `Released` — and integration is a separate typed fact carried beside them: `IntegrationEvidenceStatus` on an `Outstanding` reservation's evidence, and `ReleaseDisposition::{Integrated, RewrittenIntegration}` on a `Released` one. Derive status into a named edge-readiness enum rather than a tuple of those facts read at each call site, with one variant per row below and one acceptance case per variant:

- predecessor `Active` → awaiting checkpoint; holds unconditionally, because it has no protected tip yet
- predecessor `Outstanding`, protected tip not yet reachable from the successor → holds
- predecessor `Outstanding` or `Released { Integrated | RewrittenIntegration }`, protected tip reachable from the successor → met, **revalidated on every check** (R41), so a trunk rewrite returns it to holding
- predecessor `Released { Abandoned | RetiredOrphan }` → cancelled; both require user confirmation (R28), and neither is ever reached automatically
- predecessor's protected tip already an ancestor of the successor's `HEAD` → fulfilled, drops from the active graph

**The subject of the reachability question is the successor's `HEAD`, not trunk** (R57, R69): the successor is held until it has actually incorporated the predecessor's protected tip, so an edge whose predecessor merged to trunk still holds a successor that has not merged trunk back in. `IntegrationEvidenceStatus::ObjectUnknown` — the state phase 4 produces when git cannot resolve an oid — is a hold, never a met edge; an unresolvable object is missing evidence, not absent obstruction.
...
- `crates/cargo-berth/src/cli.rs` — **exists**; dispatch `sequence` into the engine.
- `crates/cargo-berth/src/output.rs` — **exists**; the edge payload variant.
- `crates/cargo-berth/src/ids.rs` — **exists**; `EdgeId::new` carries the dead-code expectation this phase retires.
- `crates/cargo-berth/src/git/mod.rs` — **exists**; `delete_reservation_retention_ref` carries the other dead-code expectation this phase retires.
- `crates/cargo-berth/src/ledger/journal.rs` — **exists**; `JournalEvent::event_id` is not reachable from outside `ledger`, and the ordering-edge reason below.
- `crates/cargo-berth/src/main.rs` — **exists**; owns the `mod` list.
- `crates/cargo-berth/src/reconcile.rs` — **exists**; `append_evidence_and_retention` repairs every retained reservation's retention ref and must stop repairing one whose dependents are all terminal.
- `crates/cargo-berth/src/verb/mod.rs` — **exists**; owns the verb module list, which `sequence` is added to.
- `crates/cargo-berth/src/config.rs` — **exists**; `maximum_reservations` and `maximum_ordering_edges` are parsed here and have no consumer yet.
- `crates/cargo-berth/src/reservation/mod.rs` — **exists**; the claim path is where the reservation limit is enforced.
- `crates/cargo-berth/tests/edges.rs`

**Constraints from prior phases:** Phase 6 creates edges as a `ConflictAuthorization::Sequence` payload — this phase reads them, it does not add a second creation path. Phase 4's `protected_tip` and revalidation rule decide every status. **Phase 5's `WorktreeLiveness` cancels nothing, and conflating the two is this phase's easiest mistake.** Only a user-confirmed terminal disposition — `Released { Abandoned | RetiredOrphan }` — cancels an edge. `Unavailable`, `OrphanCandidate`, and `Unknown` are reversible observations about a worktree, not verdicts about a reservation: they retain the scopes and the edges and change nothing about readiness. `Orphaned` is reversible too, since phase 5's `resolve --recovered` rebinds the reservation to a live worktree, so an edge held by an orphaned predecessor returns to its ordinary derived status the moment recovery lands and must never have been dropped from the graph. Keep the two facts in separate types: the derived edge-readiness enum answers whether the edge is met, held, cancelled, or fulfilled, and liveness is a separate per-holder observation the board renders beside it. No edge-readiness variant is ever computed from liveness. Phase 1 froze `EdgeId`; this phase is its first consumer. The dead-code expectation is **not** in `main.rs` — it is generated onto `EdgeId::new` by the `uuid_identifier_minter!(future …)` macro in `src/ids.rs`, and that is what this phase retires. An edge also records the journal `EventId` that declared it, and `JournalEvent::event_id` is not reachable from outside the `ledger` module today; widen exactly that one accessor rather than exposing the event type. Phase 2's journal already carries a `DeclareOrderingEdge` operation. Either this phase adopts it as the single creation path, or it removes it — the constraint that there is no second way to create an edge has to be true of the shipped union, not just of this phase's new code. **This phase also owns retention-ref deletion.** Phase 4 writes a reservation's retention ref and never deletes one, because the ref must outlive the release until every dependent successor is terminal — which is a fact only this phase's graph knows. `git::delete_reservation_retention_ref` is shipped and carries a dead-code expectation this phase retires; call it from the post-append committed-action stage of the transaction that makes the last dependent successor terminal, never from `release` and never outside the lock. Phase 5 shipped `WorktreeLiveness` as `Live | Unavailable | OrphanCandidate | Orphaned | Unknown`, and **everything except `Live` retains the reservation's scopes and edges** — a cancellation decision reads that enum, it does not treat a missing worktree as a freed one. **Phase 5's `reconcile` rewrites retention refs, so deleting one is not enough.** It pushes a retention repair for every retained reservation reaching `Outstanding` or `Released`, including a `Released` one whose disposition revalidates nothing, and rewrites the ref from the recorded `protected_tip`. A deletion this phase performs is therefore undone by the next reconcile unless this phase also teaches `reconcile` to stop repairing a reservation whose dependent successors are all terminal. Change both together, in `src/reconcile.rs`'s `append_evidence_and_retention`, or the ref outlives the graph fact that was supposed to retire it. **R4's limits are shipped as config with no consumer.** `maximum_reservations` and `maximum_ordering_edges` are read from `berth.toml` today and enforced nowhere, so extending R4's limits to `V` and `E` means writing the first enforcement of both: the reservation count checked on the claim path, the edge count inside this phase's locked graph mutation, each rejecting with its own typed limit outcome rather than a generic usage error. **One repository snapshot is what makes this phase's cost budget reachable.** Phase 5's `ReconciliationReport` returns alerts plus only the evidence it changed, discarding every liveness observation it made, and it resolves trunk separately for each retained reservation — so readiness built on it pays per-reservation git cost however the grouping is written. Introduce one semantically complete repository snapshot, taken once under the lock, carrying each holder's liveness, each retained reservation's evidence status, and one resolved trunk; edge readiness consumes it here and phase 10's board consumes the same value. The counted-call proof belongs to phase 10, the first phase with a board to count. **`sequence` is a stateful verb and inherits `claim`'s shipped protocol.** Phase 5 made a mutating verb reconcile first, resolve its acting run against the locked replay, and attach reconciliation alerts to its own envelope, honoring a `Marker`-derived identity only when that replay shows the same run still holding an active reservation minted in that same worktree. `sequence` does the same, and a stale marker never becomes its actor.

**Acceptance gate:** `verify.sh test cargo-berth` and `verify.sh test cargo-berth edges` both green, including: concurrent `A → B` and `B → A` declarations produce exactly one edge; a trunk rewrite that removes the predecessor returns a met edge to pending and re-holds the successor; duplicate edges are rejected; the repository snapshot is taken once per readiness computation and every edge status derives from it with no further git call; converting a `Defer` to an ordering edge is replayable and distinguishable from an edge declared outright; `sequence` no longer returns `unimplemented` and its exit status, envelope `exit_code`, and semantic status agree. Also: a released predecessor with a still-pending dependent keeps its retention ref, and the ref is deleted once the last dependent reaches a terminal state; an edge whose predecessor reached trunk still holds a successor that has not incorporated it; and `ObjectUnknown` evidence holds rather than satisfying an edge. Also: a retention ref deleted once the last dependent successor is terminal is still absent after a further reconciliation, while an outstanding reservation's ref is still repaired by that same pass; a predecessor whose worktree is `Unavailable`, `OrphanCandidate`, or `Unknown` cancels no edge and holds its successor exactly as a `Live` one does, and an `Orphaned` predecessor recovered through `resolve --recovered` returns its edge to ordinary derived status; a claim exceeding `maximum_reservations` and an edge declaration exceeding `maximum_ordering_edges` are each rejected with their own typed outcome; and `sequence` reconciles before resolving its actor, refuses a stale marker identity, and carries any reconciliation alert on its own envelope.

**Losing the middle node**, per R59 and R61: in `A → B → C`, losing `B` terminates `A → B` and **never synthesizes `A → C`** — but `B → C` is resolved from `B`'s own evidence, and an orphaned `B` with no terminal evidence is not terminal, so **`B → C` is retained and `C` keeps waiting**. A missing worktree is `Orphaned`, never `Abandoned`, and abandonment requires user confirmation (R28). Assert both halves: no synthesized `A → C`, and `C` still held by the orphaned `B`; `C` clears only once `B` reaches a user-confirmed disposition or its evidence lands.
...
**Derived edge status is not the only deny input.** A `Defer` (phase 6) holds **both** reservations at integration until an order exists — that is what makes it not an `Override` with a nicer name — and a deferral records no edge, so a gate that reads only the edge graph lets both halves of a deferred conflict land in whatever order they arrive. Evaluate both: unsatisfied predecessors from phase 7's derived status, **and** any unresolved `Defer` naming this reservation.
...
1. `CARGO_BERTH_BYPASS=1` is read **before the ledger, the projection, or anything else**. It records no reason, which makes it the one release valve the board cannot explain after the fact; the journalled record carries a typed bypass cause naming at minimum that it was the environment override, so a reader can tell it from a `--force` permit without comparing absent fields. A corrupt journal, a stuck lock, a hook timeout, or a bug in the gate can never block anyone. Journalled when the journal is writable; reported at next SessionStart when it was not.
...
- `crates/cargo-berth/src/ledger/journal.rs` — **exists**; the bypass and forced-integration records this phase must retype before their first use.
...
- `crates/cargo-berth/src/main.rs` — **exists**; owns the `mod` list, which `mod gate` is added to.
- `crates/cargo-berth/src/verb/mod.rs` — **exists**; owns the verb module list, which `integrate` is added to.
- `crates/cargo-berth/tests/gate.rs`

**Constraints from prior phases:** Phase 7's derived edge status and phase 6's unresolved deferrals are together the input to the deny decision. Phase 2's ledger location resolves the common dir, and its `gate_mode` config field selects observe versus enforce. Exit `2` is edge-blocked and exit `4` is ledger-unreadable — `integrate` must return `4` and refuse where an edit path would proceed. Phase 4 adds exit `6`, a transient lock contention that the caller should retry; it is **not** an unreadable ledger and must not take the fail-closed path, and the gate's acceptance matrix states an outcome for every code it can observe rather than treating "not zero" as one case. **Every consumer of a mutating verb needs a bounded retry and a named exhausted outcome, and this phase sets the contract the later ones follow:** one 10-second total deadline measured across all attempts, with exponential backoff from 50 ms inside it, and an exhausted deadline that is its own outcome — neither a clear nor an unreadable ledger — reported with the command to run again. A retry loop with no ceiling turns a stuck lock into a hang, and no retry at all turns ordinary concurrency into a false verdict. Phase 1 shipped `integrate` with `--force` and `--why` as a `bool` plus an `Option<String>` bound by clap `requires`; convert them at the boundary into one integration request whose forced variant carries its reason inseparably, so no consumer can hold a force with no reason or a reason with no force. Phase 2's forced-integration permit is identified by a bare `EventId` and records nothing about what was skipped, while this phase and the board both need the exact bypassed edges. Give the permit its own semantic identifier and a non-empty set of skipped edge ids, and prove one-use consumption survives replay. **The shipped journal shapes do not yet support the typed contract above and must be retyped before their first use**, because the journal is append-only and the first bypass freezes them in: `Bypass` carries `{ action, reason: String }`, which cannot express an environment override that records no reason, and the forced-integration record stores its reason as a bare `String`. Replace them with a `BypassCause` that names the environment override as its own variant — carrying no reason field at all rather than an empty one — and a non-empty `ForcedIntegrationReason` minted at the clap boundary. This phase also owns the only route back from a corrupt ledger: a confirmed reinitialization through the existing `init` verb behind an explicit flag — not a new repair verb — plus a durable marker written when the journal was unwritable, so the bypass is still reportable at the next session start after a restart. **Phase 5 shipped `init --repair-projection` as a mutually exclusive branch that never initializes and never rewrites configuration**, so confirmed reinitialization is the third thing `init` can be asked to do, not a second boolean beside it. Replace the representation-named `ProjectionRepairRequest` with a three-way initialization request — ordinary initialization, projection repair, confirmed reinitialization — mutually exclusive by construction at the clap boundary, each branch stating exactly what it touches: ordinary initialization writes configuration and creates what is missing, repair rewrites only the projection from journal truth, reinitialization discards journal state after confirmation. **Phase 5 also shipped the way out of the reblocked-release corner.** `ReplaceReleaseDisposition` lets a released reservation whose evidence a trunk rewrite invalidated accept `resolve --integrated-as <trunk-oid>` and reach a clean disposition again without rewriting history, so the denial matrix names it as the ordinary recovery from a reblocked release — it is not a force, a bypass, or a reinitialization, and a user offered only those three would rewrite history to escape a case that has a clean answer. **One total deadline, not five multiplied attempts.** Every mutating verb already waits internally for the lock before returning exit `6`, so a consumer retrying five times pays that wait five times over and turns a busy ledger into a half-minute hang on a path that must stay interactive. The retry contract every later consumer inherits is therefore a single total deadline measured across all attempts — 10 seconds — with exponential backoff from 50 ms inside it and an exhausted-deadline outcome of its own, distinguishable from both a clear result and exit `4`. Phases 12, 13, 14, and 15 inherit that deadline, not an attempt count. **`integrate` is a stateful verb and inherits `claim`'s shipped protocol** — reconcile first, resolve the acting run against the locked replay, attach reconciliation alerts to its own envelope, and honor a `Marker`-derived identity only when that replay shows the same run still holding an active reservation minted in that same worktree; a stale marker never becomes its actor and never earns its exemption.

**Acceptance gate:** `verify.sh test cargo-berth` and `verify.sh test cargo-berth gate` both green, the latter in a scratch repo with two worktrees, including: observe-only permits and logs; enforcing denies an out-of-order merge and permits an in-order one; a `Defer` with no edge holds **both** integrations; `CARGO_BERTH_BYPASS=1` succeeds with a deliberately corrupted `journal.ndjson`; a `--force` permit is consumed exactly once; `integrate` fails closed on a missing ledger while `check` reports exit `4` with no facts and the edit consumer deliberately treats that as permission to proceed — phase 3b's `check` does not invent an all-clear it cannot prove, so "fail open" is a property of the *caller*, not of the verb; installing twice does not duplicate the hook and an existing unrelated hook is preserved. Exits `2` and `4` are asserted end to end — process status, envelope `exit_code`, semantic status, and the user-facing recovery message all agreeing — and `integrate` no longer returns `unimplemented`. Also: a forced permit names every edge it skipped and their predecessors, and replay reproduces both; confirmed reinitialization restores service on a corrupt ledger and says what it discarded; and a bypass taken while the journal was unwritable is still reported after a process restart. Also: an environment-override bypass replays as its own typed cause with no reason field to leave empty, and a `--force` permit replays with its non-empty reason; a mutating verb held off by a busy lock retries and succeeds, while one held off for the full budget reports the exhausted-retry outcome and is distinguishable from both a clear and exit `4`. Also: each of the three `init` branches is exercised and its exact effect on `journal.ndjson` and on the configuration file asserted — ordinary initialization idempotent, repair leaving the journal byte-identical, reinitialization discarding only after confirmation — and supplying any two together is a usage error; a released reservation reblocked by a trunk rewrite clears the gate through `resolve --integrated-as` with no force and no bypass; the retry budget is one 10-second total deadline, proven by a verb held off for the whole of it reporting the exhausted-deadline outcome with elapsed time that does not scale with the attempt count; and `integrate` reconciles before resolving its actor and refuses a stale marker identity.

**Pending decision: does a general user get an automatic commit-time drift check, or a manual one?**

Actual problem:
Phase 11 requires the README to tell a stranger that blocking the edit itself is a Claude Code hook and outside this tool, and that "a general user gets a commit-time drift check instead — automatic, but later than blocking the keystroke." No Work Order builds that. This phase installs a `reference-transaction` hook, which fires on ref updates and governs merges; phase 9 implements the `drift` verb but nothing invokes it; phase 13's `PostToolUse` shim calls it only inside Claude Code, which is exactly the audience the README sentence excludes. As the plan stands, the README would promise an automatic check that does not exist for the person it names.

What exists now:
- Phase 8 — `cargo-berth init` installs one hook, `reference-transaction`, into the common git directory. Nothing installs a commit-time hook.
- Phase 9 — `drift` is a verb a caller must run. Its only automatic caller is phase 13's `PostToolUse` shim.
- Phase 11 — the README paragraph quoted above, under "the honest limitation, stated plainly and not buried".
- The design section `#### The hook` describes the `PreToolUse` gap and says tier 3 covers it, but never commits to a commit-time hook for anyone.

What should change, one of:
- **Build it.** `init` also installs a `pre-commit` or `post-commit` hook that runs `drift` and reports incursions, making the README sentence true. New scope: a second hook to install idempotently, a decision about whether it blocks or only reports, and the same never-trap-anyone release valve this phase already owes the trunk gate.
- **Withdraw the promise.** The README says a general user runs `cargo berth drift` themselves, and the enforcement gap is stated as wider than currently written. No new code; the product claims less.

Recommendation:
Withdraw the promise. The design's own stance is that a coordination tool overselling its enforcement is the failure it exists to avoid, and an automatic commit-time hook that only reports — which is all it could safely do, since blocking a commit for drift would trap people — is a notification, not enforcement. It also lands a second installed hook in the phase already carrying the trunk gate's install, idempotence, and bypass burden. Manual `drift` plus an honest README costs nothing and claims exactly what is true.

This is a product decision about what `cargo-berth` promises a non-Claude-Code user, and it changes phase 11's README text either way, so it cannot be settled by the implementation.

### Phase 9 — Drift detection  · status: todo

#### Work Order

**Goal:** In `cargo-liner`: `cargo-berth drift` reports what changed against what was claimed, classifying each result the way D1 requires.

**Spec:** Implements tier 3 and the D1 resolution — read `### D1 — RESOLVED` in `docs/berth-plan.md` (this plan).

**`drift` is a new verb and phase 1 did not freeze it.** Phase 1 shipped seven verbs; `drift` is not among them, yet this phase promises it and phase 13's `PostToolUse` shim invokes it. It cannot fold into an existing verb — it mutates through the widen path, which `check` must never do, and it answers a question about the worktree rather than about a proposed claim. Add `Drift` to the `CommandVerb` enum and the clap `Subcommand`, and extend phase 1's frozen-surface tests to cover it: it parses under both `cargo berth drift` and `cargo-berth drift`, and its verb name renders identically through both encoders in `src/output.rs` if that hand-written encoder still exists. The expansion is additive, so every track-B parser written against phase 1 keeps working.
...
Classification of the changed set. **"Live" is not the test and must not be used here** — phase 4's replay retains released reservations rather than dropping them, and a `Released { Integrated }` reservation returns to blocking when a trunk rewrite invalidates its evidence. The subject is every retained reservation whose materialized `EditBlockingStatus` is currently blocking, whatever its lifecycle; reading lifecycle instead would auto-widen straight over a reblocked foreign holder and call it unclaimed:

| Changed paths | Result |
| --- | --- |
| covered by this worktree's active reservation | silent; update the stored fingerprint |
| covered by no reservation that currently blocks edits | auto-widen through the locked overlap transaction; report the widen |
| covered by a **foreign** reservation that currently blocks edits | **incursion** — record, surface, tell the caller to stop |
| would collide on widen | report the collision; do not widen |

An incursion appends `incursion { reservation_id, foreign_reservation_ids, paths, at }` and gets its own board state. This is that record's first use, so its two vectors are settled here rather than migrated later: the foreign reservations and the incursion paths are each their own non-empty set type, because an incursion naming no foreign holder or no path is not an incursion. **These are two different computations and the plan has been fusing them.** The four-command block above is R46's complete comparison against the phase-start baseline: it is what `/sync check` (phase 14) and the pre-checkpoint check (phase 15) run, it costs what four `git diff` invocations cost, and it never touches the hook path. D1's post-`Bash` check is a different contract — `git status --porcelain` plus `git ls-files --others --exclude-standard`, no lock, no `cargo metadata`, no `rev-list`, budget ~0.02s — and it answers only whether anything changed since the last observation. Name the two separately, a full comparison and a cheap delta, and give the verb a flag selecting which runs, defaulting to the cheap one, so no caller has to know which commands each implies. The cheap path needs somewhere to remember its last observation: keep it in a disposable per-worktree cache file beside the ledger, never in the journal, because it is derived working-tree state and losing it costs one full comparison rather than any correctness. It is rewritten on every silent and every auto-widen result, and a missing or unreadable cache falls back to the full comparison rather than reporting no change.

A widen re-evaluates every existing answer against the new scopes (phase 6) — an answer never covers a path it was not shown.

**`drift` must name the reservation it acts on.** `Incursion` and `Widen` both require one `ReservationId`, and `EditAuthorization` identifies a run and a worktree, not a reservation — while phase 3b permits several active reservations sharing one worktree and one coordination run. R24 already supplies the rule: bind the operation to an explicit reservation, and allow implicit selection only when exactly one active reservation matches the acting identity. Add `--reservation <id>`, select implicitly when exactly one candidate exists, and make an ambiguous selection a usage error naming every candidate rather than silently picking one.

**Files:**

- `crates/cargo-berth/src/drift/mod.rs` — fingerprint, classification, incursion records.
- `crates/cargo-berth/src/verb/drift.rs`
- `crates/cargo-berth/src/ledger/journal.rs` — **exists**; the reshaped `Widen` record the Constraints below require.
- `crates/cargo-berth/src/reservation/mod.rs` — **exists**; replay currently hard-codes every widened scope as `ScopeKind::File`, which is the other half of that reshaping.
- `crates/cargo-berth/src/cli.rs` — **exists**; the new `Drift` variant, its dual spelling, and dispatch.
- `crates/cargo-berth/src/output.rs` — **exists**; the new `CommandVerb::Drift` and the drift/incursion payload variants.
- `crates/cargo-berth/src/main.rs` — **exists**; owns the `mod` list.
- `crates/cargo-berth/src/verb/mod.rs` — **exists**; owns the verb module list, which `drift` is added to.
- `crates/cargo-berth/tests/drift.rs`

**Constraints from prior phases:** Phase 6's scope binding must be re-run on every widen. Phase 3b's antichain reduction normalizes the widened set and its `--head` flag supplies the phase-start snapshot this fingerprint diffs against; phase 3 stores that snapshot as `ProtectedPhaseStartHead`. **`JournalOperation::Widen` must be reshaped before its first use.** Phase 3 shipped it with `added_scopes: Vec<ReservationScopePath>`, which loses the `ScopeKind` the claim record carries and permits an empty domain-owned set, and `WidenCause::Drift` duplicates the same paths a second time. The journal is append-only, so the first drift widen would freeze both defects in. Replace `added_scopes` with a non-empty `ReservationScopeAdditionSet` of complete `ReservationScope`s — drift-created entries are `ScopeKind::File` — and reduce `WidenCause` to the cause-specific evidence alone, with no second path vector. **Both sides of that change are this phase's.** Phase 3b's `RetainedReservationSet::replay` reads `added_scopes` and assigns `ScopeKind::File` to every entry, so reshaping only the record would leave replay discarding the kind it now carries; a widened *tree* scope would replay as a file scope and silently stop blocking its descendants. Fix the record and its replay together. Phase 4 established the shape to follow: a record that carries a conclusion a git-free reader needs stores that conclusion, not only the inputs it was derived from — `JournalOperation::EvidenceRevalidated` carries `edit_blocking_status` beside `IntegrationEvidenceStatus`, and replay assigns it rather than recomputing it. A widen that changes what blocks must record the resulting edit-blocking state the same way. `WidenCause::Explicit` also still carries an unconstrained `String`; give it a non-empty `ExplicitWidenReason` minted at the clap boundary, rejecting empty input there, so the reshaping this phase already owns leaves no bare string behind. Phase 2's typed payload carries the classification back to the hook — the shim reads structured fields, never prose. `output.rs` no longer has a hand-written encoder — phase 2 deleted `unimplemented_json` and the test pinning the two encoders together when a real payload replaced the placeholder. Extend the frozen-surface tests through serde alone. **The phase-start baseline this fingerprint diffs against survives the journal but not replay.** `JournalOperation::Claim` stores `ProtectedPhaseStartHead`, and phase 3b's `RetainedReservationSet::apply_claim` drops it when it builds the `Reservation`, so a drift run against a rebuilt projection has no baseline to diff from. Carry it through replay onto the reservation before the first full comparison depends on it. **Phase 5's marker-derived identity cannot pick a reservation** — see the `--reservation` rule in the Spec above; `EditAuthorization` answers who is acting, never which of that actor's reservations is the subject. `drift` is otherwise a stateful verb and inherits `claim`'s shipped protocol: reconcile first, resolve the acting run against the locked replay, attach reconciliation alerts to its own envelope, and honor a `Marker` identity only when replay shows that run still holding an active reservation minted in that same worktree.

**Acceptance gate:** `verify.sh test cargo-berth` and `verify.sh test cargo-berth drift` both green, including one scratch-repo case per table row, an untracked new file counting as changed, a widened tree scope surviving journal replay from a deleted projection as a tree scope and still blocking its descendants, a widened file scope replaying as a file scope, a timed assertion that the fingerprint path makes no `cargo metadata` or `rev-list` call, and the phase-1 surface tests extended to `drift` under both spellings. `drift` returns a real status, never `unimplemented`, with process status, envelope `exit_code`, and semantic status agreeing. Also: a foreign `Released { Integrated }` reservation returned to blocking by a trunk rewrite produces an **incursion** rather than an auto-widen; and an explicit widen with an empty reason is rejected at the boundary while a supplied one survives replay. Also: the cheap delta and the full comparison are separately exercised, the cheap one asserted to issue only its two commands and the full one its four; a deleted or corrupt fingerprint cache falls back to the full comparison rather than reporting no change; a worktree holding two active reservations for one run makes a parameterless `drift` a usage error naming both, while `--reservation` selects one and a single-reservation worktree still resolves implicitly; and a reservation replayed from a deleted projection exposes the same `ProtectedPhaseStartHead` the claim recorded.

### Phase 10 — The board  · status: todo

#### Work Order

**Goal:** In `cargo-liner`: `cargo-berth board` shows integration constraints truthfully, and `--json` gives the same content to a machine.
...
Also per-reservation: holder worktree, branch, plan, phase, scopes, `ReservationLifecycle`, current `IntegrationEvidenceStatus`, `WorktreeLiveness`, its release disposition where it has one, and ahead/behind vs `main` computed live and never stored — four orthogonal facts, never collapsed back into one `stage`.

**Retained history is not board state, and this phase owns the projection between them.** Phase 4's `RetainedReservationSet` keeps released reservations indefinitely, so replay hands the board a mix of live work, cleanly finished work, and finished work that a trunk rewrite returned to blocking. Define a named board-visibility type that says which section each retained reservation belongs to — never an `Option<ReleaseDisposition>`, which answers "is there a disposition" when the question is "where does this belong". A cleanly released reservation moves to a **Resolved** audit section, out of the active constraints; a released reservation whose materialized `EditBlockingStatus` is blocking again re-enters the active constraints and is labelled as reblocked, because it obstructs exactly like a live one.
...
- `crates/cargo-berth/src/alert.rs` — **exists**; `Alert::OrphanedOutstanding`, `BranchRefStatus`, `ObjectAvailability`, `RetentionRefStatus`, and `RecoverabilityVerdict`, which the board renders.
- `crates/cargo-berth/src/reconcile.rs` — **exists**; the reconciliation the board runs first, and the repository snapshot phase 7 introduces.
- `crates/cargo-berth/src/worktree/liveness.rs` — **exists**; the five `WorktreeLiveness` states each holder row shows.
- `crates/cargo-berth/src/reservation/mod.rs` — **exists**; the retained set the board projects from, and the `Renew` replay that currently discards its timestamp.
- `crates/cargo-berth/src/git/mod.rs` — **exists**; the ahead/behind and ref-status helpers the board reads live.
- `crates/cargo-berth/src/verb/mod.rs` — **exists**; owns the verb module list, which `board` is added to.
- `crates/cargo-berth/tests/board.rs` — model tests, headless.

**Constraints from prior phases:** Phase 7 supplies derived edge status and the readiness grouping — the board must not recompute it. Phase 5's `reconcile()` runs first. Phase 8 supplies bypassed-edge state and unresolved deferrals. Phase 2's typed payload is the `--json` shape, and its structured board data is what `--json` emits — never a rendering scraped back into fields. **The board owns the only user-facing surface for phase 5's orphan alerts.** `Alert::OrphanedOutstanding` in `src/alert.rs` is durable and re-shown until the user records recovery, integration, or approved abandonment, and it carries reservation id, `BranchRefStatus`, `ObjectAvailability`, `RetentionRefStatus`, protected tip, and one `RecoverabilityVerdict` of `RecoverableFromBranch | RecoverableFromProtectedTip | CommitUnavailable`. Today it reaches a user only inside a `resolve`, `release`, or `claim` envelope; the board must render it and name the `resolve` flag that answers it. Render the verdict as given — "commits are lost" is earned by `CommitUnavailable`, never inferred from a deleted branch. The board also shows `WorktreeLiveness` for each holder, since a blocking reservation whose worktree is gone reads as a stuck board row otherwise. **The alerts constraint above understates where alerts already appear** — phase 5 attaches them to `renew` and to a blocked `check` as well as to `resolve`, `release`, and `claim`, so the board is the one surface that shows them durably and on demand, not the first surface that shows them at all. **Every liveness state needs a home, and four of them currently have none.** "Unconstrained live reservations" is written for a `Live` holder; an active reservation whose worktree is `Unavailable`, `OrphanCandidate`, `Orphaned`, or `Unknown` and which carries no edge belongs nowhere, yet it blocks edits exactly like a live one and only the orphaned-outstanding case raises an alert. Give the section a name that covers all five and label each row with its liveness, so a blocked user can see the difference between a holder that is working and one whose worktree is gone. **`renew` promises a freshness signal that nothing computes.** Phase 5 appends a `Renew` event, but replay merely advances `ReservationRevision` and discards the event's `RecordedAt`, so the stale flags this Work Order lists among its alerts have no source. Define a named reservation-freshness type derived only from the events that mean someone is still working — claim, widen, renew, and checkpoint — and never from unrelated journal traffic or from `HEAD` moving, since a reservation whose owner walked away still sees commits land around it. **Take the repository snapshot phase 7 introduces rather than re-deriving it.** One snapshot under the lock carries every holder's liveness, every retained reservation's evidence status, and one resolved trunk; the board projects from that value, and the counted-call proof that a board read costs one `git worktree list` plus at most `P` ancestor checks belongs here, since this is the first phase with a board to count. **Two board-facing types stay representation-driven and must not.** Ahead/behind is `AheadBehind::{Counts { ahead, behind }, Unrelated, Unavailable}` — never an optional tuple, because "no common ancestor" and "could not be computed" are different answers a reader acts on differently. `BranchRefStatus.reference` and `RetentionRefStatus.reference` are `String` today; convert them at the board boundary into `FullRefName` and a semantic reservation-retention-ref type, so a row cannot render a branch name where a retention ref belongs.

**Acceptance gate:** `verify.sh test cargo-berth` and `verify.sh test cargo-berth board` both green, including: an empty graph renders "no integration order declared"; independent reservations are shown as an unordered tie, never numbered; a bypassed edge appears with its reason and its predecessor flagged; `--json` runs headless with no TTY and matches the rendered content; `board` no longer returns `unimplemented` in either output mode. Also: a cleanly released reservation appears in the resolved audit section and not in the active constraints, while one reblocked by a trunk rewrite appears in the active constraints labelled as reblocked; and each of the four release dispositions and each `IntegrationEvidenceStatus`, `ObjectUnknown` included, renders in both the TUI model and `--json`. Also: all five `WorktreeLiveness` states appear on a blocking holder with no edge and each is distinguishable in both output modes; each of the three `RecoverabilityVerdict` values renders with the `resolve` flag that answers it, and `CommitUnavailable` is the only one that reads as commits lost; a reservation renewed after a long idle reads fresh while one whose worktree only saw unrelated journal events and `HEAD` movement reads stale; a counted-call test proves a board read costs one `git worktree list` plus at most one ancestor check per distinct predecessor; and an unrelated branch and an uncomputable one render as different ahead/behind answers.
...
Run `cargo-berth init`, then confirm the ledger exists at `.git/cargo-berth/` with `journal.ndjson` and `reservations.json`, and that the `reference-transaction` hook is in the **common** git dir. `init` does not overwrite the config written above. `init` takes the mutation lock, so it can exit `6` when something else holds it: retry on that code rather than treating it as failure, and report an exhausted retry as its own outcome — phase 8 sets the contract (one 10-second total deadline measured across all attempts, with exponential backoff from 50 ms inside it).
...
**Goal:** In `hana`: the two hook shims exist and are proven against every exit code, ready to be switched on once the plans actually declare reservations.

**Spec:** Implements D1 — read `### D1 — RESOLVED` in `docs/berth-plan.md` (this plan).

**Write and test the shims here; do not switch them on.** R38's adoption order enables mandatory coverage and refusal **together, after backfill** — a `PreToolUse` hook active before the 28 Work Orders declare their reservations blocks nothing useful and surprises every agent that edits a path no one has claimed. So this phase creates both scripts and proves them against synthetic payloads, and **leaves `.claude/settings.local.json` untouched**. Phase 17 adds the `hooks` key as its first step, after phase 14's skill, phase 15's dispatcher lifecycle, and phase 16's backfill are all in place.

Two shim scripts that shell out to the installed `cargo-berth` binary and translate its exit codes into Claude Code hook protocol.
...
`PostToolUse` on `Bash`: call `cargo-berth drift --json`. Silent and auto-widen results say nothing; an **incursion** surfaces to the user and tells the agent to stop. Budget ~0.02s.
...
- `/Users/natemccoy/rust/hana/.claude/hooks/berth_session_start.sh` — new; the `SessionStart` reconciliation and alert surface.

`.claude/settings.local.json` is deliberately **not** in this list; phase 17 edits it.

**Constraints from prior phases:** Phase 1 froze the exit codes and the six envelope fields; phase 2 added the typed payload — parse those fields, never scrape prose. Phase 9 defines the drift classifications the post-hook reacts to and adds `drift` to the verb set. Phase 5's reconcile is *not* called by the pre-hook except the single retry when the projection already says block. Phase 11 installed the executable; invoke `cargo-berth`, and compile nothing. Phase 3 owns `EditAuthorization` and its resolution — `CARGO_BERTH_RUN` first, then the worktree marker file, failing closed to `Unidentified`. Read it; never re-derive it, and never pass a run id to `check`. Phase 3b's `claim --run` is the one provenance boundary where a run id is an argument; orchestration exports that same value as `CARGO_BERTH_RUN` — the same UUID-v7, not a second identifier. `--plan` and `--phase` travel together or the command line is a usage error. **A successful `claim` can still fail to publish the run marker**: phase 3b reports that as `marker_publication: unavailable` on an otherwise successful claim, and in that state `CARGO_BERTH_RUN` is the only way a later `check` recognizes its own run — so the shim must rely on the exported variable rather than on the marker existing. The concrete payloads to parse are phase 3b's `CheckPayload` and `ClaimPayload`; a blocked answer carries **every** holder, not just the first, so a message naming one holder is wrong whenever two hold the paths. Phase 4 adds exit `6`, transient lock contention. The shim **retries** it — phase 8's contract, one 10-second total deadline measured across all attempts, with exponential backoff from 50 ms inside it — and allows the edit only when that deadline expires with every attempt still `6`, recording that it did so. **Phase 5 changed what a marker-derived identity buys.** `EditAuthorization` is now source-preserving — `Environment(CoordinationRunId)`, `Marker { coordination_run_id, worktree_id }`, or `Unidentified` — and a `Marker` identity is honored only when replay shows that same run still holds an **active** reservation minted in that same worktree. An environment-supplied run is trusted as given. A stale marker left by a crashed run therefore no longer grants the holder's own exemption, which is exactly why exporting `CARGO_BERTH_RUN` is the reliable path and the marker file is the fallback.

Exit `6` is not exit `4`, and the two must not share a branch. Phase 4 split lock contention out of `LedgerUnreadable`, so the fail-open rule keyed on `4` fires only on `4`: a busy ledger is a retry, and jumping straight to allow converts a transient wait into permission to edit a reserved path. The keystroke is still never blocked on contention — exhausting the retry allows — but it allows after actually trying, and says so, rather than mistaking a lock for a lost ledger. **The retry budget is one 10-second total deadline, not five attempts** — phase 8's constraint explains why: each mutating call already waits internally for the lock before returning `6`, so multiplying attempts multiplies that wait and turns a busy ledger into a hang on the one path that must stay interactive. Measure the deadline across all attempts, back off exponentially from 50 ms inside it, and report an exhausted deadline as its own outcome. **The blocked-`check` retry does not exercise what this phase's exit-`6` case assumes.** Phase 5 made a blocked `check` reconcile once and then swallow a reconciliation failure, preserving the original exit `1`, so a `check` that finds a block never surfaces `6` from that path at all; the exit-`6` case belongs to the mutating calls and the shim must be tested against real lock contention — another process holding `mutation.lock` — rather than against a synthesized status. **Nothing invokes reconciliation at a session boundary, and two shipped promises depend on it.** Phase 5 shipped `reconcile()` as a callable routine, not a process-level surface; phase 8's bypass taken while the journal was unwritable is specified to be "reported at next SessionStart"; and phase 5's orphaned-outstanding alerts are durable and meant to be re-shown until the user records a disposition. No hook registers either. Build a third shim here — a `SessionStart` script that invokes the headless board once, surfaces any orphaned-outstanding alerts with the `resolve` flag that answers each, and reports any deferred bypass marker — and leave it unregistered exactly as the other two are. **`drift` now takes an explicit reservation.** Phase 9 binds the operation to one `ReservationId` and resolves implicitly only when exactly one active reservation matches the acting identity, so the `PostToolUse` shim passes `--reservation` when it knows which one the phase claimed and treats the ambiguous-selection usage error as a real result to surface, never as a silent no-op.

**Acceptance gate:** No `verify.sh`, and no compile. Each shim runs against synthetic stdin payloads covering all seven exit codes and the decision is asserted — including exit `6`, which must retry and then allow, and must be distinguishable in the shim's own output from an exit `4` allow — including a malformed envelope, an unknown status, and a payload whose `exit_code` field disagrees with the process exit status — all three are refused rather than trusted, **except** that exit `4` still permits editing, because failing open on ledger loss is the deliberate design and must survive the stricter parsing. A real claim in a scratch worktree makes a real `Edit` block when the shim is invoked directly; corrupting `journal.ndjson` still permits editing. Also: a check run by the claiming run itself is clear while the same check from an unidentified session blocks; a block held by two reservations names both in the message; and a claim whose marker publication failed is still usable when `CARGO_BERTH_RUN` is exported and unrecognized when it is not. `.claude/settings.local.json` is byte-identical to how this phase found it. Also: the exit-`6` case is driven by a second process actually holding `mutation.lock` rather than a synthesized status, and the shim's total elapsed wait stays inside the 10-second deadline regardless of how many attempts it made; the `SessionStart` shim run against a ledger carrying an orphaned-outstanding alert prints it with the `resolve` flag that answers it, run against a clean ledger prints nothing, and run against a deferred bypass marker reports the bypass; and a `PostToolUse` run in a worktree holding two active reservations for one run surfaces the ambiguous-selection error rather than reporting no drift.
...
Exit `6` is contention, not an answer: the skill retries the same invocation on phase 8's contract (one 10-second total deadline measured across all attempts, with exponential backoff from 50 ms inside it) and reports an exhausted retry as "the ledger is busy, try again" — never as a clear, and never as an unreadable ledger. On exit `3` the skill escalates to the user with the binary's payload — both plans and phases, the shared paths, the direction, the reason, the consequence — and only then re-invokes `claim` with the chosen flag. **An agent never answers its own block** (R54).
...
A phase whose Work Order has no `**Reservations:**` block is a hard stop with an actionable message, not a silent skip — a phase that claims nothing is invisible to every other worktree, which is the exact decay this design exists to prevent. **Build the hard stop here; it becomes universally binding only once phase 16 finishes the backfill**, because until then most todo Work Orders carry no block and the stop would refuse nearly every dispatch. R38 enables mandatory coverage and refusal together, after backfill — phase 17 is that moment. This phase's own dry run therefore needs a Work Order that already carries a block; hand-author one if phase 16 has not run.

On a blocked claim, `/plan:delegate` stops before dispatching and surfaces the collision the same way it surfaces a pending decision. A claim or release that exits `6` is contention rather than a verdict: retry it on phase 8's contract (one 10-second total deadline measured across all attempts, with exponential backoff from 50 ms inside it), and on an exhausted retry stop with a busy-ledger message rather than dispatching as though the claim had succeeded.
...
**Constraints from prior phases:** Phase 14's shared validator does the Work-Order resolution — `/plan:delegate` calls it rather than parsing markdown itself, and phase 14 already added that call, so keep this phase's edits to the claim/release lifecycle and do not add a second parser. Phase 4's release semantics mean checkpoint does not free the paths. Phase 3's provenance flags are what carry the plan, phase, coordination run, and starting `HEAD` into the claim. Compile nothing. Phase 3 owns `EditAuthorization` and its resolution — `CARGO_BERTH_RUN` first, then the worktree marker file, failing closed to `Unidentified`. Read it; never re-derive it, and never pass a run id to `check`. Phase 3b's `claim --run` is the one provenance boundary where a run id is an argument; orchestration exports that same value as `CARGO_BERTH_RUN` — the same UUID-v7, not a second identifier — so a run that claims and a later check in that worktree agree on who is acting. `--plan` and `--phase` are additive optional flags that travel **together**; supplying one without the other is a usage error, not a partial record. `--head` records the commit the phase started from and is what phase 9's drift fingerprint later diffs against, so the dispatcher must pass the real starting `HEAD` rather than omitting it. A claim can succeed while reporting `marker_publication: unavailable`; the dispatcher must export `CARGO_BERTH_RUN` for the phase's lifetime rather than assuming the marker file backs it up. **The claimed `ReservationId` has to survive the phase, not just the claim.** Phase 9 binds `drift` to an explicit reservation and resolves implicitly only when exactly one active reservation matches the acting identity, which a worktree running two phased claims does not satisfy — so the dispatcher retains the id the claim returned and exports it alongside `CARGO_BERTH_RUN` for the phase's lifetime, and the `PostToolUse` shim passes it. **The pre-checkpoint check runs drift's full comparison.** The cheap delta answers only whether anything changed since the last observation; before releasing, the question is what changed against the phase-start baseline, which is phase 9's four-command comparison selected explicitly. The retry budget is phase 8's single 10-second total deadline across all attempts, not five attempts each paying the engine's internal lock wait. **Phase 5 changed what a marker-derived identity buys.** `EditAuthorization` is now source-preserving — `Environment(CoordinationRunId)`, `Marker { coordination_run_id, worktree_id }`, or `Unidentified` — and a `Marker` identity is honored only when replay shows that same run still holds an **active** reservation minted in that same worktree. An environment-supplied run is trusted as given. A stale marker left by a crashed run therefore no longer grants the holder's own exemption, which is exactly why exporting `CARGO_BERTH_RUN` is the reliable path and the marker file is the fallback.

**Acceptance gate:** No `verify.sh`. A dry run over a real todo phase claims the right paths and releases at checkpoint leaving the reservation `Outstanding`; a phase with no `**Reservations:**` block stops with the actionable message; a forced collision stops before dispatch; and a claim held off by a busy lock is retried rather than treated as either success or failure, with total elapsed wait inside the 10-second deadline; the claimed reservation id is exported for the phase's lifetime and a worktree holding two phased claims still resolves each `drift` unambiguously; and the pre-checkpoint check runs the full comparison against the recorded phase-start `HEAD`.
...
- Scenarios 1, 3, 5, and 6 run first, unchanged, against the observe-only gate. Scenario 3 asserts that the hold **clears**, which is a permit either way — record the gate's stated decision at each step, not merely that the merge went through.
- Then flip `gate_mode` to `"enforce"` **temporarily**, run scenarios 2 and 4, and flip it straight back to `"observe"` if either fails. The flip-back is part of the scenario, not a follow-up: a failed run must never leave the real repo enforcing a gate that has not been proven.
- Scenario 5 deliberately corrupts `journal.ndjson`. Run it against a **disposable coordination domain** — a scratch ledger the scenario creates and deletes — not the repo's real ledger. If it must use the real one, repair the journal and verify it reads clean before enforcement is left on; enforcing against a corrupted ledger is how a working repo becomes an unusable one.

Only after all six pass, flip `gate_mode` from `"observe"` to `"enforce"` in `.claude/config/berth.toml` for good.

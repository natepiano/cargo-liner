...
**Both existing call sites are this phase's to change.** `claim` resolves its acting run from the `cargo-berth-run-id` marker before doing anything else, so sweeping stale markers *after* run selection would still let a dead run's marker seed a fresh claim — reconcile must run before `ClaimCoordinationRunSelection` resolves. `check` is the one caller allowed the blocked-path retry above: it stays git-free and lock-free on the clear path, and may reconcile only when the read-only snapshot already says block. If that retry fails for any reason, the original block stands — a reconciliation error never converts a block into an all-clear. `ReservationReplayError` is corruption, not liveness drift, and reconcile never treats it as something to repair.

**Reconcile sweeps the coordination-run markers.** Phase 3b's `claim` publishes `cargo-berth-run-id` and phase 4's `release` removes it, so a crash between them leaves a marker naming a run with no live reservation — and phase 3's `EditAuthorization` would then hand a foreign session the holder's own identity. `reconcile` removes a malformed marker, and one whose administrative locator has no matching active reservation, while preserving a live match untouched. **The sweep alone does not close the hole, because `EditAuthorization` erases where an identity came from.** It resolves `CARGO_BERTH_RUN` first and the worktree marker second and collapses both into one `Identified` value, so `check` — which must stay git-free and lock-free on the clear path, and therefore cannot reconcile first — hands a marker-derived caller the holder's own exemption before any sweep can fire. Give the authorization a source-preserving shape: an environment-derived identity is trusted as given, while a marker-derived one is honored only when replay shows that run still holds an active retained reservation. That validation reads the snapshot `check` already has, so the git-free clear path survives intact. A crash is not the only way to reach this state — phase 4's `release` reports a retirement it could not perform as `marker: unavailable` on an otherwise successful release, leaving a stale marker behind a command that reported success.
...
**The recovery verbs.** Phase 2 declared `resolve` and `renew` and left them `unimplemented`; their engines land here, beside the alerts and liveness verdicts they answer.

**`--retire-orphan` does not exist yet and this phase adds it.** The shipped `ResolveArguments` carries `--recovered`, `--integrated-as`, and `--abandon` in a required exclusive argument group, and nothing else; `RetiredOrphan` is therefore currently unreachable by any route. Add `--retire-orphan` to that group as a fourth alternative, with its own long help, requiring `--why` exactly as `--abandon` does. The expansion is additive — the three existing flags keep their spelling and behavior — matching the precedent phase 3b set when it added the provenance flags to `claim`. `resolve --recovered` rebinds the reservation to the worktree the command runs in, appending phase 2's rebinding operation — this is the path for work recovered into a *new* worktree, which reconciliation cannot detect on its own because the recorded `WorktreeId` no longer matches. `resolve --integrated-as <trunk-oid>` appends `release` with `ReleaseDisposition::RewrittenIntegration`, and validates first: the oid must resolve in this repository and be reachable from trunk, so a typo or a commit from an unrelated branch is a usage error rather than a false all-clear. **A reblocked `Released` reservation must be able to receive this answer, and today it cannot.** Phase 4 retains a `Released` reservation and revalidates its git evidence, so a trunk rewrite returns an already-integrated record to blocking — while `ReservationLifecycle::release` rejects every transition out of `Released`, leaving the user blocked with no command that clears it. Add a replacement disposition transition: it appends a new record naming both the superseded disposition and the new one, replay takes the last, and the earlier release stays readable as history rather than being rewritten. It is a distinct transition, not a relaxation of `release` — an ordinary first release must still refuse an already-released reservation. `resolve --abandon --why <text>` appends `release` with `Abandoned`, stores the reason, and is the only route to that disposition (R28). `resolve --retire-orphan --why <text>` is the one route to `RetiredOrphan`, is never reached automatically, and stays distinguishable from a deliberate abandonment after replay. `renew` appends the activity record R29 derives freshness from. All four run inside phase 2's mutation lock, and each stops the alert only as a consequence of the state it recorded — never by marking the alert itself.
...
- `crates/cargo-berth/src/verb/claim.rs` — **exists**; reconcile before the acting run is resolved from the marker.
- `crates/cargo-berth/src/verb/check.rs` — **exists**; the single blocked-path reconcile retry.
- `crates/cargo-berth/src/cli.rs` — **exists**; wire `resolve` and `renew` to their engines, and add the `--retire-orphan` alternative to the resolve disposition group. The other verbs and flags are phase 2's and do not change here.
- `crates/cargo-berth/src/main.rs` — **exists**; owns the `mod` list.
- `crates/cargo-berth/tests/liveness.rs`

**Constraints from prior phases:** Phase 4 supplies `protected_tip`, retention refs, and reachability; recoverability verdicts are computed from them. `WorktreeLiveness` is **this phase's type**, deliberately kept out of phase 4's lifecycle enum — whether a holder still exists is orthogonal to how far its work progressed. Phase 2 mints the opaque `WorktreeId` into the administrative directory and owns `RepoInstanceId`; this phase validates repo instance, current administrative directory, its backlink, and the recorded root before granting same-owner status (R39). Phase 2's lock wraps every append reconcile makes. Phase 2's recovery-surface decision resolved to two verbs. Phase 2 declares and parses `resolve` and `renew`, adds their `CommandVerb` variants and `--help` long forms, and adds the worktree-rebinding journal operation `--recovered` needs; **this phase writes their engines** and adds exactly one piece of wire surface, the `--retire-orphan` flag above. Phase 4 owns the ordinary marker removal that rides on `release`; this phase owns only the crash sweep. Phase 4 shipped that removal as a **post-append committed action** inside `Ledger::transact_with_committed_action`, still under `mutation.lock`, and it renames the marker to a private pathname before reading its content so a marker republished for another run between the read and the unlink cannot be deleted; the crash sweep must use the same detach-then-classify sequence rather than a read-then-remove. Any git side effect this phase adds to a mutating verb belongs in that post-append stage, never in the transaction validator — a ref write inside the validator is observable even when the append that describes it fails. That ordering carries a consequence this phase owns: the journal commits **before** the action runs, so a failed retention-ref write or marker retirement leaves a durable record describing a side effect that never happened, and the command reports the action error over an outcome that is already permanent. `reconcile` repairs that idempotently — every retained reservation that still needs a retention ref gets one written or corrected from its recorded `protected_tip` — and the repair is safe to run repeatedly and after a restart. Phase 4 also established that every verb revalidating git evidence appends its **conclusion**, not only its inputs: `JournalOperation::EvidenceRevalidated` carries `edit_blocking_status` beside `IntegrationEvidenceStatus`, and replay reads it. Reconcile must do the same. Exit `6` (`BerthExit::BlockedByContention`) now means lock contention in **every** verb including `init`, so a recovery path must treat it as retry, never as an unreadable ledger. Phase 3b shipped `verb/claim.rs` and `verb/check.rs`, whose call-site ordering this phase changes, and `RetainedReservationSet::replay`, whose `ReservationReplayError` signals a corrupt journal rather than a stale one. `ReleaseDisposition::{RewrittenIntegration, Abandoned}` and the rebinding operation all exist by the time this phase starts — do not add a journal variant. Phase 2 ships `WorktreeIdentity` with only an id and a kind, and the journal's only relocation operation is `RebindWorktree`, which changes identity. R39 requires a moved worktree to keep its identity while its recorded root is updated under the lock, so this phase owns a same-identity relocation transition distinct from a rebind. Collapse the shipped `ResolveArguments` — a bool plus two `Option<String>`s, and the `--retire-orphan` flag this phase adds — into one named recovery-request enum built at the clap boundary, each variant carrying exactly the evidence and reason its disposition requires, so an impossible combination cannot reach the engine and no `Option<T>` survives the parser.

**Acceptance gate:** `verify.sh test cargo-berth` and `verify.sh test cargo-berth liveness` both green, including: an `rm -rf`'d-but-unpruned worktree classifies `OrphanCandidate` and keeps blocking; a pruned one classifies `Orphaned` and still keeps its scopes; a locked worktree is `Unavailable`; no path reaches `Abandoned` without explicit confirmation; an `OrphanedOutstanding` alert survives a process restart and reports the right recoverability verdict for a surviving branch, a deleted branch with a retention ref, and neither; `resolve --recovered` run from a replacement worktree rebinds the reservation and stops the alert; `resolve --integrated-as` rejects an oid unreachable from trunk and accepts one reachable from it; `resolve --abandon --why` is the only route to `Abandoned` and its reason survives replay; `renew` refreshes freshness without touching scopes or edges; and replaying each recovery verb from a deleted projection reproduces the same state. Also: a ledger copied into a different repository is rejected on its `RepoInstanceId` rather than silently adopted; a worktree whose administrative directory was swapped for another repository's is refused; and a worktree moved on disk keeps its identity while its recorded root is updated. Also: `--retire-orphan --why` is the only route to `RetiredOrphan`, is rejected without `--why`, is rejected when combined with any other disposition flag, and stays distinguishable from `Abandoned` after replay; a claim whose worktree holds a stale marker reconciles before selecting its run, so the dead run's id never reaches the appended record; a `check` that the read-only snapshot clears makes no git call and takes no lock, while one the snapshot blocks reconciles once and still blocks if that retry errors; a marker left by a crashed run with no active reservation is swept while a live one is preserved; a generation-ahead projection and an offset-ahead projection each report `--repair-projection`, recover through it, and leave `journal.ndjson` byte-identical; and before recovery phase 13's pre-edit hook still fails open on exit `4`. Also: a `Released { Integrated }` reservation reblocked by a trunk rewrite accepts `resolve --integrated-as`, replays to the replacement disposition, and stops blocking, while a first `release` against an already-released reservation is still refused; a retention-ref write failed after its journal append is repaired by the next `reconcile` and survives a process restart, and the reservation is never reported as having no ref; and a release that reported `marker: unavailable` leaves a marker that cannot produce a clear `check` for an unrelated session, while the same session with `CARGO_BERTH_RUN` exported is still recognized.
...
On a blocked claim, `/plan:delegate` stops before dispatching and surfaces the collision the same way it surfaces a pending decision. A claim or release that exits `6` is contention rather than a verdict: retry it on phase 8's schedule (five attempts, exponential backoff from 50 ms), and on an exhausted retry stop with a busy-ledger message rather than dispatching as though the claim had succeeded.
...
**Acceptance gate:** No `verify.sh`. A dry run over a real todo phase claims the right paths and releases at checkpoint leaving the reservation `Outstanding`; a phase with no `**Reservations:**` block stops with the actionable message; a forced collision stops before dispatch; and a claim held off by a busy lock is retried rather than treated as either success or failure.
...
**First, switch the hooks on.** Phase 13 built both shims and tested them by hand but deliberately left `/Users/natemccoy/rust/hana/.claude/settings.local.json` untouched, so nothing invokes them yet. Add the `hooks` key registering the `PreToolUse` shim for `Edit`/`Write` and the `Bash` incursion shim, preserving the existing `permissions` and `outputStyle` values **byte for byte** — this file is the user's own settings and the only allowed change is the added key. Until this lands, every scenario below tests nothing.

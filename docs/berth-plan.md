...
**Derived edge status is not the only deny input.** A `Defer` (phase 6) holds **both** reservations at integration until an order exists — that is what makes it not an `Override` with a nicer name — and a deferral records no edge, so a gate that reads only the edge graph lets both halves of a deferred conflict land in whatever order they arrive. The two held endpoints are the reservation whose parent claim carries `ConflictAuthorization::Defer` and that authorization's named `blocker`; do not treat every historical overlap binding as another endpoint. Evaluate both: unsatisfied predecessors from phase 7's derived status, **and** any unresolved `Defer` naming this reservation.

**This phase owns the read path onto the constraints, and phase 10 reuses it.** Phase 7 shipped `OrderingEdge`, `OrderingGraph`, and `RepositorySnapshot` with private fields, and the only way to obtain a snapshot is to run mutating reconciliation. Introduce one named read-only constraint projection — built under the same lock as the decision — carrying the edges that currently hold, the unresolved deferrals, each hold's exact scopes and recorded reason, and the reservation facts a denial message must name. The gate reads that projection; it never reaches into the graph's internals and never re-derives readiness. Phase 10 renders the same value, so it is a shared surface from its first use rather than a gate-private one retrofitted later.

**An enforcement decision needs one transaction, not two.** `sequence` (phase 7) reconciles, releases the lock, then takes a second lock to append while reading the snapshot from before it — acceptable for a verb whose output is informational, unsafe for a decision that permits or denies a merge, because an edge can be recorded between the two. Bind the constraint projection, the repository snapshot, the journal generation they were read at, and the proposed old and new `main` object ids into one validated decision taken under a single lock hold. `integrate` and the hook both go through it.

**The hook needs a stated way to call the binary.** The frozen CLI has no gate verb and this phase must not add an unexplained public one. Define a private dispatch contract instead: the installed `reference-transaction` script invokes `cargo-berth` on a hidden entry point, and git's transaction input — the `prepared`/`committed`/`aborted` phase word on the command line and the `<old-oid> <new-oid> <ref-name>` lines on stdin — is parsed at that boundary into a semantic reference-update type rather than passed around as text. It carries every proposed update in the transaction, not just the first, since one push or merge can update several refs at once and only the `refs/heads/main` updates are the gate's subject. The entry point is not advertised in help output and is not part of the frozen surface phase 1 pinned.
...
- `crates/cargo-berth/src/answer/conflict_authorization.rs` — **exists**; `ConflictAuthorization::Defer` names the parent reservation's exact counterpart and preserves its scopes and reason.
- `crates/cargo-berth/src/edge/mod.rs`, `crates/cargo-berth/src/edge/graph.rs`, `crates/cargo-berth/src/edge/snapshot.rs` — **exist**; the edge record, the locked graph, and the repository observation the deny decision reads. Their fields are private today and the graph is reachable only through mutating reconciliation, so this phase owns opening a read path.
- `crates/cargo-berth/src/reconcile.rs` — **exists**; builds the repository observation and currently the only route to it.
...
**A DAG is a partial order; never render a numbered queue.** Sections: **Ready now** (no unsatisfied predecessors), **Waiting** (each predecessor named, with the covered paths), **Unresolved overlaps**, **Recorded overlap answers**, **Unconstrained live reservations**, and alerts (orphaned-outstanding, bypassed edges with reason and date, stale flags). The recorded-answer audit includes `Override`, `Sequence`, and `Defer`; each row carries the exact approved scopes, named blocker, direction where one exists, authorization reason, and consequence. Ties within a readiness level are labelled unordered. With no edges recorded it says **"no integration order declared"** — never an empty queue, which reads as an all-clear.

**"Waiting" is not one state and must not render as one word.** Phase 7's readiness value names why each edge holds, and each reason has a different answer for the person reading the row: waiting for the predecessor to reach a checkpoint means nobody can act yet; waiting for the predecessor to reach trunk means either wait, re-record evidence a trunk rewrite invalidated with `resolve --integrated-as <trunk-oid>`, or repair an object that does not resolve, depending on the evidence case the value carries; and waiting for *this* worktree to incorporate a predecessor that already landed is cleared by the reader's own rebase and by nothing else. Render the applicable action beside every waiting row. Collapsing all of them into an unexplained "Waiting" is the specific failure this section exists to prevent — it tells a user who could act right now that they are blocked by someone else. A settled edge renders in the same vocabulary: cancelled because the constraint ended, or fulfilled because the successor already contains the predecessor.

**A defer that has been resolved must stop rendering as a defer.** Phase 7 records on each edge whether it was declared directly or created by resolving a prior deferral, and that provenance is already in the `sequence` output, so it is a fact the board owes its reader rather than an implementation detail. A resolved deferral leaves **Unresolved overlaps** entirely, appears in the recorded-answer audit as an ordering created from a deferral, and carries both reasons — the one recorded when the two sides agreed to defer and the one recorded when the order was chosen — with the constraint's current consequence. It must never continue to display the symmetric hold it no longer imposes.
...
- `crates/cargo-berth/src/answer/conflict_authorization.rs`, `crates/cargo-berth/src/answer/scope_binding.rs` — **exist**; the durable answer audit and its exact approved overlap scopes.
- `crates/cargo-berth/src/main.rs` — **exists**; owns the `mod` list.
...
- `crates/cargo-berth/src/reconcile.rs` — **exists**; the reconciliation the board runs first, and where the repository observation is constructed.
- `crates/cargo-berth/src/edge/snapshot.rs` — **exists**; the repository observation itself and its evidence and reachability types, which phase 7 put here rather than in `reconcile.rs`.
- `crates/cargo-berth/src/edge/mod.rs`, `crates/cargo-berth/src/edge/graph.rs` — **exist**; the edge record and the locked graph, read through the shared constraint projection phase 8 introduces.
...
**Constraints from prior phases:** Phase 7 supplies the per-edge derived status and the board must not recompute it — but it shipped `OrderingEdge::readiness()` alone, one edge at a time, with no reusable grouping. The shared read-only constraint projection is phase 8's, built because the gate needed it first; take that value here rather than writing a second one, and take the repository observation from `edge/snapshot.rs`, not from `reconcile.rs`, which only constructs it. Phase 5's `reconcile()` runs first. Phase 8 supplies bypassed-edge state and unresolved deferrals. Phase 6 supplies the complete durable overlap answer: exact scope bindings, named blocker, optional direction, reason, and consequence. Render those facts in both the TUI model and JSON. A proposal token is derived, transient approval material and is never journaled, projected, or displayed on the board. Phase 2's typed payload is the `--json` representation, and its structured board data is what `--json` emits — never a rendering scraped back into fields. **The board owns the only user-facing surface for phase 5's orphan alerts.** `Alert::OrphanedOutstanding` in `src/alert.rs` is durable and re-shown until the user records recovery, integration, or approved abandonment, and it carries reservation id, `BranchRefStatus`, `ObjectAvailability`, `RetentionRefStatus`, protected tip, and one `RecoverabilityVerdict` of `RecoverableFromBranch | RecoverableFromProtectedTip | CommitUnavailable`. Today it reaches a user only inside a `resolve`, `release`, or `claim` envelope; the board must render it and name the `resolve` flag that answers it. Render the verdict as given — "commits are lost" is earned by `CommitUnavailable`, never inferred from a deleted branch. The board also shows `WorktreeLiveness` for each holder, since a blocking reservation whose worktree is gone reads as a stuck board row otherwise. **The alerts constraint above understates where alerts already appear** — phase 5 attaches them to `renew` and to a blocked `check` as well as to `resolve`, `release`, and `claim`, so the board is the one surface that shows them durably and on demand, not the first surface that shows them at all. **Every liveness state needs a home, and four of them currently have none.** "Unconstrained live reservations" is written for a `Live` holder; an active reservation whose worktree is `Unavailable`, `OrphanCandidate`, `Orphaned`, or `Unknown` and which carries no edge belongs nowhere, yet it blocks edits exactly like a live one and only the orphaned-outstanding case raises an alert. Give the section a name that covers all five and label each row with its liveness, so a blocked user can see the difference between a holder that is working and one whose worktree is gone. **`renew` promises a freshness signal that nothing computes.** Phase 5 appends a `Renew` event, but replay merely advances `ReservationRevision` and discards the event's `RecordedAt`, so the stale flags this Work Order lists among its alerts have no source. Define a named reservation-freshness type derived only from the events that mean someone is still working — claim, widen, renew, and checkpoint — and never from unrelated journal traffic or from `HEAD` moving, since a reservation whose owner walked away still sees commits land around it. **Take the repository observation rather than re-deriving it.** One observation under the lock carries every holder's liveness, every retained reservation's evidence status, and one resolved trunk; the board projects from that value. **Its real cost is larger than this Work Order previously claimed, and the true bound is what gets asserted.** Building it resolves trunk, revalidates each retained reservation's evidence through one or two ancestry calls, and then issues a `cat-file` plus a grouped `rev-list` per protected graph predecessor; the board adds an ahead/behind computation per worktree on top. "One `git worktree list` plus at most `P` ancestor checks" was never achievable and must not be written as though it were. Measure what a board read actually costs, state that bound here, and pin it with a counted-call test — phase 8 sets the first bound on the hook path where the budget is interactive, and this phase states its own, which is allowed to be larger because a board read is a deliberate user action. Do not add batching or query fusion to hit a number nobody has measured a need for: state the true cost first, and optimize only if the measurement says to. **Two board-facing types stay representation-driven and must not.** Ahead/behind is `AheadBehind::{Counts { ahead, behind }, Unrelated, Unavailable}` — never an optional tuple, because "no common ancestor" and "could not be computed" are different answers a reader acts on differently. `BranchRefStatus.reference` and `RetentionRefStatus.reference` are `String` today; convert them at the board boundary into `FullRefName` and a semantic reservation-retention-ref type, so a row cannot render a branch name where a retention ref belongs. **Direction is a property of the answer, not an optional field beside it.** Only some overlap answers order anything — a sequence names a before and an after, an override and a defer do not — so the board must read direction out of the `ConflictAuthorization` variant it is already rendering rather than flattening every answer into a row carrying an optional direction. An answer that orders nothing has no direction to be absent; a row built to hold one invites a reader to wonder which case an empty cell is. **Retention-ref status is not a board column.** Phase 7 deletes a reservation's retention ref only once its predecessor is released and every dependent successor is released too — the point at which no recovery remains possible or necessary — so its presence or absence tells an ordinary reader nothing they can act on. Render retention status only where it changes an orphan alert's recoverability verdict, and nowhere else.

**Acceptance gate:** `verify.sh test cargo-berth` and `verify.sh test cargo-berth board` both green, including: an empty graph renders "no integration order declared"; independent reservations are shown as an unordered tie, never numbered; `Override`, `Sequence`, and `Defer` each render the exact approved scopes, named blocker, direction where present, authorization reason, and consequence in both the TUI model and JSON, with no proposal token field; a bypassed edge appears with its reason and its predecessor flagged; `--json` runs headless with no TTY and matches the rendered content; `board` no longer returns `unimplemented` in either output mode. Also: a cleanly released reservation appears in the resolved audit section and not in the active constraints, while one reblocked by a trunk rewrite appears in the active constraints labelled as reblocked; and each of the four release dispositions and each `IntegrationEvidenceStatus`, `ObjectUnknown` included, renders in both the TUI model and `--json`. Also: all five `WorktreeLiveness` states appear on a blocking holder with no edge and each is distinguishable in both output modes; each of the three `RecoverabilityVerdict` values renders with the `resolve` flag that answers it, and `CommitUnavailable` is the only one that reads as commits lost; a reservation renewed after a long idle reads fresh while one whose worktree only saw unrelated journal events and `HEAD` movement reads stale; a counted-call test pins a board read against this phase's own measured bound, counting the worktree listing, the per-reservation evidence revalidation, the per-protected-predecessor ancestry query, and the per-worktree ahead/behind computation separately, so a later change that multiplies any one of them fails the test rather than hiding inside a single total; and an unrelated branch and an uncomputable one render as different ahead/behind answers. Also: each waiting reason renders with its own applicable action and none of them collapse to a bare "Waiting" — the checkpoint wait, each of the three not-on-trunk evidence cases with its distinct recovery, and the incorporate-the-predecessor case that names the reader's own rebase — asserted in both the TUI model and `--json`; and cancelled and fulfilled edges render as settled rather than as waiting. Also: a deferral resolved by `sequence` disappears from unresolved overlaps, appears in the recorded-answer audit as an ordering created from a deferral carrying both recorded reasons, and no longer renders the symmetric hold. Also: an overlap answer that orders nothing renders with no direction cell at all rather than an empty one, and a retention ref that has been deleted changes no board row unless it changes an orphan alert's recoverability verdict.
...
- **The enforcement limits must be prominent.** The trunk gate is a git hook, so merge ordering is enforced for anybody with no discipline required. Editing is different: blocking the write itself is a Claude Code `PreToolUse` hook and is *not* part of this tool; a general user gets a commit-time drift check instead — automatic, but later than blocking the keystroke. A coordination tool that oversells its enforcement is the failure this design exists to avoid. **Describe the second gap beside the first.** A permissive overlap answer takes two deliberate invocations and records the submitting repository, worktree, and coordination run, why the answer was chosen, and the exact overlap it covers. The journal does not identify a person and cannot prove a person rather than an agent supplied the answer, because a published binary has nowhere to send an escalation its own caller cannot read. Say what it guarantees (the answer is deliberate, reasoned, scoped to the conflict it was shown, attributed to the submitting coordination identity, and visible on the board) and what it does not (a person's identity or that a human was in the loop), and name the invoking harness as where that last part is enforced.
...
**Acceptance gate:** `verify.sh check cargo-berth` and `lint cargo-berth` green (`missing_docs` is denied, so the crate-level docs must be complete). `cargo publish --dry-run -p cargo-berth` succeeds. Every command in the README runs as written against a scratch repo, and the collision transcript matches the real exit-`1` block, exit-`3` proposal, and token-bearing application output byte-for-byte. The README attributes approval only to the submitting repository, worktree, and coordination run, never to a person. Every verb's `--help` runs and names each of its flags. The installed `cargo-berth` executable runs from a directory outside the workspace and reports the expected version.
...
- **Claimed** — the separately approved token-bearing invocation returned exit `0`. Return its `ReservationId` to the caller; phase 15 exports it as `CARGO_BERTH_RESERVATION_ID` for the phase lifetime.
...
- `~/.claude/scripts/` — the shared `**Reservations:**` parser, validator, offline pairwise comparison, and claim-state coordinator, new. **Edit anything under `~/.claude/commands` with the Write/Edit tool, never a shell write** — it is a protected path and shell writes fail with `Operation not permitted`.

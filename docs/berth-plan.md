...
**Stage the artifact for track B.** Track-B phases compile nothing, so the binary must exist before phase 12 runs and no track-B phase may build it. Install it here — `cargo install --path crates/cargo-berth` from the track-A checkout this phase runs in — and record four things in this Work Order's as-built notes: the resulting version, the absolute path to the installed executable, **the absolute path of the source checkout it was built from, and the commit that checkout was on**. Phases 12–14 name that checkout and commit when they cite an engine source file, because the Delegation Context's main checkout can be at a different revision and a relative path does not resolve from a track-B repository. Phases 12–17 invoke that executable by name (`cargo-berth`) and never compile.
...
- `crates/cargo-berth/src/exit.rs` — read only; the eight process exit codes and the one-line meaning the doc table gives each, which the README's exit table must agree with word for word.
- `crates/cargo-berth/src/board/tui.rs` — read only; the terminal board's panes, its key bindings, and the two failure outcomes the README documents for a terminal that will not open or dies mid-session.
- `crates/cargo-berth/src/verb/board.rs` — read only; the authoritative mapping from a terminal outcome to an exit code and a response status, and the source for the three-output-mode account above.
- `Cargo.toml` (workspace root) and `crates/cargo-berth/Cargo.toml`, `Cargo.lock` — read only; the dependency set to license-check and resolve. The root manifest already declared all four before phase 10c; only the crate manifest and the lockfile changed.

...
### Phase 10b — Pre-publish contract hardening  · status: todo

#### Work Order

**Goal:** In `cargo-liner`: an unreadable journal version says so instead of claiming corruption, the board names the recovery it just completed and the verb that answers a stale reservation, and the two answer values phase 11 freezes state their guarantee.

**Spec:** Four repairs to what phases 9b and 10 shipped, grouped because phase 11 freezes the journal and board schemas and each of these is unfixable afterwards without a version bump. Nothing here adds a user-facing verb or flag.

**1. Reject an unsupported journal schema version before decoding version-specific fields.** `replay_complete_records` deserializes each complete record as a whole `JournalEvent` and only then compares `schema_version`. A record written by a future version whose operation layout v1 cannot decode therefore fails deserialization first and is reported as `CorruptInteriorRecord` — the corrupt-ledger diagnostic that sends a user to confirmed reinitialization and loses their journal — when the truth is a newer writer and the answer is to upgrade the tool. Decode a semantic schema header first and reject an unsupported `SchemaVersion` before `JournalEvent` is decoded at all. Malformed v1 keeps reporting `CorruptInteriorRecord` exactly as it does today; the two cases must stay distinguishable. R19 and R68 both require this before v1 ships.

**2. Say which pending bypasses this read just adopted.** Phase 10 imports each `cargo-berth-pending-bypass-<unique>.json` marker into the journal, deletes the file, and renders the result as an ordinary environment-override bypass row — deliberately indistinguishable from a bypass journalled normally when it happened. Phase 13's `SessionStart` shim is written to report that recovery once and then never again, and the board as shipped leaves it nothing to read: after the import there is only history. Reconciliation already separates the markers it completed from the ones it could not append; carry the completed set out to the board as its own value, distinct from the durable audit history and from `BoardAlert::UnrecordedBypasses`, which reports the markers still on disk. It is per-invocation and transient by construction — a second board read reports an empty set, because the markers are gone. Model it as a named per-invocation type, never an `Option` and never a count.

**3. Give a stale reservation the action that answers it.** `BoardAlert::OrphanedOutstanding` carries an `OrphanResolutionAction` and `BoardAlert::UnrecordedBypasses` carries an instruction, while `BoardAlert::StaleReservation` carries only the reservation id and its freshness — so the one alert whose answer is a single verb is the one that does not name it. That verb is `renew`. Model the action as a semantic type in the same form its sibling already uses, never a bare string.

**4. Rename the two answer values whose serialized tags phase 11 freezes.** `ConflictAuthorization::Revalidated` and `RecordedAnswer::RevalidatedWiden` name the action taken rather than the guarantee earned — that the answers already recorded cover every overlap the widen currently has. Rename both to state that guarantee. Their serialized `kind` tags change with them, which is exactly why this lands before the freeze rather than after it. The remaining representation-driven names are internal, carry no serialized tag, and stay in `docs/berth-plan-next.md` where no freeze constrains them.

**Files:**

- `crates/cargo-berth/src/ledger/journal.rs` — **exists**; the replay path and the schema-version comparison this reorders.
- `crates/cargo-berth/src/board/mod.rs` — **exists** after phase 10; the recovered-bypass value, the stale-reservation action, and `RecordedAnswer`.
- `crates/cargo-berth/src/board/tests.rs` — **exists** after phase 10; the model-level assertions.
- `crates/cargo-berth/src/reconcile.rs` — **exists**; already separates completed marker imports from unrecorded occurrences, and is where the completed set reaches the board.
- `crates/cargo-berth/src/gate/permit.rs` — **exists**; the marker recovery preparation those two sets come from.
- `crates/cargo-berth/src/answer/conflict_authorization.rs` — **exists**; `ConflictAuthorization::Revalidated`.
- `crates/cargo-berth/tests/board.rs` — **exists** after phase 10; the `--json` assertions.

**Constraints from prior phases:** Phase 2 owns the journal schema and its version field; this phase changes the order in which replay reads them and changes no record on disk. Phase 8 writes the pending-bypass markers and phase 10 adopts them — take reconciliation's existing completed-import set rather than recomputing which markers were adopted, and do not change the adoption itself, which is already proven idempotent under interruption. Phase 10 owns the board model and its `--json` representation; the two new values join it under phase 2's typed payload and no rendering is scraped back into a field. Phase 9b owns `ConflictAuthorization` and the widen path that records it; the rename changes the value's name and its serialized tag and changes no widening behavior, no ordering edge, and no audit row. Phase 10's board already renders the `Revalidated` authorization by name, so its assertions move with the rename. Phase 11 freezes both schemas, so every serialized-tag change this plan intends must land here.

**Acceptance gate:** `verify.sh test cargo-berth`, `verify.sh test cargo-berth board`, and `verify.sh lint cargo-berth` all green, including: a journal record carrying an unsupported `schema_version` together with an operation layout v1 cannot decode reports `UnsupportedSchemaVersion`, while a malformed v1 record still reports `CorruptInteriorRecord`, and the two are asserted separately rather than through one error match. Also: a board read that adopts a pending-bypass marker reports that marker in its recovered set and also renders it as an ordinary environment-override bypass row; a second board read reports an empty recovered set while the bypass row remains; a marker that still cannot be appended appears in `BoardAlert::UnrecordedBypasses` and not in the recovered set; and a read that adopts nothing reports an empty recovered set rather than omitting the value. Also: a stale reservation's alert names `renew` and its reservation id in both the board model and `--json`, and a fresh reservation raises no such alert. Also: both renamed values serialize under their new `kind` tags in every payload that carries them, and `verify.sh test cargo-berth` proves no consumer still reads the old names.
...
- `/Users/natemccoy/rust/cargo-liner/crates/cargo-tile/src/app.rs`, `keymap.rs`, `terminal.rs`, `render.rs` — read only; the complete worked example of a `cargo-*` binary embedding the framework: the `AppContext` impl, `register_pane` binding registration, terminal acquisition and restoration, and manual render dispatch.

**Constraints from prior phases:** Phase 10 owns the board model, its sections, its incident and alert projection, and the `--json` representation; take those values rather than recomputing anything. Phase 10b adds the recovered-bypass set and the stale-reservation action to that model — render them like any other fact and add none of your own. Phase 2's typed payload is the `--json` representation and this phase does not change it. Phase 1's envelope still applies. **The `tui_pane` pin already exists.** The workspace root already declares `tui_pane = { path = "crates/tui_pane" }` and `.claude/config/release.toml` already carries its versionless `[[publish_path_pins]]` entry, added for `cargo-port` and `cargo-tile`; this phase only makes `cargo-berth` a third consumer of both. No new pin entry is needed, and phase 11 must not be written as though this phase created one.

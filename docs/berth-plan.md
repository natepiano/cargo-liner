...
### Phase 9b — Drift records and hook identity, finalized  · status: todo

#### Work Order

**Goal:** In `cargo-liner`: an incursion is an incident a user can clear, a commit-time widen is attributed to exactly one reservation, and a hook can learn which run and reservation it is acting for.

**Spec:** Three changes to the record and identity surface phase 9 shipped, all of them to things phase 11 freezes and publishes. Nothing here renders; phase 10 does that.

**An incursion becomes a clearable incident.** Phase 9 appends `incursion { reservation_id, foreign_reservation_ids, paths, at }` with no identity and no way to express resolution, so two observations of one unresolved incursion append two indistinguishable records and nothing can ever clear one — the failure mode phase 5's orphan-alert design already rejected. Add an `IncursionIncidentId` to the record and a disposition journal operation that answers it, in `crates/cargo-berth/src/ledger/journal.rs`. Replay projects incursions so an answered incident stops being outstanding, and re-observing an incident that is already recorded and unanswered appends nothing new rather than a second row. The `resolve` verb gains the flag that records the disposition, alongside phase 5's four recovery decisions.

**Auto-widen attributes an unclaimed path to one reservation or to none.** `DriftReservationSelection::EveryActiveForPostCommit` in `crates/cargo-berth/src/drift/mod.rs` widens every active reservation the invoking worktree holds. That is right for incursion reporting and wrong for widening: once phase 15 makes several phased claims in one worktree normal, an unclaimed file silently joins every phase's reservation and the phase boundaries stop meaning anything. Split the two jobs. Incursion and collision reporting still covers every held reservation — that is the warning the hook exists for, and it mutates nothing. Auto-widen resolves to exactly one reservation: the one the session mapping below names, the one `--reservation` names, or the single candidate when only one is active. When it can name none it mutates nothing and returns an ambiguous-attribution outcome naming `drift --reservation <id>`. This satisfies both halves of phase 9's own argument — the commit-time warning keeps working in the case it matters most, and no widen is attributed to a reservation nobody chose — and a non-mutating report is recoverable where a wrong widen is not.

**Identity reaches a hook through a session-keyed durable mapping, not an exported environment variable.** Phases 13, 14, 15, and 17 were written assuming `/plan:delegate` can export `CARGO_BERTH_RUN` and `CARGO_BERTH_RESERVATION_ID` for a phase's lifetime and that a later hook will see them. Claude Code does not work that way: a Bash tool call's environment does not persist to the next call, and hooks inherit the Claude Code process environment rather than that of any command it ran. Nothing in `~/.claude/commands/plan/delegate.md` sets either variable, and no mechanism there could make an exported value visible to a `PostToolUse` hook. Hooks do receive a `session_id`. Write a mapping under the ledger keyed by that harness session id, naming the coordination run and the claimed reservation, created when `claim` succeeds and retired when the reservation is released or the run checkpoints. Extend phase 3's `EditAuthorization` resolution to consult it **first**, then the per-worktree marker file, then `CARGO_BERTH_RUN`, then fail closed to `Unidentified` — the mapping goes in front of phase 3's order, it does not replace it, and the answering source stays preserved in the authorization. This is what makes phase 3b's `marker_publication: unavailable` genuinely recoverable rather than nominally so, and it is what lets the auto-widen above name exactly one reservation.

The mapping is ledger state, not git state, so it lives under `.git/cargo-berth/` beside the projection and is disposable in the same sense: losing it costs identity resolution for live sessions, never journal truth.

**Files:**
- `crates/cargo-berth/src/ledger/journal.rs` — incursion incident identity, disposition operation
- `crates/cargo-berth/src/ledger/mod.rs` — re-exports for the new domain types
- `crates/cargo-berth/src/drift/mod.rs` — reporting split from widening in post-commit selection
- `crates/cargo-berth/src/reservation/mod.rs` — incursion projection, `EditAuthorization` resolution order
- `crates/cargo-berth/src/session/mod.rs` — the session-keyed mapping, written at claim, retired at release
- `crates/cargo-berth/src/cli.rs` — the `resolve` disposition flag
- `crates/cargo-berth/tests/drift.rs`, `crates/cargo-berth/tests/gate.rs` — acceptance

**Constraints from prior phases:** The journal is append-only and `reservations.json` is a disposable projection; every new record replays. Phase 4 commits the journal record before any git side effect, and adds exit `6` for lock contention whose ten-second deadline the engine has already spent — add no retry anywhere here. Phase 5's `EditAuthorization` is source-preserving and honors a `Marker` identity only when replay shows that run still holds an active reservation minted in that worktree; the session mapping is subject to the same liveness test, so a mapping naming a released reservation resolves to nothing rather than to a stale holder. Phase 8's fail-closed rule for an unreadable journal is unchanged, but an unreadable or absent session mapping is not a ledger failure — it falls through to the next source rather than blocking an edit. Phase 9's `drift --reservation` flag and its ambiguous-selection usage error already exist; reuse them rather than minting a second selection path. Phase 9's domain sets are non-empty by construction — private field, fallible constructor, rejecting empty on both parse and deserialize — and every set added here is built the same way.

**Acceptance gate:** `verify.sh test cargo-berth`, `verify.sh test cargo-berth drift`, and `verify.sh lint cargo-berth` green, including: an incursion record carries an identity that survives a journal round trip and replay; re-observing the same unanswered incident appends no second record and leaves one outstanding incident; a disposition recorded through `resolve` clears the incident from outstanding while leaving both records in the journal; and a disposition naming an unknown incident is a usage error rather than a silent no-op. Also: a `post-commit` drift in a worktree holding two active reservations reports incursions across both and widens neither, returning the ambiguous-attribution outcome that names `drift --reservation <id>`; the same call in a worktree holding one active reservation widens it exactly as before; and an explicit `--reservation` widens only the named one even when two are held. Also: a hook process given only the harness `session_id`, with no `CARGO_BERTH_RUN` exported and no marker file present, resolves the claiming run and its reservation through the mapping; the same lookup after release resolves to nothing; a mapping naming a reservation replay shows released grants no holder exemption; an absent or corrupt mapping falls through to the marker file and then to `CARGO_BERTH_RUN` without blocking an edit; and a claim whose `marker_publication` reported `unavailable` is still resolvable through the mapping alone.

### Phase 10 — The board: model, incidents, and `--json`  · status: todo

#### Work Order

**Goal:** In `cargo-liner`: `cargo-berth board --json` reports integration constraints accurately to a machine, and the headless model behind it is complete. The terminal rendering is phase 10b.
...
`--json` emits the phase-1 envelope and must not require a terminal. **This phase ships no terminal rendering.** Everything here is the headless model and its `--json` projection, which is what phases 13, 14, and 15 actually consume; phase 10b builds the `tui_pane` interface on top of it and owns the only dependencies that need a terminal.

Board read calls `reconcile()` first (phase 5).

**Files:**

- `crates/cargo-berth/src/board/mod.rs` — the model: sections, readiness levels, alerts, and the incursion incident projection.
...
- `crates/cargo-berth/src/git/mod.rs` — **exists**; the git command runner and ref helpers. **There is no ahead/behind helper yet.** `AheadBehind::{Counts, Unrelated, Unavailable}` is named in R51's type skeleton but nothing in `src/git/` implements it, so this phase writes it here rather than calling one that already exists.
...
**Spec:** A `tui_pane` front end over phase 10's headless board model. Implement `tui_pane`'s `AppContext` trait, register panes through `PaneRegistry`, use `Keymap`/`KeymapBuilder` for bindings and `StatusBar` for the footer. Follow `cargo-tile` for how a `cargo-*` binary embeds the framework.
...
`--json` continues to run headless with no TTY and must not acquire a terminal, so the TUI entry point is selected after the output mode is resolved, never before.

**Files:**

- `crates/cargo-berth/src/board/tui.rs` — `AppContext` impl, panes, keymap.
- `crates/cargo-berth/src/board/mod.rs` — **exists** after phase 10; read only, for the model this renders.
- `crates/cargo-berth/src/verb/board.rs` — **exists** after phase 10; selects the terminal path when the output mode is human.
...
**Constraints from prior phases:** Phase 10 owns the board model, its sections, its incident and alert projection, and the `--json` representation; take those values rather than recomputing anything. Phase 2's typed payload is the `--json` representation and this phase does not change it. Phase 1's envelope still applies. The `tui_pane` path dependency this phase adds is what makes phase 11's `[[publish_path_pins]]` entry necessary, so phase 11 must run after it.

**Acceptance gate:** `verify.sh test cargo-berth` and `verify.sh test cargo-berth board` both green. `board --json` still runs headless with no TTY and its content is byte-identical to what phase 10 produced. Every section, waiting reason with its action, alert, incursion incident, and recorded-answer row that `--json` reports is reachable in the terminal view, proven against the model rather than against a rendered string. No fact is computed in `tui.rs` that `--json` does not also report. `verify.sh lint cargo-berth` green.
...
- `.claude/config/release.toml` — read only; add a pin entry only if a path dep exists. Phase 10b adds a `tui_pane` path dependency, so a `[[publish_path_pins]]` entry is expected here.
...
- `crates/cargo-berth/src/drift/mod.rs` — read for the two comparison modes, the reservation-selection rule and its ambiguity error, the post-commit all-local selection, and the three consequences a drift result can carry.
...
- **Claimed** — the separately approved token-bearing invocation returned exit `0`. Return its `ReservationId` to the caller; phase 15 records it in phase 9b's session mapping for the phase lifetime.

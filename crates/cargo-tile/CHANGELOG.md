...
### Changed
- `scripts/install-desktop-entry.sh` installs cargo-handler's desktop entry too, for a binary named `cargo-handler` or when passed `--crate cargo-handler`. With no arguments, a binary, or a binary and an entry name, it installs cargo-tile's entry exactly as before.

...
### Added
- Show how far along a compiling invocation is. The reading is cargo's own: while it compiles it draws `Building [========>    ] 149/403: globset, regex-automata`, and those are units of its build plan finished and planned, so nothing is estimated. The reading rules the working-directory heading standing over the command, in the summary and in the command's own cell alike, which costs the table no width. A heading stands over every command started from that directory, so where two of them report at once the summary falls back to a `done` column with room for a reading per row -- the percentage beside a six-cell bar at eighth-block resolution, fine enough that a bar that short still moves on every unit. Neither appears where nothing on the cell has a capture behind it.

...
- Count what a cell actually draws when working out what it asks for. The summary counted one row per command and left out the working-directory heading and gap each of them draws under; a command's own cell counted the ancestry chain not at all, and counted a driver's own row and heading even where it draws the driver as the foot of that chain instead.

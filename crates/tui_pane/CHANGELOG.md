...
- `BarPalette::themed` styles the status line from the installed theme, and `StatusLineNote::flag` is a note that is a label alone. `draw_keymap_overlay` and `draw_global_shortcuts_overlay` draw the keymap overlay and the `?` popup over the whole frame. `render_toasts` draws the toasts over the whole frame, and `draw_framework_overlay` draws whichever framework overlay is open, settings with the app's rows among them.
...
- `TileGrid<Id>`, the animated grid of bordered cells `cargo-tile` draws: a summary cell and one cell per group the app names by its own `Id`, divided by what each cell asks for through `TileDemands` and laid out as `TilePlacement`s a frame at a time. `TileAction` and `TileGrid::apply` carry out adding and removing cells and moving focus; `TABLE_CELL` and `MIN_INITIAL_ROWS` fix the summary's cell number and the floor on the first column's rows. `draw_tile_grid` draws it, with the app's `TileCells` supplying each cell's rows and contents and the grid adding the shared borders, a rows readout along each cell's foot and the number an empty cell carries; `TileGridContents` hides the contents while leaving the frames, and `draw_tile_cell` draws one cell's interior alone. `TILE_ROWS_CONTENT_LABEL` is the readout's leading label. An app whose body is the grid implements `TileGridHost` and registers `TileGridPane`, which keeps the status line's navigation region off and walks the cells on Tab; `handle_tile_click` is its click, focusing a cell or selecting a row of an open framework overlay by the `TilePick` the hit-test ladder answers. The doc-hidden `TileGrid::settle_for_test` finishes every queued step at once for tests.
- With the `backdrop` feature, what an app's frame calls around the attract screen: `Updates` says whether the display takes new work in or is held still, `fade_to_background` carries a region's text toward the colour each cell is painted on, and `attract_ground` is the colour it goes toward where a cell is painted on nothing. `Attract<P>` is the attract screen's controller, moved from `cargo-tile`: when it comes on and goes, its fade, which `AttractMode` it draws with which `AttractSettings`, randomizing and undoing a replacement, and `frame_due`, the frames it asks the event loop for; it answers each frame with an `AttractGrid` for the app's `AttractWork`, and writes to the app's `FrameProbe`. An app implements `AttractHost` to carry it, naming the app pane ids its `MovingBandPane`, `MovingTextPane` and `PixelatePane` scopes register under (`MovingBandAction`, `MovingTextAction` and `PixelateAction`, in the `attract_moving_band`, `attract_moving_text` and `attract_pixelate` tables); `dispatch_attract_key` is the attract hook, `undo_attract_replacement` the undo global's toast and `attract_pane_id` the id for a mode. `draw_attract_layers` draws the attract screen's share of a frame: the advance, the app's panes as the `AttractGrid` calls for, the animation over them and the notice. `draw_backdrop_notice` writes the `BackdropNotice` line where there is no desktop to draw, saying capture is unavailable in the words of the app probe's `FrameProbe::BACKDROP_UNAVAILABLE_NOTICE`. The attract favorites moved from `cargo-tile` too, kept in `favorites.toml` beside the app's `config.toml` and read without dropping anything the running version cannot read; a load answers a `FavoritesFileState` holding `FavoriteRows`, each recognized row a `Favorite`. The favorites overlay moved from `cargo-tile` as well: an app implements `FavoritesHost` to keep a `FavoritesOverlay` and name the app pane id its `FavoritesOverlayPane` scope (`FavoritesOverlayAction`, in the `favorites` table) registers under, calls `dispatch_favorites_key`, `favorites_resize_settled` and `poll_favorites` from its modal-key, resize-settled and poll hooks, and dispatches its save, open and random-favorite globals to `save_favorite`, `open_favorites` and `show_random_favorite`. The overlay finds the save and open keys it names by their TOML names, `save_favorite` and `open_favorites`.

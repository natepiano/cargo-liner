...
### Fixed
- The backdrop reads the terminal's reported text area as points rather than pixels, and scales the cell up to the capture's pixels instead of dividing it down. Read backwards it left every cell half its size and the reduce grid twice as fine, so the drawing was placed and sampled at half scale -- invisible on a display with one pixel to the point, and wrong on every Retina panel.

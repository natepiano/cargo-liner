...
### Added
- Add `TravelingBand::toggle_variable_tail`. On, the band runs back its own distance at each offset across itself -- every row of one crossing sideways, every column of one crossing up or down -- and those distances grow and shrink between a third of its width and all of it, each walking to a depth drawn for it over a few seconds and standing there a couple more. The leading edge stays flat either way.

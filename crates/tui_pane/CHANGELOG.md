...
- With the `backdrop` feature, what an app's frame calls around the attract screen: `Updates` says whether the display takes new work in or is held still, `fade_to_background` carries a region's text toward the colour each cell is painted on, and `attract_ground` is the colour it goes toward where a cell is painted on nothing. `Attract<P>` is the attract screen's controller, moved from `cargo-tile`: when it comes on and goes, its fade, which `AttractMode` it draws with which `AttractSettings`, randomizing and undoing a replacement, and `frame_due`, the frames it asks the event loop for; it answers each frame with a `Grid` for the app's `Work`, and writes to the app's `FrameProbe`. An app implements `AttractHost` to carry it, naming the app pane ids its `MovingBandPane`, `MovingTextPane` and `PixelatePane` scopes register under (`MovingBandAction`, `MovingTextAction` and `PixelateAction`, in the `attract_moving_band`, `attract_moving_text` and `attract_pixelate` tables); `dispatch_attract_key` is the attract hook, `undo_attract_replacement` the undo global's toast and `attract_pane_id` the id for a mode. `draw_backdrop_notice` writes the `BackdropNotice` line where there is no desktop to draw. The attract favorites moved from `cargo-tile` too, kept in `favorites.toml` beside the app's `config.toml`: `load_favorites`, `push_favorite` and `remove_favorite`, generic over the app's `AppIdentity`, read the file without dropping anything it cannot read, save the running `AttractSettings` as a `Favorite` (a `FavoriteSaveOutcome` says whether it added a row or refreshed one), and delete the row a `FavoriteRemovalTarget` names under the file's lock. A load answers a `FavoritesFileState` holding `FavoriteRows`, each row a `FavoriteRowRecognition`: a `Favorite` with its `FavoriteId`, or an `UnrecognizedFavoriteValue` kept with the `UnrecognizedFavoriteRemovalLocator` that deletes it. `favorite_refusal_message` explains a refused `FavoritesMutation` with the `FavoritesRetryInstruction` and `ResolvedBinding`s that retry it. The favorites overlay moved from `cargo-tile` as well: an app implements `FavoritesHost` to keep a `FavoritesOverlay` and name the app pane id its `FavoritesOverlayPane` scope (`FavoritesOverlayAction`, in the `favorites` table) registers under, calls `dispatch_favorites_key`, `favorites_resize_settled` and `poll_favorites` from its modal-key, resize-settled and poll hooks, and dispatches its save, open and random-favorite globals to `save_favorite`, `open_favorites` and `show_random_favorite`. The overlay finds the save and open keys it names by their TOML names, `save_favorite` and `open_favorites`.

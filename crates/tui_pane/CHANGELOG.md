...
### Added
- On KDE Plasma the backdrop now draws what is really underneath the terminal window -- the other windows standing there, over the actual wallpaper -- rather than a reconstruction of the wallpaper alone. `KWin` offers no capture limited by stacking order, so the picture is assembled: the window stack is read through `org.kde.kwin.Scripting`, and every window below this one on the same output and virtual desktop is captured through `org.kde.KWin.ScreenShot2` and drawn over the output's desktop window. This terminal's own window is absent by construction, so the animation can never photograph itself. `KWin` answers the screenshot interface only for a process whose desktop entry names it, and without that entry -- or on any other failure -- the wallpaper reconstruction stands as before.
- `TravelingBand`, `DriftingText`, and `ResolvingPixels` expose their steerable parameters through `BandSettings`, `TextSettings`, and `PixelSettings`. Each animation can restore settings through its steering transitions or generate a deterministic valid set from a seed.
- The keymap overlay lays its sections out in as many columns as the popup has room for, so a keymap too tall for the terminal is read rather than scrolled. A section that will not fit in what is left of a column starts the next one, and only a section taller than a whole column is split, with its heading drawn again above the part that carries on. Clicking a row now reads both axes, so a row in the second column is the row that gets selected.
...
### Fixed
- `Desktop::placement` reads the terminal's current cell count rather than the one the capture was reduced at. The two disagree for as long as it takes a fresh capture to arrive after a resize, and the capture's own count subtracted from the current frame gives a negative padding: a window taken full width and then halved was placed 86 columns off the left of the display, so half of it fell outside the grid and drew nothing while the rest read the wrong part of the desktop. The cell *size* still comes from the capture, which is the size its grid was reduced at.
- The KDE backdrop keeps working for a window the user has dragged. `KWin` reports such a window's geometry with a fraction on the end, which parsed as no whole coordinate at all and dropped the window from the stack it was being looked for in, so the composite gave way to the wallpaper -- silently, and only for a window that had been moved by hand.
- The backdrop captures the desktop with CoreGraphics `CGWindowListCreateImage` instead of ScreenCaptureKit, whose screenshot and shareable-content calls block one another when several processes call them at once.
...
- The backdrop monitor replaces a capture worker only after a second consecutive missed deadline,
  and a returned result restores its replacement allowance.
- `window_titles`, `window_titled` and `window_at` ask CoreGraphics rather than `SCShareableContent`, which `window_frame` had already measured at seventy milliseconds against a few hundred microseconds. `identify` calls them from the thread that draws, so a pass cost several frames and a run spent its first seconds drawing none.
- The marker title `identify` sets is left on until a pass finds it or the passes run out. It used to come off within the pass that set it, which only worked while asking the window server was slow enough to be the waiting; asked the cheap way, no window was ever caught wearing it.
...
### Changed
- `DriftingText`'s lanes merge into one another rather than meeting at an edge. `merged` pulls the interpolation's smoothstep back toward a straight ramp by `TEXT_LANE_BODY_PERCENT`; the bare curve held both ends of a span flat and did the whole handover in the middle, which put every bit of the speed change into a narrow run of lines and drew a boundary there.
- `DEFAULT_TEXT_SPEED` drops from 12 cells a second to 9.
- The drifting field's lanes are dealt uneven thicknesses, each within `TEXT_LANE_SPREAD_PERCENT` of the nominal, rather than one thickness apiece. Bands all one size read as a ruled grid.

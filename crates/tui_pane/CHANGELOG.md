...
- A `TravelingBand` now starts standing across the whole grid with a ragged trailing edge: it runs back its own distance at each offset across itself, and those distances grow and shrink between a third of its width and all of it. `toggle_variable_tail` turns the fraying off for a trailing edge as flat as the leading one, and `tail_faster` and `tail_slower` change how fast it moves -- one number governing both the walk to a fresh depth and the stand at it.

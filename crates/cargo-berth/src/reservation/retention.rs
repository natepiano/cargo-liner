//! Every reservation replay retains, and the incursion incidents replayed alongside them.
//!
//! [`RetainedReservationSet`] is the only thing a journal replay produces: the reservations
//! it accumulated and the incursion incidents recorded against them. Every conflict,
...
use super::scoped_patch_evaluation::ScopedPatchEvaluatorVersion;
use super::scoped_patch_evaluation::ScopedPatchTargetEvaluationSchedule;
use super::scoped_patch_evaluation::ScopedPatchVerdictEvidence;
use super::scoped_patch_evaluation::SuccessorScopedPatchEquivalenceVerdict;
...
impl RetainedReservationSet {
    ...
    fn apply(&mut self, event: &JournalEvent) -> Result<(), ReservationReplayError> {
        match &event.operation {
            ...
            | JournalOperation::SuccessorScopedPatchComparisonAttempted { .. } => {
                self.apply_integration_evidence_journal_event(event)
            },
            JournalOperation::RebindWorktree { .. } | JournalOperation::RelocateWorktree { .. } => {
                self.apply_worktree_binding_journal_event(event)
            },
    ...
    fn apply_holder_lifecycle_journal_event(
        &mut self,
        event: &JournalEvent,
    ...
    /// Apply the operations that protect, resnapshot, or judge a holder's integration subject.
    fn apply_integration_evidence_journal_event(
        &mut self,
        event: &JournalEvent,
    ) -> Result<(), ReservationReplayError> {
        match &event.operation {
    ...
    /// Apply the operations that move a reservation to another worktree or another root.
    fn apply_worktree_binding_journal_event(
        &mut self,
        event: &JournalEvent,
    ) -> Result<(), ReservationReplayError> {
        match &event.operation {
    ...
    fn apply_scoped_patch_equivalence_check(
        ...
        target: &GitObjectId,
        evidence: &ScopedPatchVerdictEvidence,
        generation: ProjectionGeneration,
    ) -> Result<(), ReservationReplayError> {
        let reservation = self.scoped_patch_comparison_subject_mut(reservation_id, subject)?;
        reservation.retained_scoped_patch_target_verdicts.record(
            subject,
            target,
            evidence.verdict,
            &evidence.witness,
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_verdict_version_precedence()?;
        Ok(())
    }

    /// Exercise both verdict domains with conflicting versions in either journal order.
    fn assert_verdict_version_precedence() -> Result<(), Box<dyn std::error::Error>> {
        let reservation_id = RESERVATION_ID.parse::<ReservationId>()?;
        let target = TRUNK_OID.parse::<GitObjectId>()?;
        let [claim, checkpoint, ..] = lifecycle_events()?;
        for domain in [VerdictDomain::Target, VerdictDomain::Successor] {
            for current_comparison in [
                ...
                let legacy_only = RetainedReservationSet::replay(&[
                    claim.clone(),
                    checkpoint.clone(),
                    journal_event(3, &legacy)?,
                ])?;
                ...
                for current_first in [false, true] {
                    ...
                    let retained = RetainedReservationSet::replay(&[
                        claim.clone(),
                        checkpoint.clone(),
                        journal_event(3, first)?,
                        journal_event(4, second)?,
                    ])?;
                    assert_versioned_verdict_lookup(
    ...
    /// Check a cache hit or retry without reaching into retained storage.
    ...
    fn scoped_patch_equivalence_checked(
        ...
        journal_event(
            ...
            &json!({
                "op": "scoped_patch_equivalence_checked",
                "evaluator_version": "historical_candidate",
                "reservation_id": RESERVATION_ID,
                "subject": subject,
                "target": TRUNK_OID,
                "verdict": verdict

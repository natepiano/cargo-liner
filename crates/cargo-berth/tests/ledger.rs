...
fn init_reports_invalid_configured_trunk_as_input_error() {
    ...
    repo.set_trunk("refs/remotes/origin/main");
    let output = repo.run(repo.root(), &["init", "--json"]);
...
fn init_enrolls_lane_against_configured_target_merge_base() {
    ...
    repo.commit_file(&lane, "lane-work.txt", "committed lane work\n", "lane work");
    fs::write(lane.join("dirty-lane.txt"), "uncommitted lane work\n")
        .expect("dirty lane file writes");
    let expected_phase_start = repo.git_stdout(&lane, &["merge-base", "HEAD", "integration"]);
    let expected_target_commit = repo.git_stdout(&lane, &["rev-parse", "integration"]);
    let output = repo.run(repo.root(), &["init", "--json"]);
...
fn linked_worktree_uses_main_configuration_for_repository_trunk() {
    ...
    let target = &json(&claim)["payload"]["data"]["target"];

...
/// drag that began in another pane never reaches here. Motion off the
/// captured-output rows yields no row and leaves the range as-is.
...
pub fn dispatch_output_action(action: OutputAction, app: &mut App) {
    match action {
        // Select-all covers the captured-output buffer.
        OutputAction::SelectAll => {
            let live = app.inflight.owned_run().output().to_vec();
            app.panes.output.select_all(&live);
        },
        // Stopping and clearing act only on Cargo Port's own run.
        OutputAction::Cancel => cancel_owned_output(app),
    }
}
